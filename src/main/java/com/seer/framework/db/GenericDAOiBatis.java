 ...
 * @version 1.0
 * 
 * Updated the class to implement GenericDao by Bryan Noll. This class used to
 * be named BaseDaoiBATIS.
 */
@SuppressWarnings({ "unchecked", "rawtypes" })
public class GenericDAOiBatis<T extends Entity> extends SqlMapClientDaoSupport implements GenericDAO<T>{
    /** The log. */
    protected final Logger log = Logger.getLogger(DAOAdapter.class);
    ...
    public void delete(final T object){
        getSqlMapClientTemplate().update(iBatisDAOUtils.getDeleteQuery(ClassUtils.getShortName(this.persistentClass)), object.getId());
    }
}