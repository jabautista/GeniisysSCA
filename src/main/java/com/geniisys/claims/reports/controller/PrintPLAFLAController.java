...
@WebServlet(name="PrintPLAFLAController", urlPatterns={"/PrintPLAFLAController"})
public class PrintPLAFLAController extends BaseController{
	
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private Logger log = Logger.getLogger(PrintPLAFLAController.class);
		...
		try{
			if("printReport".equals(ACTION)){
				...
				if("GICLR050A".equals(reportId)){
					...
					params.put("P_ALL_USERS", request.getParameter("allUsers"));
				} else if("GICLR051A".equals(reportId)){
					params.put("P_LINE_CD", request.getParameter("lineCd"));
					params.put("P_USER", (request.getParameter("userId") != null && !request.getParameter("userId").equals("")) ? request.getParameter("userId") : null);
					params.put("P_ALL_USERS", request.getParameter("allUsers"));
				}					
