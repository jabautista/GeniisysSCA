...
Modalbox.Methods = {
	...
	options: {
		...
		headerClose: true, // andrew 03.28.2011
		...
		asynchronous : true, //nok 06-03-2010
		onComplete: null, //alfie 02-09-2011
		slideDown: true
	},
	_options: new Object,
	...
	_init: function(options) {
		...
		if(!this.options.headerClose) $(this.MBclose).hide();
	...
	hide: function(options) { // External hide method to use from external HTML and JS
		if(this.initialized) {
			...
			/* This part executes the function specified on option onHideFunc when Modalbox.hide() is called
			 * Added an option on options parameter (executeOnHideFunc) to check if function will be executed or not
			 * Value of parameter is true or false
			 */
			// if onHideFunc is not null, and executeOnHideFunc option is true, execute after Hide (emman 04.06.2011)
			if (nvl(options, false)) {
				if (nvl(this.options.onHideFunc, false) && nvl(options.executeOnHideFunc, false)) {
					this.options.onHideFunc();
				}
			}
			
			/* Set onHideFunc to null after function is called, to clear onHideFunc function and to avoid being used
			 * again when another Modalbox has been invoked
			 */
			this.options.onHideFunc = null;
							...
							new Effect.SlideDown(this.MBwindow, {
								...
								afterFinish: function(){ 
									...
									if (this.options.onComplete) {
										this.options.onComplete(); //alfie 02/18/2011
									}
								}.bind(this)
							});
						} else {
	...
	activate: function(options){
		...
		if(this.options.headerClose) $(this.MBclose).show();
	...
	_kbdHandler: function(event) {
		...
		switch(event.keyCode) {
			case Event.KEY_TAB:
				...
				break;			
			case Event.KEY_ESC:
				if(this.options.headerClose) if(this.active) this._hide(event);
				break;
			case 32:
