...
//******************************************************************************
	...
	// scwDateNow.getFullYear();

    // How many years do want to be valid and to show in the drop-down list?
	
    //var scwDropDownYears   = scwDateNow.getFullYear()-scwBaseYear+1;
    var scwDropDownYears   = scwDateNow.getFullYear()-scwBaseYear+275;	//changed from 30 to 275 - set year to 2287 Gzelle 10.9.2013
    ...
    var scwLanguage;
    var scwOriginalDate; // added by irwin
    ...
    document.writeln(
        '<style type="text/css">'                                       +
            ...
            'td.scwCellsExMonth         {background-color:  #CCCCCC;'   +
                                        'color:             #666666;}'  +
            '/* Today selector */'                                      +
            'td.scwFoot                 {background-color:  #474747;'   +
                                        'color:             #FFFFFF;}'  +
...
// ****************************************************************************
    ...
    function showCal(scwEle,scwSourceEle, targetAge)    {scwShow(scwEle,scwSourceEle, targetAge);};
    function scwShow(scwEle,scwSourceEle, targetAge)
        {scwTriggerEle = scwSourceEle;
        scwOriginalDate = "";
        scwOriginalDate = scwEle.value;
		theTargetAge = targetAge;	
            ...
            {scwSelYears.options.selectedIndex=Math.floor(scwMonthSum/12);
             ...
             function scwSetOutput(scwOutputDate)
             
                {
				 ...
				 //PART WHERE WE COMPUTE AGE ###############################################################################################################
				 
				 calculateAge(scwOutputDate.scwFormat(scwDateOutputFormat));				 				 
                 scwHide(); 
                 if(nvl(scwTargetEle.getAttribute("triggerChange"),"N") == "Y"){ //added by irwin - 7/20.2012
                	 fireEvent(scwTargetEle,"change");
                 }
                 scwTargetEle.focus();
                };
			function calculateAge(date){
					var moyr = date.substr(date.search('/')+1 ,date.length);
					
					var bday=date.substr(0, date.search('/'));
					if(bday.substr(0, 1)=='0'){
						bday = bday.substr(1, 2)
					}
					
					var bmo=moyr.substr(0, moyr.search('/'));
					if(bmo.substr(0, 1)=='0'){
						bmo = bmo.substr(1, 2)
					}
					var byr=moyr.substr(moyr.search('/')+1, moyr.length);
					var now = new Date();
					tday=now.getDate();
					tmo=(now.getMonth()+1);
					tyr=(now.getFullYear());
				...
				if(ageNow<0){
					...
					if(theTargetAge!=null){
						theTargetAge.value=ageNow;
						
					}
					
				}
				if($('day')){
					displayDate(date);//found in common.js for client.jsp
				}
...
// ***************************
    ...
    document.write(
     "<!--[if IE]>" +
        ...
        "</iframe>" +
     "<![endif]-->" +
 //    "<table id='scw' class='scw' style='z-index: 10000;'>" +		modified by d.alcantara, 06-22-2011
      //"<table id='scw' class='scw' style='z-index: 2000003;'>" + modified by andrew - 08.17.2011
      //"<table id='scw' class='scw' style='z-index: 90000000;'>" + 
     "<table id='scw' class='scw' style='z-index: "+(Number(90000000) + Number(Windows.maxZIndex) + Number(2000001))+";'>" +  //nok modified 11.15.11
       "<tr class='scw'>" +
         "<td class='scw'>" +
