/**
 * Same as the function checkPopupsTableWithTotalAmountbyObject but a condition
 * must be satisfied before the amount can be added to the total amount.
 * 
 * @author Veronica V. Raymundo
 */

function checkPopupsTableWithTotalAmountbyObjectWithCond(objArray, tableName, tableRow, rowName, amountName, totalAmtDiv, amountId, attribute, comparedValue){
	try{
		var holder = 0;
		var exist = 0;
		...
		for(var i=0, length=filteredArray.length; i < length; i++){
			if(filteredArray[i][attribute] == comparedValue){
				var amount;
		...
		showErrorMessage("checkPopupsTableWithTotalAmountbyObjectWithCond", e);
	}		
}