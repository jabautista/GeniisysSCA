 ...
 * 	==========	===============	==============================
 * 	08.31.2011	mark jm			tablegrid version of showItemInfo 
 */
function showItemInfoTG(){
	try {
		...
		lineCd = (objUWGlobal.packParId != null ? /*objCurrPackPar.lineCd*/ getOrigLineCd(objCurrPackPar.menuLineCd) : $F("globalLineCd")); 	
		
		if(objUWGlobal.lineCd == objLineCds.MC || objUWGlobal.menuLineCd == objLineCds.MC || lineCd == "MC"){	// mark jm 07.08.2011 hard-coded for this side for package. these are the original line cds
			...
			linePage = "/GIPIWCargoController?action=" + (parType == "E" ? "showEndtMarineCargoItemInfo&" : "getGIPIWItemTableGridMN&");	
		} else if(objUWGlobal.lineCd == objLineCds.AV || objUWGlobal.menuLineCd == objLineCds.AV || lineCd== "AV"){
			linePage = "/GIPIWAviationItemController?action=" + (parType == "E" ? "showEndtAviationItemInfo&" : "getGIPIWItemTableGridAV&");
			...
			linePage = "/GIPIWItemVesController?action=" + (parType == "E" ? "showEndtMarineHullItemInfo&" : "getGIPIWItemTableGridMH&");
		} else if(objUWGlobal.lineCd == objLineCds.AC || objUWGlobal.menuLineCd == objLineCds.AC || objUWGlobal.menuLineCd == "AC" || lineCd== "AC"){
			linePage = "/GIPIWAccidentItemController?action=" + (parType == "E" ? "showEndtAccidentItemInfo&" : "getGIPIWItemTableGridAC&");	
		} else if(objUWGlobal.lineCd == objLineCds.EN || objUWGlobal.menuLineCd == objLineCds.EN || lineCd== "EN") {	
			linePage = "/GIPIWEngineeringItemController?action=" + (parType == "E" ? "showEndtEngineeringItemInfo&" : "getGIPIWItemTableGridEN&");
		} else{
		...
		//hideNotice(""); //hide notice message just after Ajax.Update to prevent javascript error upon selecting an item during the loading of Endt Item Information module by MAC 06/03/2013.
	} catch(e){
		showErrorMessage("showItemInfoTG", e);
	}
}