<div id="agingSortPopUp" style="width: 95%; margin-top: 5px;">
		...
		<div class="sectionDiv" id="sortOptionDiv" style="width: 45%; height: 100px; margin: 8px 8px 8px 8px;">
			<table border="0" id="chkboxTbl" align="center" style="margin-left: 10px; margin-top: 10px; margin-bottom: 0px;">
				...
				<tr name="columnTR" id="assdNoTR" >
					<td>
						<input id="chkAssdNo" name="columnCB" type="checkbox" value="a020_assd_no" style="float: left;">
	...
	<div>
		...
		<input id="columnNo"   type="hidden"  value="${columnNo}"/>
	</div>
</div>
...
try{
	...
	if (objGIACS202.msortOrder != ""){
		for (var i=1; i<=3; i++){
			$("chkboxTbl").down("tr", i).down("td", 0).down("label", 0).innerHTML = objGIACS202.msortOrder[i-1].columnLabel;
			$("chkboxTbl").down("tr", i).down("td", 0).down("input", 0).value = objGIACS202.msortOrder[i-1].columnValue;
			$("chkboxTbl").down("tr", i).down("td", 0).down("input", 0).checked = objGIACS202.msortOrder[i-1].columnState;
		}
		ascDesc = objGIACS202.msortOrder[0].ascDesc;
		if (ascDesc == " ASC"){
			$("ascRB").checked = true;
		}else{
			$("descRB").checked = true;
	...
	$("btnOk").observe("click", function(){
		...
		for (var i=1; i <= 3; i++){
			...
			objGIACS202.msortOrder.push({columnLabel: $("chkboxTbl").down("tr", i).down("td", 0).down("label", 0).innerHTML,
		...
		objGIACS202.multiSort = columns.substr(0, columns.length - 1);	
		
		if(objGIACS202.multiSort != null){
			tbgBillsByAssdAndAge.url = contextPath + "/GIACInquiryController?action=showGIACS202&refresh=1&fundCd=" + $F("fundCd")
			+ "&branchCd=" + $F("branchCd") + "&columnNo=" + $F("columnNo") + "&multiSort=" + objGIACS202.multiSort;
		...
		overlayAgingSortPopUp.close();
	});
	
	$("btnCancel").observe("click", function(){
		overlayAgingSortPopUp.close();
		delete overlayAgingSortPopUp;
	});
