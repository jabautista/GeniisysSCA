<div style="padding: 10px;">
	...
	<input id="replenishId"  type="hidden" value="${replenishId}"/>
</div>
<script>

	var objSumAcctEntriesTG = JSON.parse('${sumAcctEntriesJSON}'.replace(/\\/g, '\\\\'));
	try{
		
	
		var acctEntriesModel = { 
			url: contextPath+"/GIACReplenishDvController?action=getGIACS016SumAcctEntriesTableGrid&refresh=1&replenishId="+nvl($F("replenishId"), 0),
			options:{
				title: '',
			...
			rows: objSumAcctEntriesTG.rows
		};
		
		acctEntriesTableGrid = new MyTableGrid(acctEntriesModel);
		acctEntriesTableGrid.pager = objSumAcctEntriesTG;
