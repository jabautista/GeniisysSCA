...
<script type="text/javascript">
		...
		try {
			new Ajax.Request(
					contextPath + "/GIACUploadingController",
					{
						method : "POST",
						parameters : {
							action : "checkDcbNoGiacs603",
							branchCd : objGIACS603.branchCd,
							orDate : $F("txtOrDate")
						},
	...
	function checkClaim(){
		new Ajax.Request(contextPath+"/GIACUploadingController", {
			method: "POST",
			asynchronous: true,
			parameters : {action : "processGiacs603",
						orDate : $F("txtOrDate"),
			...
			onComplete: function(response){
				...
				if(checkCustomErrorOnResponse(response) && checkErrorOnResponse(response)){
					...
					if (obj.exists == "Y"){ // nieko Accounting Uploading GIACS603
						if(hasCCFunction == "Y"){
							checkForOverride();
						} else {
	...
	function proceedUploadPayments(){
		new Ajax.Request(contextPath+"/GIACUploadingController", {
			...
			parameters : {action : "giacs603UploadPayments",
						...
						dcbBankCd : $("txtBankCd").value,
						dcbBankAcctCd : $("txtBankAcctCd").value,
			...
			onComplete: function(response){
				...
				if(checkCustomErrorOnResponse(response) && checkErrorOnResponse(response)){
					...
					showWaitingMessageBox(objCommonMessage.SUCCESS, imgMessage.SUCCESS, function(){
						showGiacs603(); //nieko Accounting Uploading GIACS603
						closeOverlay();
					});
				}
			}
