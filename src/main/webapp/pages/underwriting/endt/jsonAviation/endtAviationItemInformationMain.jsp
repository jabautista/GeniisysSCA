...
<div id="parItemInformationDiv" name="parItemInformationDiv" style="margin-top: 1px;" changeTagAttr="true">
	<form id="itemInformationForm" name="itemInformationForm">
		...
		<div id="addDeleteItemDiv">
			<jsp:include page="/pages/underwriting/endt/jsonAviation/subPages/endtAviationItemInformation.jsp"></jsp:include>
		</div>	
		...
		<!-- <div id="perilInformationDiv" name="perilInformationDiv" style="float: left;"> -->
...
try{
	...
	setCursor("wait");
	if(objUWGlobal.packParId == null) setDocumentTitle("Item Information - Aviation - Endorsement");
	initializeAccordion();
	addStyleToInputs();
	initializeAll();
	...
	observeCancelForm("btnCancel", function(){saveEndtAviationItems(false);}, showEndtParListing);
	observeSaveForm("btnSave", function(){saveEndtAviationItems(true);});
	...
	//showEndtPerilInfoTGPage();
	
	setCursor("default");
	changeTag = 0;
	initializeChangeTagBehavior(saveEndtAviationItems);
	initializeChangeAttribute();
	hideNotice("");
	checkIfCancelledEndorsement(); // added by: Nica 07.23.2012 - to check if to disable fields if PAR is a cancelled endt
}catch(e){
	showErrorMessage("Endorsement Aviation Main Page", e);
}
</script>
