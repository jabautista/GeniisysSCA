...
<script type="text/javascript">
	...
	$("btnOk").observe("click", function(){
		...
		var objParams = new Object(); // robert 10.31.2012
		objParams.gipiWPolbas	= prepareJsonAsParameter(setEndtBasicObj()); // robert 10.31.2012
		
		new Ajax.Request(contextPath + "/GIPIParInformationController?action=processEndtCancellation", {
			method : "GET",
			parameters : {
				parId : $F("globalParId"),
				...
				effDate : $F("b540EffDate") == "" ? $F("endtEffDate") : $F("b540EffDate"), //jdiago 08.05.2014 added checking if b540EffDate is empty.
				parameters : JSON.stringify(objParams), //robert 10.31.2012
				cancellationType : $("endtCancellation").checked ? "ENDT" : "COI" // robert 10.31.2012
			},
			asynchronous : false,
			evalScripts : true,
				...
				function(response){
					if (checkErrorOnResponse(response)){
						...
						if(nvl(result.msgAlert,null) == null){
							...
							changeTag = 0; //robert 10.31.2012
							showMessageBox("Endorsement successfully cancelled.", imgMessage.INFO);
							showBasicInfo(); //added by jdiago 08.11.2014 : reload page
						}else{
