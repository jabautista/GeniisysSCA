...
<script type="text/javascript">
	...
	objUW.hidObjGIPIS031 = {};
	objUW.hidObjGIPIS031.gipiWPolGenin = JSON.parse('${gipiWPolGeninJSON}'.replace(/\\/g, '\\\\'));
	objUW.hidObjGIPIS031.gipiWEndtText = JSON.parse('${gipiWEndtTextJSON}'.replace(/\\/g, '\\\\'));

	//
	objUW.hidObjGIPIS031.gipiWItem = JSON.parse('${gipiWItem}'.replace(/\\/g, '\\\\'));
	objUW.hidObjGIPIS031.gipiWItmperl = JSON.parse('${gipiWItmperl}'.replace(/\\/g, '\\\\'));	
</script>
...
<div id="endtBasicInformationDiv" style=" margin-top: 1px;"> <!--  display: none; -->
	...
	<form id="endtBasicInformationForm">
		...
		<input type="hidden" id="b540InceptDate"			name="b540InceptDate"			value="<c:if test="${not empty gipiWPolbas}"><fmt:formatDate value="${gipiWPolbas.inceptDate }" pattern="MM-dd-yyyy HH:mm:ss a" /></c:if>" />	<!-- adpascual - 05.09.2012 - added a AM/PM indicator -->
		<input type="hidden" id="b540InceptTag"				name="b540InceptTag"			value="<c:if test="${not empty gipiWPolbas}">${gipiWPolbas.inceptTag}</c:if>" />	
		<input type="hidden" id="b540ExpiryDate"			name="b540ExpiryDate"			value="<c:if test="${not empty gipiWPolbas}"><fmt:formatDate value="${gipiWPolbas.expiryDate }" pattern="MM-dd-yyyy HH:mm:ss a" /></c:if>" /> <!-- adpascual - 05.09.2012 - added a AM/PM indicator -->	
		...
		<input type="hidden" id="b540EffDate"				name="b540EffDate"				value="<c:if test="${not empty gipiWPolbas}"><fmt:formatDate value="${gipiWPolbas.effDate }" pattern="MM-dd-yyyy HH:mm:ss a" /></c:if>" />	<!-- adpascual - 05.09.2012 - added a AM/PM indicator -->
		<input type="hidden" id="b540EndtExpiryDate"		name="b540EndtExpiryDate"		value="<c:if test="${not empty gipiWPolbas}"><fmt:formatDate value="${gipiWPolbas.endtExpiryDate }" pattern="MM-dd-yyyy HH:mm:ss a" /></c:if>" />	<!-- adpascual - 05.09.2012 - added a AM/PM indicator -->
		<input type="hidden" id="b540PlaceCd"				name="b540PlaceCd"				value="<c:if test="${not empty gipiWPolbas}">${gipiWPolbas.placeCd}</c:if>" />	
		<input type="hidden" id="b540IssueDate"				name="b540IssueDate"			value="<c:if test="${not empty gipiWPolbas}"><fmt:formatDate value="${gipiWPolbas.issueDate }" pattern="MM-dd-yyyy HH:mm:ss a" /></c:if>" /> <!-- adpascual - 05.09.2012 - added a AM/PM indicator -->
		...
		<!-- GIPI_WENDTTEXT -->
		<!-- comment by Nok - converted into JSON 05.17.2011
		...
		<!-- GIPI_WPOLGENIN -->
		<!-- comment by Nok - converted into JSON 05.16.2011
	...
	<jsp:include page="../../subPages/otherDetails.jsp"></jsp:include>
		<div id="mortgageePopups">
			...
			<div id="outerDiv" name="outerDiv">
				<div id="innerDiv" name="innerDiv" subPage="1">
			   		<label>MortgageeInformation</label>
...
<script type="text/javascript">
	...
	*/
	...
	$("endorseTax").observe("click", function(){
		...
		if($("endorseTax").checked){
			...
			if($("nbtPolFlag").checked | $("prorateSw").checked){
				...
				objUW.hidObjGIPIS031.gipiWEndtText.endtTax = "N";
			...
			if(getRecordCount("endtItmPerls") > 0){
				...
				//objUW.hidObjGIPIS031.gipiWEndtText.endtTax = "N"; //$("b360EndtTax").value = "N";
				objUW.hidObjGIPIS031.gipiWEndtText.endtTax = $("endorseTax").checked ? "Y" : "N";
				$("endorseTax").value = "N";
			...
			*/
			...
			if((objUW.hidObjGIPIS031.gipiWItem.filter(function(obj){	return obj.recFlag == "A";	})).length > 0){
				$("parEndtTaxSw").value = "X";
			}			

			if(objUW.hidObjGIPIS031.gipiWItmperl.length > 0){
				$("parEndtTaxSw").value = "N";
				$("endorseTax").value = "N";
				$("endorseTax").checked = false;
				objUW.hidObjGIPIS031.gipiWEndtText.endtTax = "N";				
			...
			objUW.hidObjGIPIS031.gipiWEndtText.endtTax = "Y";
			$("parVendt").value = "Y";
		}else{
			$("endorseTax").value = "N";
			objUW.hidObjGIPIS031.gipiWEndtText.endtTax = "N";
	...
	function initializeFields(){
		$("shortRatePercent").value = $F("shortRatePercent").blank() ? "0" : $F("shortRatePercent");
	...
	function checkPendingRecordChangesInBasicInfo(){
		...
		$$("div#outerDiv").each(function(div){
			...
			subPage.isSubPage = div.down("div").getAttribute("subPage");
		...
		for (i=0; i<subPages.length; i++){
			if(subPages[i].changedElements.length > 0) {
				if(subPages[i].isSubPage == 1){
					if (subPages[i].tranMode == "Update"){
						var message = "You have changes in " + subPages[i].title + " portion. Press Update button first to apply changes otherwise unselect the " + subPages[i].title + " record to clear changes.";
	...
	function startSave(){
		try{
			if(!checkPendingRecordChangesInBasicInfo()){				
				return false;
			}
			/* pre-validation */
			...
			if($F("globalCg$BackEndt") == "Y" && $F("parBackEndtSw") == "Y"){				
				Dialog.alert({
					url: contextPath+"/GIPIParInformationController?action=showBackwardEndt&parId=" + $F("globalParId"), 
					options: {method: 'get'}}, {
						className: "alphacube", 
						width:540, 
						okLabel: "Ok",						
				...
				onComplete :
					function(response){
						if (checkErrorOnResponse(response)) {
							...
							if(nvl(result.msgType,null) == null){
	...
	observeCancelForm("btnCancel", startSave, cancelFuncMain);
	...
	function saveEndorsementBasicInfo(){	
		try{	
			parseOtherDetails();	
			...
			//objUW.hidObjGIPIS031.gipiWItmperl = null;
			objUW.hidObjGIPIS031.gipiWItem = getAddedAndModifiedJSONObjects(objUW.hidObjGIPIS031.gipiWItem);
			objUW.hidObjGIPIS031.gipiWItmperl = getAddedAndModifiedJSONObjects(objUW.hidObjGIPIS031.gipiWItmperl);			
			
			var hidObjGIPIS031 = prepareJsonAsParameter(objUW.hidObjGIPIS031);
			controller = "GIPIParInformationController";
			action = "saveEndtBasicInfo";
			showNoticeMessage = "Saving endorsement, please wait...";
			
			var newObj = serializeToObject("endtBasicInformationForm");
			/* newObj.inceptTag = $("inceptTag").checked ? "Y" : "N";
			newObj.expiryTag = $("expiryTag").checked ? "Y" : "N";
			newObj.endtExpDateTag = $("endtExpDateTag").checked ? "Y" : "N"; */
			...
			new Ajax.Request(contextPath + "/"+controller+"?action="+action//+
					...
					//"&parameters=" + encodeURIComponent(JSON.stringify(objParameters))+Form.serialize("endtBasicInformationForm")
					, {
				method : "POST",
				//postBody : Form.serialize("endtBasicInformationForm"),
				parameters:{
					...
					lineCd: $F("lineCd"),
					issCd: $F("issCd"),
					urlParNbtPolFlag: nbtPolFlag,
					urlParProrateSw: prorateSw,
					urlParEndtCancellation: endtCancellation,
					urlParCoiCancellation: coiCancellation,
					creditingBranch: creditingBranch,
					hidObjGIPIS031: hidObjGIPIS031,
					parameters: (prepareJsonAsParameter(objParameters)),
					basicInfoParams: prepareJsonAsParameter(newObj)
				},
			
			/*new Ajax.Request(contextPath + "/"+controller+"?action="+action+
					"&urlParNbtPolFlag=" + nbtPolFlag + "&urlParProrateSw=" + prorateSw + "&urlParEndtCancellation=" + endtCancellation +
					"&urlParCoiCancellation=" + coiCancellation + "&creditingBranch=" + creditingBranch +
					"&parameters=" + encodeURIComponent(JSON.stringify(objParameters))+Form.serialize("endtBasicInformationForm"), {
				method : "POST",
				//postBody : Form.serialize("endtBasicInformationForm"),
				parameters:{
						...
						if (checkErrorOnResponse(response)) {
							if(response.responseText != "SUCCESS"){
								...
								showWaitingMessageBox(objCommonMessage.SUCCESS, imgMessage.SUCCESS, 
									function() {
										updateParParameters(); // andrew - 05.01.2011
										$("polParId") ? $("polParId").remove() :null; // andrew - 05.01.2011
										getBasicInfo(); 
									});
							}						
						}
			...
			showErrorMessage("saveEndorsementBasicInfo", e);
		}
	}

	function parseOtherDetails(){
		objUW.hidObjGIPIS031.gipiWPolGenin.parId	= $F("parId");
		objUW.hidObjGIPIS031.gipiWPolGenin.genInfo 	= ($F("generalInformation"));
		objUW.hidObjGIPIS031.gipiWEndtText.parId	= $F("parId");
		//objUW.hidObjGIPIS031.gipiWEndtText.endtTax	= escapeHTML2($F("b360EndtTax"));
		objUW.hidObjGIPIS031.gipiWEndtText.endtText	= ($F("endtInformation"));
		for(var index=1; index <=17; index++){
			objUW.hidObjGIPIS031.gipiWPolGenin["genInfo"+ index.toPaddedString(2)] = ($F("generalInformation").substr((index-1)*2000, 2000));
			objUW.hidObjGIPIS031.gipiWEndtText["endtText"+ index.toPaddedString(2)] = ($F("endtInformation").substr((index-1)*2000, 2000));
	...
	*/
	...
	$("btnCancelEndt").observe("click", function(){
		...
		if($F("varCancellationFlag") == "Y"){
			showMessageBox("Only one endorsement record can be cancelled per endorsement PAR.", imgMessage.ERROR);
			return false;
		}

		if($F("b540PolFlag") == "4" && ($("endtCancellation").checked && $("coiCancellation").checked)){
		...
		if($F("recExistsInGipiWItmperl") == "Y"){
			$("processStatus").value = "cancelButton";
			existSw = "Y";
			showConfirmBox("Endorsement", "Existing item and perils for this PAR would be deleted. Do you want to continue? ", "Yes", "No", continueProcess2, stopProcess2);
		}
	});
	...
	function continueCancellation(){
		...
		if(existSw == "N"){
			if($F("recExistsInGipiWItem") == "Y"){
				$("processStatus").value = "continueCancellation";
				showConfirmBox("Endorsement", "Existing item(s) for this PAR would be deleted. Do you want to continue? ", "Yes", "No", continueProcess2, stopProcess2);
			}
		}else{
			continueProcess2();
		}
	...
	function continueCancellation1(){		
		...
		setCommonFields();
		showOverlayContent(contextPath+"/GIPIParInformationController?action=showRecordsForCancellation&parId=" + $F("globalParId") +
		...
		/*Modalbox.show(contextPath+"/GIPIParInformationController?action=showRecordsForCancellation&parId=" + $F("globalParId") +
				"&lineCd=" + lineCd + "&sublineCd=" + sublineCd + "&issCd=" + issCd + "&issueYy=" + 
				issueYy + "&polSeqNo=" + polSeqNo + "&renewNo=" + renewNo + "&flag=" + flag, {
			title: "Mortgagee Information",
			width: 600
	...
	function continueCheck(){
		...
		//var inceptDate = dateFormat($F("recordStatus") > 0 ? $F("doi") : $F("b540InceptDate"), "mm-dd-yyyy HH:MM:ss");
		//var effDate = dateFormat($F("recordStatus") > 0 ? $F("endtEffDate") : $F("b540EffDate"), "mm-dd-yyyy HH:MM:ss");
		
		var inceptDate = dateFormat($F("b540InceptDate"), "mm-dd-yyyy HH:MM:ss");
		var effDate = dateFormat($F("b540EffDate"), "mm-dd-yyyy HH:MM:ss");
		...
		if(flag == "polFlag" | flag == "prorateFlag"){
			...
			*/
			var itemCount = objUW.hidObjGIPIS031.gipiWItem.length;
			var perilCount = objUW.hidObjGIPIS031.gipiWItmperl.length;
			var messageText = "";
	...
	function createNegatedRecords(){
		...
		new Ajax.Request(contextPath + "/GIPIParInformationController?action=createNegatedRecords", {
			method : "GET",
			parameters : {
				parId : $F("globalParId"),
				lineCd : $F("globalLineCd"),
			...
			onComplete : function(response){
				...
				if (checkErrorOnResponse(response)){
					...
					if(nvl(result.msgAlert,null) == null){										
						$("endtEffDate").value = nvl(result.effDate,null) != null ? (result.effDate).substr(0,10): $F("endtEffDate");
						$("b540EffDate").value = nvl(result.effDate,null) != null ? (result.effDate) : $F("b540EffDate");						
						...
						$("doi").value = nvl(result.inceptDate,null) != null ? (result.inceptDate).substr(0,10) : $F("doi");
						$("b540InceptDate").value = nvl(result.inceptDate,null) != null ? (result.inceptDate) : $F("b540InceptDate");
						$("doe").value = nvl(result.expiryDate,null) != null ? (result.expiryDate).substr(0,10) : $F("doe");
						$("b540ExpiryDate").value = nvl(result.expiryDate,null) != null ? (result.expiryDate) : $F("b540ExpiryDate");						
						$("endtExpDate").value = nvl(result.endtExpiryDate,null) != null ? (result.endtExpiryDate).substr(0,10) : $F("endtExpDate");
						$("b540EndtExpiryDate").value = nvl(result.endtExpiryDate,null) != null ? (result.endtExpiryDate) : $F("b540EndtExpiryDate");						
	...
	function commitSwitchHandler(){
		try{
			...
			showErrorMessage("commitSwitchHandler", e);
		}	
	}
	...
	function continuePreValidation2(){
		try{
			...
			showErrorMessage("continuePreValidation2", e);
		}
	}	
	...
	function getRequiredFields(){
		...
		$$("select").each(function(elem){
			if(elem.hasClassName("required")){
				if(elem.getAttribute("id") != "inputDeductible1"){
					requiredFields = requiredFields + elem.getAttribute("id");
					requiredFields = requiredFields + " ";
				}				
			}
	...
	$("reloadForm").observe("click", function() {
		if($("message").innerHTML != "SUCCESS"){
			...
			if (changeTag == 1){		
				showConfirmBox("Confirmation", objCommonMessage.WITH_CHANGES, "Yes", "No", saveEndorsementBasicInfo, getBasicInfo);
			} else {
				getBasicInfo();
			}
		}		
