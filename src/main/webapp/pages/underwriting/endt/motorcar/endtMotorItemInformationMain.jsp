...
<div id="endtItemInformationMainDiv" name="endtItemInformationMainDiv" style="margin-top: 1px;">	
	<form id="itemInformationForm" name="itemInformationForm">
		...
		<jsp:include page="/pages/underwriting/endt/motorcar/subPages/endtMotorItemInformation.jsp"></jsp:include>
		
		<!-- end of item information-->
		<div id="addDeleteItemDiv" style="display: none;">
		</div>
		<c:forEach var="towing" items="${towingParams}" varStatus="ctr">
			<div id="towingParams${ctr.index }" name="towingParams">
				<input type="hidden" id="towingParamName${ctr.index }" name="towingParamValueName" value="${towing.paramName }" />
				<input type="hidden" id="towingParamN${ctr.index }" name="towingParamValueN" value="${towing.paramValueN }" />
			</div>
		</c:forEach>
		<c:forEach var="plateNo" items="${plateNoParams}" varStatus="ctr">
			<div id="plateNoParams${ctr.index }" name="plateNoParams">
				<input type="hidden" id="plateNoParamName${ctr.index }" name="plateNoParamValueName" value="${towing.paramName }" />
				<input type="hidden" id="plateNoParamV${ctr.index }" name="plateNoParamValueV" value="${towing.paramValueV }" />
			</div>
		</c:forEach> 
	
		<jsp:include page="/pages/underwriting/subPages/endtMotorItemInformationAdditional.jsp"></jsp:include>
		...
		<div id="accessoryPopups">
			...
			<div id="accessory" class="sectionDiv" style="display: none;"></div>
		</div>
		<div id="outerDiv" name="outerDiv">
...
<script type="text/javascript" defer="defer">	
	...
	$("showPerilInfoSubPage").observe("click", function() { // added by andrew 07.08.2010
		try {
			...
			showErrorMessage("endtMotorItemInformationMain.jsp - showPerilInfoSubPage", e);
	...
	$("showDeductible2").observe("click", function() { // added by andrew 08.09.2010
		try {
			...
			showErrorMessage("endtMotorItemInformationMain.jsp - showDeductible2", e);
	...
	$("showDeductible3").observe("click", function() { // added by andrew 07.08.2010
		try {
			...
			showErrorMessage("endtMotorItemInformationMain.jsp - showDeductible3", e);
			//showMessageBox("showPerilDeductible : " + e.message);
		}
	});
		...
		/*if(itemNos.trim() == "" && delItemNos.trim() == "" && mortgageeItemNos.trim() == "" && deductibleItemNos.trim() == "" &&
			...
			//if(itemNos.trim() != "" || delItemNos.trim() != ""){
				...
				if($F("tempVariable") == "SUCCESS"){
					...
					var controller = "GIPIParMCItemInformationController";
					var action = "saveGipiParVehicle";
