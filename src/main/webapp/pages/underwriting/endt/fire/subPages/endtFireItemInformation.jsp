	...
	<div style="margin: 10px;" id="parItemForm">
		...
		<table width="100%" cellspacing="1" border="0">					
			...
			<tr>
				...
				<td class="rightAligned" style="width: 20%;display: none">Region </td>
...
//             	$(itemFields[index]).value =  row.down("input", index+1).value;
				...
				new Ajax.Request(contextPath + "/GIPIWItemController?action=getDistinctItemNos",{
					...
					onComplete : function(response){
						if (checkErrorOnResponse(response)) {
							...
							if (size < 1) {
								showMessageBox("All item(s) for this policy is already inserted.", imgMessage.INFO);
								return false;
							}
	...
	if ($F("globalLineCd") == "FI") {
		...
		function filterCityByProvince(paramVal, province){		
			new Ajax.Updater($("city").up("td", 0), contextPath+"/GIPIWFireItmController?action=filterCityByProvince", {
				evalScripts: true,
				asynchronous: false,
				method: "GET",
				onCreate: function () {
					$("city").up("td", 0).update("<span style='font-size: 9px;'>Refreshing...</span>");
					showNotice("Refreshing city list, please wait...");
					$("notice").show();	
				},
				parameters: {
					globalParId : $F("globalParId"),
					province: province /*$("province").options[$("province").selectedIndex].text*/,
					elementWidth : $("city").getWidth()	
				},
				onComplete : function(){
					$("city").value = paramVal;
					$("isLoaded").value = 0;
		...
		function filterDistrictByProvinceByCity(paramVal, province, city){
			new Ajax.Updater($("district").up("td", 0), contextPath+"/GIPIWFireItmController?action=filterDistrictByProvinceByCity", {
				...
				onCreate: function () {
					...
					showNotice("Refreshing district list, please wait...");
				},
				parameters: {
					globalParId : $F("globalParId"),
					province: province /*$("province").options[$("province").selectedIndex].text*/,
		...
		function filterBlock (paramVal, province, city, district){
			new Ajax.Updater($("block").up("td", 0), contextPath+"/GIPIWFireItmController?action=filterBlock", {
				...
				onCreate: function () {
					...
					showNotice("Refreshing block list, please wait...");
				},
				parameters: {
					globalParId : $F("globalParId"),
					province: province /*$("province").options[$("province").selectedIndex].text*/,
					city: city /*$("city").options[$("city").selectedIndex].text*/,
					district: district /*$("district").options[$("district").selectedIndex].text*/,
		...
		function filterRisk(paramVal, block){
			new Ajax.Updater($("risk").up("td", 0), contextPath+"/GIPIWFireItmController?action=filterRisk", {
				...
				onCreate: function () {
					...
					showNotice("Refreshing risk list, please wait...");
				},
				parameters: {
					globalParId : $F("globalParId"),
					block: block /*$("block").options[$("block").selectedIndex].text*/,
