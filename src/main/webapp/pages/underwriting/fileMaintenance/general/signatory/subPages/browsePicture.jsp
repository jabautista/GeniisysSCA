<div class="sectionDiv" style="padding: 5px; margin-top: 10px; margin-bottom: 10px; float: left; width: 340px;">
	<div id="browsePictureDiv" style="width: 100%; padding-top: 10px;">
		<form id="uploadForm" name="uploadForm" enctype="multipart/form-data" method="POST" action="" target="uploadTarget">	
			...
			<input type="hidden" id="signatoryId" name="signatoryId" value="${signatoryId}">
...
<script>
	...
	$("btnUpload").observe("click", function () {
		if($F("file") == ''){
			showMessageBox("No file to be uploaded", imgMessage.ERROR);
		}else{
	...
	function uploadFile(){
		try{
			...
			$("uploadForm").action = "GIISSignatoryController?action=uploadFile&"+ fixTildeProblem(Form.serialize("uploadForm")+"&signatoryId="+$F("signatoryId"));
			$("uploadForm").submit();
			
			
		    updater = new Ajax.PeriodicalUpdater('updaterDiv','GIISSignatoryController', {
		        frequency: 0.1,
		        method: "GET",
