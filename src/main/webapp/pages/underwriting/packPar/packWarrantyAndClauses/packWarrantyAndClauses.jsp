...
<div id="packWarrantyAndClauseMainDiv" name="packWarrantyAndClauseMainDiv"	style="margin-top: 1px; display: none;">
	...
	<form id="packWarrantyAndClauseForm" name="packWarrantyAndClauseForm">
		...
		<div id="packWarrantyAndClauseDiv" name="packWarrantyAndClauseDiv" class="sectionDiv">
			<jsp:include page="/pages/underwriting/packPar/packWarrantyAndClauses/packWarrantyAndClausesTable.jsp"></jsp:include>
...
<script type="text/javascript">
	...
	var changeTextSw = "N";
	var lastRowNo = 0;
	var objWC = JSON.parse('${warranty}'/* .replace(/\\/g, '\\\\') */);
	...
	parType = '${packParType}';
	changeTag = 0;
	initializeChangeTagBehavior(saveGIPIPackWPolWC);
	
	if(objWC.length > 0){
		for(var i=0; i<objWC.length; i++){
			var rowContent = generateWCRow(objWC[i], i);
			addTableRow("wcRow"+objWC[i].parId+objWC[i].wcCd, "wcRow","packWCList", rowContent, clickWCRow);
			$("wcRow"+objWC[i].parId+objWC[i].wcCd).setAttribute("parId", objWC[i].parId);
		...
		checkIfToResizeTable2("packWCList", "wcRow");
	}else{
		checkTableIfEmpty2("wcRow", "packWCList");
	}

	initializeSubPagesTableListing("packWCList", "wcRow");
	
	if(parType == "P") {
		setDocumentTitle("Package Policy - Warranties and Clauses");
	...
	function clickWCRow(row){
		try{
			...
			//currentRow = parseInt(row.down("input", 0).value); replaced by: Nica 11.21.2011
			currentRow = row.getAttribute("index");
	
			row.toggleClassName("selectedRow");
			if(row.hasClassName("selectedRow")){
				populatePackWarrantyAndClauseForm(objWC[currentRow]);
	...
	function populatePackWarrantyAndClauseForm(objWC){
		try{
			//$("selectWarrantyTitle").value 	= objWC.wcCd == null ? "" : objWC.wcCd;			
			//$("warrantyTitleDisplay").value = objWC.wcTitle == null ? "" : changeSingleAndDoubleQuotes(objWC.wcTitle);
			...
			showErrorMessage("populatePackWarrantyAndClauseForm", e);
		}
	}
	...
	function synchWCListDetails(){
		try{
			...
			var selectedPol = getSelectedPolicy();
			var max = 0;
	
			if(title.value != null || title.value != ""){
				for(var i=0; i<objWC.length; i++){
					if(selectedPol == objWC[i].parId && parseInt(objWC[i].printSeqNo) > max && objWC[i].recordStatus != -1){
	...
	function setWCObject(recStatus, recFlag){
		try{
			...
			var policyId   = getSelectedPolicy();
			var lineCd	= getSelectedPolicyLineCd();
			//var selectedWCTitle = //$("selectWarrantyTitle").options[$("selectWarrantyTitle").selectedIndex];
			var wcText = $("inputChangeTag").checked ? $F("inputWarrantyText"): $F("hidOrigWarrantyText"); //selectedWCTitle.getAttribute("wcText");  
			
			objWarrCla.packParId = objUWGlobal.packParId;
			objWarrCla.wcCd 	= changeSingleAndDoubleQuotes2($F("hidWcCd"));
			objWarrCla.parId	= parseInt(policyId);
			objWarrCla.lineCd 	= changeSingleAndDoubleQuotes2(lineCd);
			objWarrCla.wcSw 	= changeSingleAndDoubleQuotes2($("inputWarrantyType").value);
			objWarrCla.wcTitle 	= changeSingleAndDoubleQuotes2($F("txtWarrantyTitle")/* $("selectWarrantyTitle").options[$("selectWarrantyTitle").selectedIndex].text */);
	...
	function resetText() {
		try{
			...
			if($F("inputWarrantyText") != ""){
				defaultText = $F("hidOrigWarrantyText");//$("selectWarrantyTitle").options[$("selectWarrantyTitle").selectedIndex].getAttribute("wcText");
			}
...
/* 	$("selectWarrantyTitle").observe("change", function(){
		if(checkIfWarrantyTitleExist(this.value)){
			synchWCListDetails();
		}
	}); */
...
/* 	$("selectWarrantyTitle").observe("keypress", function(evt){
	...
	$("inputWarrantyText").observe("change", function(){
		if(!$("inputChangeTag").checked){
			showConfirmBox("Confirm", "Do you really want to change this text?", "Yes", "No",
							...
							function(){
								$("inputWarrantyText").value = $F("hidOrigWarrantyText");//$("selectWarrantyTitle").options[$("selectWarrantyTitle").selectedIndex].getAttribute("wcText");
								limitText($("inputWarrantyText"), 34000);
							});
		}else{
			limitText($("inputWarrantyText"), 34000);
	...
	$("btnAdd").observe("click", function(){
		...
		if(checkWCRequiredFields() && checkRemarksLength() && checkWarrantyTextLength()){
			changeTag = 1;
			if(this.value == "Add"){
				var recFlag = checkExistWPolwcPolwc();
				var objAdded = setWCObject(0, recFlag);
				if(checkIfWarrantyTitleExist(objAdded.wcCd)){
					objWC.push(objAdded);
					rowContent = generateWCRow(objAdded,lastRowNo);
					addTableRow("wcRow"+objAdded.parId+objAdded.wcCd, "wcRow","packWCList", rowContent, clickWCRow);
					$("wcRow"+objAdded.parId+objAdded.wcCd).setAttribute("parId", objAdded.parId);
				...
				for(var i=0; i<objWC.length; i++){
					if(objModified.parId == objWC[i].parId && objModified.wcCd == objWC[i].wcCd){
						...
						$("wcRow"+objModified.parId+objModified.wcCd).setAttribute("parId", objModified.parId);
	...
	$("btnDelete").observe("click", function(){
		...
		objWC[currentRow].packParId = objUWGlobal.packParId;
		row.remove();
		row.removeClassName("selectedRow");
		checkTableIfEmpty2("wcRow", "packWCList");
		checkIfToResizeTable2("packWCList", "wcRow");
		resetWarrAndClauseFormFields();
		//showDeletedWCOpt(row.id);
	});
	...
	$("btnCancel").observe("click", function(){
		...
		/*if(changeTag == 1){
			pAction = pageActions.cancel;
			showConfirmBox4("Confirmation", objCommonMessage.WITH_CHANGES, "Yes", "No", "Cancel", saveGIPIPackWPolWC,
							function(){
								fireEvent($("parExit"), "click");
							}, "");
		}else{
	...
	/* function hideAddedWCOpt(){
		try{
			var wcOpt = $("selectWarrantyTitle").options;
			var policy = getSelectedPolicy();
			
			$$("div[name='wcRow']").pluck("id").each(function(row){
				for(var i=0; i<wcOpt.length; i++){
					if(row.substring(5) == (policy+wcOpt[i].value)){
						wcOpt[i].hide();
			...
			showErrorMessage("hideAddedWCOpt", e);
		}
	}
 */
	/* function showDeletedWCOpt(row){
		try{
			var wcOpt = $("selectWarrantyTitle").options;
			var policy = getSelectedPolicy();
			...
			showErrorMessage("showDeletedWCOpt", e);
		}
	}
 */
	function checkIfPrintSeqNoExist(printSeqNo){
		try{
			var exist = false;
			var parId = getSelectedPolicy();
	
			for(var i=0; i<objWC.length; i++){
				if(objWC[i].parId == parId && printSeqNo == objWC[i].printSeqNo && objWC[i].recordStatus != -1){
					exist = true;
				}
			}
			return exist;
	...
	function checkIfWarrantyTitleExist(wcCd){
		try{
			...
			for(var i=0; i<objWC.length; i++){
				if(objWC[i].parId == parId && objWC[i].wcCd == wcCd && objWC[i].recordStatus != -1){
	...
	function saveGIPIPackWPolWC(){
		try{
			...
			new Ajax.Request(contextPath+"/GIPIPackWarrantyAndClausesController?action=savePackWPolWC",{
				method: "POST",
				asynchronous: true,
				parameters:{
					...
					if(checkErrorOnResponse(response)){
						if(response.responseText == "SUCCESS"){
							if(pAction == pageActions.save){
								showWaitingMessageBox(objCommonMessage.SUCCESS, imgMessage.SUCCESS, showWPackWarrantyAndClausePage);
							}else if(pAction == pageActions.reload){
								showWaitingMessageBox(objCommonMessage.SUCCESS, imgMessage.SUCCESS, showWPackWarrantyAndClausePage);
							}else{
								showWaitingMessageBox(objCommonMessage.SUCCESS, imgMessage.SUCCESS, goBackToPackagePARListing);
							}
			...
			showErrorMessage("saveGIPIPackWPolWC", e);
		}
	}

	function checkExistWPolwcPolwc(){
		try{
			var recFlag = "A";
			var parId = getSelectedPolicy();
			var lineCd = getSelectedPolicyLineCd();
			var sublineCd = $("polRow"+parId).getAttribute("sublineCd");
			var issCd = $("polRow"+parId).getAttribute("issCd");
			var issueYy = $("polRow"+parId).getAttribute("issueYy");
			var polSeqNo = $("polRow"+parId).getAttribute("polSeqNo");
			var wcCd = $F("hidWcCd");//$("selectWarrantyTitle").options[$("selectWarrantyTitle").selectedIndex].value;
			var swcSeqNo = 0;
			
			new Ajax.Request(contextPath+ "/GIPIPackWarrantyAndClausesController?action=checkExistWPolwcPolwc", {
						method: "POST",
						parameters:{
							lineCd: 	lineCd,
							sublineCd: 	sublineCd,
			...
			showErrorMessage("checkExistWPolwcPolwc", e);
		}
	}
	...
	$$("div[name='polRow']").each(function(row){
		...
		row.observe("click", function(){
			...
			synchronizeWarrAndClausePage(row);
			resetWarrAndClauseFormFields();
			//$("selectWarrantyTitle").update("");;
			/* if(row.hasClassName("selectedRow")){
				//getWarrantyListing(row.getAttribute("lineCd")); // andrew - 10.20.2011
			} */

			$$("div[name='wcRow']").each(function(wcRow){
				wcRow.removeClassName("selectedRow");
			});

		});
	});

	function synchronizeWarrAndClausePage(row){
		$$("div[name='wcRow']").each(function(wc){
			var parId = wc.getAttribute("parId");
			var id = wc.getAttribute("id");
			$(id).hide();
			if(row.getAttribute("parId") == parId && row.hasClassName("selectedRow")){
				$(id).show();
			}
		});
		checkTableIfEmpty2("wcRow", "packWCList");
	...
	/* function getWarrantyListing(lineCd){ // andrew - 10.20.2011
		new Ajax.Request(contextPath+"/GIPIPackWarrantyAndClausesController", {
			method: "GET",
			parameters: {
				action: "getWarrantyListing",
				lineCd: lineCd
			},
			onCreate:function(){
				setCursor("wait");
			...
			onComplete:function(response){
				...
				generateWCTitleOptions(optList);
			}
		});
	} */

	/* function generateWCTitleOptions(list){
		//$("selectWarrantyTitle").update("");
		var opt = '<option value=""/>';
		for(var i=0; i<list.length; i++){
			var wcCd = nvl(list[i].wcCd, "");
			var wcSw = nvl(list[i].wcSw, "");
			var printSw = nvl(list[i].printSw, "N");
			var wcText = nvl(list[i].wcText, "");
			var changeTag = nvl(list[i].changeTag, "N");
			var wcTitle = nvl(list[i].wcTitle, "");
	...
	$("searchWarrantyTitle").observe("click", function(){ // andrew - 10.20.2011
		var notIn = "";
		var withPrevious = false;
		$$("div#packWCList div[name='wcRow']").each(function(row){
		...
		notIn = (notIn != "" ? "("+notIn+")" : "");
		showWarrantyAndClauseLOV(getSelectedPolicyLineCd(), notIn);		
	});
</script>
