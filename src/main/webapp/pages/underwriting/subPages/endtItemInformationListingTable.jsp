            ...
            <div id="parItemTableContainer" class="tableContainer">
                ...
                <c:forEach var="item" items="${items}" varStatus="ctr">
                    <div id="row${item.itemNo}" name="row" class="tableRow" style="padding-left:1px;" /> <!-- style="height: 20px; border-bottom: 1px solid #E0E0E0; padding-top: 10px;"> -->
						...
						<input type="hidden" name="masterItemGrps" 			value="${item.itemGrp}" />
						<input type="hidden" name="masterItemDescs" 		value="${item.itemDesc}" />
						<input type="hidden" name="masterItemDesc2s" 		value="${item.itemDesc2}" />
						<input type="hidden" name="masterTsiAmts" 			value="${item.tsiAmt}" />
						<input type="hidden" name="masterPremAmts" 			value="${item.premAmt}" />
						<input type="hidden" name="masterAnnPremAmts" 		value="${item.annPremAmt}" />
						<input type="hidden" name="masterAnnTsiAmts" 		value="${item.annTsiAmt}" />
						<input type="hidden" name="masterRecFlags" 			value="${item.recFlag}" />
						<input type="hidden" name="masterCurrencyCds" 		value="${item.currencyCd}" />
						<input type="hidden" name="masterCurrencyRts" 		value="${item.currencyRt}" />
						<input type="hidden" name="masterGroupCds" 			value="${item.groupCd}" />
						...
						<input type="hidden" name="masterPackLineCds" 		value="${item.packLineCd}" />
						<input type="hidden" name="masterPackSublineCds" 	value="${item.packSublineCd}" />
						<input type="hidden" name="masterDiscountSws" 		value="${item.discountSw}" />
						<input type="hidden" name="masterCoverageCds" 		value="${item.coverageCd}" />
						<input type="hidden" name="masterOtherInfos" 		value="${item.otherInfo}" />
						<input type="hidden" name="masterSurchargeSws" 		value="${item.surchargeSw}" />
						<input type="hidden" name="masterRegionCds" 		value="${item.regionCd}" />
						<input type="hidden" name="masterChangedTags" 		value="${item.changedTag}" />
						<input type="hidden" name="masterProrateFlags" 		value="${item.prorateFlag}" />
						<input type="hidden" name="masterCompSws" 			value="${item.compSw}" />
						<input type="hidden" name="masterShortRtPercents" 	value="${item.shortRtPercent}" />
						<input type="hidden" name="masterPackBenCds" 		value="${item.packBenCd}" />
						<input type="hidden" name="masterPaytTermss" 		value="${item.paytTerms}" />
						<input type="hidden" name="masterRiskNos" 			value="${item.riskNo}" />
						<input type="hidden" name="masterRiskItemNos" 		value="${item.riskItemNo}" />                 
                        ...
                        <label name="textItem" style="width: 20%; text-align: left;">${item.itemTitle}<c:if test="${empty item.itemTitle}">---</c:if></label>
                        <label name="textItem" style="width: 20%; text-align: left;">${item.itemDesc}<c:if test="${empty item.itemDesc}">---</c:if></label>
                        <label name="textItem" style="width: 20%; text-align: left;">${item.itemDesc2}<c:if test="${empty item.itemDesc2}">---</c:if></label>
                        <label name="textItem" style="width: 10%; text-align: left;">${item.currencyDesc}<c:if test="${empty item.currencyDesc}">---</c:if></label>
                        ...
                        <label name="textItem" style="text-align: left;">${item.coverageDesc}<c:if test="${empty item.coverageDesc}">---</c:if></label>
                    </div>
                </c:forEach>               
            </div>                    
        </div>       
