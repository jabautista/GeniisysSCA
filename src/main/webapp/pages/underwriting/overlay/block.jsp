			...
			<div class="tableContainer" id="blockListing" name="blockListing" style="height: 310px; overflow: auto;">
				<c:forEach var="block" items="${blockListing}">
					<div id="${block.provinceCd}_${block.cityCd}_${block.districtNo}_${block.blockId}"  name="rowBlock" class="tableRow"
						...
						eqZone="${block.eqZone}" eqDesc="${block.eqDesc}" typhoonZone="${block.typhoonZone}" typhoonZoneDesc="${block.typhoonZoneDesc}" floodZone="${block.floodZone}" floodZoneDesc="${block.floodZoneDesc}">
...
<script type="text/javascript">		
	...
	$("btnBlockOk").observe("click", function(){		
		if($$("div#blockListing .selectedRow").length > 0){
			...
			//}else if(column == "block"){
				...
				$("blockId").value 		= row.getAttribute("blockId");
				$("eqZone").value 		= row.getAttribute("eqZone");
				$("eqZoneDesc").value 	= row.getAttribute("eqDesc");
				$("typhoonZone").value 	= row.getAttribute("typhoonZone");
				$("typhoonZoneDesc").value 	= row.getAttribute("typhoonZoneDesc");
				$("floodZone").value 	= row.getAttribute("floodZone");
				$("floodZoneDesc").value = row.getAttribute("floodZoneDesc");
	...
	$("filterColumn").observe("change", function(){
		fireEvent($("filterContent"), "keyup");
	});
