...
<div id="assumedTabMainDiv" class="" style="float: left; padding: 15px 0 5px 0">
	...
	<div id="reportLayoutDiv" name="reportLayoutDiv" class="sectionDiv" style="padding-top: 5px; width: 280px; height: 180px;" align="center">
		<table id="reportLayoutTbl" align="center" style="margin-top: 25px;">
			<tr>
				<td style="padding: 0 0 25px 20px;">Report Layout</td>
			</tr>
			<tr>
				<td>
					<input id="postedRB" name="rptLayoutRG" type="radio" value="Posted" style="float: left; margin: 2px 5px 10px 0px;" checked="checked"><label for="postedRB" style="margin: 2px 0 4px 0">Posted</label>
				</td>
			</tr>
			<tr>
				<td id="comparativeTD">
					<input id="comparativeRB" name="rptLayoutRG" type="radio" value="Comparative" style="float: left; margin: 2px 5px 4px 0px;"><label for="comparativeRB" style="margin: 2px 0 4px 0">Comparative Study</label>
				</td>
			</tr>
		</table>
	</div>
	...
	<div class="sectionDiv" id="printDialogFormDiv" style="margin-left: 15px; margin-bottom: 5px; float: left; width: 890px; height: 251px;" >
		...
		<table style="float: left; padding: 65px 0 0 50px;">
			<tr><td><input type="button" class="button disabledButton" style="width: 100px; margin-left: 15px;" id="btnGenerate" name="btnGenerate" value="Generate"></td></tr>
...
<script type="text/javascript">
	...
	observeReloadForm("reloadForm", function(){showGenerateRIReportsTabPage("assumedTab");});
	...
	$("postedRB").observe("click", function(){
		disableButton("btnGenerate");
	});
	
	//$("comparativeTD").hide();
	
	$("comparativeRB").observe("click", function(){
	...
	$("btnGenerate").observe("click", function(){
		//reportMonth and reportYear parameter
		//call GIRIS104
		//showMessageBox("GIRIS104 does not exist.", "I");	//error FRM-40010: cannot read form GIRIS104 in CS
	});
	
	$("btnPrint").observe("click", function(){
		objRiReports.rep_type_sw = 3;
		
		if ($("comparativeRB").checked && $F("selReportMonth") == ""){
			showMessageBox("Invalid value for field REPORT MONTH", "E");
		}else{
			printAssumedReport();
		}		
	});
	
	function printAssumedReport(){
		if($F("selDestination") == "PRINTER" && ($("selPrinter").value == "" || $("txtNoOfCopies").value == "")){
			...
			if($("postedRB").checked){
				reportId = "GIRIR103";
				reportTitle = "Assumed Premium Production (Posted)";
			}else if($("comparativeRB").checked){
				reportId = "GIRIR104";
				reportTitle = "Assumed Production Comparative Study";
			}
			
			var content = contextPath+"/UWReinsuranceReportPrintController?action=printUWRiAssumedReport&reportId="+reportId+
