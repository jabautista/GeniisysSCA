...
<script type="text/javascript">
	...
	$("btnPrint").observe("click", function(){
		if(validatePrint()){
			var binderNo = $F("txtBinderNo").split("-");
			var lineCd = binderNo[0];
			var binderYy = binderNo[1];
			var binderSeqNo = binderNo[2];
			printRIBinder(lineCd, binderYy, binderSeqNo);
		}
	});
	...
	function printRIBinder(lineCd, binderYy, binderSeqNo){
		var content = contextPath+"/PrintRiBinderReportController?action=printRIBinderReport&lineCd="+lineCd+"&binderYy="+binderYy+"&binderSeqNo="+binderSeqNo+ "&noOfCopies=" 
				+ $F("txtNoOfCopies") + "&printerName=" + $F("selPrinter") + "&destination=" + $F("selDestination");
			if($F("selDestination") == "screen"){
				showPdfReport(content, "RI Binder Report");
