	...
	<form id="prelimLossRepForm" name="prelimLossRepForm">
		...
		<div id="groDivBot" name="groDivBot">
			<div id="itemInfoDiv" name="itemInfoDiv" class="sectionDiv" style="height: 225px; padding: 15px 15px 15px 15px; width: 891px;">
				<div id="hullTableGridDiv" name="hullTableGridDiv" style="height: 220px;"></div>
			</div>
		</div>
		<c:if test="${prelim=='N'}">
			<div id="outerDiv" name="outerDiv" style="width: 100%; margin-bottom: 1px;">
...
<script>
	...
	var objTGHullInfoDetails = JSON.parse('${mhInfoTableGrid}'.replace(/\\/g,'\\\\'));
	try{
		var hullInfoModel = {
			...
			columnModel:[
  				...
  				rows: objTGHullInfoDetails.rows,
  				id: 1122
		};
		itemInfoTableGrid = new MyTableGrid(hullInfoModel);
		itemInfoTableGrid.pager = objTGHullInfoDetails;
		itemInfoTableGrid.render('hullTableGridDiv');
	}catch(e){
		showMessageBox("Error in Marine Hull Item Information: " + e, imgMessage.ERROR);		
	}
