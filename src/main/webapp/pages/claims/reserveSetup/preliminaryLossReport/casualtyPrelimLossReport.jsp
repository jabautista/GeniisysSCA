	...
	<form id="prelimLossRepForm" name="prelimLossRepForm">
		...
		<div id="groDivBot" name="groDivBot">
			<div id="itemInfoDiv" name="itemInfoDiv" class="sectionDiv" style="height: 225px; padding: 15px 15px 15px 15px; width: 891px;">
				<div id="casualtyItemInfoTableGrid" name="casualtyItemInfoTableGrid" style="height: 220px;"></div>
			</div>
		</div>
		<c:if test="${prelim=='N'}">
			<div id="outerDiv" name="outerDiv" style="width: 100%; margin-bottom: 1px;">
...
<script>
	...
	var objTGCasualtyInfoDetails = JSON.parse('${casualtyInfoTableGrid}'.replace(/\\/g,'\\\\'));
	try{
		var casualtyInfoModel = {
			...
			columnModel:[
  				...
  				rows: objTGCasualtyInfoDetails.rows,
  				id: 1126
		};
		itemInfoTableGrid = new MyTableGrid(casualtyInfoModel);
		itemInfoTableGrid.pager = objTGCasualtyInfoDetails;
		itemInfoTableGrid.render('casualtyItemInfoTableGrid');
	}catch(e){
		showMessageBox("Error in Casualty Item Information: " + e, imgMessage.ERROR);		
	}
