	...
	<form id="prelimLossRepForm" name="prelimLossRepForm">
		...
		<div id="groDivBot" name="groDivBot">
			<div id="itemInfoDiv" name="itemInfoDiv" class="sectionDiv" style="height: 225px; padding: 15px 15px 15px 15px; width: 891px;">
				<div id="engItemInfoTableGrid" name="engItemInfoTableGrid" style="height: 220px;"></div>
			</div>
		</div>
		<c:if test="${prelim=='N'}">
			<div id="outerDiv" name="outerDiv" style="width: 100%; margin-bottom: 1px;">
...
<script>
	...
	var objTGEngInfoDetails = JSON.parse('${engInfoTableGrid}'.replace(/\\/g,'\\\\'));
	try{
		var engInfoModel = {
			...
			columnModel:[
  				...
  				rows: objTGEngInfoDetails.rows,
  				id: 1120
		};
		itemInfoTableGrid = new MyTableGrid(engInfoModel);
		itemInfoTableGrid.pager = objTGEngInfoDetails;
		itemInfoTableGrid.render('engItemInfoTableGrid');
	}catch(e){
		showMessageBox("Error in Engineering Item Information: " + e, imgMessage.ERROR);		
	}
