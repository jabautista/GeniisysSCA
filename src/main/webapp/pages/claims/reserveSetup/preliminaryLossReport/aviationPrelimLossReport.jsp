	...
	<form id="prelimLossRepForm" name="prelimLossRepForm">
		...
		<div id="groDivBot" name="groDivBot">
			<div id="itemInfoDiv" name="itemInfoDiv" class="sectionDiv" style="height: 225px; padding: 15px 15px 15px 15px; width: 891px;">
				<div id="aviationItemInfoTableGrid" name="aviationItemInfoTableGrid" style="height: 220px;"></div>
			</div>
		</div>
		<c:if test="${prelim=='N'}">
			<div id="outerDiv" name="outerDiv" style="width: 100%; margin-bottom: 1px;">
...
<script>
	...
	var objTGAviationInfoDetails = JSON.parse('${aviationInfoTableGrid}'.replace(/\\/g,'\\\\'));
	try{
		var aviationInfoModel = {
			url: contextPath+"/GICLReserveSetupController?action=getItemInformation&refresh=1&claimId=" + objCLMGlobal.claimId + "&menuLineCd=" + nvl(objCLMGlobal.menuLineCd, "") +
			...
			columnModel:[
						...
						{
							id:	'estUtilHrs',
							title: 'Estimated Util. Hours',
							titleAlign: 'center',
							align: 'right',
							width: '122px'
						}
  					],
  				rows: objTGAviationInfoDetails.rows,
  				id: 1114
		};
		itemInfoTableGrid = new MyTableGrid(aviationInfoModel);
		itemInfoTableGrid.pager = objTGAviationInfoDetails;
		itemInfoTableGrid.render('aviationItemInfoTableGrid');
	}catch(e){
		showMessageBox("Error in Aviation Item Information: " + e, imgMessage.ERROR);		
	}
