	...
	<form id="prelimLossRepForm" name="prelimLossRepForm">
		...
		<div id="groDivBot" name="groDivBot">
			<div id="itemInfoDiv" name="itemInfoDiv" class="sectionDiv" style="height: 225px; padding: 15px 15px 15px 15px; width: 891px;">
				<div id="fireItemInfoTableGrid" name="fireItemInfoTableGrid" style="height: 220px;"></div>
			</div>
		</div>
		<c:if test="${prelim=='N'}">
			<div id="outerDiv" name="outerDiv" style="width: 100%; margin-bottom: 1px;">
...
<script>
	...
	var objTGFireInfoDetails = JSON.parse('${fireInfoTableGrid}'.replace(/\\/g,'\\\\'));
	try{
		var fireInfoModel = {
			...
			columnModel:[
  				...
  				rows: objTGFireInfoDetails.rows,
  				id: 1121
		};
		itemInfoTableGrid = new MyTableGrid(fireInfoModel);
		itemInfoTableGrid.pager = objTGFireInfoDetails;
		itemInfoTableGrid.render('fireItemInfoTableGrid');
	}catch(e){
		showMessageBox("Error in Fire Item Information: " + e, imgMessage.ERROR);		
	}
