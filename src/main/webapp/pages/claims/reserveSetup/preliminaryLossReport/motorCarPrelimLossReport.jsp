	...
	<form id="prelimLossRepForm" name="prelimLossRepForm">
		...
		<div id="groDivBot" name="groDivBot">
			<div id="itemInfoDiv" name="itemInfoDiv" class="sectionDiv" style="height: 225px; padding: 15px 15px 15px 15px; width: 891px;">
				<div id="mcItemInfoTableGrid" name="mcItemInfoTableGrid" style="height: 220px;"></div>
			</div>
		</div>
		<c:if test="${prelim=='N'}">
			<div id="outerDiv" name="outerDiv" style="width: 100%; margin-bottom: 1px;">
...
<script>
	...
	var objTGMCInfoDetails = JSON.parse('${mcInfoTableGrid}'.replace(/\\/g,'\\\\'));
	try{
		var mcInfoModel = {
			...
			columnModel:[
  				...
  				rows: objTGMCInfoDetails.rows,
  				id: 1124
		};
		itemInfoTableGrid = new MyTableGrid(mcInfoModel);
		itemInfoTableGrid.pager = objTGMCInfoDetails;
		itemInfoTableGrid.render('mcItemInfoTableGrid');
	}catch(e){
		showMessageBox("Error in Motor Car Item Information: " + e, imgMessage.ERROR);		
	}
