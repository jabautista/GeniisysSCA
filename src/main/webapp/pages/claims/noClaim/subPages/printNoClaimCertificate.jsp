...
<script>
	...
	function populateGICLR026(){
		try {
			var content = contextPath+"/PrintNoClaimCertificateController?action=populateGICLR026&noClaimId="+objCLM.noClaims.noClaimId+"&noClaimNo="+objCLM.noClaims.noClaimNo;
	...
	function populateGICLR026B(){
		try {
			var content = contextPath+"/PrintNoClaimCertificateController?action=populateGICLR026B&noClaimId="+objCLM.noClaims.noClaimId+"&noClaimNo="+objCLM.noClaims.noClaimNo;
	...
	function validateBeforePrint(){
		...
		new Ajax.Request(contextPath+"/GICLNoClaimController?action=getGICLR026Desname&reportId="+objCLM.noClaims.reportId, {
			evalScripts: true,
			asynchronous: false,
			method: "GET",
			onComplete: function(response) {
	...
	$("btnPrint").observe("click", function(){
		if (validateBeforePrint()){
			if(objCLM.noClaims.version == "UCPB" || objCLM.noClaims.versionB == "UCPB"){
				new Ajax.Request(contextPath+"/GICLNoClaimController?action=getSignatoryGICLS026", {
					evalScripts: true,
					asynchronous: false,
					method: "GET",
					parameters: {
							reportId			: objCLM.noClaims.reportId,
							issCd				: objCLM.noClaims.issCd,
							lineCd				: objCLM.noClaims.lineCd
					},
					onComplete: function(response) {
						if (checkErrorOnResponse(response)) {
							...
							if(msg != ""){
								showWaitingMessageBox(msg, "I", function(){
									if(nvl(objCLM.noClaims.menuLineCd, objCLM.noClaims.lineCd) != objCLM.noClaims.lineCdMC){
										populateGICLR026B();
									}else{
										populateGICLR026();
									}
