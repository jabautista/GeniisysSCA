   ...
   BEGIN
      ...
      THEN
         ...
         DELETE FROM giac_function_columns
               WHERE module_id = v_module_id AND function_cd = 'AA';
         ...
         DBMS_OUTPUT.put_line
            ('Record with function_code = AA under GIACS030 is successfully deleted in GIAC_FUNCTIONS.'
            );         
      END IF;
   EXCEPTION
      WHEN NO_DATA_FOUND
      THEN
         DBMS_OUTPUT.put_line
            ('Record with function_code = AA under GIACS030 is not existing in GIAC_FUNCTIONS.'
            );
   END;
END;