...
BEGIN
   MERGE INTO giac_modules a
      USING (SELECT (SELECT MAX (module_id) + 1
                    ...
                    'GIRIS056A' AS module_name,
                    'CREATE INWARD TREATY RECORD' AS scrn_rep_name,
                    'S' AS scrn_rep_tag
               FROM DUAL) b
   ...
   THEN
      COMMIT;
      DBMS_OUTPUT.put_line ('GIRIS056A inserted in GIAC_MODULES.');
   ELSE
      DBMS_OUTPUT.put_line ('GIRIS056A is already existing in GIAC_MODULES.');
   END IF;
END;