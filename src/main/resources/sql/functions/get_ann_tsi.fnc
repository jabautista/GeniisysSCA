DROP FUNCTION CPI.GET_ANN_TSI;

CREATE OR REPLACE FUNCTION CPI.Get_Ann_Tsi
  ...
  */
  ...
  BEGIN
   ...
   /*AND Date_Risk(x.acct_ent_date, TO_DATE(x.eff_date,'dd-mm-yyyy'), TO_DATE(x.issue_date,'dd-mm-yyyy'),x.booking_mth,x.booking_year,
       x.spld_acct_ent_date, x.pol_flag, p_basis, p_from, p_to) = 1*/ -- bonok :: 11.12.2012 to fix tsi_amt when policy has Non-affecting endorsement with eff_date < pol_eff_date 
         --AND DIST_FLAG = '3'
