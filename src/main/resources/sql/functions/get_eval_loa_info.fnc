DROP FUNCTION CPI.GET_EVAL_LOA_INFO;

CREATE OR REPLACE FUNCTION CPI.Get_Eval_Loa_Info (p_claim_id IN gicl_claims.claim_id%TYPE,
...
RETURN VARCHAR2 AS
/* Beth 11032006
** Get evaluation LOA information
*/
     p_loa_dtl  VARCHAR2(700);
  p_loa_amt  NUMBER :=0;
 ...
 END LOOP;
   ...
   FOR get_loa_info IN(
     SELECT 'LOA No. : '||DECODE(d.subline_cd,NULL,NULL,d.subline_cd||'-'||d.iss_cd||'-'||trim(TO_CHAR(d.loa_yy,'09'))||'-'||trim(TO_CHAR(d.loa_seq_no,'00009'))) ||CHR(10)||
   ...
   'LOA Amount : ' || LTRIM(RTRIM(TO_CHAR(p_loa_amt,'999,999,999,990.99'))) loa_info
       FROM giis_payee_class b, giis_payees c, gicl_eval_loa d 
   ...
   LOOP
     p_loa_dtl := get_loa_info.loa_info;
  EXIT;
   END LOOP;
