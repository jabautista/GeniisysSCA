DROP FUNCTION CPI.LOC_DIST_EXISTS;

CREATE OR REPLACE FUNCTION CPI.loc_dist_exists (v_location_cd gipi_casualty_item.location_cd%TYPE, v_loss_date DATE)
   RETURN BOOLEAN
IS
   /*
   ...
   **  Description   : Converted procedure from GICLS024 - loc_dist_exists program unit
   */

BEGIN
   FOR a IN (SELECT   d.share_cd, f.share_type, f.trty_yy, f.acct_trty_type, f.prtfolio_sw, v_location_cd,
                ...
                WHERE f.share_cd = d.share_cd
                  ...
                  AND EXISTS (
                         SELECT 1
                           FROM gipi_polbasic sub_a, gipi_item sub_b, gipi_casualty_item sub_c
             ...
             GROUP BY d.share_cd, f.share_type, f.trty_yy, f.acct_trty_type, f.prtfolio_sw, v_location_cd, f.expiry_date)
   LOOP
      RETURN (TRUE);
   END LOOP;
