DROP FUNCTION CPI.GET_SU_PARTICULARS;

CREATE OR REPLACE FUNCTION CPI.get_su_particulars(
   p_tran_id            NUMBER,
...
IS
 ...
 v_su_particulars       giac_parameters.param_value_v%TYPE := GIACP.V('SU_PARTICULARS');
BEGIN
  ...
  LOOP
        ...
        IF NVL(giacp.v('PREM_COLLN_PARTICULARS'),'PN') = 'PB' THEN
          ...
          --IF LENGTH(v_particulars) + LENGTH(i.policy_no || '/' || i.iss_cd || '/' || TO_CHAR(i.prem_seq_no,'99999999') || ', ') <= 502 THEN          
            v_particulars := v_particulars ||v_policy_no || '/' || i.iss_cd || '-' || TO_CHAR(i.prem_seq_no,'fm099999999999') || ', ';
          --END IF;
   ...
   IF v_package_particulars <> 'XX' AND v_pack_pol_flag = 'Y' THEN
      ...
      IF NVL(v_particulars, 'XX') NOT LIKE '%'||v_pack_pol_no||'%' THEN
         ...
         ELSE
            ...
            FOR pack IN (SELECT DECODE(a.line_cd, 'SU', DECODE(v_su_particulars, 'Y', a.ref_pol_no, a.line_cd
                                     || '-'
                                     || a.subline_cd
                                     || '-'
                                     || a.iss_cd
