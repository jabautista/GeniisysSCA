DROP FUNCTION CPI.GET_PREM_RESERVE;

CREATE OR REPLACE FUNCTION CPI.get_prem_reserve(p_session_id NUMBER, p_line_cd VARCHAR2, p_subline_cd VARCHAR2, p_iss_cd VARCHAR2, p_intm_no NUMBER, p_assd_no NUMBER)
RETURN NUMBER AS
  v_prem  NUMBER;
BEGIN
  FOR rec IN (SELECT (NVL(curr_prem_amt,0)*.4) prem_res_cy
