DROP FUNCTION CPI.GET_LATEST_SERIAL_NO;

CREATE OR REPLACE FUNCTION CPI.get_latest_serial_no (
...
*/
     FOR rec IN (SELECT y.item_no, y.serial_no, x.endt_seq_no,
   ...
   --check for endt in serial_no
     FOR rec3 IN (SELECT a.policy_id, a.endt_seq_no,b.serial_no
        ...
        LOOP
          v_serial_no  := rec3.serial_no;
        EXIT;
        END LOOP;
     --check for change in serial_no using backward endt.
        FOR rec2 IN (SELECT b.item_no, b.serial_no, a.endt_seq_no
                       FROM gipi_polbasic a,
                            gipi_vehicle  b
                      WHERE 1=1
                        AND a.policy_id  = b.policy_id
