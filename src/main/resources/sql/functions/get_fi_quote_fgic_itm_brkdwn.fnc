DROP FUNCTION CPI.GET_FI_QUOTE_FGIC_ITM_BRKDWN;

CREATE OR REPLACE FUNCTION CPI.get_fi_quote_fgic_itm_brkdwn(p_quote_id   gipi_quote.quote_id%TYPE)
RETURN VARCHAR2
IS
    v_count                 NUMBER := 0;
    v_item_no				NUMBER;
...
--     v_item_tsi_amt    := a.item_tsi_amt;
        ...
        IF v_count = 1 THEN
            v_breakdown_tsi := a.item_title; --v_item_title;
        ELSE
            v_breakdown_tsi := a.item_no||' - '||nvl(a.item_title, ' ')||' - '||nvl(a.item_tsi_amt, ' ');
        END IF;
