DROP FUNCTION CPI.CHECK_TOTAL_LOSS_GICLS010;

CREATE OR REPLACE FUNCTION CPI.check_total_loss_gicls010
( 
...
*/          
 ...
 p_total_loss_level  GIIS_LINE.TOTAL_LOSS_LEVEL%TYPE -- added parameter by MAGeamoga 09/12/2011
 ) RETURN BOOLEAN IS
  ...
  CURSOR tsi_non_agg IS
   ...
   AND A.policy_id = C.policy_id   
    --pau 06AUG07 end 
      ...
      AND b.pol_flag IN ('1','2','3','X') --added by RonCD 03/08/2012 to validate the policy flag
  ...
  CURSOR amt_settled_for_agg(p_peril GICL_CLM_LOSS_EXP.peril_cd%TYPE,
   ...
   SELECT SUM(A.losses_paid) paid_amt
    ...
    WHERE 1=1
      ...
      AND b.pol_seq_no = p_pol_Seq_no
      AND b.renew_no = p_renew_no
      AND A.item_no = NVL(p_item,A.item_no)--modified by MAGeamoga 09/12/2011
      --comment out by MAGeamoga 09/12/2011
      --AND nvl(A.GROUPED_ITEM_NO, 0) = p_grouped_item_no -- added by marlo 07082010
      AND A.GROUPED_ITEM_NO = NVL(p_grouped_item_no,0)--added by MAGeamoga 09/12/2011
...
BEGIN   
   ...
   ** and for claim level validation, only return true if all item-peril is total loss.*/
   ...
   --IF p_item_no IS NULL THEN   comment out by MAGeamoga 09/12/2011
      ...
      FOR i IN tsi_non_agg LOOP
         ...
         END LOOP;
         --nested if condition created by MAGeamoga 09/12/2011
         IF p_total_loss_level = '3' THEN--for total_loss_level 3 MAGeamoga 09/12/2011
             ...
             ELSE
                ...
                EXIT;
             END IF;
         ELSE   --for total_loss_level 1 and 2 MAGeamoga 09/12/2011
             IF v_paid_amount >= i.tsi_amt THEN
                v_total_loss := 'Y';
                EXIT;
             END IF;
      ...
      /*comment out and replaced by the below statement by MAC 06/14/2012.
      IF v_total_loss = 'Y'
      THEN
         RETURN (FALSE);
      ELSE
      ...
      --Will always return TRUE to disable validation since enhancement is not yet final by MAC 06/14/2012.
      RETURN (TRUE);
      
   /*ELSE  comment out by MAGeamoga 09/12/2011
      --checks w/o aggreagate peril
      DBMS_OUTPUT.PUT_LINE('tsi_non_arg');
      FOR i IN tsi_non_agg LOOP
          FOR j IN amt_settled(i.peril_cd, i.item_no) LOOP
