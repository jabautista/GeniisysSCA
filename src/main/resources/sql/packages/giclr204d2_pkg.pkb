CREATE OR REPLACE PACKAGE BODY CPI.GICLR204D2_PKG AS
...
BEGIN
    FOR a IN(
        SELECT  b.intm_no, 
                ...
                d.assd_name, 
                a.policy_id,
                --'PREMIUMS WRITTEN FOR ' ||  TO_CHAR(p_curr_start_date,'MM-DD-YYYY') || ' TO ' ||  TO_CHAR(p_curr_end_date,'MM-DD-YYYY') time_period,
                'PREMIUMS WRITTEN FOR ' || p_curr_start_date || ' TO ' || p_curr_end_date time_period,
                b.intm_no || ' ' || c.intm_name intermediary_
        ...
        GROUP BY b.intm_no, 
                ...
                d.assd_name, 
                a.policy_id,
                --'PREMIUMS WRITTEN FOR ' ||  TO_CHAR(p_curr_start_date,'MM-DD-YYYY') || ' TO ' ||  TO_CHAR(p_curr_end_date,'MM-DD-YYYY'),
        ...
        LOOP
            ...
            v_rec.intermediary_     := a.intermediary_;
            v_rec.v_date            := GICLR204D2_PKG.GET_DATEVALUE(p_print_date,a.policy_id);
            PIPE ROW (v_rec);
...
BEGIN
    FOR a IN(
        SELECT  b.intm_no ||' - ' ||c.intm_name intermediary_,
                b.intm_no,
                b.assd_no, 
                a.line_cd||'-'||a.subline_cd||'-'||a.iss_cd||'-'||LTRIM (TO_CHAR (a.issue_yy, '09'))||'-'||LTRIM (TO_CHAR (a.pol_seq_no, '0999999'))||'-'||LTRIM (TO_CHAR (a.renew_no, '09')) policy_no,
                a.endt_iss_cd, 
...
BEGIN
    FOR a IN(
            SELECT  e.intm_no||' - ' ||e.intm_name intermediary_,
                    e.intm_name, 
                    a.intm_no, 
                    a.assd_no, 
                    d.assd_name, 
            ...
            GROUP BY    a.intm_no, 
                        ...
                        --'OUTSTANDING LOSS AS OF ' ||  TO_CHAR( p_curr_end_date,'YYYY')  
            ORDER BY claim_no
...
BEGIN
    FOR a IN(
            ...
            GROUP BY    a.intm_no, 
                        ...
                        b.line_cd||'-'||b.subline_cd||'-'||b.iss_cd||'-'||LTRIM(TO_CHAR(b.clm_yy, '09'))||'-'||LTRIM(TO_CHAR(b.clm_seq_no, '0000009')),
                        e.intm_no||' - ' ||e.intm_name
                        --'OUTSTANDING LOSS AS OF' ||  TO_CHAR( p_prev_end_date,'YYYY')  
            ORDER BY claim_no
...
BEGIN
    FOR a IN(
            SELECT      a.intm_no ||' - ' || e.intm_name intermediary_,
                        a.intm_no, 
                        e.intm_name, 
                        a.assd_no, 
                        c.assd_name, 
...
BEGIN
    FOR a IN(
            SELECT      a.intm_no ||' - '||f.intm_name intermediary_,
                        a.intm_no, 
                        f.intm_name, 
                        a.assd_no, 
                        b.assd_name, 
