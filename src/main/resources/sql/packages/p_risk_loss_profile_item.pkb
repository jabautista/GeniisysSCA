CREATE OR REPLACE PACKAGE BODY CPI.p_risk_loss_profile_item AS
...
BEGIN
  ...
  fire_risk_profile_item_ext(p_user,      p_line_cd,   p_subline_cd,
        ...
        p_cred_branch, p_include_expired, p_include_endt);  -- aaron additional record filter
  ELSE
  dbms_output.put_line('apdc');
...
BEGIN
  ...
  FOR exst IN (SELECT 'EXIST'
                 FROM gipi_risk_loss_profile_item --new table, issa
  ...
  ** marked this bulk collect as Q2*/
  ...
  UPDATE gipi_risk_loss_profile_item
     SET risk_count  = 0,
   ...
   **
     ...
     SELECT A.line_cd,       A.subline_cd,    A.iss_cd,
         ...
         DECODE(p_basis,'AD',sign_tsi(p_date_from,  p_date_to,  A.acct_neg_date,
                 A.acct_ent_date,A.pol_flag),1),0)) tsi_amt
                        FROM gipi_polriskloss_item_ext A, --new table, issa
   ...
   FOR rec IN vv_range_to.FIRST..vv_range_to.LAST LOOP
     ...
     ELSE
        ...
        tab_polrisk_ext(rec).claim_count := tab_polrisk_ext(rec).claim_count + 0; -- Marvin 04292013 Added Claim count. UW-SPECS-2013-072 GIPIS902 GIPIR902 IE-2012-018-CIC-RISK AND LOSS PROFILE
   ...
   END IF;
    ...
    v_tarfcount := 0;
    --start insertion of collected records to gipi_risk_loss_profile_item
    ...
    IF v_polcount != 0 THEN
      ...
      DBMS_OUTPUT.PUT_LINE('INSERT');
         /* Formatted on 2013/04/26 23:14 (Formatter Plus v4.8.8) */
INSERT INTO gipi_risk_loss_profile_item
     ...
     VALUES (p_line_cd, tab_polrisk_ext (risk).subline_cd, p_user,
             ...
             tab_polrisk_ext (risk).facultative, p_date_from, p_date_to,
             p_line_tag, tab_polrisk_ext (risk).treaty2_tsi,
...
BEGIN
  ...
  END LOOP;
--      v_exist := 'Y';
  DBMS_OUTPUT.PUT_LINE(v_line_tag||'exist '||v_exist);
  ...
  ** marked this bulk collect as Q4*/
  SELECT range_from, range_to
    BULK COLLECT INTO
         range_from, range_to
       FROM gipi_risk_loss_profile_item A
  ...
  FROM (
      SELECT A.line_cd,       A.subline_cd,    A.iss_cd,
       ...
       SUM(b.dist_prem*DECODE(v_basis,'AD',sign_tsi(v_date_from,
                 ...
                 FROM gipi_polriskloss_item_ext A,
                    GIUW_ITEMPERILDS_DTL b,
                 GIPI_SHARISK_EXT2 c
              WHERE A.policy_id >= 0
  ...
  */
  ...
  -- should be changed
    ...
    SET risk_count = ext_risk_row(transfer).policy_count,
        claim_count = ext_risk_row(transfer).claim_count, -- Marvin 04292013 Added Claim count. UW-SPECS-2013-072 GIPIS902 GIPIR902 IE-2012-018-CIC-RISK AND LOSS PROFILE
...
IS
  ...
  TYPE risk_cd_tab     IS TABLE OF GIPI_FIREITEM.risk_cd%TYPE;  -- aaron per risk
  TYPE block_id_tab        IS TABLE OF GIPI_FIREITEM.block_id%TYPE; -- aaron per risk
...
BEGIN
  DELETE FROM gipi_polriskloss_item_ext
    WHERE user_id = USER;
  COMMIT;
  SELECT A.policy_id,          A.line_cd,          A.subline_cd,
   ...
   DECODE(a.line_cd, giisp.v('LINE_CODE_FI'),get_risk_cd(a.policy_id, c.item_no),NULL),
   DECODE(a.line_cd, giisp.v('LINE_CODE_FI'),get_block_cd(a.policy_id, c.item_no),NULL)
/*   get_ann_tsi(a.line_cd,     a.subline_cd,      a.iss_cd,
   ...
   vv_item_no, vv_risk_cd, vv_block_id
...
--     AND b.dist_flag = decode(a.pol_flag,'5','4','3');--if pol_flag is 5, dist_flag shld be 4, else dist_flag shld be 3
  ...
  */
  IF NOT SQL%NOTFOUND THEN
    ...
    --dbms_output.put_line(vv_policy_id);
    INSERT INTO gipi_polriskloss_item_ext --new table, issa
         (policy_id,          item_no,    line_cd,          
    subline_cd,         iss_cd,               issue_yy,       
    pol_seq_no,        renew_no,            dist_no,              
    ann_tsi_amt,    user_id,      acct_ent_date,  
    acct_neg_date,    pol_flag, risk_cd, block_id)
       VALUES
    ...
    vv_acct_neg_date(i),vv_pol_flag(i), vv_risk_cd(i), vv_block_id(i));
     COMMIT;
  END IF;
    
  -- selects and inserts endorsements aaron 032509 
  
  FOR X IN (SELECT policy_id, line_cd, subline_cd, iss_cd, issue_yy, pol_seq_no, renew_no--, item_no
             FROM gipi_polriskloss_item_ext
   WHERE user_id = USER
   GROUP BY policy_id, line_cd, subline_cd, iss_cd, issue_yy, pol_seq_no, renew_no/*, item_no*/)
 LOOP
 -- IF x.policy_id = 239658 THEN
...
/*   get_ann_tsi(a.line_cd,     a.subline_cd,      a.iss_cd,
  ...
  AND A.policy_id != X.policy_id
    ...
    --AND c.item_no = x.item_no  to be able extract additional items 
    -- AND A.endt_seq_no = 0  -- aaron 032509 extract policies first
/*
  AND decode(a.pol_flag,'4',
         check_date_dist_peril(a.line_cd,    a.subline_cd,
...
FUNCTION get_risk_cd (p_policy_id NUMBER,
         p_item_no NUMBER)
RETURN VARCHAR2 IS
  v_risk_cd VARCHAR2(7);
BEGIN
  FOR x IN (SELECT risk_cd
              FROM gipi_fireitem
...
FUNCTION get_block_cd (p_policy_id NUMBER,
         p_item_no NUMBER)
RETURN VARCHAR2 IS
  v_block_id VARCHAR2(7);
BEGIN
  FOR x IN (SELECT block_id
              FROM gipi_fireitem
    WHERE policy_id = p_policy_id
      AND item_no = p_item_no)
  LOOP
