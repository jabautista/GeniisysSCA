CREATE OR REPLACE PACKAGE BODY CPI.Nlto_Fromto_Test AS
  ...
  AS
	...
	v_coverage       gixx_nlto_stat.coverage%TYPE;
  BEGIN
    v_coverage := get_coverage_desc(v_zone_type);
	...
	SELECT peril_cd, peril_name
      bulk collect INTO vv_peril_cd, vv_peril_name
	  FROM giis_peril a
	...
	IF SQL%FOUND THEN
	   ...
	   vv_pc_count.extend(vv_peril_name.last);
	   vv_cv_count.extend(vv_peril_name.last);
	   vv_mc_count.extend(vv_peril_name.last);
	   vv_pc_prem.extend(vv_peril_name.last);
	   vv_cv_prem.extend(vv_peril_name.last);
	   vv_mc_prem.extend(vv_peril_name.last);
	   FOR rec IN vv_peril_name.first..vv_peril_name.last
	   LOOP
	    SELECT --count policy with subline 'pcp'
          ...
          INTO vv_pc_count(rec), vv_cv_count(rec), vv_mc_count(rec),
         ...
         WHERE 1=1
           ...
           --*--
    	   AND c.peril_cd  = vv_peril_cd(rec)
           ...
           AND Nlto_Fromto.date_ok(a.acct_ent_date, a.eff_date, a.issue_date, a.booking_mth, a.booking_year,
