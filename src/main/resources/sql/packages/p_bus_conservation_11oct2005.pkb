CREATE OR REPLACE PACKAGE BODY CPI.p_bus_conservation_11oct2005 AS
  PROCEDURE get_data(
       ...
       FORALL cnt IN vv_line_cd.FIRST..vv_line_cd.LAST
		  ...
		  VALUES
		    (vv_line_cd(cnt),	vv_subline_cd(cnt),   vv_iss_cd(cnt),
			 vv_policy_cnt(cnt),vv_renew_cnt(cnt),	  vv_prem_amt(cnt),
			 vv_exp_month(cnt),	vv_exp_year(cnt),	  USER,
			 SYSDATE,			vv_renew_amt(cnt),	  vv_region_cd(cnt));
	END IF;
	COMMIT;
	--get dummy data
--	IF p_del_table = 'N' THEN
         ...
         FROM (SELECT DISTINCT a.line_cd,a.subline_cd,a.iss_cd,b.YEAR
			  ...
			  (SELECT DISTINCT YEAR
                 FROM giex_ren_ratio
                WHERE user_id = USER
   	   		     GROUP BY iss_cd,line_cd,subline_cd,YEAR) b) d
...
DROP PACKAGE BODY CPI.P_BUS_CONSERVATION_11OCT2005;
