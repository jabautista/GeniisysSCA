CREATE OR REPLACE PACKAGE BODY CPI.GIACR171A_PKG AS
   ...
   FUNCTION populate_giacr171a_records (
        p_line_cd           VARCHAR2,
        p_ri_cd             VARCHAR2,
        p_user_id           VARCHAR2
       )
       RETURN giacr171a_record_tab PIPELINED
    AS
       v_rec                giacr171a_record_type;
       v_company_name       VARCHAR2(200);
       v_company_address    VARCHAR2(400);
       v_from_date          DATE;
       v_to_date            DATE;
    ...
    BEGIN
       FOR i IN (SELECT assd_no, assd_name, a.line_cd, fund_cd, branch_cd, ri_cd, ri_name, 
                        amt_insured, policy_no, gross_prem_amt, ri_comm_exp, booking_date,
                        net_premium, prem_vat, comm_vat, b.line_name
       ...
       LOOP
            ...
            v_rec.to_date         := cf_to_dateformula(p_user_id);
