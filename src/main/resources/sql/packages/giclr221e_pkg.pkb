CREATE OR REPLACE PACKAGE BODY CPI.GICLR221E_PKG
AS
/*
    **  Created by   :  Kenneth Mark Labrador
    **  Date Created : 04.01.2013
    **  Reference By : GICLR221E- EXPENSE PAID BORDEREAUX (PER ENROLLEE)
    */
   ...
   IS
      ...
      v_loss         NUMBER;
      v_ref_pol      gipi_polbasic.ref_pol_no%type;
   BEGIN
      FOR i IN (SELECT   a.brdrx_record_id, a.claim_id, a.line_cd,
                         ...
                         a.clm_loss_id, NVL (a.expenses_paid, 0) paid_losses
                    FROM gicl_res_brdrx_extr a
                   WHERE a.session_id = p_session_id
                     AND a.claim_id = NVL (p_claim_id, a.claim_id)
                     AND a.enrollee = NVL (p_enrollee, a.enrollee)
                     AND ABS (NVL (a.expenses_paid, 0)) > 0
                ORDER BY a.claim_no, a.peril_cd)
      LOOP
         ...
         IF v_ref_pol IS NOT NULL THEN
             v_detail.cf_policy_no := v_detail.policy_no||'/'||CHR(10)||v_ref_pol;
          ELSE
         ...
         LOOP
            ...
            LOOP
               ...
               LOOP
                  ...
                  FOR m IN (SELECT   a.brdrx_ds_record_id,
                                     DECODE (a.prnt_ri_cd,
                                             NULL, a.ri_cd,
                                             a.prnt_ri_cd
                                            ) facul_ri_cd,
   ...
   /* Added by : J. Diago
   ** Date Added : 02.27.2014
   ** Remarks : To handle difficult matrix.
   */
   
   FUNCTION get_giclr221e_parent(
      p_session_id         GICL_RES_BRDRX_EXTR.session_id%TYPE,
      p_claim_id           GICL_RES_BRDRX_EXTR.claim_id%TYPE
   )
     RETURN giclr221e_parent_tab PIPELINED
   IS
      v_row                giclr221e_parent_type;
   ...
   FUNCTION get_giclr221e_claim(
      p_session_id         GICL_RES_BRDRX_EXTR.session_id%TYPE,
      p_claim_id           GICL_RES_BRDRX_EXTR.claim_id%TYPE,
      p_enrollee           GICL_RES_BRDRX_EXTR.enrollee%TYPE
   )
     RETURN giclr221e_claim_tab PIPELINED
   IS
      v_row                giclr221e_claim_type;
   BEGIN
   ...
   FUNCTION get_giclr221e_item_main(
      p_session_id         GICL_RES_BRDRX_EXTR.session_id%TYPE,
      p_claim_id           GICL_RES_BRDRX_EXTR.claim_id%TYPE,
      p_enrollee           GICL_RES_BRDRX_EXTR.enrollee%TYPE
   )
     RETURN giclr221e_item_main_tab PIPELINED
   IS
      v_row                giclr221e_item_main_type;
   BEGIN
   ...
   FUNCTION get_giclr221e_item(
     ...
     RETURN giclr221e_item_tab PIPELINED
   IS
      v_row                giclr221e_item_type;
   BEGIN
      FOR item IN (SELECT   a.claim_id, a.claim_no, a.policy_no, a.enrollee, a.item_no, a.tsi_amt,
                            ...
                            NVL(a.expenses_paid, 0) paid_expenses, a.brdrx_record_id, a.grouped_item_no
                     FROM gicl_res_brdrx_extr a
                    WHERE a.session_id = p_session_id
                      AND a.claim_id = NVL (p_claim_id, a.claim_id)
                      AND NVL (a.expenses_paid, 0) > 0
...
--         v_row.dv_no := cf_dv_noformula(item.claim_id, item.item_no, item.clm_loss_id, item.paid_losses, item.paid_expenses, p_paid_date, p_from_date, p_to_date);
         v_row.dv_no := gicls202_pkg.get_voucher_check_no(item.claim_id, item.item_no, item.peril_cd, item.grouped_item_no, p_from_date, p_to_date, p_paid_date, 'E');
         v_row.intm_name := giclr222le_pkg.CF_INTM_RIFormula(item.claim_id, item.item_no, item.peril_cd);
   ...
   BEGIN
      ...
      FOR enrollee IN (SELECT enrollee, grp_seq_no1, grp_seq_no2, grp_seq_no3, grp_seq_no4
                         FROM TABLE (giclr221e_pkg.get_giclr221e_parent (p_session_id, p_claim_id))
      ...
      LOOP
         ...
         LOOP
            ...
            v_loop := 'SELECT paid_losses
                         FROM TABLE (giclr221e_pkg.get_treaty_details (
                                                '||p_session_id||',
                                                '||v_claim_id||',
                                                '''||enrollee.enrollee||''',
                                                '||p_brdrx_record_id||'
      ...
      IF v_table.EXISTS(v_index) THEN
         v_row.paid_expenses1 := v_table(v_index);
         v_index := v_index + 1;
      END IF;
      
      IF v_table.EXISTS(v_index) THEN
         v_row.paid_expenses2 := v_table(v_index);
         v_index := v_index + 1;
      END IF;
      
      IF v_table.EXISTS(v_index) THEN
         v_row.paid_expenses3 := v_table(v_index);
         v_index := v_index + 1;
      END IF;
      
      IF v_table.EXISTS(v_index) THEN
         v_row.paid_expenses4 := v_table(v_index);
         v_index := v_index + 1;
      END IF;
   ...
   BEGIN
      ...
      LOOP
         FOR facul IN (SELECT *
                         FROM TABLE(GICLR221E_PKG.get_facul(p_session_id, p_claim_id, enrollee.enrollee, p_brdrx_record_id))
   ...
   FUNCTION get_giclr221e_expenses_total(
      p_session_id         GICL_RES_BRDRX_DS_EXTR.session_id%TYPE,
      p_claim_id           GICL_RES_BRDRX_EXTR.claim_id%TYPE,
      p_enrollee           GICL_RES_BRDRX_EXTR.enrollee%TYPE
   )
     RETURN total_paid_expenses_tab PIPELINED
   IS
      v_total              total_paid_expenses_type;
   BEGIN
      FOR total IN (SELECT SUM (NVL (a.expenses_paid, 0)) total_paid_expenses
                      FROM gicl_res_brdrx_extr a
                     WHERE a.session_id = p_session_id
                       AND a.claim_id = NVL (p_claim_id, a.claim_id)
                       AND NVL (a.expenses_paid, 0) > 0
   ...
   FUNCTION get_giclr221e_treaty_total(
      p_session_id         GICL_RES_BRDRX_DS_EXTR.session_id%TYPE,
      p_claim_id           GICL_RES_BRDRX_EXTR.claim_id%TYPE,
      p_enrollee_dummy     GICL_RES_BRDRX_EXTR.enrollee%TYPE
   )
         ...
         LOOP
            ...
            v_loop := 'SELECT SUM(paid_losses)
                         FROM ( SELECT paid_losses
                                  FROM TABLE (giclr221e_pkg.get_facul ('||p_session_id||',
                                                                       '||v_claim_id||',
                                                                       '''||enrollee.enrollee||''',
                                                                       '||v_brdrx_record_id||'
                                                                      )
