CREATE OR REPLACE PACKAGE BODY CPI.GIPIS161_PKG
AS

    FUNCTION get_policy_listing (
        p_user_id          giis_users.user_id%TYPE,
        p_pol_line_cd      gipi_polbasic.line_cd%TYPE,
        p_pol_subline_cd   gipi_polbasic.subline_cd%TYPE,
        p_pol_iss_cd       gipi_polbasic.iss_cd%TYPE,
        p_pol_issue_yy     gipi_polbasic.issue_yy%TYPE,
        p_pol_seq_no       gipi_polbasic.pol_seq_no%TYPE,
        p_pol_renew_no     gipi_polbasic.renew_no%TYPE,
        p_par_line_cd      gipi_parlist.line_cd%TYPE,
        p_par_iss_cd       gipi_parlist.iss_cd%TYPE,
        p_par_yy           gipi_parlist.par_yy%TYPE,
        p_par_seq_no       gipi_parlist.par_seq_no%TYPE,
        p_par_quote_seq_no gipi_parlist.quote_seq_no%TYPE,
        p_endt_iss_cd      gipi_polbasic.endt_iss_cd%TYPE,      
        p_endt_yy          gipi_polbasic.endt_yy%TYPE,
        p_endt_seq_no      gipi_polbasic.endt_seq_no%TYPE,
        p_assd_name        giis_assured.assd_name%TYPE
    )
        RETURN policy_tab PIPELINED
    IS
        v_pol   policy_type;
    BEGIN
        FOR i IN (SELECT b.policy_id, b.line_cd, b.subline_cd, b.iss_cd, b.issue_yy, b.pol_seq_no, b.renew_no,
                         line_cd
                         || '-'
                         || subline_cd
                         || '-'
                         || iss_cd
                         || '-'
                         || LTRIM (TO_CHAR (issue_yy, '09'))
                         || '-'
                         || LTRIM (TO_CHAR (pol_seq_no, '0999999'))
                         || '-'
                         || LTRIM (TO_CHAR (renew_no, '09')) policy_no, 
                         b.par_id, b.endt_iss_cd, b.endt_yy, b.endt_seq_no,
                         endt_iss_cd
                         || '-'
                         || LTRIM (TO_CHAR (endt_yy, '09'))
                         || '-'
                         || LTRIM (TO_CHAR (endt_seq_no, '0999999')) endt_no,
                         a.assd_name, b.pack_pol_flag, b.pack_policy_id
                    FROM gipi_polbasic b, giis_assured a
                   WHERE 1=1
                     AND b.assd_no = a.assd_no
                     AND check_user_per_line2 (b.line_cd,b.iss_cd,'GIPIS161',p_user_id) = 1
                     AND check_user_per_iss_cd2 (b.line_cd,b.iss_cd,'GIPIS161',p_user_id) = 1
                     AND UPPER (b.line_cd) LIKE UPPER(NVL(p_pol_line_cd, b.line_cd))
                     AND UPPER (b.subline_cd) LIKE UPPER(NVL(p_pol_subline_cd, b.subline_cd))
                     AND UPPER (b.iss_cd) LIKE UPPER(NVL(p_pol_iss_cd, b.iss_cd))
                     AND b.issue_yy LIKE UPPER(NVL(p_pol_issue_yy, b.issue_yy))
                     AND b.pol_seq_no LIKE UPPER(NVL(p_pol_seq_no, b.pol_seq_no))
                     AND b.renew_no LIKE UPPER(NVL(p_pol_renew_no, b.renew_no))
                     AND a.assd_name LIKE UPPER(NVL(p_assd_name, a.assd_name))
                     AND b.endt_iss_cd LIKE UPPER (NVL(p_endt_iss_cd, b.endt_iss_cd))
                     AND b.endt_yy LIKE UPPER (NVL(p_endt_yy, b.endt_yy))
                     AND b.endt_seq_no LIKE UPPER (NVL(p_endt_seq_no, b.endt_seq_no))
                     AND b.par_id IN (SELECT par_id
                                        FROM gipi_parlist 
                                       WHERE line_cd = UPPER(NVL(p_par_line_cd, line_cd))
                                         AND iss_cd = UPPER(NVL(p_par_iss_cd, iss_cd))
                                         AND (par_yy = UPPER(NVL(p_par_yy, par_yy)))
                                         AND (par_seq_no = UPPER(NVL(p_par_seq_no, par_seq_no)))
                                         AND (quote_seq_no = UPPER(NVL(p_par_quote_seq_no, quote_seq_no)))
                                     )
                )
        LOOP
            v_pol.policy_id         := i.policy_id; 
            v_pol.pack_policy_id    := i.pack_policy_id;
            v_pol.pol_line_cd       := i.line_cd;
            v_pol.pol_subline_cd    := i.subline_cd;
            v_pol.pol_iss_cd        := i.iss_cd;
            v_pol.pol_issue_yy      := i.issue_yy;
            v_pol.pol_seq_no        := i.pol_seq_no;
            v_pol.pol_renew_no      := i.renew_no;
            v_pol.policy_no         := i.policy_no;
            v_pol.par_id            := i.par_id;
            v_pol.assd_name         := i.assd_name;
            v_pol.pack_pol_flag     := i.pack_pol_flag;
            
            SELECT a.line_cd
                        || '-'
                        || a.iss_cd
                        || '-'
                        || TRIM(TO_CHAR (a.par_yy, '09'))
                        || '-'
                        || TRIM(TO_CHAR (a.par_seq_no, '000009'))
                        || '-'
                        || TRIM(TO_CHAR (a.quote_seq_no, '09')) par_no, a.line_cd, a.iss_cd, 
                        TRIM(TO_CHAR (a.par_yy, '09')) par_yy, TRIM(TO_CHAR (a.par_seq_no, '000009')) par_seq_no, TRIM(TO_CHAR (a.quote_seq_no, '09')) quote_seq_no
              INTO v_pol.par_no, v_pol.par_line_cd, v_pol.par_iss_cd, v_pol.par_yy, v_pol.par_seq_no, v_pol.par_quote_seq_no
              FROM gipi_parlist a
             WHERE a.par_id = i.par_id;
            
            IF i.endt_seq_no != 0 THEN
                v_pol.endt_iss_cd       := i.endt_iss_cd;
                v_pol.endt_yy           := i.endt_yy;
                v_pol.endt_seq_no       := i.endt_seq_no;
                v_pol.endt_no           := i.endt_no;
            ELSE
                v_pol.endt_iss_cd       := NULL;
                v_pol.endt_yy           := NULL;
                v_pol.endt_seq_no       := NULL;
                v_pol.endt_no           := NULL;
            END IF;
            
            BEGIN
            SELECT line_cd
                    || '-'
                    || subline_cd
                    || '-'
                    || iss_cd
                    || '-'
                    || TRIM(TO_CHAR (issue_yy, '009'))
                    || '-'
                    || TRIM(TO_CHAR (pol_seq_no, '00009')
                    || '-00') pack_no 
               INTO v_pol.pack_no
		       FROM gipi_pack_polbasic
		      WHERE pack_policy_id = i.pack_policy_id;
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    v_pol.pack_no := NULL;
            END;
            PIPE ROW (v_pol);
        END LOOP;

    END get_policy_listing;

    FUNCTION get_gen_init_info(
        p_policy_id     gipi_polbasic.policy_id%TYPE
    ) 
        RETURN gen_init_info_tab PIPELINED
    AS
        v_ gen_init_info_type;
    BEGIN
        SELECT /*gen_info*/
               gen_info01,gen_info02,gen_info03,gen_info04,gen_info05,gen_info06,
               gen_info07,gen_info08,gen_info09,gen_info10,gen_info11,gen_info12,
               gen_info13,gen_info14,gen_info15,gen_info16,gen_info17,
               /*first_info*/
               initial_info01,initial_info02,initial_info03,initial_info04,initial_info05,
               initial_info06,initial_info07,initial_info08,initial_info09,initial_info10,
               initial_info11,initial_info12,initial_info13,initial_info14,initial_info15,
               initial_info16,initial_info17,
               user_id, TO_CHAR(last_update, 'MM-DD-YYYY HH:MI:SS AM') last_update
          INTO /*gen_info*/
               v_.gen_info01,v_.gen_info02,v_.gen_info03,v_.gen_info04,v_.gen_info05,v_.gen_info06,
               v_.gen_info07,v_.gen_info08,v_.gen_info09,v_.gen_info10,v_.gen_info11,v_.gen_info12,
               v_.gen_info13,v_.gen_info14,v_.gen_info15,v_.gen_info16,v_.gen_info17,
               /*first_info*/
               v_.initial_info01,v_.initial_info02,v_.initial_info03,v_.initial_info04,v_.initial_info05,
               v_.initial_info06,v_.initial_info07,v_.initial_info08,v_.initial_info09,v_.initial_info10,
               v_.initial_info11,v_.initial_info12,v_.initial_info13,v_.initial_info14,v_.initial_info15,
               v_.initial_info16,v_.initial_info17,
               v_.user_id, v_.last_update
          FROM gipi_polgenin
         WHERE policy_id = p_policy_id;
         PIPE ROW(v_);    
    END get_gen_init_info;

    FUNCTION get_gen_init_pack_info(
        p_policy_id     gipi_polbasic.policy_id%TYPE
    ) 
        RETURN gen_init_info_pack_tab PIPELINED
    AS
        v_                gen_init_info_pack_type;
        v_pack_policy_id  gipi_pack_polgenin.pack_policy_id%TYPE;
    BEGIN
        SELECT pack_policy_id
          INTO v_pack_policy_id
          FROM gipi_polbasic    
         WHERE policy_id = p_policy_id;
        
        SELECT /*gen_info*/
    		   gen_info01,gen_info02,gen_info03,gen_info04,gen_info05,gen_info06,
               gen_info07,gen_info08,gen_info09,gen_info10,gen_info11,gen_info12,
               gen_info13,gen_info14,gen_info15,gen_info16,gen_info17,
               --first_info
               initial_info01,initial_info02,initial_info03,initial_info04,initial_info05,
               initial_info06,initial_info07,initial_info08,initial_info09,initial_info10,
               initial_info11,initial_info12,initial_info13,initial_info14,initial_info15,
               initial_info16,initial_info17,
               user_id, TO_CHAR(last_update, 'MM-DD-YYYY HH:MI:SS AM') last_update, pack_policy_id
          INTO /*gen_info*/
      		   v_.gen_info01,v_.gen_info02,v_.gen_info03,v_.gen_info04,v_.gen_info05,v_.gen_info06,
               v_.gen_info07,v_.gen_info08,v_.gen_info09,v_.gen_info10,v_.gen_info11,v_.gen_info12,
               v_.gen_info13,v_.gen_info14,v_.gen_info15,v_.gen_info16,v_.gen_info17,
               --first_info
               v_.initial_info01,v_.initial_info02,v_.initial_info03,v_.initial_info04,v_.initial_info05,
               v_.initial_info06,v_.initial_info07,v_.initial_info08,v_.initial_info09,v_.initial_info10,
               v_.initial_info11,v_.initial_info12,v_.initial_info13,v_.initial_info14,v_.initial_info15,
               v_.initial_info16,v_.initial_info17,
               v_.user_id, v_.last_update, v_.pack_policy_id
          FROM gipi_pack_polgenin
         WHERE pack_policy_id = v_pack_policy_id;
         PIPE ROW(v_);    
    END get_gen_init_pack_info;

    FUNCTION get_endt_info(
        p_policy_id     gipi_polbasic.policy_id%TYPE
    ) 
        RETURN endt_info_tab PIPELINED
    AS
        v_                endt_info_type;
    BEGIN
        SELECT endt_text01, endt_text02, endt_text03, endt_text04, endt_text05, endt_text06, 
               endt_text07, endt_text08, endt_text09, endt_text10, endt_text11, endt_text12,
 			   endt_text13, endt_text14, endt_text15, endt_text16, endt_text17,
               user_id, TO_CHAR(last_update, 'MM-DD-YYYY HH:MI:SS AM') last_update
          INTO v_.endt_text01, v_.endt_text02, v_.endt_text03, v_.endt_text04, v_.endt_text05, v_.endt_text06, 
               v_.endt_text07, v_.endt_text08, v_.endt_text09, v_.endt_text10, v_.endt_text11, v_.endt_text12,
 			   v_.endt_text13, v_.endt_text14, v_.endt_text15, v_.endt_text16, v_.endt_text17,
               v_.user_id, v_.last_update 
 		  FROM gipi_endttext
 		 WHERE policy_id = p_policy_id;       
         PIPE ROW(v_);    
    END get_endt_info;  

    PROCEDURE set_gen_info_gipi_polgenin(
        p_set   gipi_polgenin%ROWTYPE
    )
    IS
    BEGIN
        MERGE INTO gipi_polgenin
             USING DUAL
                ON (policy_id = p_set.policy_id)
            WHEN NOT MATCHED THEN
                INSERT (policy_id, 
                        gen_info01,gen_info02,gen_info03,gen_info04,gen_info05,gen_info06,
                        gen_info07,gen_info08,gen_info09,gen_info10,gen_info11,gen_info12,
                        gen_info13,gen_info14,gen_info15,gen_info16,gen_info17,
                        user_id, last_update)
                VALUES (p_set.policy_id,
                        p_set.gen_info01,p_set.gen_info02,p_set.gen_info03,p_set.gen_info04,p_set.gen_info05,p_set.gen_info06,
                        p_set.gen_info07,p_set.gen_info08,p_set.gen_info09,p_set.gen_info10,p_set.gen_info11,p_set.gen_info12,
                        p_set.gen_info13,p_set.gen_info14,p_set.gen_info15,p_set.gen_info16,p_set.gen_info17,
                        p_set.user_id, SYSDATE)
            WHEN MATCHED THEN
                UPDATE
                    SET gen_info01 = p_set.gen_info01,
                        gen_info02 = p_set.gen_info02,
                        gen_info03 = p_set.gen_info03,
                        gen_info04 = p_set.gen_info04,
                        gen_info05 = p_set.gen_info05,
                        gen_info06 = p_set.gen_info06,
                        gen_info07 = p_set.gen_info07,
                        gen_info08 = p_set.gen_info08,
                        gen_info09 = p_set.gen_info09,
                        gen_info10 = p_set.gen_info10,
                        gen_info11 = p_set.gen_info11,
                        gen_info12 = p_set.gen_info12,
                        gen_info13 = p_set.gen_info13,
                        gen_info14 = p_set.gen_info14,
                        gen_info15 = p_set.gen_info15,
                        gen_info16 = p_set.gen_info16,
                        gen_info17 = p_set.gen_info17,
                        user_id    = p_set.user_id,
                        last_update = SYSDATE;
    END set_gen_info_gipi_polgenin;

    PROCEDURE set_init_info_gipi_polgenin(
        p_set   gipi_polgenin%ROWTYPE
    )
    IS
    BEGIN
        MERGE INTO gipi_polgenin
             USING DUAL
                ON (policy_id = p_set.policy_id)
            WHEN NOT MATCHED THEN
                INSERT (policy_id, 
                        initial_info01,initial_info02,initial_info03,initial_info04,initial_info05,initial_info06,
                        initial_info07,initial_info08,initial_info09,initial_info10,initial_info11,initial_info12,
                        initial_info13,initial_info14,initial_info15,initial_info16,initial_info17,
                        user_id, last_update)
                VALUES (p_set.policy_id,
                        p_set.initial_info01,p_set.initial_info02,p_set.initial_info03,p_set.initial_info04,p_set.initial_info05,p_set.initial_info06,
                        p_set.initial_info07,p_set.initial_info08,p_set.initial_info09,p_set.initial_info10,p_set.initial_info11,p_set.initial_info12,
                        p_set.initial_info13,p_set.initial_info14,p_set.initial_info15,p_set.initial_info16,p_set.initial_info17,
                        p_set.user_id, p_set.last_update)
            WHEN MATCHED THEN
                UPDATE
                    SET initial_info01 = p_set.initial_info01,
                        initial_info02 = p_set.initial_info02,
                        initial_info03 = p_set.initial_info03,
                        initial_info04 = p_set.initial_info04,
                        initial_info05 = p_set.initial_info05,
                        initial_info06 = p_set.initial_info06,
                        initial_info07 = p_set.initial_info07,
                        initial_info08 = p_set.initial_info08,
                        initial_info09 = p_set.initial_info09,
                        initial_info10 = p_set.initial_info10,
                        initial_info11 = p_set.initial_info11,
                        initial_info12 = p_set.initial_info12,
                        initial_info13 = p_set.initial_info13,
                        initial_info14 = p_set.initial_info14,
                        initial_info15 = p_set.initial_info15,
                        initial_info16 = p_set.initial_info16,
                        initial_info17 = p_set.initial_info17,
                        user_id    = p_set.user_id,
                        last_update = p_set.last_update;
    END set_init_info_gipi_polgenin;
    
    PROCEDURE set_gen_gipi_pack_polgenin(
        p_set   gipi_pack_polgenin%ROWTYPE
    )
    IS
    BEGIN
        MERGE INTO gipi_pack_polgenin
             USING DUAL
                ON (pack_policy_id = p_set.pack_policy_id)
            WHEN NOT MATCHED THEN
                INSERT (pack_policy_id, 
                        gen_info01,gen_info02,gen_info03,gen_info04,gen_info05,gen_info06,
                        gen_info07,gen_info08,gen_info09,gen_info10,gen_info11,gen_info12,
                        gen_info13,gen_info14,gen_info15,gen_info16,gen_info17,
                        user_id, last_update)
                VALUES (p_set.pack_policy_id,
                        p_set.gen_info01,p_set.gen_info02,p_set.gen_info03,p_set.gen_info04,p_set.gen_info05,p_set.gen_info06,
                        p_set.gen_info07,p_set.gen_info08,p_set.gen_info09,p_set.gen_info10,p_set.gen_info11,p_set.gen_info12,
                        p_set.gen_info13,p_set.gen_info14,p_set.gen_info15,p_set.gen_info16,p_set.gen_info17,
                        p_set.user_id, p_set.last_update)
            WHEN MATCHED THEN
                UPDATE
                    SET gen_info01 = p_set.gen_info01,
                        gen_info02 = p_set.gen_info02,
                        gen_info03 = p_set.gen_info03,
                        gen_info04 = p_set.gen_info04,
                        gen_info05 = p_set.gen_info05,
                        gen_info06 = p_set.gen_info06,
                        gen_info07 = p_set.gen_info07,
                        gen_info08 = p_set.gen_info08,
                        gen_info09 = p_set.gen_info09,
                        gen_info10 = p_set.gen_info10,
                        gen_info11 = p_set.gen_info11,
                        gen_info12 = p_set.gen_info12,
                        gen_info13 = p_set.gen_info13,
                        gen_info14 = p_set.gen_info14,
                        gen_info15 = p_set.gen_info15,
                        gen_info16 = p_set.gen_info16,
                        gen_info17 = p_set.gen_info17,
                        user_id    = p_set.user_id,
                        last_update = p_set.last_update;
    END set_gen_gipi_pack_polgenin;
    
    PROCEDURE set_init_gipi_pack_polgenin(
        p_set   gipi_pack_polgenin%ROWTYPE
    )   
    IS
    BEGIN
        MERGE INTO gipi_pack_polgenin
             USING DUAL
                ON (pack_policy_id = p_set.pack_policy_id)
            WHEN NOT MATCHED THEN
                INSERT (pack_policy_id, 
                        initial_info01,initial_info02,initial_info03,initial_info04,initial_info05,initial_info06,
                        initial_info07,initial_info08,initial_info09,initial_info10,initial_info11,initial_info12,
                        initial_info13,initial_info14,initial_info15,initial_info16,initial_info17,
                        user_id, last_update)
                VALUES (p_set.pack_policy_id,
                        p_set.initial_info01,p_set.initial_info02,p_set.initial_info03,p_set.initial_info04,p_set.initial_info05,p_set.initial_info06,
                        p_set.initial_info07,p_set.initial_info08,p_set.initial_info09,p_set.initial_info10,p_set.initial_info11,p_set.initial_info12,
                        p_set.initial_info13,p_set.initial_info14,p_set.initial_info15,p_set.initial_info16,p_set.initial_info17,
                        p_set.user_id, p_set.last_update)
            WHEN MATCHED THEN
                UPDATE
                    SET initial_info01 = p_set.initial_info01,
                        initial_info02 = p_set.initial_info02,
                        initial_info03 = p_set.initial_info03,
                        initial_info04 = p_set.initial_info04,
                        initial_info05 = p_set.initial_info05,
                        initial_info06 = p_set.initial_info06,
                        initial_info07 = p_set.initial_info07,
                        initial_info08 = p_set.initial_info08,
                        initial_info09 = p_set.initial_info09,
                        initial_info10 = p_set.initial_info10,
                        initial_info11 = p_set.initial_info11,
                        initial_info12 = p_set.initial_info12,
                        initial_info13 = p_set.initial_info13,
                        initial_info14 = p_set.initial_info14,
                        initial_info15 = p_set.initial_info15,
                        initial_info16 = p_set.initial_info16,
                        initial_info17 = p_set.initial_info17,
                        user_id    = p_set.user_id,
                        last_update = p_set.last_update;
    END set_init_gipi_pack_polgenin;
    
    PROCEDURE set_endt_gipi_endt_text(
        p_set   gipi_endttext%ROWTYPE
    )
    IS
    BEGIN
        MERGE INTO gipi_endttext
             USING DUAL
                ON (policy_id = p_set.policy_id)
            WHEN NOT MATCHED THEN
                INSERT (policy_id, 
                        endt_text01,endt_text02,endt_text03,endt_text04,endt_text05,endt_text06,
                        endt_text07,endt_text08,endt_text09,endt_text10,endt_text11,endt_text12,
                        endt_text13,endt_text14,endt_text15,endt_text16,endt_text17,
                        user_id, last_update)
                VALUES (p_set.policy_id,
                        p_set.endt_text01,p_set.endt_text02,p_set.endt_text03,p_set.endt_text04,p_set.endt_text05,p_set.endt_text06,
                        p_set.endt_text07,p_set.endt_text08,p_set.endt_text09,p_set.endt_text10,p_set.endt_text11,p_set.endt_text12,
                        p_set.endt_text13,p_set.endt_text14,p_set.endt_text15,p_set.endt_text16,p_set.endt_text17,
                        p_set.user_id, p_set.last_update)
            WHEN MATCHED THEN
                UPDATE
                    SET endt_text01 = p_set.endt_text01,
                        endt_text02 = p_set.endt_text02,
                        endt_text03 = p_set.endt_text03,
                        endt_text04 = p_set.endt_text04,
                        endt_text05 = p_set.endt_text05,
                        endt_text06 = p_set.endt_text06,
                        endt_text07 = p_set.endt_text07,
                        endt_text08 = p_set.endt_text08,
                        endt_text09 = p_set.endt_text09,
                        endt_text10 = p_set.endt_text10,
                        endt_text11 = p_set.endt_text11,
                        endt_text12 = p_set.endt_text12,
                        endt_text13 = p_set.endt_text13,
                        endt_text14 = p_set.endt_text14,
                        endt_text15 = p_set.endt_text15,
                        endt_text16 = p_set.endt_text16,
                        endt_text17 = p_set.endt_text17,
                        user_id    = p_set.user_id,
                        last_update = p_set.last_update;
        --added by MArkS 05.06.2016 SR-5381           
            MERGE INTO gipi_polgenin
             USING DUAL
                ON (policy_id = p_set.policy_id)
            WHEN NOT MATCHED THEN
                INSERT (policy_id, 
                        endt_text01,endt_text02,endt_text03,endt_text04,endt_text05,endt_text06,
                        endt_text07,endt_text08,endt_text09,endt_text10,endt_text11,endt_text12,
                        endt_text13,endt_text14,endt_text15,endt_text16,endt_text17,
                        user_id, last_update)
                VALUES (p_set.policy_id,
                        p_set.endt_text01,p_set.endt_text02,p_set.endt_text03,p_set.endt_text04,p_set.endt_text05,p_set.endt_text06,
                        p_set.endt_text07,p_set.endt_text08,p_set.endt_text09,p_set.endt_text10,p_set.endt_text11,p_set.endt_text12,
                        p_set.endt_text13,p_set.endt_text14,p_set.endt_text15,p_set.endt_text16,p_set.endt_text17,
                        p_set.user_id, p_set.last_update)
            WHEN MATCHED THEN
                UPDATE
                    SET endt_text01 = p_set.endt_text01,
                        endt_text02 = p_set.endt_text02,
                        endt_text03 = p_set.endt_text03,
                        endt_text04 = p_set.endt_text04,
                        endt_text05 = p_set.endt_text05,
                        endt_text06 = p_set.endt_text06,
                        endt_text07 = p_set.endt_text07,
                        endt_text08 = p_set.endt_text08,
                        endt_text09 = p_set.endt_text09,
                        endt_text10 = p_set.endt_text10,
                        endt_text11 = p_set.endt_text11,
                        endt_text12 = p_set.endt_text12,
                        endt_text13 = p_set.endt_text13,
                        endt_text14 = p_set.endt_text14,
                        endt_text15 = p_set.endt_text15,
                        endt_text16 = p_set.endt_text16,
                        endt_text17 = p_set.endt_text17,
                        user_id    = p_set.user_id,
                        last_update = p_set.last_update;
            -- end SR-5381   
    END set_endt_gipi_endt_text;         

    FUNCTION check_package(
        p_pack_policy_id    gipi_polbasic.pack_policy_id%TYPE       
    )
        RETURN VARCHAR
    IS
        v_check     VARCHAR2(1) := 'N';
    BEGIN
        FOR x IN (SELECT *
 		            FROM gipi_pack_polgenin
				   WHERE pack_policy_id = p_pack_policy_id)

		LOOP
  			v_check := 'Y';
		END LOOP;
        RETURN v_check; 
    END check_package;
    
    --marco - 08.18.2014 - optimized get_policy_listing
    FUNCTION get_policy_listing2 (
      p_user_id          giis_users.user_id%TYPE,
      p_pol_line_cd      gipi_polbasic.line_cd%TYPE,
      p_pol_subline_cd   gipi_polbasic.subline_cd%TYPE,
      p_pol_iss_cd       gipi_polbasic.iss_cd%TYPE,
      p_pol_issue_yy     gipi_polbasic.issue_yy%TYPE,
      p_pol_seq_no       gipi_polbasic.pol_seq_no%TYPE,
      p_pol_renew_no     gipi_polbasic.renew_no%TYPE,
      p_par_line_cd      gipi_parlist.line_cd%TYPE,
      p_par_iss_cd       gipi_parlist.iss_cd%TYPE,
      p_par_yy           gipi_parlist.par_yy%TYPE,
      p_par_seq_no       gipi_parlist.par_seq_no%TYPE,
      p_par_quote_seq_no gipi_parlist.quote_seq_no%TYPE,
      p_endt_iss_cd      gipi_polbasic.endt_iss_cd%TYPE,      
      p_endt_yy          gipi_polbasic.endt_yy%TYPE,
      p_endt_seq_no      gipi_polbasic.endt_seq_no%TYPE,
      p_assd_name        giis_assured.assd_name%TYPE
   )
      RETURN policy_tab PIPELINED
   IS
      TYPE cur_type IS REF CURSOR;
      v_pol              policy_type;
      v_query            VARCHAR2(32000);
      v_access_list      VARCHAR2(20000);
      cur                cur_type;
   BEGIN
      --v_access_list := CHECK_USER_ACCESS_PKG.get_accessible_list('GIPIS161', p_user_id);
   
      v_query := 'SELECT b.policy_id, b.line_cd, b.subline_cd, b.iss_cd, b.issue_yy, b.pol_seq_no, b.renew_no, ' ||
                         'line_cd ' ||
                         '|| ''-'' ' ||
                         '|| subline_cd ' ||
                         '|| ''-'' ' ||
                         '|| iss_cd ' ||
                         '|| ''-'' ' ||
                         '|| LTRIM (TO_CHAR (issue_yy, ''09'')) ' ||
                         '|| ''-'' ' ||
                         '|| LTRIM (TO_CHAR (pol_seq_no, ''0999999'')) ' ||
                         '|| ''-'' ' ||
                         '|| LTRIM (TO_CHAR (renew_no, ''09'')) policy_no, ' || 
                         'b.par_id, b.endt_iss_cd, b.endt_yy, b.endt_seq_no, ' ||
                         'endt_iss_cd ' ||
                         '|| ''-'' ' ||
                         '|| LTRIM (TO_CHAR (endt_yy, ''09'')) ' ||
                         '|| ''-'' ' ||
                         '|| LTRIM (TO_CHAR (endt_seq_no, ''0999999'')) endt_no, ' ||
                         'a.assd_name, b.pack_pol_flag, b.pack_policy_id ' ||
                    'FROM gipi_polbasic b, giis_assured a ' ||
                   'WHERE b.assd_no = a.assd_no ' ||
                     'AND b.line_cd = NVL(''' || p_pol_line_cd || ''', b.line_cd) ' ||
                     'AND b.subline_cd = NVL(''' || p_pol_subline_cd || ''', b.subline_cd) ' ||
                     'AND b.iss_cd = NVL(''' || p_pol_iss_cd || ''', b.iss_cd) ' ||
                     'AND b.issue_yy = NVL(''' || p_pol_issue_yy || ''', b.issue_yy) ' ||
                     'AND b.pol_seq_no = NVL(''' || p_pol_seq_no || ''', b.pol_seq_no) ' ||
                     'AND b.renew_no = NVL(''' || p_pol_renew_no || ''', b.renew_no) ' ||
                     'AND a.assd_name = NVL(''' || p_assd_name || ''', a.assd_name) ' ||
                     'AND b.endt_iss_cd = NVL(''' || p_endt_iss_cd || ''', b.endt_iss_cd) ' ||
                     'AND b.endt_yy = NVL(''' || p_endt_yy || ''', b.endt_yy) ' ||
                     'AND b.endt_seq_no = NVL(''' || p_endt_seq_no || ''', b.endt_seq_no) ' ||
                     'AND check_user_per_iss_cd2(b.line_cd, b.iss_cd, ''GIPIS161'', ''' || p_user_id || ''') = 1';
                     --'AND (b.line_cd, b.iss_cd) IN ' || v_access_list;
                     
        IF p_par_line_cd IS NOT NULL OR p_par_iss_cd IS NOT NULL OR p_par_yy IS NOT NULL OR p_par_seq_no IS NOT NULL OR p_par_quote_seq_no IS NOT NULL THEN
            v_query := v_query || 'AND b.par_id IN (SELECT par_id ' ||
                                                     'FROM gipi_parlist ' || 
                                                    'WHERE line_cd = NVL(''' || p_par_line_cd || ''', line_cd) ' ||
                                                      'AND iss_cd = NVL(''' || p_par_iss_cd || ''', iss_cd) ' ||
                                                      'AND par_yy = NVL(''' || p_par_yy || ''', par_yy) ' ||
                                                      'AND par_seq_no = NVL(''' || p_par_seq_no || ''', par_seq_no) ' ||
                                                      'AND quote_seq_no = NVL(''' || p_par_quote_seq_no || ''', quote_seq_no))';
        END IF;
        
        -- SR-21812 JET MAY-12-2016
        IF cpi.giisp.v('ALLOW_UPD_POLGENIN_PACK_SUBPOL') = 'N' OR cpi.giisp.v('ALLOW_UPD_POLGENIN_PACK_SUBPOL') IS NULL THEN
           v_query := v_query || 'AND b.pack_policy_id IS NULL';
        END IF;
                     
        OPEN cur FOR v_query;
        LOOP
            FETCH cur
             INTO v_pol.policy_id, v_pol.pol_line_cd, v_pol.pol_subline_cd, v_pol.pol_iss_cd, v_pol.pol_issue_yy, v_pol.pol_seq_no, v_pol.pol_renew_no,
                  v_pol.policy_no, v_pol.par_id, v_pol.endt_iss_cd, v_pol.endt_yy, v_pol.endt_seq_no, v_pol.endt_no, v_pol.assd_name,
                  v_pol.pack_pol_flag, v_pol.pack_policy_id;
                  
            SELECT a.line_cd
                   || '-'
                   || a.iss_cd
                   || '-'
                   || TRIM(TO_CHAR (a.par_yy, '09'))
                   || '-'
                   || TRIM(TO_CHAR (a.par_seq_no, '000009'))
                   || '-'
                   || TRIM(TO_CHAR (a.quote_seq_no, '09')) par_no, a.line_cd, a.iss_cd, 
                      TRIM(TO_CHAR (a.par_yy, '09')) par_yy, TRIM(TO_CHAR (a.par_seq_no, '000009')) par_seq_no, TRIM(TO_CHAR (a.quote_seq_no, '09')) quote_seq_no
              INTO v_pol.par_no, v_pol.par_line_cd, v_pol.par_iss_cd, v_pol.par_yy, v_pol.par_seq_no, v_pol.par_quote_seq_no
              FROM gipi_parlist a
             WHERE a.par_id = v_pol.par_id
               AND ROWNUM = 1;
            
            IF v_pol.endt_seq_no = 0 THEN
                v_pol.endt_iss_cd := NULL;
                v_pol.endt_yy := NULL;
                v_pol.endt_seq_no := NULL;
                v_pol.endt_no := NULL;
            END IF;
            
            IF v_pol.pack_policy_id IS NOT NULL THEN
               BEGIN
                  SELECT line_cd
                          || '-'
                          || subline_cd
                          || '-'
                          || iss_cd
                          || '-'
                          || TRIM(TO_CHAR (issue_yy, '009'))
                          || '-'
                          || TRIM(TO_CHAR (pol_seq_no, '00009')
                          || '-00') pack_no 
                     INTO v_pol.pack_no
                     FROM gipi_pack_polbasic
                    WHERE pack_policy_id = v_pol.pack_policy_id;
                  EXCEPTION
                      WHEN NO_DATA_FOUND THEN
                          v_pol.pack_no := NULL;
               END;
            END IF;
            
            EXIT WHEN cur%NOTFOUND;
            PIPE ROW (v_pol);
        END LOOP;
        CLOSE cur;
   END;
                   
END GIPIS161_PKG;
/


