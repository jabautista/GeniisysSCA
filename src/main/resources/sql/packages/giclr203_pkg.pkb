CREATE OR REPLACE PACKAGE BODY CPI.GICLR203_PKG
AS
   ...
   BEGIN
      FOR i IN (SELECT b.claim_no, DECODE (p_line_cd_tag, '0', p_line_cd_tag, b.line_cd) line_cd, DECODE (p_iss_cd_tag, '0', p_iss_cd_tag, b.iss_cd) iss_cd,
                 DECODE (p_intermediary_tag, '0', p_intermediary_tag, b.intm_type) intm_type, DECODE (p_intermediary_tag, '0', p_intermediary_tag, b.buss_source
                 ) intm_ri_no, b.claim_id, b.line_cd line_cd2, b.policy_no, b.assd_no, b.incept_date, b.iss_cd iss_cd2, b.expiry_date, b.recovered_amt,
                 b.loss_cat_cd, b.item_no, b.grouped_item_no, b.peril_cd, b.loss_date, b.clm_file_date, b.clm_stat_cd, b.tsi_amt, b.prem_amt,
                 b.loss_reserve, b.expense_reserve, b.losses_paid, b.expenses_paid--, B.BRDRX_RECORD_ID
           ...
           WHERE b.session_id = p_session_id
             ...
             AND NVL (b.buss_source, '*') = NVL (p_intm_no, NVL (b.buss_source, '*'))      
      )
      LOOP
...
--         v_list.expiry_date := i.expiry_date;
         v_list.incept_date := TO_CHAR(i.incept_date, 'mm-dd-yyyy'); -- bonok :: 7.9.2015 :: UCPB Fullweb :: SR 19674 :: set to string for print to excel
         v_list.expiry_date := TO_CHAR(i.expiry_date, 'mm-dd-yyyy');
--         v_list.clm_file_date := i.clm_file_date;
--         v_list.loss_date := i.loss_date;
         v_list.clm_file_date := TO_CHAR(i.clm_file_date, 'mm-dd-yyyy'); -- bonok :: 7.9.2015 :: UCPB Fullweb :: SR 19674 :: set to string for print to excel
         ...
         BEGIN
           v_list.cf_break := NULL;
           IF NVL(p_line_cd_tag,'1') != '0' then
              v_list.cf_break := 'LINE';
           END IF;
           IF NVL(p_iss_cd_tag,'1') != '0' then
              IF v_list.cf_break is not null then
                 v_list.cf_break := v_list.cf_break || '-ISSUE SOURCE';
              ELSE
                 v_list.cf_break := v_list.cf_break || 'ISSUE SOURCE';
              END IF;
           END IF;
         ...
         BEGIN
           SELECT line_name 
             ...
             FROM giis_line
            WHERE line_cd = i.line_cd2; --i.line_cd; benjo 10.01.2015 GENQA-SR-4925
         EXCEPTION
             WHEN NO_DATA_FOUND THEN
         ...
         BEGIN
           SELECT iss_name
             ...
             FROM giis_issource
            WHERE iss_cd = i.iss_cd2; --i.iss_cd; benjo 10.01.2015 GENQA-SR-4925
         EXCEPTION
             WHEN NO_DATA_FOUND THEN
         ...
         BEGIN
           SELECT loss_cat_des
            ...
            WHERE loss_cat_cd = i.loss_cat_cd
             AND line_cd = i.line_cd2; --i.line_cd; benjo 10.01.2015 GENQA-SR-4925
         EXCEPTION
             WHEN NO_DATA_FOUND THEN
   ...
   */      
   FUNCTION CSV_GICLR203 ( 
      ...
      RETURN GICLR203_CSV_TABLE PIPELINED
   IS
      v_list                GICLR203_CSV_REPORT;
      v_pol_iss_cd          gicl_claims.pol_iss_cd%type;
   ...
   BEGIN
      FOR i IN (SELECT b.claim_no, DECODE (p_line_cd_tag, '0', p_line_cd_tag, b.line_cd) line_cd, DECODE (p_iss_cd_tag, '0', p_iss_cd_tag, b.iss_cd) iss_cd,
           ...
           WHERE b.session_id = p_session_id
             ...
             AND b.peril_cd = NVL (TO_NUMBER (p_peril_cd), b.peril_cd)
             AND NVL (b.buss_source, '*') = NVL (p_intm_no, NVL (b.buss_source, '*'))
           ORDER BY line_cd, iss_cd, claim_no, policy_no
      )
      LOOP
         v_list.claim_no := i.claim_no;
         v_list.policy_no := i.policy_no;
         ...
         BEGIN
           SELECT loss_cat_des
             ...
             FROM giis_loss_ctgry
            WHERE loss_cat_cd = i.loss_cat_cd
             AND line_cd = i.line_cd2; --Changed to line_cd2 CarloR SR22508 06.28.2016
         EXCEPTION
             WHEN NO_DATA_FOUND THEN
                  v_list.loss_cat_des := null;
         END;
         ...
         BEGIN
           SELECT peril_name 
             INTO v_list.peril_name
             FROM giis_peril
            WHERE line_cd  = i.line_cd2 --Changed to line_cd2 CarloR SR22508 06.28.2016
              AND peril_cd = i.peril_cd;
         EXCEPTION
             WHEN NO_DATA_FOUND THEN
                  v_list.peril_name := null;
      ...
      RETURN;
   END CSV_GICLR203; -- Dren 09.15.2015 SR 0020264 : Added CSV Report for GICLR203 - End      
