CREATE OR REPLACE PACKAGE BODY CPI.GICLR206E_PKG
AS
/*
    **  Created by   :  Kenneth Mark Labrador
    **  Date Created : 04.10.2013
    **  Reference By : GICLR206E - LOSS EXPENSE BORDEREAUX
    */
   ...
   BEGIN
      FOR i IN (SELECT a.brdrx_record_id, a.buss_source, a.iss_cd, a.line_cd,
                       ...
                       NVL (a.expenses_paid, 0) paid_losses, a.expenses_paid
                  FROM gicl_res_brdrx_extr a
                 WHERE a.session_id = p_session_id
                   AND a.claim_id = NVL (p_claim_id, a.claim_id)
                   AND NVL (a.expenses_paid, 0) <> 0
   ...
   FUNCTION get_giclr206e_main(
      ...
      RETURN giclr206e_main_tab PIPELINED
   IS
      v_row                giclr206e_main_type;
   ...
   BEGIN
      FOR i IN (SELECT DISTINCT DECODE (a.iss_cd, 'RI', 'RI', 'DI') iss_type,
                          ...
                          ORDER BY DECODE(p_intm_break, 1, DECODE (a.iss_cd, 'RI', 'RI', b.intm_type), a.buss_source), a.iss_cd, a.line_cd, a.subline_cd, a.loss_year)
      LOOP
         v_row := NULL;
   ...
   FUNCTION get_giclr206e_detail(
     ...
     RETURN giclr206e_detail_tab PIPELINED
   IS
      v_row                giclr206e_detail_type;
   BEGIN
   ...
   FUNCTION get_giclr206e_item(
     ...
     RETURN giclr206e_item_tab PIPELINED
   IS
      v_row                giclr206e_item_type;
   BEGIN
   ...
   FUNCTION get_giclr206e_peril(
     ...
     RETURN giclr206e_peril_tab PIPELINED
   IS
      v_row                giclr206e_peril_type;
   BEGIN
      FOR i IN(SELECT a.tsi_amt, a.peril_cd, b.peril_name, a.intm_no,
                      NVL(a.expenses_paid,0) paid_losses, a.brdrx_record_id --added by gab 03.21.2016 SR 21796
                 FROM GICL_RES_BRDRX_EXTR a,
                      GIIS_PERIL b
                WHERE a.session_id = p_session_id
                  AND a.claim_id = NVL (p_claim_id, a.claim_id)
      ...
      LOOP
         ...
         v_row.intm_cedant := CPI.GICLR206E_PKG.intm_ri_formula(p_claim_id, p_session_id, p_item_no, i.peril_cd, i.intm_no, p_intm_break);
         v_row.dv_no := CPI.GICLS202_PKG.get_voucher_check_no(p_claim_id, p_item_no, i.peril_cd, p_grouped_item_no, p_from_date, p_to_date, p_paid_date, 'E');
   ...
   BEGIN
      FOR i IN(SELECT grp_seq_no1, grp_seq_no2, grp_seq_no3, grp_seq_no4
                 FROM TABLE(CPI.GICLR206E_PKG.get_giclr206e_main(NULL, p_session_id, NULL, NULL, NULL, NULL, NULL))
                WHERE dummy = p_dummy
   ...
   FUNCTION get_giclr206e_treaty_total(
      p_session_id         GICL_RES_BRDRX_EXTR.session_id%TYPE,
      p_buss_source        GICL_RES_BRDRX_EXTR.buss_source%TYPE,
      p_iss_cd             GICL_RES_BRDRX_EXTR.iss_cd%TYPE,
      p_line_cd            GICL_RES_BRDRX_EXTR.line_cd%TYPE,
   ...
   BEGIN
      ...
      LOOP
         ...
         FOR s IN(SELECT SUM(NVL(a.expenses_paid,0)) paid_losses
                    FROM GICL_RES_BRDRX_DS_EXTR a
                   WHERE a.session_id = p_session_id
                     AND NVL(a.expenses_paid,0) <> 0 
                     AND a.buss_source = p_buss_source
   ...
   FUNCTION get_giclr206e_treaty_ri(
      p_session_id         GICL_RES_BRDRX_EXTR.session_id%TYPE,
      p_buss_source        GICL_RES_BRDRX_EXTR.buss_source%TYPE,
      p_iss_cd             GICL_RES_BRDRX_EXTR.iss_cd%TYPE,
      p_line_cd            GICL_RES_BRDRX_EXTR.line_cd%TYPE,
