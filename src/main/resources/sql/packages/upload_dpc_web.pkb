CREATE OR REPLACE PACKAGE BODY CPI.UPLOAD_DPC_WEB AS
  ...
  */
  ...
  BEGIN
   ...
   IF v_closed_tag = 'T' THEN
      RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#You are no longer allowed to create a transaction for '
  ...
  */
  ...
  BEGIN
   ...
   INTO v_cashier_cd, v_dcb_user_id, v_valid_tag,
         v_eff_dt    , v_exp_dt;
   IF C%NOTFOUND THEN
      RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#You are not authorized to issue an O.R. for '
   ...
   END IF;
    ...
    IF v_eff_dt > p_date OR v_eff_dt > TRUNC(SYSDATE) THEN
      RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#Your authority to issue an O.R. for '
                                    ||'this company/branch ('||pvar_fund_cd||'/'||pvar_branch_cd||') '
  ...
  BEGIN
   p_dcb_no := NULL;
    ...
    IF p_dcb_no IS NULL THEN
      RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#There is no open DCB No. dated '
  ...
  */
  ...
  EXCEPTION
   WHEN NO_DATA_FOUND THEN
      RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#GL account code: '||trim(TO_CHAR(cca_gl_acct_category))
 ...
 CURSOR pr_cur_cw IS
  ...
  BEGIN
   ...
   pvar_tran_id := aeg_tran_id;
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in table GIAC_MODULES: '||aeg_module_nm||'.');
    END;


    --Jason 07/27/2007: Call the deletion of accounting entry procedure.

    Upload_Dpc_Web.AEG_Delete_Acct_Entries(aeg_tran_id,
                                    pvar_gen_type);

    --call the accounting entry generation procedure.
    /*FOR pr_rec IN pr_cur
    LOOP
    Upload_Dpc_Web.aeg_create_acct_entries(pvar_module_id,
                                         1,
                                       pr_rec.amount,
                                       pvar_gen_type);
 ...
 FOR pr_rec IN pr_cur LOOP
   Upload_Dpc_Web.aeg_create_cib_acct_entries(aeg_tran_id,
                             aeg_branch_cd,
                            aeg_fund_cd,
                                          pr_rec.dcb_bank_cd,
  ...
  *****************************************************************************/
  ...
  BEGIN
   ...
   EXCEPTION
     WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in giac_module_entries: '
   ...
   Upload_Dpc_Web.aeg_check_chart_of_accts(ws_gl_acct_category, ws_gl_control_acct, ws_gl_sub_acct_1,
   ...
   Upload_Dpc_Web.aeg_insert_update_acct_entries(ws_gl_acct_category, ws_gl_control_acct, ws_gl_sub_acct_1,
                                             ws_gl_sub_acct_2   , ws_gl_sub_acct_3  , ws_gl_sub_acct_4,
                                             ws_gl_sub_acct_5   , ws_gl_sub_acct_6  , ws_gl_sub_acct_7,
                                             aeg_gen_type       , ws_gl_acct_id     , ws_debit_amt    ,
                                             ws_credit_amt, p_user_id);
  ...
  BEGIN
   ...
   IF NVL(Giacp.v('PREM_REC_GROSS_TAG'),'Y') = 'Y' THEN
    IF NVL(Giacp.v('ENTER_ADVANCED_PAYT'),'N') = 'Y' THEN
      Upload_Dpc_Web.aeg_parameters_y_prem_rec(aeg_tran_id, aeg_module_nm, p_user_id);
      Upload_Dpc_Web.aeg_parameters_y_prem_dep(aeg_tran_id, aeg_module_nm, p_user_id);
    ELSE
     Upload_Dpc_Web.aeg_parameters_y(aeg_tran_id, aeg_module_nm, p_user_id);
    END IF;
   ELSE
     Upload_Dpc_Web.aeg_parameters_n(aeg_tran_id, aeg_module_nm, p_user_id);
   END IF;
  END;
  ...
  */
  ...
  BEGIN
   ...
   EXCEPTION
    ...
    LOOP
      --call the accounting entry generation procedure.
      Upload_Dpc_Web.aeg_create_dpc_acct_entries (pr_rec.assd_no,  pvar_module_id       ,
                                              1             ,  pr_rec.iss_cd        ,
                                              pr_rec.bill_no,  pr_rec.line_cd       ,
                                              pr_rec.type_cd,  pr_rec.collection_amt,
                                              pvar_gen_type, p_user_id);
    ...
    IF NVL(Giacp.v('OUTPUT_VAT_COLLN_ENTRY'),'N') = 'Y' THEN
     ...
     LOOP
       ...
       Upload_Dpc_Web.aeg_create_dpc_acct_entries (NULL          ,  pvar_module_id  ,
                                               7             ,  pr_rec.iss_cd   ,
                                               pr_rec.bill_no,  pr_rec.line_cd  ,
  ...
  *****************************************************************************/
  ...
  BEGIN
   ...
   IF ws_intm_type_level != 0 THEN
     ...
     EXCEPTION
       WHEN NO_DATA_FOUND THEN
          RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in giis_intm_type.');
     END;
     Upload_Dpc_Web.aeg_check_level(ws_intm_type_level, ws_acct_intm_cd , ws_gl_sub_acct_1,
                                ws_gl_sub_acct_2  , ws_gl_sub_acct_3, ws_gl_sub_acct_4,
                                ws_gl_sub_acct_5  , ws_gl_sub_acct_6, ws_gl_sub_acct_7);
   END IF;
   ...
   IF ws_line_dep_level != 0 THEN
     ...
     EXCEPTION
       WHEN NO_DATA_FOUND THEN
          RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in giis_line.');
     END;
    Upload_Dpc_Web.aeg_check_level(ws_line_dep_level, ws_line_cd      , ws_gl_sub_acct_1,
                               ws_gl_sub_acct_2 , ws_gl_sub_acct_3, ws_gl_sub_acct_4,
                               ws_gl_sub_acct_5 , ws_gl_sub_acct_6, ws_gl_sub_acct_7);
   END IF;
    ...
    **************************************************************************/
    ...
    IF ws_old_new_acct_level != 0 THEN
      BEGIN
        ...
        END;
        Upload_Dpc_Web.aeg_check_level(ws_old_new_acct_level, ws_new_acct_cd  , ws_gl_sub_acct_1,
                                   ws_gl_sub_acct_2     , ws_gl_sub_acct_3, ws_gl_sub_acct_4,
                                   ws_gl_sub_acct_5     , ws_gl_sub_acct_6, ws_gl_sub_acct_7);
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in giac_parameters.');
      END;
    END IF;
    ...
    **************************************************************************/
    ...
    IF ws_pol_type_tag = 'Y' THEN
      ...
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in giac_policy_type_entries.');
      END;
    END IF;
   ...
   Upload_Dpc_Web.aeg_ins_upd_dpc_acct_entries(ws_gl_acct_category, ws_gl_control_acct, ws_gl_sub_acct_1,
                                           ws_gl_sub_acct_2   , ws_gl_sub_acct_3  , ws_gl_sub_acct_4,
                                           ws_gl_sub_acct_5   , ws_gl_sub_acct_6  , ws_gl_sub_acct_7,
                                           aeg_sl_cd          , ws_sl_type_cd     , aeg_gen_type    ,
                                           ws_gl_acct_id      , ws_debit_amt      , ws_credit_amt, p_user_id);
    ...
    IF NVL(iuae_acct_entry_id,0) = 0 THEN
     ...
     VALUES
      (pvar_tran_id        ,  pvar_fund_cd         ,
       ...
       iuae_generation_type,  p_user_id                 ,
       SYSDATE);
    ELSE
      UPDATE GIAC_ACCT_ENTRIES
         SET debit_amt  = debit_amt  + iuae_debit_amt,
  ...
  */
  ...
  BEGIN
   ...
   EXCEPTION
    WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in table GIAC_MODULES: '||p_module_nm||'.');
    END;
    ...
    -- assd_no is sometimes null therefore look into GIPI_PARLIST
     ...
     EXCEPTION
      WHEN NO_DATA_FOUND THEN
       RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#' || pr_rec.iss_cd||'-'||TO_CHAR(pr_rec.bill_no)||' has no assured.');
     END;
      END IF;

      IF v_rec_gross_tag = 'Y' THEN
     Upload_Dpc_Web.aeg_create_dpc_acct_entries(NVL(pr_rec.assd_no,v_assd_no), pvar_module_id  ,
     ...
     IF pr_rec.tax_amt != 0 THEN
      ...
      LOOP
       Upload_Dpc_Web.aeg_create_acct_entries_tax_n(rec.tax_cd,rec.tax_amt, pvar_gen_type, p_user_id);
          END LOOP; -- end of rec
  ...
  BEGIN
    ...
    **************************************************************************/
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        BEGIN
          RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#GL account code '
   ...
   Upload_Dpc_Web.aeg_ins_upd_acct_tax_n(ws_gl_acct_category, ws_gl_control_acct, ws_gl_sub_acct_1,
                                     ws_gl_sub_acct_2   , ws_gl_sub_acct_3  , ws_gl_sub_acct_4,
                                     ws_gl_sub_acct_5   , ws_gl_sub_acct_6  , ws_gl_sub_acct_7,
                                     NULL               , aeg_gen_type      , ws_gl_acct_id   ,
                                     ws_debit_amt       , ws_credit_amt     , p_user_id);
  ...
  BEGIN
    ...
    IF prem_deposit_rec.ri_cd IS NULL THEN
     ...
     ws_sl_cd := prem_deposit_rec.assd_no;
      END IF;
      --call the accounting entry generation procedure.
      Upload_Dpc_Web.aeg_create_pdep_acct_entries(prem_deposit_rec.collection_amt, pvar_gen_type,  pvar_module_id,
                                  v_item_no                      , ws_sl_cd     ,  ws_sl_type_cd ,
                                              ws_sl_source_cd, p_user_id);
    END LOOP;
  ...
  */
  ...
  BEGIN
    ...
    ****************************************************************************/
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#GL account code '||trim(TO_CHAR(ws_gl_acct_category))
                                      ||'-'||trim(TO_CHAR(ws_gl_control_acct,'09'))
                                      ||'-'||trim(TO_CHAR(ws_gl_sub_acct_1,'09'))
                                      ||'-'||trim(TO_CHAR(ws_gl_sub_acct_2,'09'))
                                      ||'-'||trim(TO_CHAR(ws_gl_sub_acct_3,'09'))
    ...
    END;
     ...
     Upload_Dpc_Web.aeg_ins_upd_pdep_acct_entries (ws_gl_acct_category, ws_gl_control_acct, ws_gl_sub_acct_1,
                                               ws_gl_sub_acct_2   , ws_gl_sub_acct_3  , ws_gl_sub_acct_4,
                                               ws_gl_sub_acct_5   , ws_gl_sub_acct_6  , ws_gl_sub_acct_7,
  ...
  */
  ...
  BEGIN
    ...
    --call the accounting entry generation procedure.
    Upload_Dpc_Web.aeg_create_misc_acct_entries(aeg_collection_amt,  pvar_gen_type,  pvar_module_id,
  ...
  */
  ...
  BEGIN
   ...
   Upload_Dpc_Web.aeg_check_chart_of_accts(ws_gl_acct_category, ws_gl_control_acct, ws_gl_sub_acct_1,
    ...
    Upload_Dpc_Web.aeg_ins_upd_misc_acct_entries (ws_gl_acct_category, ws_gl_control_acct, ws_gl_sub_acct_1,
                                              ws_gl_sub_acct_2   , ws_gl_sub_acct_3  , ws_gl_sub_acct_4,
                                              ws_gl_sub_acct_5   , ws_gl_sub_acct_6  , ws_gl_sub_acct_7,
    ...
    IF pvar_comm_take_up IN (1,2) THEN
      BEGIN
        ...
        LOOP
          ...
          Upload_Dpc_Web.comm_payable_proc (premium_rec.intm_no,       premium_rec.assd_no,
      ...
      ** This is handled by the function ABS(). */
      END;
    ELSE
      RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#This module was designed to upload data only if the parameter: COMM_PAYABLE_TAKE_UP = 1 or 2.');
    END IF;
    ...
    ELSIF pvar_sl_type_cd1 = 'INTM_SL_TYPE' THEN
      BEGIN
        ...
        WHILE v_loop LOOP
          ...
          EXCEPTION
            WHEN NO_DATA_FOUND THEN
              RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#Parent Intermediary No. does not exist. Intermediary No. = '||TO_CHAR(v_intm_no));
          END;
        END LOOP;
      END;
    ELSE
  ...
  */
         ...
         IF SQL%NOTFOUND THEN  --insert the record if there were no records updated
           INSERT INTO giac_prepaid_comm
              ...
              VALUES (j.gacc_tran_id,  j.policy_id,    j.tran_type,
                      ...
                      p_user_id,            SYSDATE,        j.multi_booking_mm,
                      j.multi_booking_yy);
        END IF;
      END LOOP;
  ...
  END IF;
    ...
    Upload_Dpc_Web.get_sl_code (p_intm_no,    p_assd_no,    p_acct_line_cd,
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in table giac_module_entries. Item no. '||pvar_item_num||' for GIACS020.');
    END;

    IF v_intm_type_level != 0 THEN
      ws_acct_intm_cd := v_gl_intm_no;
      Upload_Dpc_Web.aeg_check_level (v_intm_type_level, ws_acct_intm_cd,
    ...
    IF v_line_dependency_level != 0 THEN
      ...
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in table giis_line. Line code '||p_line_cd||'.');
      END;

      Upload_Dpc_Web.aeg_check_level (v_line_dependency_level, ws_line_cd,
                                  v_gl_sub_acct_1,         v_gl_sub_acct_2,
                                  v_gl_sub_acct_3,         v_gl_sub_acct_4,
                                  v_gl_sub_acct_5,         v_gl_sub_acct_6,
                                  v_gl_sub_acct_7);
    ...
    Upload_Dpc_Web.aeg_check_chart_of_accts(v_gl_acct_category, v_gl_control_acct,
                                        v_gl_sub_acct_1,    v_gl_sub_acct_2,
                                        v_gl_sub_acct_3,    v_gl_sub_acct_4,
                                        v_gl_sub_acct_5,    v_gl_sub_acct_6,
    ...
    Upload_Dpc_Web.aeg_ins_upd_comm_acct_entries (v_gl_acct_category, v_gl_control_acct, v_gl_sub_acct_1, v_gl_sub_acct_2,
    ...
    BEGIN
      IF NVL(p_wtax_amt,0) <> 0 THEN
        ...
        EXCEPTION
          WHEN NO_DATA_FOUND THEN
            RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in table giac_module_entries. Item no. 4 for GIACS020.');
        END;

        Upload_Dpc_Web.aeg_check_level (v_intm_type_level, p_sl_cd2,
                                    v_gl_sub_acct_1,   v_gl_sub_acct_2,
                                    v_gl_sub_acct_3,   v_gl_sub_acct_4,
                                    v_gl_sub_acct_5,   v_gl_sub_acct_6,
                                    v_gl_sub_acct_7);
    ...
    IF NVL(p_input_vat_amt,0) != 0 THEN
      ...
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in table giac_module_entries. Item no. 5 for GIACS020.');
      END;
 ...
 v_acct_entry_id     GIAC_ACCT_ENTRIES.acct_entry_id%TYPE;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20210,'Geniisys Exception#I#No data found in giac_bank_accounts for bank_cd/bank_acct_cd: '
 ...
 EXCEPTION
   WHEN NO_DATA_FOUND THEN
  RAISE_APPLICATION_ERROR(-20210,'Geniisys Exception#I#No record in the Chart of Accounts for this GL ID '
  ...
  */
    ...
    IS
       ...
       ** nieko Accounting Uploading GIACS608
       ...
       IS
          SELECT c.b140_iss_cd iss_cd, c.collection_amt,
                 ...
                 DECODE (aeg_sl_type_cd1,
                         'ASSD_SL_TYPE', g.a020_assd_no,
                         'RI_SL_TYPE', g.a180_ri_cd,
       ...
       CURSOR premium_cur
       IS
          SELECT c.b140_iss_cd iss_cd, (c.premium_amt + c.tax_amount
                                       ) premium_amt, c.b140_prem_seq_no bill_no,
                 a.line_cd,
                 DECODE (aeg_sl_type_cd1,
                         'ASSD_SL_TYPE', g.a020_assd_no,
                         'RI_SL_TYPE', g.a180_ri_cd,
       ...
       IS
          SELECT c.b140_iss_cd iss_cd, SUM(c.collection_amt) collection_amt,
                 ...
                 a.type_cd, SUM(c.tax_amount) tax_amount, SUM(c.comm_vat) comm_vat
            FROM gipi_polbasic a,
                 gipi_invoice b,
                 giac_inwfacul_prem_collns c,
                 giac_aging_ri_soa_details g,
       ...
       CURSOR premium_cur
       IS
          SELECT c.b140_iss_cd iss_cd, SUM(c.premium_amt + c.tax_amount) premium_amt, 
                 c.b140_prem_seq_no bill_no,
                 a.line_cd,
                 DECODE (aeg_sl_type_cd1,
                         'ASSD_SL_TYPE', g.a020_assd_no,
       ...
       CURSOR commission_cur
       IS
          SELECT c.b140_iss_cd iss_cd, SUM(c.comm_amt) comm_amt, c.b140_prem_seq_no bill_no,
                 a.line_cd,
                 DECODE (aeg_sl_type_cd2,
                         'ASSD_SL_TYPE', g.a020_assd_no,
                         'RI_SL_TYPE', g.a180_ri_cd,
       ...
       v_gen_type          giac_modules.generation_type%TYPE;
    BEGIN
       pvar_tran_id := aeg_tran_id;         --nieko Accounting Uploading GIACS608
       ...
       THEN
          FOR pr_rec IN pr_cur
          LOOP
             aeg_create_inwfacul_entries (pr_rec.sl_cd,
                                          v_module_id,
                                          1,
                                          pr_rec.iss_cd,
                                          pr_rec.bill_no,
             ...
             END IF;
          END LOOP;
       ELSIF v_inwprem_take_up = 2
       THEN
          FOR premium_rec IN premium_cur
          LOOP
             aeg_create_inwfacul_entries (premium_rec.sl_cd,
                                          v_module_id,
                                          1,
                                          premium_rec.iss_cd,
                                          premium_rec.bill_no,
          ...
          LOOP
             aeg_create_inwfacul_entries (commission_rec.sl_cd,
                                          v_module_id,
                                          2,
 ...
 IF ws_old_new_acct_level != 0 THEN
      ...
      EXCEPTION
        WHEN NO_DATA_FOUND THEN
          RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in giac parameters. ');
      END;
    END IF;
    ...
    BEGIN
       ...
       THEN
          ...
          INSERT INTO giac_acct_entries
               ...
               VALUES (pvar_tran_id, pvar_fund_cd, pvar_branch_cd,
       ...
       ELSE
          UPDATE giac_acct_entries
             SET debit_amt = debit_amt + iuae_debit_amt,
                 credit_amt = credit_amt + iuae_credit_amt
           WHERE NVL2 (iuae_sl_cd, sl_cd, 0) = NVL (iuae_sl_cd, 0)
             AND generation_type = iuae_generation_type
             AND gl_acct_id = iuae_gl_acct_id
             AND gacc_gibr_branch_cd = pvar_branch_cd
             AND gacc_gfun_fund_cd = pvar_fund_cd
 ...
 v_ri_sl_cd     GIAC_PARAMETERS.param_value_v%TYPE;
  ...
  BEGIN
    ...
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#No data found in GIAC MODULES.');
  END;
 ...
 IF NVL(Giacp.v('DEF_RI_VAT_ENTRY'), 'Y') = 'Y' THEN
  ...
  BEGIN
   ...
   AND gl_sub_acct_4     = ws_gl_sub_acct_4
      ...
      EXCEPTION
       WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#GL account code '||TO_CHAR(ws_gl_acct_category)
  ...
  BEGIN
    ...
    IF v_sl_type <> v_ri_sl_cd THEN
      RAISE_APPLICATION_ERROR(-20210,'Geniisys Exception#I#The SL type of this account(gl_acct_id: ' ||
    ...
    IF v_leaf_tag = 'N' THEN
      RAISE_APPLICATION_ERROR(-20210, 'Geniisys Exception#I#This account(gl_acct_id: ' ||
                                      TO_CHAR(aeg_gl_acct_id) ||
                                      ') ' ||
