CREATE OR REPLACE PACKAGE BODY CPI.extract_paid_le_enrollee
AS
      ...
      IS
         SELECT   b.claim_id, b.item_no, b.peril_cd, b.loss_cat_cd,
            ...
            WHERE a.peril_cd = b.peril_cd
              ...
              AND g.grouped_item_title =
                                        NVL (p_enrollee, g.grouped_item_title)
              AND NVL (g.control_type_cd, 0) =
                              NVL (p_control_type, NVL (g.control_type_cd, 0))
              AND NVL (g.control_cd, 0) =
                                 NVL (p_control_number, NVL (g.control_cd, 0))
         ...
         SELECT   b.claim_id, b.item_no, b.peril_cd, b.loss_cat_cd,
            ...
            WHERE a.peril_cd = b.peril_cd
              ...
              **checked if within date parameters.*/
              ...
              AND NVL (g.control_cd, 0) =
                                 NVL (p_control_number, NVL (g.control_cd, 0))
            --AND clm_stat_cd NOT IN ('WD', 'DN', 'CC')		--commented out by MJ 03/08/2013. Confirmed by Ms Jen.
            --AND (TRUNC(a.cancel_date) NOT BETWEEN TRUNC(p_from_date) AND TRUNC(p_to_date)
                 --OR a.cancel_date IS NULL) --Edison 06.05.2012  --comment out jcDY 10.04.2012 to query cancelled transactions
              /*Added by: jcDY.20121017
   ...
   IS
      CURSOR brdrx_extr_paid_ds (
         brdrx_extr_session_id   IN   gicl_res_brdrx_ds_extr.session_id%TYPE
      )
      IS
       SELECT   a.claim_id, a.item_no, a.peril_cd, a.clm_loss_id, a.grp_seq_no,
         a.shr_loss_exp_pct,         
            /*modified by Edison 04.13.2012, changed payee_type to
            **get_payee_type(b.claim_id, b.clm_loss_id
            **changed also b.currency_rate to b.convert_rate*/
         DECODE
            (get_payee_type (b.claim_id, b.clm_loss_id),
...
GROUP BY a.claim_id,
      ...
      IS
        ...
        WHERE    a.claim_id = paid_rids_claim_id
            ...
            /*added by: jcDY 10/19/2012
            **exclude cancelled distributions*/
			/* Commented out by MJ 03/08/2013. Confirmed by Ms Jen.
             AND DECODE (b.cancel_tag,
                         'Y', TRUNC (b.cancel_date),
                        p_to_date + 1
                       ) > p_to_date
