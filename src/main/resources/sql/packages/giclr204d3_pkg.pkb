CREATE OR REPLACE PACKAGE BODY CPI.GICLR204D3_PKG AS
...
BEGIN
    FOR a IN(
        SELECT      b.intm_no || ' - ' || c.intm_name intermediary_,
                    ...
                    c.intm_name,
                    d.assd_name,
                    TO_CHAR(TO_DATE(TO_CHAR(date_for_24th,'MONTH YYYY'),'MONTH YYYY') ,'MONTH YYYY') transaction_month,
                ...
                GROUP BY    b.intm_no, 
                            ...
                            c.intm_name,
                            d.assd_name, 
                            TO_CHAR(TO_DATE(TO_CHAR(date_for_24th,'MONTH YYYY'),'MONTH YYYY') ,'MONTH YYYY'),
                            'PREMIUMS WRITTEN FOR '|| p_curr_start_date || ' TO ' ||  p_curr_end_date 
                            ORDER BY TO_CHAR(TO_DATE(TO_CHAR(date_for_24th,'MONTH YYYY'),'MONTH YYYY') ,'MONTH YYYY') DESC, policy_no
...
BEGIN
    FOR a IN(
        ...
        FROM    gipi_polbasic a, gicl_lratio_prev_prem_ext b, giis_intermediary c, giis_assured d
        WHERE   b.policy_id = a.policy_id
        AND     b.assd_no = d.assd_no (+)
        AND     b.intm_no = c.intm_no (+)
        AND     b.session_id = p_session_id
