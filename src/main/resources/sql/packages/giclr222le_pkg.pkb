CREATE OR REPLACE PACKAGE BODY CPI.GICLR222LE_PKG
AS
    /**
    **  Created By:     Shan Bati
    **  Date Created:   04.08.2013
    **  Referenced BY:  GICLR222LE - LOSS EXPENSE PAID BORDEREAUX (PER POLICY)
    **/
        ...
        BEGIN
            SELECT DISTINCT b.ref_pol_no -- Modified by Marlo 02172010
             ...
             WHERE a.line_cd = b.line_cd
               ...
               AND b.ref_pol_no IS NOT NULL; --added by steven 05.02.2014
        EXCEPTION
            WHEN NO_DATA_FOUND THEN
                v_ref_pol_no := NULL; 
        END;
        ...
        LOOP
            FOR c in (SELECT A.BRDRX_RECORD_ID, A.CLAIM_ID, A.LINE_CD, A.CLAIM_NO,
                       ...
                       WHERE A.SESSION_ID = p_session_id
                         ...
                         AND (ABS(NVL(A.LOSSES_PAID,0)) > 0
                              OR ABS(NVL(A.EXPENSES_PAID, 0)) > 0) 
                         AND A.POLICY_NO = /*NVL(P_POLICY_NO, A.POLICY_NO)--*/b.POLICY_NO
            ...
            LOOP
                ...
                rep.paid_expenses       := c.paid_expenses;
                ...
                rep.cf_dv_no            := CF_DV_NOFormula(c.claim_id, c.item_no, c.clm_loss_id, c.paid_losses, c.paid_expenses, p_paid_date, p_from_date, p_to_date);
    ...
    BEGIN
        ...
        LOOP
            ...
            LOOP
                ...
                FOR h IN (SELECT A.BRDRX_RECORD_ID, A.GRP_SEQ_NO, A.SHR_PCT, 
                                 SUM(NVL(A.LOSSES_PAID,0)) PAID_LOSSES, SUM(NVL(A.EXPENSES_PAID,0)) PAID_EXPENSES
                            FROM GICL_RES_BRDRX_DS_EXTR A
                           WHERE A.SESSION_ID = p_session_id
                             AND (ABS(NVL(A.LOSSES_PAID,0)) > 0
                                  OR ABS(NVL(A.EXPENSES_PAID, 0)) > 0)
                             ...
                             AND A.BRDRX_RECORD_ID = c.brdrx_record_id
                             GROUP BY a.brdrx_record_id, a.grp_seq_no, a.shr_pct
                           ORDER BY A.GRP_SEQ_NO )
                LOOP
    ...
    BEGIN
        ...
        LOOP
            ...
            LOOP
                ...
                FOR h IN (SELECT A.BRDRX_RECORD_ID, A.BRDRX_DS_RECORD_ID, A.GRP_SEQ_NO, A.SHR_PCT, 
                ...
                LOOP
                    ...
                    FOR i IN (SELECT A.BRDRX_DS_RECORD_ID,
                                     ...
                                     SUM(DECODE(SIGN(NVL(A.LOSSES_PAID,0)), 1, NVL(A.LOSSES_PAID,0), 0)) PAID_LOSSES2,
                                     SUM(DECODE(SIGN(NVL(A.EXPENSES_PAID,0)), 1, NVL(A.EXPENSES_PAID,0), 0)) PAID_EXPENSES2
                    ...
                    LOOP 
                        ...
                        rep.paid_expenses2      := i.paid_expenses2;
    ...
    BEGIN
        ...
        LOOP
            ...
            FOR h IN (SELECT DISTINCT treaty_name
                        FROM TABLE(GICLR222LE_PKG.GET_TREATY_RI(P_SESSION_ID, P_CLAIM_ID, P_POLICY_NO))
                       WHERE RI_CD <> P_RI_CD
            ...
            FOR j IN (SELECT DISTINCT A.RI_CD, A.LINE_CD, A.GRP_SEQ_NO, A.BRDRX_RIDS_RECORD_ID,
                ...
                LOOP
                    ...
                    FOR l IN (SELECT DISTINCT A.LINE_CD, A.GRP_SEQ_NO, B.POLICY_NO
                               ...
                               ORDER BY A.GRP_SEQ_NO)
                    LOOP            
                        FOR n IN (SELECT A.BRDRX_RIDS_RECORD_ID, A.GRP_SEQ_NO, 
                                         DECODE(SIGN(NVL(A.LOSSES_PAID, 0)), 1, NVL(A.LOSSES_PAID,0), 0) PAID_LOSSES, 
                                         DECODE(SIGN(NVL(A.EXPENSES_PAID, 0)), 1, NVL(A.EXPENSES_PAID,0), 0) PAID_EXPENSES
                                    FROM GICL_RES_BRDRX_RIDS_EXTR A
                                   WHERE A.GRP_SEQ_NO <> 999
                                     AND A.SESSION_ID = P_SESSION_ID
                                     AND (ABS(NVL(A.LOSSES_PAID,0)) > 0
                        ...
                        LOOP                        
                            ...
                            rep.paid_expenses3  := n.PAID_EXPENSES;
    ...
    ** Remarks : To handle difficult matrix
    */
   ...
   IS
      v_row                giclr222le_claim_type;
   BEGIN
      FOR i IN(SELECT a.brdrx_record_id, a.claim_id, a.claim_no, a.policy_no, a.loss_date, a.item_no
                 FROM GICL_RES_BRDRX_EXTR a
                WHERE a.session_id = p_session_id 
                  AND a.claim_id = nvl(p_claim_id,a.claim_id)
                  AND (ABS(NVL(A.LOSSES_PAID,0)) > 0
   ...
   IS
      v_row                giclr222le_item_main_type;
   BEGIN
      FOR i IN(SELECT a.brdrx_record_id, a.claim_id, a.claim_no, a.policy_no, a.loss_date,
                      a.clm_loss_id, a.tsi_amt, a.loss_cat_cd, a.intm_no, NVL(a.losses_paid, 0) paid_losses,
                      NVL(a.expenses_paid, 0) paid_expenses, a.line_cd, a.item_no, a.peril_cd, a.grouped_item_no
                 FROM GICL_RES_BRDRX_EXTR a
                WHERE a.session_id = p_session_id 
                  AND a.claim_id = nvl(p_claim_id,a.claim_id)
                  AND (ABS(NVL(A.LOSSES_PAID,0)) > 0
                        OR ABS(NVL(A.EXPENSES_PAID, 0)) > 0)
                  AND a.policy_no = p_policy_no
                  --AND a.brdrx_record_id = p_record_id
                ORDER BY a.claim_no)
      LOOP
        --v_row.record_id := i.brdrx_record_id;
        v_row.claim_id := i.claim_id;
   ...
   IS
      v_row                giclr222le_item_type;
   BEGIN
      FOR i IN(SELECT a.brdrx_record_id, a.claim_id, a.claim_no, a.policy_no, a.loss_date,
                      a.clm_loss_id, a.tsi_amt, a.loss_cat_cd, a.intm_no, NVL(a.losses_paid, 0) paid_losses,
                      NVL(a.expenses_paid, 0) paid_expenses, a.line_cd, a.item_no, a.peril_cd, a.grouped_item_no, a.from_date, a.to_date, a.pd_date_opt
                 FROM GICL_RES_BRDRX_EXTR a
                WHERE a.session_id = p_session_id 
                  AND a.claim_id = nvl(p_claim_id,a.claim_id)
                  AND (ABS(NVL(A.LOSSES_PAID,0)) > 0
      ...
      LOOP
        ...
        IF i.paid_losses > 0 THEN
            v_row.dv_no := gicls202_extraction_pkg.get_voucher_check_no(i.claim_id, i.item_no, i.peril_cd, i.grouped_item_no, i.from_date, i.to_date, i.pd_date_opt, 'L');
        ELSE
            v_row.dv_no := gicls202_extraction_pkg.get_voucher_check_no(i.claim_id, i.item_no, i.peril_cd, i.grouped_item_no, i.from_date, i.to_date, i.pd_date_opt, 'E');
        END IF;
        
        --v_row.dv_no := cf_dv_noformula(i.claim_id, i.item_no, i.clm_loss_id, i.paid_losses, i.paid_expenses, p_paid_date, p_from_date, p_to_date);
        v_row.intm_name := giclr222le_pkg.CF_INTM_RIFormula(i.claim_id, i.item_no, i.peril_cd);
   ...
   BEGIN
      ...
      FOR i IN(SELECT policy_no, grp_seq_no1, grp_seq_no2, grp_seq_no3, grp_seq_no4
                 FROM TABLE(GICLR222LE_PKG.get_giclr222le_header(p_session_id, p_claim_id, p_user_id))
      ...
      LOOP
        ...
        LOOP
            ...
            v_loop := 'SELECT paid_losses ' ||
                       ' FROM TABLE(GICLR222LE_PKG.get_treaty_details(' || p_session_id || ',' || p_claim_id || ','''
                                                                       || i.policy_no || ''',' || p_brdrx_record_id || '))' ||
   ...
   BEGIN
      ...
      LOOP
         FOR b IN(SELECT *
                    FROM TABLE(GICLR222LE_PKG.get_treaty_facul(p_session_id, p_claim_id, i.policy_no, p_brdrx_record_id))
         ...
         LOOP
            ...
            IF p_grp_seq_no1 = 999 THEN
               ...
               v_row.paid_losses1 := b.paid_losses2;
               v_row.paid_expenses1 := b.paid_expenses2;
            ELSIF p_grp_seq_no2 = 999 THEN
               v_row.ri_name2 := b.facul_ri_name;
               v_row.paid_losses2 := b.paid_losses2;
               v_row.paid_expenses2 := b.paid_expenses2;
            ELSIF p_grp_seq_no3 = 999 THEN
               v_row.ri_name3 := b.facul_ri_name;
               v_row.paid_losses3 := b.paid_losses2;
               v_row.paid_expenses3 := b.paid_expenses2;
            ELSIF p_grp_seq_no4 = 999 THEN
               v_row.ri_name4 := b.facul_ri_name;
               v_row.paid_losses4 := b.paid_losses2;
               v_row.paid_expenses4 := b.paid_expenses2;
            END IF;
   ...
   IS
      v_total              total_paid_le_type;
   BEGIN
      FOR i IN(SELECT SUM(NVL(a.losses_paid, 0)) total_paid_losses, SUM(NVL(a.expenses_paid, 0)) total_paid_expenses 
                 FROM GICL_RES_BRDRX_EXTR a
                WHERE a.session_id = p_session_id 
                  AND a.claim_id = NVL(p_claim_id, a.claim_id)
                  AND (ABS(NVL(A.LOSSES_PAID,0)) > 0
                ...
                ORDER BY a.claim_no)
      LOOP 
        v_total.total_paid_losses := i.total_paid_losses;
        v_total.total_paid_expenses := i.total_paid_expenses;
         ...
         LOOP
            ...
            v_loop := 'SELECT SUM(paid_losses)
                         FROM (SELECT paid_losses ' ||
                               ' FROM TABLE(GICLR222LE_PKG.get_treaty_facul(' || p_session_id || ', :A ,'''
                                                                             || i.policy_no || ''', :B))' ||
                              ' WHERE grp_seq_no = ' || v_grp_seq_no ||
            ...
            v_loop2 := 'SELECT SUM(paid_expenses)
                         FROM (SELECT paid_expenses ' ||
                               ' FROM TABLE(GICLR222LE_PKG.get_treaty_facul(' || p_session_id || ', :A ,'''
                                                                             || i.policy_no || ''', :B))' ||
                              ' WHERE grp_seq_no = ' || v_grp_seq_no ||
                              ' GROUP BY brdrx_ds_record_id, paid_expenses)';
            ...
            OPEN cur2 FOR v_loop2 USING -1, -1;
            LOOP
   ...
   BEGIN
        FOR j IN (SELECT DISTINCT a.ri_cd, a.line_cd, a.grp_seq_no,
                   ...
                   WHERE a.grp_seq_no <> 999
                     ...
                     AND a.grp_seq_no = b.trty_seq_no
                     AND a.ri_cd = b.ri_cd
                     AND (NVL(a.expenses_paid, 0) > 0 OR NVL(a.losses_paid, 0) > 0) 
        ...
        LOOP
            ...
            FOR a IN (SELECT NVL(SUM(a.losses_paid),0) paid_losses, NVL(SUM(a.expenses_paid),0) paid_expenses
                        FROM GICL_RES_BRDRX_RIDS_EXTR a
                       WHERE a.grp_seq_no <> 999
                         AND a.session_id = p_session_id
