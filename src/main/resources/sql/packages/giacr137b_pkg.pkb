CREATE OR REPLACE PACKAGE BODY CPI.GIACR137B_PKG
AS
   /*
   ...
   */    
    ...
    BEGIN
        ...
        FOR q IN(SELECT a.line_cd, b.trty_name, d.line_name || ' - ' || b.trty_name line_trty_name, a.cession_year,  a.trty_com_rt, a.share_cd, 
                  ...
                  WHERE a.user_id    = p_user_id  
                    ...
                    AND cession_year = TO_NUMBER(NVL(p_cession_year, TO_CHAR(SYSDATE, 'YYYY')))
                 HAVING SUM(a.commission_amt) <> 0
               GROUP BY a.share_cd, a.line_cd, d.line_name, b.trty_name, a.trty_com_rt, a.cession_year,  a.cession_mm,  a.branch_cd, a.prnt_ri_cd, c.ri_sname, a.trty_shr_pct, a.share_cd
        ...
        LOOP
            ...
            v_rep.trty_com_rt     := q.trty_com_rt;
    ...
    BEGIN
        FOR q IN(SELECT DISTINCT a.line_cd, b.trty_name, a.cession_year,  a.trty_com_rt, a.share_cd,
                  ...
                  WHERE a.user_id    = p_user_id  
                    ...
                    AND cession_year = TO_NUMBER(NVL(p_cession_year, TO_CHAR(SYSDATE, 'YYYY')))
                 HAVING SUM(a.commission_amt) <> 0
               GROUP BY a.share_cd, a.line_cd, d.line_name, b.trty_name, a.trty_com_rt, a.cession_year,  a.cession_mm,  a.branch_cd
               ORDER BY  a.share_cd, a.cession_mm)
        LOOP
            
            FOR w IN(SELECT DISTINCT a.line_cd line_cd_grand, c.trty_name, a.cession_year, a.cession_mm,
                                     TO_CHAR(TO_DATE(cession_mm,'MM'),'MONTH') month_grand, a.trty_com_rt, 
                               ...
                               WHERE a.prnt_ri_cd = b.ri_cd    
                                 ...
                                 AND a.trty_com_rt = q.trty_com_rt
                            GROUP BY a.line_cd, c.trty_name, a.cession_year, a.cession_mm, a.trty_com_rt, b.ri_sname, a.share_cd
                            ORDER BY a.line_cd, c.trty_name, a.cession_year, a.cession_mm, a.trty_com_rt, b.ri_sname, a.share_cd)
            LOOP
                ...
                v_rep.trty_com_rt1      := w.trty_com_rt;
