CREATE OR REPLACE PACKAGE BODY CPI.GICLR204F3_PKG
AS
   ...
   BEGIN
      ...
      LOOP
         ...
         cp.peril_name := i.peril_name;       
         cp.assd_name := i.assd_name;          
         cp.policy_id := i.policy_id;
         cp.date_for_24th := i.date_for_24th;
         v_grand_total := v_grand_total + i.sum_prem_amt;
         cp.grand_total := v_grand_total;
   ...
   BEGIN
      ...
      LOOP
         ...
         pp.peril_name := i.peril_name;       
         pp.assd_name := i.assd_name;          
         pp.policy_id := i.policy_id;
         pp.date_for_24th := i.date_for_24th;
   ...
   BEGIN
      FOR i IN (SELECT a.peril_cd, a.assd_no, c.assd_name, 
                 ...
                 ORDER BY 1, 4)
      LOOP
         lp.peril_cd := i.peril_cd;
         lp.peril_name := i.peril_name;
   ...
   BEGIN
      FOR i IN (SELECT a.peril_cd, f.peril_name, a.assd_no, b.assd_name, d.rec_type_desc, SUM(a.recovered_amt) sum_recovered_amt, e.dsp_loss_date, 
                 ...
                 ORDER BY 1, 8)
      LOOP
         cr.peril_cd := i.peril_cd;
         cr.peril_name := i.peril_name;
   ...
   BEGIN
      FOR i IN (SELECT a.peril_cd, f.peril_name, a.assd_no, b.assd_name, d.rec_type_desc, SUM(a.recovered_amt) sum_recovered_amt, e.dsp_loss_date,
                 ...
                 ORDER BY 1, 8)
      LOOP
         pr.peril_cd := i.peril_cd;
         pr.peril_name := i.peril_name;
