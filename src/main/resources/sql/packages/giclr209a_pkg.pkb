CREATE OR REPLACE PACKAGE BODY CPI.GICLR209A_PKG
AS
    ...
    BEGIN
        FOR i IN(SELECT  a.brdrx_record_id, a.buss_source, a.iss_cd, a.line_cd, a.subline_cd,
                         a.claim_id, a.assd_no, a.claim_no, a.policy_no, a.clm_file_date,
                         a.loss_date, a.loss_cat_cd, a.intm_no, a.clm_loss_id,
                         NVL (a.losses_paid, 0) paid_loss,a.pd_date_opt,a.from_date, a.to_date
                    FROM gicl_res_brdrx_extr a
                   WHERE a.session_id = p_session_id
                     AND a.claim_id = NVL (p_claim_id, a.claim_id)
                     AND NVL (a.losses_paid, 0) <> 0
                ORDER BY a.intm_no, a.iss_cd, a.line_cd, a.clm_loss_id
        )
        LOOP
            ...
            BEGIN
              ...
              LOOP
                ...
                ELSE
                   v_list.clm_stat := v_clm_stat||'/'||var_clm_stat;
                END IF;
