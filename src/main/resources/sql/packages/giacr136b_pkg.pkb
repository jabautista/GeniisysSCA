CREATE OR REPLACE PACKAGE BODY CPI.giacr136b_pkg
AS
   ...
   BEGIN
     ...
     END;   
      FOR i IN (SELECT   a.line_cd, trty_name,
                         ...
                         a.trty_shr_pct, SUM (a.premium_amt) premium,
                         a.share_cd
   ...
   BEGIN
      FOR i IN
            (SELECT DISTINCT TO_CHAR (TO_DATE (cession_mm, 'MM'),
                                      'MONTH'
                                     ) month_grand,
                             a.cession_year, a.cession_mm,
                             a.line_cd line_cd_grand, trty_name,
                             b.ri_sname ri_sname_grand,
                       ...
                       WHERE a.prnt_ri_cd = b.ri_cd
                         ...
                         AND TO_CHAR (TO_DATE (cession_mm, 'MM'),
                                      'MONTH'
                                     ) = NVL(p_cf_month,TO_CHAR (TO_DATE (cession_mm, 'MM'),
                                      'MONTH'
                                     ))
                    GROUP BY a.line_cd,
                             trty_name,
