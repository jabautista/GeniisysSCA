CREATE OR REPLACE PACKAGE BODY CPI.giacr120_pkg
AS
   ...
   BEGIN
      FOR i IN
         (SELECT   i.iss_name, l.line_name, r.ri_name, c.assured_name,
                   ...
                   f.fla_date fla_date, NVL (f.adv_shr_amt, 0) total_share,
                   SUM (NVL (j.collection_amt, 0)) total_payments,
                   (NVL (f.adv_shr_amt, 0) - SUM (NVL (j.collection_amt, 0))
                   ) total_amount_due
              FROM giis_line l,
                   giis_reinsurer r,
                   gicl_claims c,
             ...
             WHERE f.claim_id = c.claim_id
               ...
               --Ladz 03252013
               AND check_user_per_iss_cd_acctg (NULL, i.iss_cd, 'GIACS119') =
                                                                             1
...
--ORDER BY 1, 2, 3, 4, 6
          ...
          SELECT   i.iss_name, l.line_name, r.ri_name, c.assured_name,
                   ...
                   || '-'
                   || LTRIM (TO_CHAR (f.fla_seq_no, '0999999')) advice_no,
                   f.fla_date fla_date, NVL (f.adv_shr_amt, 0) total_share, 0,
                   
                   -- 0
                   NVL (f.adv_shr_amt, 0) total_amount_due
              FROM giis_line l,
                   giis_reinsurer r,
                   gicl_claims c,
                   giis_issource i,
               ...
               AND (   1 =
                          (SELECT DECODE
                                     (p,
                                      1, (SELECT 1
                                            FROM gicl_advice ga,
                                                 giac_direct_claim_payts gdcp,
                                                 giac_acctrans gacc
                                           WHERE ga.claim_id = gdcp.claim_id
                                             AND ga.advice_id = gdcp.advice_id
                                             AND f.claim_id = ga.claim_id
                                             AND f.adv_fla_id = ga.adv_fla_id
                                             AND gdcp.gacc_tran_id =
                                                                  gacc.tran_id
                                             AND gacc.tran_flag NOT IN
                                                                   ('O', 'D')
                                             AND NOT EXISTS (
                                                    SELECT 1
                    ...
                    OR 1 =
                          (SELECT DECODE
                                     (p,
                                      1, (SELECT 1
                                            FROM gicl_advice ga,
                                                 giac_inw_claim_payts gicp,
                                                 giac_acctrans gacc
                                           WHERE ga.claim_id = gicp.claim_id
                                             AND ga.advice_id = gicp.advice_id
                                             AND f.claim_id = ga.claim_id
                                             AND f.adv_fla_id = ga.adv_fla_id
                                             AND gicp.gacc_tran_id =
                                                                  gacc.tran_id
                                             AND gacc.tran_flag NOT IN
                                                                   ('O', 'D')
                                             AND NOT EXISTS (
