CREATE OR REPLACE PACKAGE BODY CPI.NLTO AS
   PROCEDURE MAIN_NLTO(
     v_date      varchar2,
   ...
   BEGIN
     ...
     IF v_basis in ('AD', 'ED','ID')  THEN
        NLTO_BD_AD(v_basis,v_date_from, v_date_to, v_zone,v_zone_type, v_iss_cd, v_user, v_sys);
     ELSE
        DBMS_OUTPUT.PUT_LINE('BOOKDATE');
        NLTO_BD_BD(v_basis,v_date_from, v_date_to, v_zone,v_zone_type, v_iss_cd, v_user, v_sys);
     END IF;
    END;
...
-------------------
    PROCEDURE NLTO_BD_BD(
     ...
     AS
        ...
        v_to varchar2(20);
     BEGIN
      DBMS_OUTPUT.PUT_LINE('BOOKDATE2');
        FOR C IN (SELECT a.param_value_v MC,
                         b.param_value_v PC,
                         c.param_value_v CV,
                         d.param_value_v   line
        ...
        END LOOP;
           ...
           DBMS_OUTPUT.PUT_LINE(v_to   || 'v_to');
