CREATE OR REPLACE PACKAGE BODY CPI.P_Bus_Conservation_Dtl092606 AS
  PROCEDURE get_data(
  ...
  BEGIN
	...
	SELECT DISTINCT
		   ...
		   pol_seq_no,
		   renew_no,
		   TO_CHAR(expiry_date,'YYYY') expiry_year,
		   TO_CHAR(expiry_date,'MM') expiry_month
    BULK COLLECT INTO
	       vv_line_cd,
		   vv_subline_cd,
		   vv_iss_cd,
		   ...
		   vv_expiry_month
	  FROM gipi_polbasic
	 WHERE TRUNC(expiry_date) BETWEEN p_from_date AND p_to_date
	   AND iss_cd <> v_ri_cd;
       ...
       FORALL cnt IN vv_line_cd.FIRST..vv_line_cd.LAST
		  ...
		  VALUES
		    (vv_line_cd(cnt),	  vv_subline_cd(cnt), vv_iss_cd(cnt),
			 ...
			 vv_region_cd(cnt),	  vv_pol_flag(cnt));
	END IF;
	COMMIT;
...
DROP PACKAGE BODY CPI.P_BUS_CONSERVATION_DTL092606;
