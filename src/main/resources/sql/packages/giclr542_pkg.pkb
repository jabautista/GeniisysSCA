CREATE OR REPLACE PACKAGE BODY CPI.giclr542_pkg
AS
   FUNCTION get_giclr_542_report (
      p_assd_no     NUMBER,
      p_assured     VARCHAR2,
   ...
   BEGIN
      ...
      LOOP
         ...
         IF p_assd_no IS NULL
         THEN
            v_list.cf_assured :=
                   'Assured : ' || TO_CHAR (i.assd_no) || ' - '
                   || i.assd_name;
         ELSE
            v_list.cf_assured := NULL;
         END IF;
   ...
   END get_giclr_542_report;

   FUNCTION get_giclr_542_dtls (
      p_line_cd       VARCHAR2,
      p_claim_id      NUMBER,
      p_loss_exp      VARCHAR2,
   ...
   BEGIN
      FOR i IN (SELECT DISTINCT c.peril_cd, c.peril_sname peril_sname,
                          ...
                          WHERE b.peril_cd = c.peril_cd
                            ...
                            ORDER BY c.peril_cd, c.peril_sname,
                                b.claim_id, c.line_cd)
      LOOP
      ...
      THEN
         ...
         v_list.exp_amt := 0;
         v_list.exp_retention := 0;
         v_list.exp_treaty := 0;
         v_list.exp_xol := 0;
         v_list.exp_facultative := 0;
         PIPE ROW (v_list);
      END IF;
   END get_giclr_542_dtls;
END;
/
