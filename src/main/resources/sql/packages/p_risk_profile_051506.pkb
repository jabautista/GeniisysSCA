CREATE OR REPLACE PACKAGE BODY CPI.p_risk_profile_051506 AS
		  ...
		  FOR prl IN (SELECT peril_cd
		    ...
		    LOOP
			  ...
			  FOR rec IN vv_range_to.FIRST..vv_range_to.LAST LOOP
			    ...
			    IF ABS(v_range_tsi) >= tab_polrisk_ext(rec).range_from AND
			       ABS(v_range_tsi) <= tab_polrisk_ext(rec).range_to THEN
		   ...
		   --A.R.C. 05.10.2006
		      FOR ctr IN vv_range_to.FIRST..vv_range_to.LAST LOOP
				  IF ABS(v_item_tsi) >= vv_range_from(ctr) AND
					 ...
					 tab_polrisk_ext_dtl(v_pol_ctr).ann_tsi_amt := v_item_tsi;
     ...
     FORALL i IN vv_policy_id.FIRST..vv_policy_id.LAST
       ...
       VALUES
	     (vv_policy_id(i),    vv_line_cd(i), 	    vv_subline_cd(i),
		  ...
		  USER,				  vv_acct_ent_date(i),	vv_acct_neg_date(i),
		  vv_pol_flag(i));
     COMMIT;
  END IF;
END;
...
DROP PACKAGE BODY CPI.P_RISK_PROFILE_051506;
