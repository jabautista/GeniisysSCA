CREATE OR REPLACE PACKAGE BODY CPI.giclr205l_pkg
AS
   /*
   **  Created by   :  Maria Gzelle Ison
   **  Date Created : 03.14.2013
   **  Reference By : GILR205L - Outstanding Bordereaux (LOSSES)
   */
...
--         raise_application_error (-20001, p_session_id||' c '||i.claim_id||' i '||i.item_no||' p '||i.peril_cd||' in '||i.intm_no||' pbre '||p_intm_break);
         v_detail.intm_ri :=
            cf_intm (p_session_id,
                     i.claim_id,
                     i.item_no,
                     i.peril_cd,
                     i.intm_no,
                     p_intm_break
                    );    -- Kris 01.29.2014: changed p_claim_id to i.claim_id
         v_detail.outstanding_loss := i.outstanding_loss;
         PIPE ROW (v_detail);
      END LOOP;
   ...
   BEGIN
      ...
      LOOP
         ...
         rep.treaty_name := treaty_name (i.grp_seq_no, i.line_cd);
         PIPE ROW (rep);
      END LOOP;
      ...
      LOOP
         ...
         LOOP
            ...
            LOOP
               ...
               v_treaty.outstanding_loss2_sum := l.outstanding_loss;
               PIPE ROW (v_treaty);
            END LOOP;
         END LOOP;
         ...
         LOOP
            ...
            LOOP
               ...
               LOOP
                  ...
                  v_facul.outstanding_loss3 := f.outstanding_loss3;
                  PIPE ROW (v_facul);
               END LOOP;
            END LOOP;
         END LOOP;
   ...
   BEGIN
      FOR a IN (SELECT   a.brdrx_rids_record_id, a.iss_cd, a.buss_source,
   ...
   BEGIN
      ...
      LOOP
         ...
         LOOP
            ...
            LOOP
               v_treaty_ri.outstanding_loss4 := c.outstanding_loss;
   ...
   BEGIN
      FOR i IN (SELECT   line_cd, subline_cd, iss_cd,
                   ...
                   WHERE a.session_id = p_session_id
                     ...
                     AND a.iss_cd = p_iss_cd
                     AND a.buss_source = p_buss_source
                GROUP BY line_cd, subline_cd, iss_cd)
      LOOP
         v_total.iss_cd := i.iss_cd;
         v_total.subline_cd := i.subline_cd;
         v_total.line_cd := i.line_cd;
         v_total.outstanding_loss := i.outstanding_loss;
         PIPE ROW (v_total);
      END LOOP;
   ...
   BEGIN
      FOR i IN (SELECT   b.iss_cd,
                         ...
                         c.trty_name, a.line_cd,
                         NVL (NVL (d.parent_intm_no, d.intm_no),
                              0
                             ) buss_source
                    FROM gicl_res_brdrx_extr a,
                ...
                GROUP BY b.iss_cd,
                         c.trty_name,
                         a.line_cd,
                         NVL (NVL (d.parent_intm_no, d.intm_no), 0))
      LOOP
         v_total.iss_cd := i.iss_cd;
         v_total.outstanding_loss := i.outstanding_loss;
      ...
      FOR i IN (SELECT   b.iss_cd,
                   ...
                   WHERE b.session_id = p_session_id
                     ...
                     AND (NVL (a.loss_reserve, 0) - NVL (a.losses_paid, 0)) >
                                                                             0
                     AND a.buss_source = d.intm_no(+)
                GROUP BY b.iss_cd, NVL (NVL (d.parent_intm_no, d.intm_no), 0))
      LOOP
         v_total.outstanding_loss := i.outstanding_loss;
         v_total.buss_source := i.buss_source;
         v_total.iss_cd := i.iss_cd;
   ...
   IS
      v_total   total_per_buss_source_type;
   BEGIN
      FOR i IN (SELECT   line_cd, subline_cd, iss_cd,
                   ...
                   WHERE a.session_id = p_session_id
                     ...
                     AND (NVL (a.loss_reserve, 0) - NVL (a.losses_paid, 0)) >
                                                                             0
                     AND a.buss_source = p_buss_source
                GROUP BY line_cd, subline_cd, iss_cd, buss_source)
      LOOP
         v_total.subline_cd := i.subline_cd;
         v_total.line_cd := i.line_cd;
         v_total.outstanding_loss := i.outstanding_loss;
   ...
   IS
      v_total   total_treaty_type;
   BEGIN
      FOR i IN (SELECT   SUM ((NVL (a.loss_reserve, 0)
                               - NVL (a.losses_paid, 0)
                              )
                             ) "OUTSTANDING_LOSS",
                         a.buss_source
      ...
      LOOP
         ...
         PIPE ROW (v_total);
      END LOOP;
   END get_total_treaty_buss_source;
   ...
   IS
      v_total   total_per_buss_src_type_type;
   BEGIN
      ...
      FOR i IN (SELECT   SUM ((NVL (a.loss_reserve, 0)
                   ...
                   WHERE b.session_id = p_session_id
                     ...
                     AND (NVL (a.loss_reserve, 0) - NVL (a.losses_paid, 0)) >
                                                                             0
                     AND a.buss_source = d.intm_no(+)
                     AND DECODE (a.iss_cd, 'RI', 'RI', d.intm_type) =
                                                            p_buss_source_type
                GROUP BY DECODE (b.iss_cd, 'RI', 'RI', d.intm_type))
      LOOP
         v_total.outstanding_loss := i.outstanding_loss;
         v_total.buss_source_type := i.buss_source_type;
         PIPE ROW (v_total);
      END LOOP;
   END get_total_treaty_buss_src_type;
   ...
   BEGIN
      FOR i IN (SELECT   SUM ((NVL (b.loss_reserve, 0)
                               - NVL (b.losses_paid, 0)
                              )
                             ) grand_treaty_loss,
      ...
      LOOP
         v_grand.grand_treaty_loss := i.grand_treaty_loss;
         v_grand.treaty_name := i.trty_name;
         v_grand.grp_seq_no := i.grp_seq_no;
         PIPE ROW (v_grand);
      END LOOP;
   ...
   IS
      v_row          giclr205l_parent_type;
      v_treaty_tab   treaty_tab;
      v_index        NUMBER                := 0;
      v_id           NUMBER                := 0;
   BEGIN
      FOR buss_source_type IN
      ...
      LOOP
         ...
         FOR treaty IN (SELECT DISTINCT a.iss_cd, a.buss_source, a.line_cd,
                                        a.subline_cd, a.loss_year,
                                  ...
                                  WHERE a.line_cd = b.line_cd
                                    ...
                                    AND a.session_id = p_session_id                                   
                                    AND a.buss_source =
                                                  buss_source_type.buss_source
                                    AND a.iss_cd = buss_source_type.iss_cd
                                    AND a.line_cd = buss_source_type.line_cd
                                    AND a.subline_cd =
                                                   buss_source_type.subline_cd
                                    AND a.loss_year =
                                                    buss_source_type.loss_year
                               ORDER BY a.grp_seq_no)
         LOOP
            v_index := v_index + 1;
            v_treaty_tab.EXTEND;
         ...
         LOOP
            ...
            v_row.iss_type := buss_source_type.iss_type;
            ...
            IF buss_source_type.iss_type = 'RI'
            THEN
               BEGIN
                  SELECT ri_name
            ...
            IF v_treaty_tab.EXISTS (v_index)
            THEN
   ...
   IS
      v_row   giclr205l_claim_type;
   BEGIN
      FOR claim IN (SELECT   a.claim_id, a.claim_no, a.policy_no,
                             b.assd_name, a.incept_date, a.expiry_date,
                             a.loss_date, a.item_no
                       ...
                       WHERE a.assd_no = b.assd_no
                         ...
                         AND a.subline_cd = p_subline_cd
                         AND a.iss_cd = p_iss_cd
                    GROUP BY claim_id, a.claim_no, a.policy_no, --Added by carlo
                             b.assd_name, a.incept_date, a.expiry_date,
                             a.loss_date, a.item_no
                    ORDER BY a.claim_no)
      LOOP
   ...
   IS
      v_row   giclr205l_item_main_type;
   BEGIN
      ...
      LOOP
         ...
         v_row.tsi_amt := item_name.tsi_amt; --Added by carlo
         v_row.item_title :=
            cpi.get_gpa_item_title (item_name.claim_id,
                                    item_name.line_cd,
                                    item_name.item_no,
   ...
   IS
      v_row   giclr205l_item_type;
   BEGIN
      FOR item IN (SELECT   a.claim_id, a.claim_no, a.policy_no, a.item_no,
                            a.tsi_amt, a.loss_cat_cd, a.line_cd, a.peril_cd,
                            a.clm_loss_id, a.brdrx_record_id, a.intm_no, --Added intm_no by carlo
                            (NVL (a.loss_reserve, 0) - NVL (a.losses_paid, 0)
                            ) outstanding_loss
                       FROM gicl_res_brdrx_extr a
                      WHERE a.session_id = p_session_id
      ...
      LOOP
         ...
         v_row.intm_no := item.intm_no;

         BEGIN
            SELECT peril_name
              INTO v_row.peril_name
              FROM giis_peril
             WHERE peril_cd = item.peril_cd AND line_cd = item.line_cd;
         END;

         v_row.intm_name :=
            giclr205l_pkg.cf_intm_riformula (item.claim_id,
                                             item.item_no,
                                             item.peril_cd,
                                             item.intm_no
                                            );
   ...
   END;
    ...
    BEGIN
       ...
       FOR buss_source IN
          (SELECT buss_source, iss_cd, line_cd, subline_cd, loss_year,
                  grp_seq_no1, grp_seq_no2, grp_seq_no3, grp_seq_no4
             FROM TABLE (giclr205l_pkg.get_giclr205l_parent (p_session_id,
                                                             p_claim_id
                                                            )
                        )
       ...
       LOOP
          ...
          LOOP
             ...
             IF v_grp_seq_no IS NOT NULL          
             THEN              
                v_loop :=
                      'SELECT outstanding_loss2
                                FROM TABLE (giclr205l_pkg.get_treaty_details ('
                   || p_session_id
                   || ',
                                                        '
                   || v_claim_id
   ...
   BEGIN
      ...
      LOOP
         FOR facul IN
            (SELECT *
               FROM TABLE
                       (giclr205l_pkg.get_treaty_facul
                                                     (p_session_id,
                                                      p_claim_id,
                                                      p_brdrx_record_id,
                                                      buss_source.buss_source,
   ...
   BEGIN
      ...
      LOOP
         ...
         LOOP
            ...
            THEN
                  v_loop :=
                      'SELECT SUM (outstanding_loss2)
                            FROM (SELECT outstanding_loss2
                                    FROM TABLE (giclr205l_pkg.get_treaty_facul ('
                   || p_session_id
                   || ',
                                                                                '
                   || v_claim_id
   ...
   BEGIN
      ...
      LOOP
         ...
         FOR a IN
            (SELECT   (NVL (a.loss_reserve, 0) - NVL (a.losses_paid, 0)
                      ) outstanding_loss
                 FROM gicl_res_brdrx_rids_extr a
                WHERE a.grp_seq_no <> 999
                  AND a.session_id = p_session_id
                  ...
                  AND a.grp_seq_no = j.grp_seq_no
                  AND a.ri_cd = j.ri_cd
                  AND a.brdrx_rids_record_id IN (
                         SELECT a.brdrx_rids_record_id
                           FROM gicl_res_brdrx_rids_extr a,
                                giis_trty_panel b,
                                gicl_res_brdrx_extr c,
                                gicl_res_brdrx_ds_extr d
   ...
   BEGIN
      FOR i IN (SELECT   a.line_cd, b.trty_name, a.iss_cd, a.grp_seq_no,
                         a.buss_source,
                         SUM ((NVL (a.loss_reserve, 0)
                               - NVL (a.losses_paid, 0)
                              )
                   ...
                   WHERE (NVL (a.loss_reserve, 0) - NVL (a.losses_paid, 0)) >
                                                                            0
                     AND a.session_id = p_session_id
                     AND a.claim_id = NVL (p_claim_id, a.claim_id)
                     AND a.line_cd = b.line_cd
         ...
         BEGIN
            ...
            LOOP
               v_intm_ri :=
                  ...
                  || m.intm_name;
                --|| CHR (10)
                --|| v_intm_ri; comment out by carlo
            END LOOP;
         END;
      END IF;
   ...
   BEGIN
      ...
      LOOP
         ...
         rep.grp_seq_no := j.grp_seq_no;
         rep.treaty_name := GICLR205L_PKG.cf_treaty_nameformula (j.grp_seq_no, j.line_cd);
         PIPE ROW (rep);
      END LOOP;
      ...
      LOOP
         FOR h IN (SELECT   a.brdrx_record_id, a.buss_source, a.iss_cd,
                            a.line_cd, a.subline_cd, a.loss_year, a.claim_id,
         ...
         LOOP
            ...
            LOOP
               rep.grp_seq_no := j.grp_seq_no;
               rep.treaty_name :=
                              GICLR205L_PKG.cf_treaty_nameformula (j.grp_seq_no, j.line_cd);
               ...
               LOOP
                  ...
                  LOOP
                     ...
                     rep.facul_ri_name := GICLR205L_PKG.cf_ri_nameformula (l.facul_ri_cd);
    ...
    BEGIN
       FOR d IN (SELECT DISTINCT a.iss_cd, a.buss_source, a.line_cd,
                           ...
                           WHERE a.session_id = p_session_id
                             ...
                             AND a.loss_year = NVL (p_loss_year, a.loss_year)
                 MINUS
                 SELECT DISTINCT a.iss_cd, a.buss_source, a.line_cd, a.subline_cd,
       ...
       LOOP
          FOR h IN (SELECT   a.brdrx_record_id, a.buss_source, a.iss_cd,
                             ...
                             a.assd_no, a.claim_no, a.policy_no, a.incept_date,
                             a.expiry_date, a.loss_date, a.clm_file_date,
                             a.item_no, a.grouped_item_no,      --EMCY da042506te
                                                          a.peril_cd,
                             a.loss_cat_cd, a.tsi_amt, a.intm_no,
          ...
          LOOP
             ...
             LOOP
                rep.grp_seq_no := j.grp_seq_no;
                rep.treaty_name :=
                                  cf_treaty_nameformula (j.grp_seq_no, j.line_cd);
