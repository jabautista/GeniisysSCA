CREATE OR REPLACE PACKAGE BODY CPI.GICLR057B_PKG
AS

   FUNCTION get_giclr057b(
   ...
   RETURN giclr057b_tab PIPELINED
   IS
   v_list giclr057b_type;
   BEGIN
    FOR i IN (
        SELECT   a.claim_id, a.catastrophic_cd,
         ...
         a.assured_name, TO_CHAR (a.loss_date, 'MM-DD-RR') ldate,
         a.in_hou_adj, a.clm_stat_cd,
         a.loss_loc1 || ' ' || a.loss_loc2 || ' ' || a.loss_loc3 LOCATION
                FROM gicl_claims a
               WHERE NVL (a.catastrophic_cd, 0) =
                                      NVL (p_catastrophic_cd, NVL (a.catastrophic_cd, 0))
                 AND a.clm_stat_cd NOT IN
                        ('CC', 'WD', 'DN')
                 AND a.line_cd = 'FI'
                 ...
                 AND TRUNC (a.loss_date) BETWEEN NVL (TO_DATE (p_from_date,'DD-MON-RRRR'),
                                                      TRUNC (a.loss_date))
                                             AND NVL (TO_DATE (p_to_date, 'DD-MON-RRRR'),
                                                      TRUNC (a.loss_date)
                                                     )
