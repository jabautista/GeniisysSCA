CREATE OR REPLACE PACKAGE BODY CPI.GICLR221L_PKG
AS
/*
    **  Created by   :  Kenneth Mark Labrador
    **  Date Created : 03.19.2013
    **  Reference By : GICLR221L - LOSSES PAID BORDEREAUX (PER ENROLLEE)
    */
   ...
   BEGIN
      FOR claim IN (SELECT a.claim_id, a.claim_no, a.policy_no, a.enrollee, b.assd_name,
                     ...
                     GROUP BY a.claim_id, a.claim_no, a.policy_no, a.enrollee, b.assd_name,
                              a.incept_date, a.expiry_date, a.loss_date, a.item_no --jm SR5363
                     ORDER BY a.claim_no)
      LOOP
   ...
   IS
      v_row                giclr221l_item_main_type;
   BEGIN
      FOR item_name IN (SELECT a.claim_id, a.claim_no, a.policy_no, a.enrollee, 
                         ...
                         WHERE a.assd_no = b.assd_no
                           ...
                           AND NVL (a.losses_paid, 0) > 0
                           AND a.enrollee = p_enrollee
                         GROUP BY a.claim_id, a.claim_no, a.policy_no, a.enrollee,
                                  a.item_no, a.line_cd, a.grouped_item_no --jm SR5363
                         ORDER BY a.claim_no)
      LOOP
...
--         v_row.dv_no := cf_dv_noformula(item.claim_id, item.item_no, item.clm_loss_id, item.paid_losses, item.paid_expenses, p_paid_date, p_from_date, p_to_date);
         v_row.dv_no := gicls202_pkg.get_voucher_check_no(item.claim_id, item.item_no, item.peril_cd, item.grouped_item_no, p_from_date, p_to_date, p_paid_date, 'L');
         v_row.intm_name := giclr221l_pkg.CF_INTM_RIFormula(item.claim_id, item.item_no, item.peril_cd);
         PIPE ROW(v_row);
      END LOOP;
   ...
   BEGIN
      ...
      FOR enrollee IN (SELECT enrollee, grp_seq_no1, grp_seq_no2, grp_seq_no3, grp_seq_no4
                         FROM TABLE (giclr221l_pkg.get_giclr221l_parent (p_session_id, p_claim_id))
      ...
      LOOP
         ...
         LOOP
            ...
            v_loop := 'SELECT paid_losses
                         FROM TABLE (giclr221l_pkg.get_treaty_details (
                                                '||p_session_id||',
                                                '||v_claim_id||',
                                                '''||enrollee.enrollee||''',
                                                '||p_brdrx_record_id||'
   ...
   BEGIN
      ...
      LOOP
         FOR facul IN (SELECT *
                         FROM TABLE(GICLR221L_PKG.get_facul(p_session_id, p_claim_id, enrollee.enrollee, p_brdrx_record_id))
   ...
   IS
      v_total              total_paid_losses_type;
   BEGIN
         ...
         LOOP
            ...
            v_loop := 'SELECT SUM(paid_losses)
                         FROM ( SELECT paid_losses
                                  FROM TABLE (giclr221l_pkg.get_facul ('||p_session_id||',
                                                                       '||v_claim_id||',
                                                                       '''||enrollee.enrollee||''',
                                                                       '||v_brdrx_record_id||'
                                                                      )
   ...
   BEGIN
      ...
      LOOP
         ...
         FOR a IN (SELECT NVL(SUM(a.losses_paid),0) paid_losses
                     FROM GICL_RES_BRDRX_RIDS_EXTR a
                    WHERE a.grp_seq_no <> 999
                      AND a.session_id = p_session_id
                      AND NVL(a.losses_paid, 0) > 0
