CREATE OR REPLACE PACKAGE BODY CPI.populate_giclr033_fla_xol2_pkg AS
   ...
   BEGIN
      FOR i IN (SELECT a.line_cd, a.subline_cd, a.pol_iss_cd, a.issue_yy,
                       ...
                       b.fla_title,
                       TO_CHAR (a.loss_date, 'Month dd, yyyy') loss_date,
      ...
      LOOP
         ...
         -- losses pd/ exp pd
         FOR lose IN (SELECT   SUM (a.net_amt) * b.convert_rate amt,
                               payee_type
         ...
         FOR x1 IN (SELECT SUM (a.shr_le_adv_amt * b.convert_rate) xol
         ...
         v_ri_shr_pct := ROUND ((v_ri_shr / /*v_sum_res*/ v_xol) * 100, 2);
   ...
   BEGIN
      ...
      FOR i IN (SELECT   a.share_type,
                         ...
                         SUM (shr_le_adv_amt) * b.convert_rate shr_amt,
                         g.trty_name trty_nm
      ...
      LOOP
         ...
         THEN
            ...
            vrep.wrd_xol_desc := v_xolclmdist;
   ...
   FUNCTION get_fla_xol_dist2 (
      ...
      v_exp_res      NUMBER;
   BEGIN
      FOR i IN (SELECT   SUM (a.net_amt) * b.convert_rate amt, payee_type
                    FROM gicl_clm_loss_exp a, gicl_advice b
                   WHERE a.claim_id = p_claim_id
                     AND NVL (a.dist_sw, 'N') = 'Y'
                     AND a.claim_id = b.claim_id
