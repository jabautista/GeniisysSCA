CREATE OR REPLACE PACKAGE BODY CPI.Extract_Loss_Profile_V2 AS
/* BY: PAU 
   DATE: 28NOV06
...
*/   
/* Modified By: Sherwin
** Date Modified: 092107
** Modifications: Used the function check_user_per_iss_cd2 to filter rows for specific user.
*/
...
BEGIN
 ...
 WHERE c003.clm_stat_cd NOT IN ('WD','DN','CC')
   ...
   AND check_user_per_iss_cd2(p_line_cd,c003.pol_iss_cd,'GICLS211',USER) = 1  --sherwin 092107
  ...
  vv_renew_no
   ...
   WHERE 1 = 1
     ...
     AND b250.endt_seq_no = 0
                   ...
                   ||'1,'||TO_CHAR(c.booking_year),'FMMONTH DD,YYYY'))),SYSDATE)
                              <= TRUNC(p_date_to))))
           AND check_user_per_iss_cd2(p_line_cd,b250.iss_cd,'GICLS211',USER) = 1  --sherwin 092107
