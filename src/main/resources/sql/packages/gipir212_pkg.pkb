CREATE OR REPLACE PACKAGE BODY CPI.gipir212_pkg
AS
   ...
   BEGIN
      ...
      FOR i IN
         (SELECT DISTINCT    a.line_cd
                          ...
                          a.policy_id, a.assd_no, b.assd_name, a.eff_date,
                          a.acct_of_cd, a.acct_of_cd_sw, a.acct_ent_date,
                          a.issue_date, c.grouped_item_no
                    ...
                    WHERE a.assd_no = b.assd_no
                      ...
                      AND check_user_per_iss_cd (a.line_cd,
                                                 a.iss_cd,
                                                 'GIPIS212'
                                                ) = 1
                 ...
                 ORDER BY policy_number, assd_name)
      LOOP
         ...
         FOR j IN (SELECT DISTINCT d.package_cd, c.control_cd,
                                      a.endt_iss_cd
                                   || '-'
                                   ...
                                   c.policy_id, c.grouped_item_title,
                                   c.grouped_item_no, a.eff_date,
                                   a.expiry_date, c.tsi_amt, c.prem_amt,
                                   c.delete_sw
                              FROM gipi_polbasic a,
                                   giis_assured b,
                                   gipi_grouped_items c,
                             ...
                             WHERE a.assd_no = b.assd_no
                               ...
                               AND c.grouped_item_no = i.grouped_item_no
                          ORDER BY c.grouped_item_title, control_cd)
         LOOP
            ...
            v_rec.cf_enrollee := 'Enrollee Name : ' || j.grouped_item_title;
      ...
      PIPE ROW (v_rec);
      RETURN;
   END get_gipir212_dtls_old ;
   
   FUNCTION get_gipir212_dtls (p_line_cd       VARCHAR2,
                                p_subline_cd    VARCHAR2,
                                p_iss_cd        VARCHAR2,
                                p_issue_yy      VARCHAR2,
                                p_pol_seq_no    VARCHAR2,
    ...
    BEGIN
       ...
       FOR i
          IN (  SELECT DISTINCT
                       ...
                       a.issue_date,
                       c.grouped_item_no,
                       a.line_cd , a.iss_cd , c.grouped_item_title, c.item_no
                  FROM gipi_polbasic a, giis_assured b, gipi_grouped_items c, giis_line d
                 WHERE     a.assd_no = b.assd_no
                       AND a.policy_id = c.policy_id
                       AND a.line_cd = d.line_cd 
                       AND NVL(d.menu_line_cd, d.line_cd) = 'AC'
                       AND c.grouped_item_title IS NOT NULL
                       AND (   (    a.line_cd = p_line_cd
                                AND a.subline_cd = p_subline_cd
                                AND a.iss_cd = p_iss_cd
                                           ...
                                           'MM-RRRR') BETWEEN TO_DATE (
                                                          ...
                                                          AND TO_DATE (
                                                                 ...
                                                                 || TO_CHAR (
                                                                       ...
                                                                       'YYYY'),
                                                                 'MM-RRRR'))))
              ORDER BY policy_number, assd_name, c.grouped_item_title, c.item_no, c.grouped_item_no )
       LOOP
