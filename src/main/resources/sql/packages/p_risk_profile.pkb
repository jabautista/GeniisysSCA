CREATE OR REPLACE PACKAGE BODY CPI.P_Risk_Profile AS
...
IS
BEGIN
  IF NVL(p_line_tag,'Y') = 'P' THEN
     risk_profile_per_peril(p_user,        p_line_cd,         p_subline_cd,
                         	p_date_from,   p_date_to,         p_basis,        p_line_tag,
       					 	p_cred_branch, p_include_expired, p_include_endt, p_exist); -- mark jm additional record filter --Added p_exist by Hero 09-11-2012
  ELSE
...
IS
  ...
  TYPE grp_tab_dtl   IS TABLE OF GIPI_RISK_PROFILE_DTL%ROWTYPE INDEX BY BINARY_INTEGER;
...
BEGIN
  ...
  giis_users_pkg.app_user := UPPER(p_user);  -- {jhing 04.01.2015} SR23240 june mark [10.19.16] 
  ...
  IF v_exists = 'Y' THEN
     ...
     */
     ...
     IF SQL%FOUND THEN
        ...
        --get_poldist_ext(NVL(p_line_cd,'%'),NVL(p_subline_cd,'%'),p_basis,p_date_from,p_date_to,NVL(p_cred_branch,'%'),p_include_expired, p_include_endt); -- mark jm additional 3 last parameters (cred_branch, include_expired, include_endt) for extraction filter
        --get_share_ext;
        get_poldist_ext(NVL(p_line_cd,'%'),NVL(p_subline_cd,'%'),p_basis,p_date_from,p_date_to,NVL(p_cred_branch,'%'),p_include_expired, p_include_endt,p_user); -- bonok :: 4.26.2016 :: FGIC SR 22230 :: added parameter p_user
        get_share_ext(p_user); -- bonok :: 4.26.2016 :: FGIC SR 22230 :: added parameter p_user
        ...
        */
        ...
        WHERE user_id = p_user; -- bonok :: 4.26.2016 :: FGIC SR 22230 
        ...
        FOR tarf IN (SELECT DISTINCT SUBSTR(tarf_cd,1,1) tarf_cd
          ...
          FOR prl IN (SELECT peril_cd
            ...
            v_polcount := 0;
            v_old_record := NULL; --added by aaron 02.19.2008 to correct policy count.
            ...
            END LOOP;
            -------
            FOR pol_main IN (SELECT A.line_cd, A.subline_cd, A.iss_cd,
                               A.issue_yy, A.pol_seq_no, A.renew_no,
                               SUM(b.tsi_amt*d.currency_rt) peril_tsi,
                               SUM(Get_Item_Ann_Tsi(A.line_cd, A.subline_cd, A.iss_cd, A.issue_yy,
                                                    A.pol_seq_no, A.renew_no, d.item_no, p_date_from,
                                                    p_date_to, p_basis, NULL)) item_tsi
                         ...
                         WHERE 1=1
                           ...
                           AND ((p_line_cd  <> v_fi_line)
                            ...
                            AND A.user_id = p_user -- bonok :: 4.26.2016 :: FGIC SR 22230 :: changed from USER to p_user_id
            ...
            LOOP
              v_policy_no := pol_main.line_cd||pol_main.subline_cd||pol_main.iss_cd||pol_main.issue_yy||pol_main.pol_seq_no||pol_main.renew_no;
            ...
            FOR pol IN (SELECT c.peril_cd, A.line_cd, A.subline_cd, A.iss_cd,
                         ...
                         WHERE 1=1
                           ...
                           AND b.item_no   = d.item_no
                           AND A.line_cd   = b.line_cd
                           AND a.line_cd = pol_main.line_cd        -- added by aaron 021508 to correct prem_amt and tsi_amt
                           AND a.subline_cd = pol_main.subline_cd  -- added by aaron 021508
                           AND a.iss_cd = pol_main.iss_cd          -- added by aaron 021508
                           AND a.issue_yy = pol_main.issue_yy      -- added by aaron 021508
                           AND a.pol_seq_no = pol_main.pol_seq_no  -- added by aaron 021508
                           AND a.renew_no = pol_main.renew_no      -- added by aaron 021508
                            AND NVL(b.peril_cd,b.peril_cd)  = prl.peril_cd
                           AND b.peril_cd  = c.peril_cd
                           AND b.line_cd   = c.line_cd
                           AND ((p_line_cd  <> v_fi_line)
              ...
              IF pol.line_cd = v_fi_line THEN
                 v_range_tsi := Get_Item_Ann_Tsi
                                  (pol.line_cd, pol.subline_cd, pol.iss_cd, pol.issue_yy,
                                   ...
                                   p_date_to, p_basis, NULL);
              ELSE
                 v_range_tsi  := pol.ann_tsi_amt;
              END IF;
            --------------------------------
           --A.R.C. 05.10.2006
              FOR ctr IN vv_range_to.FIRST..vv_range_to.LAST LOOP
                  IF ABS(v_pol_tsi) >= vv_range_from(ctr) AND
                     ABS(V_pol_tsi) <= vv_range_to(ctr) THEN --if within the range
                     v_policy_no := pol.line_cd||pol.subline_cd||pol.iss_cd||pol.issue_yy||pol.pol_seq_no||pol.renew_no;
                     ...
                     END IF;
                     tab_polrisk_ext_dtl(v_pol_ctr).range_from := vv_range_from(ctr);
                     tab_polrisk_ext_dtl(v_pol_ctr).range_to := vv_range_to(ctr);
                     tab_polrisk_ext_dtl(v_pol_ctr).line_cd := pol.line_cd;
                     tab_polrisk_ext_dtl(v_pol_ctr).subline_cd := pol.subline_cd;
                     tab_polrisk_ext_dtl(v_pol_ctr).iss_cd := pol.iss_cd;
                     tab_polrisk_ext_dtl(v_pol_ctr).issue_yy := pol.issue_yy;
                     tab_polrisk_ext_dtl(v_pol_ctr).pol_seq_no := pol.pol_seq_no;
                     tab_polrisk_ext_dtl(v_pol_ctr).renew_no := pol.renew_no;
                     tab_polrisk_ext_dtl(v_pol_ctr).user_id := p_user; -- bonok :: 4.26.2016 :: FGIC SR 22230 :: changed from USER to p_user
                     tab_polrisk_ext_dtl(v_pol_ctr).date_from := p_date_from;
                     tab_polrisk_ext_dtl(v_pol_ctr).date_to := p_date_to;
                     tab_polrisk_ext_dtl(v_pol_ctr).all_line_tag := p_line_tag;
                     tab_polrisk_ext_dtl(v_pol_ctr).ann_tsi_amt := NVL(tab_polrisk_ext_dtl(v_pol_ctr).ann_tsi_amt,0)  + NVL(pol.peril_tsi,0); --v_pol_tsi;
                     tab_polrisk_ext_dtl(v_pol_ctr).tarf_cd := tarf.tarf_cd;
                     tab_polrisk_ext_dtl(v_pol_ctr).peril_cd := prl.peril_cd;
                     tab_polrisk_ext_dtl(v_pol_ctr).peril_tsi  := NVL(tab_polrisk_ext_dtl(v_pol_ctr).peril_tsi,0)  + NVL(pol.peril_tsi,0);
                     tab_polrisk_ext_dtl(v_pol_Ctr).peril_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).peril_prem,0) + NVL(pol.peril_prem,0);
            ...
            --------------------------------
              FOR rec IN vv_range_to.FIRST..vv_range_to.LAST LOOP
                ...
                IF ABS(v_pol_tsi) >= tab_polrisk_ext(rec).range_from AND
                   ABS(v_pol_tsi) <= tab_polrisk_ext(rec).range_to THEN
        ...
        END LOOP; --> PERL TARIFF LOOP
         --A.R.C. 05.10.2006
		 IF NVL(tab_polrisk_ext_dtl.COUNT,0) > 0 THEN --added by steven 06.18.2014 it will cause an error pag walang laman ung collection :D
			 FOR transfer_dtl IN tab_polrisk_ext_dtl.FIRST..tab_polrisk_ext_dtl.LAST LOOP
			   ...
			   INSERT INTO GIPI_RISK_PROFILE_DTL(range_from, range_to, line_cd,
												 subline_cd, iss_cd, issue_yy,
												 pol_seq_no, renew_no, user_id,
												 net_retention, quota_share, treaty,
												 facultative, date_from, date_to,
												 all_line_tag, tarf_cd,
												 peril_cd, peril_tsi, peril_prem,
												 ann_tsi_amt)
			   VALUES(tab_polrisk_ext_dtl(transfer_dtl).range_from, tab_polrisk_ext_dtl(transfer_dtl).range_to, tab_polrisk_ext_dtl(transfer_dtl).line_cd,
					  tab_polrisk_ext_dtl(transfer_dtl).subline_cd, tab_polrisk_ext_dtl(transfer_dtl).iss_cd, tab_polrisk_ext_dtl(transfer_dtl).issue_yy,
					  tab_polrisk_ext_dtl(transfer_dtl).pol_seq_no, tab_polrisk_ext_dtl(transfer_dtl).renew_no, tab_polrisk_ext_dtl(transfer_dtl).user_id,
					  0, 0, 0,
					  0, tab_polrisk_ext_dtl(transfer_dtl).date_from, tab_polrisk_ext_dtl(transfer_dtl).date_to,
					  tab_polrisk_ext_dtl(transfer_dtl).all_line_tag, tab_polrisk_ext_dtl(transfer_dtl).tarf_cd,
					  tab_polrisk_ext_dtl(transfer_dtl).peril_cd, tab_polrisk_ext_dtl(transfer_dtl).peril_tsi, tab_polrisk_ext_dtl(transfer_dtl).peril_prem,
...
PROCEDURE fire_risk_profile_ext
  ...
  */
  (p_user           GIPI_RISK_PROFILE.user_id%TYPE,
   ...
   p_basis          VARCHAR2,
   p_line_tag           VARCHAR2, --***
   p_cred_branch        GIPI_POLBASIC.cred_branch%TYPE, -- mark jm additional parameter for extraction filter
   p_include_expired    VARCHAR2,                        -- mark jm additional parameter for extraction filter
   p_include_endt       VARCHAR2,                        -- mark jm additional parameter for extraction filter
   p_exist          OUT BOOLEAN)   -- Added by Hero 09-11-2012; Used for validation if there were records extracted to GIPI_POLRISK_EXT1
IS
  ...
  TYPE number_varray IS TABLE OF NUMBER INDEX BY BINARY_INTEGER;
  TYPE grp_tab       IS TABLE OF GIPI_RISK_PROFILE%ROWTYPE INDEX BY BINARY_INTEGER;
  TYPE grp_tab_dtl   IS TABLE OF GIPI_RISK_PROFILE_DTL%ROWTYPE INDEX BY BINARY_INTEGER; --A.R.C. 05.10.2006
  ...
  **to extract records OTHERWISE exit this procedure.*/
  IF v_exists = 'Y' THEN
     ...
     LOOP
       ...
       END IF;
       IF UPPER(shrcd.param_name) = 'FACULTATIVE' THEN
     ...
     LOOP
       ...
       END IF;
       IF UPPER(shrtyp.rv_meaning) = 'TREATY' THEN
          v_trty_shr_typ             := SUBSTR(shrtyp.rv_low_value,1,1);
       END IF;
       IF UPPER(shrtyp.rv_meaning) = 'FACULTATIVE' THEN
  ...
  SELECT COUNT (policy_id) --Added condition by Hero 09-11-2012; Used to check if there were records extracted per specific user
    INTO v_risk_count
    FROM gipi_polrisk_ext1
   WHERE line_cd = p_line_cd
     AND NVL (subline_cd, 1) = NVL (p_subline_cd, NVL (subline_cd, 1))
     AND user_id = p_user; -- bonok :: 4.26.2016 :: FGIC SR 22230 :: changed from USER to p_user

  IF v_risk_count = 0 THEN
     p_exist := FALSE; --Added by Hero 09-11-2012; Used for validation if there were records extracted to GIPI_POLRISK_EXT1
     RETURN;
  ELSE
     p_exist := TRUE;
  END IF;
        ...
        LOOP
          ...
          --A.R.C. 05.11.2006
          FOR pol_main IN (--policies to be processed
              SELECT A.line_cd,                A.subline_cd,                A.iss_cd,
                        A.issue_yy,             A.pol_seq_no,                A.renew_no,
                     SUM(Get_Item_Ann_Tsi(A.line_cd,          A.subline_cd,         A.iss_cd,
                                      A.issue_yy,          A.pol_seq_no,        A.renew_no,
                                      b.item_no,          p_date_from,         p_date_to,
                                      p_basis,              A.acct_neg_date))   item_tsi,
                      SUM(b.dist_prem*d.currency_rt) prem_amt,
                      SUM(DECODE(c.peril_type,'B',NVL(b.dist_tsi*d.currency_rt,0)*
                       ...
                       WHERE 1 = 1 --a.policy_id >= 0
                         ...
                         AND A.user_id    = p_user -- bonok :: 4.26.2016 :: FGIC SR 22230 :: changed from USER to p_user
         ...
         LOOP
          ...
          LOOP
           ...
           --A.R.C. 05.10.2006
              FOR ctr IN vv_range_to.FIRST..vv_range_to.LAST LOOP
                  IF ABS(v_item_tsi) >= vv_range_from(ctr) AND
                     ABS(v_item_tsi) <= vv_range_to(ctr) THEN --if within the range
                     v_policy_no := pol.line_cd||pol.subline_cd||pol.iss_cd||pol.issue_yy||pol.pol_seq_no||pol.renew_no;
                     IF var_policy_no <> v_policy_no OR var_policy_no IS NULL THEN
                        -- POLICYNO BEING PROCESSED IS ALREADY A DIFFERENT POLICY
                        var_policy_no := v_policy_no;
                     ...
                     DBMS_OUTPUT.PUT_LINE(v_policy_no||' '||v_pol_ctr);
                     tab_polrisk_ext_dtl(v_pol_ctr).range_from := vv_range_from(ctr);
                     tab_polrisk_ext_dtl(v_pol_ctr).range_to := vv_range_to(ctr);
                     tab_polrisk_ext_dtl(v_pol_ctr).line_cd := pol.line_cd;
                     tab_polrisk_ext_dtl(v_pol_ctr).subline_cd := pol.subline_cd;
                     ...
                     tab_polrisk_ext_dtl(v_pol_ctr).date_to := p_date_to;
                     tab_polrisk_ext_dtl(v_pol_ctr).all_line_tag := p_line_tag;
                     tab_polrisk_ext_dtl(v_pol_ctr).ann_tsi_amt := NVL(tab_polrisk_ext_dtl(v_pol_ctr).ann_tsi_amt,0) + NVL(pol.tsi_amt,0);--v_item_tsi;
                     tab_polrisk_ext_dtl(v_pol_ctr).tarf_cd := tarf.tarf_cd;
                     IF pol.share_type  = v_reten_shr_typ THEN
                           IF pol.share_cd = v_reten_shr_cd THEN
                           tab_polrisk_ext_dtl(v_pol_ctr).net_retention_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).net_retention_tsi,0) + NVL(pol.tsi_amt,0);
                           tab_polrisk_ext_dtl(v_pol_ctr).net_retention := NVL(tab_polrisk_ext_dtl(v_pol_ctr).net_retention,0) + NVL(pol.prem_amt,0);
                           ELSE
                           tab_polrisk_ext_dtl(v_pol_ctr).sec_net_retention_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).sec_net_retention_tsi,0) + NVL(pol.tsi_amt,0);
                           tab_polrisk_ext_dtl(v_pol_ctr).sec_net_retention_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).sec_net_retention_prem,0) + NVL(pol.prem_amt,0);
                           END IF;
                      ELSIF pol.share_type = v_facul_shr_typ THEN
                        IF pol.share_cd = v_facul_shr_cd THEN
                           tab_polrisk_ext_dtl(v_pol_ctr).facultative_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).facultative_tsi,0) + NVL(pol.tsi_amt,0);
                           tab_polrisk_ext_dtl(v_pol_ctr).facultative := NVL(tab_polrisk_ext_dtl(v_pol_ctr).facultative,0) + NVL(pol.prem_amt,0);
                           END IF;
                      ELSIF pol.share_type = v_trty_shr_typ THEN
                          IF NVL(pol.acct_trty_type,0) = 3 THEN
                           tab_polrisk_ext_dtl(v_pol_ctr).quota_share_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).quota_share_tsi,0) + NVL(pol.tsi_amt,0);
                           tab_polrisk_ext_dtl(v_pol_ctr).quota_share     := NVL(tab_polrisk_ext_dtl(v_pol_ctr).quota_share,0) + NVL(pol.prem_amt,0);
                           ELSE
                           IF q2_exists = 'Y' THEN
                              rec_counter := 0;
                              FOR trty IN vv_trty_share_cd.FIRST..vv_trty_share_cd.LAST LOOP
                                  ...
                                  IF pol.share_cd = vv_trty_share_cd(trty) THEN
                                     IF rec_counter = 1 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty_name := vv_index_trty_name(trty);
                                      ELSIF  rec_counter = 2 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty2_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty2_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty2_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty2_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty2_name := vv_index_trty_name(trty);
                                      ELSIF  rec_counter = 3 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty3_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty3_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty3_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty3_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty3_name := vv_index_trty_name(trty);
                                      ELSIF  rec_counter = 4 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty4_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty4_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty4_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty4_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty4_name := vv_index_trty_name(trty);
                                      ELSIF rec_counter = 5 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty5_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty5_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty5_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty5_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty5_name := vv_index_trty_name(trty);
                                      ELSIF rec_counter = 6 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty6_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty6_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty6_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty6_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty6_name := vv_index_trty_name(trty);
                                      ELSIF rec_counter = 7 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty7_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty7_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty7_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty7_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty7_name := vv_index_trty_name(trty);
                                      ELSIF rec_counter = 8 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty8_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty8_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty8_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty8_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty8_name := vv_index_trty_name(trty);
                                      ELSIF rec_counter = 9 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty9_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty9_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty9_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty9_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty9_name := vv_index_trty_name(trty);
                                      ELSIF rec_counter = 10 THEN
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty10_prem := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty10_prem,0) + NVL(pol.prem_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).treaty10_tsi := NVL(tab_polrisk_ext_dtl(v_pol_ctr).treaty10_tsi,0) + NVL(pol.tsi_amt,0);
                                        tab_polrisk_ext_dtl(v_pol_ctr).trty10_name := vv_index_trty_name(trty);
                                      END IF;
                                   END IF; --v_share_cd = vv_trty_share_cd(trty)
                               END LOOP;
                             END IF; --Q2_exists = 'Y'
            ...
            FOR rec IN vv_range_to.FIRST..vv_range_to.LAST LOOP
              tab_polrisk_ext(rec).tarf_cd := tarf.tarf_cd;
              --evaluate if policy falls within the range
              IF ABS(v_item_tsi) >= tab_polrisk_ext(rec).range_from AND
                 ABS(v_item_tsi) <= tab_polrisk_ext(rec).range_to THEN
        ...
        END LOOP; --end of tarf (break by tariff)
         ...
         FOR transfer_dtl IN tab_polrisk_ext_dtl.FIRST..tab_polrisk_ext_dtl.LAST LOOP
           ...
           INSERT INTO GIPI_RISK_PROFILE_DTL(range_from, range_to, line_cd,
                                             ...
                                             net_retention, quota_share, treaty,
                                             facultative, date_from, date_to,
                                             all_line_tag, treaty_tsi, treaty2_tsi,
                                             treaty3_tsi, treaty4_tsi, treaty5_tsi,
                                             treaty6_tsi, treaty7_tsi, treaty8_tsi,
                                             treaty9_tsi, treaty10_tsi, treaty_prem,
                                             treaty2_prem, treaty3_prem, treaty4_prem,
                                             treaty5_prem, treaty6_prem, treaty7_prem,
                                             treaty8_prem, treaty9_prem, treaty10_prem,
                                             net_retention_tsi, facultative_tsi, quota_share_tsi,
                                             sec_net_retention_tsi, sec_net_retention_prem, tarf_cd,
                                             peril_cd, peril_tsi, peril_prem,
                                             trty_name, trty2_name, trty3_name,
                                             trty4_name, trty5_name, trty6_name,
                                             trty7_name, trty8_name, trty9_name,
                                             trty10_name, ann_tsi_amt)
           VALUES(tab_polrisk_ext_dtl(transfer_dtl).range_from, tab_polrisk_ext_dtl(transfer_dtl).range_to, tab_polrisk_ext_dtl(transfer_dtl).line_cd,
                  tab_polrisk_ext_dtl(transfer_dtl).subline_cd, tab_polrisk_ext_dtl(transfer_dtl).iss_cd, tab_polrisk_ext_dtl(transfer_dtl).issue_yy,
                  tab_polrisk_ext_dtl(transfer_dtl).pol_seq_no, tab_polrisk_ext_dtl(transfer_dtl).renew_no, tab_polrisk_ext_dtl(transfer_dtl).user_id,
                  tab_polrisk_ext_dtl(transfer_dtl).net_retention, tab_polrisk_ext_dtl(transfer_dtl).quota_share, tab_polrisk_ext_dtl(transfer_dtl).treaty,
                  tab_polrisk_ext_dtl(transfer_dtl).facultative, tab_polrisk_ext_dtl(transfer_dtl).date_from, tab_polrisk_ext_dtl(transfer_dtl).date_to,
                  tab_polrisk_ext_dtl(transfer_dtl).all_line_tag, tab_polrisk_ext_dtl(transfer_dtl).treaty_tsi, tab_polrisk_ext_dtl(transfer_dtl).treaty2_tsi,
                  tab_polrisk_ext_dtl(transfer_dtl).treaty3_tsi, tab_polrisk_ext_dtl(transfer_dtl).treaty4_tsi, tab_polrisk_ext_dtl(transfer_dtl).treaty5_tsi,
                  tab_polrisk_ext_dtl(transfer_dtl).treaty6_tsi, tab_polrisk_ext_dtl(transfer_dtl).treaty7_tsi, tab_polrisk_ext_dtl(transfer_dtl).treaty8_tsi,
                  tab_polrisk_ext_dtl(transfer_dtl).treaty9_tsi, tab_polrisk_ext_dtl(transfer_dtl).treaty10_tsi, tab_polrisk_ext_dtl(transfer_dtl).treaty_prem,
                  tab_polrisk_ext_dtl(transfer_dtl).treaty2_prem, tab_polrisk_ext_dtl(transfer_dtl).treaty3_prem, tab_polrisk_ext_dtl(transfer_dtl).treaty4_prem,
                  tab_polrisk_ext_dtl(transfer_dtl).treaty5_prem, tab_polrisk_ext_dtl(transfer_dtl).treaty6_prem, tab_polrisk_ext_dtl(transfer_dtl).treaty7_prem,
                  tab_polrisk_ext_dtl(transfer_dtl).treaty8_prem, tab_polrisk_ext_dtl(transfer_dtl).treaty9_prem, tab_polrisk_ext_dtl(transfer_dtl).treaty10_prem,
                  tab_polrisk_ext_dtl(transfer_dtl).net_retention_tsi, tab_polrisk_ext_dtl(transfer_dtl).facultative_tsi, tab_polrisk_ext_dtl(transfer_dtl).quota_share_tsi,
                  tab_polrisk_ext_dtl(transfer_dtl).sec_net_retention_tsi, tab_polrisk_ext_dtl(transfer_dtl).sec_net_retention_prem, tab_polrisk_ext_dtl(transfer_dtl).tarf_cd,
                  tab_polrisk_ext_dtl(transfer_dtl).peril_cd, tab_polrisk_ext_dtl(transfer_dtl).peril_tsi, tab_polrisk_ext_dtl(transfer_dtl).peril_prem,
                  tab_polrisk_ext_dtl(transfer_dtl).trty_name, tab_polrisk_ext_dtl(transfer_dtl).trty2_name, tab_polrisk_ext_dtl(transfer_dtl).trty3_name,
                  tab_polrisk_ext_dtl(transfer_dtl).trty4_name, tab_polrisk_ext_dtl(transfer_dtl).trty5_name, tab_polrisk_ext_dtl(transfer_dtl).trty6_name,
                  tab_polrisk_ext_dtl(transfer_dtl).trty7_name, tab_polrisk_ext_dtl(transfer_dtl).trty8_name, tab_polrisk_ext_dtl(transfer_dtl).trty9_name,
                  tab_polrisk_ext_dtl(transfer_dtl).trty10_name, tab_polrisk_ext_dtl(transfer_dtl).ann_tsi_amt);
 ...
 */
 (v_user           GIPI_RISK_PROFILE.user_id%TYPE,
  ...
  v_basis          VARCHAR2,
  v_line_tag       VARCHAR2, --***
  v_cred_branch        GIPI_POLBASIC.cred_branch%TYPE, -- mark jm additional parameter for extraction filter
  v_include_expired    VARCHAR2,                       -- mark jm additional parameter for extraction filter
...
BEGIN
  ...
  giis_users_pkg.app_user := UPPER(v_user); -- {jhing 04.01.2015} SR23240 june mark [10.19.16] 
...
--      v_exist := 'Y';
     ...
     IF q4_exists = 'Y' THEN
         FOR indx IN range_to.FIRST..range_to.LAST LOOP
          ...
          -- INITIALIZATION OF COLLECTIONS --
           ...
           ext_risk_row(indx).treaty10_tsi  :=0;      ext_risk_row(indx).treaty_prem :=0; --nieko 02212017, SR 23886
         ...
         END LOOP;
     END IF;
     --get_poldist_ext(NVL(v_line_cd,'%'),NVL(v_subline_cd,'%'),v_basis,v_date_from,v_date_to,NVL(v_cred_branch,'%'), v_include_expired,v_include_endt); -- mark jm additional 3 last parameters (cred_branch, include_expired, include_endt) for extraction filter
     --get_share_ext;
     get_poldist_ext(NVL(v_line_cd,'%'),NVL(v_subline_cd,'%'),v_basis,v_date_from,v_date_to,NVL(v_cred_branch,'%'), v_include_expired,v_include_endt, v_user); -- bonok :: 4.26.2016 :: FGIC SR 22230 :: added parameter p_user
     get_share_ext(v_user); -- bonok :: 4.26.2016 :: FGIC SR 22230 :: added parameter p_user
     --A.R.C. 05.11.2006
  ...
  SELECT COUNT (policy_id) --Added condition by Hero 09-11-2012; Used to check if there were records extracted per specific user
   ...
   WHERE line_cd = v_line_cd
     AND NVL (subline_cd, 1) = NVL (v_subline_cd, NVL (subline_cd, 1))
     AND user_id = v_user; -- bonok :: 4.26.2016 :: FGIC SR 22230 :: changed from USER to v_user
  ...
  END IF;
       ...
       (SELECT A.line_cd,            A.subline_cd,              A.iss_cd,
        ...
        FROM (
               SELECT A.line_cd,              A.subline_cd,                A.iss_cd,
                      ...
                      SUM(b.dist_prem*d.currency_rt*DECODE(v_basis,'AD',sign_tsi(v_date_from,
                                                                        v_date_to,
                                                                   A.acct_neg_date,
                                                                    A.acct_ent_date,
                                                                    A.pol_flag),1)) prem_amt,
                          SUM(DECODE(c.peril_type,'B',b.dist_tsi*d.currency_rt*DECODE(v_basis,'AD',sign_tsi(v_date_from,
                                                                                               v_date_to,
                                                                                           A.acct_neg_date,
                                                                                           A.acct_ent_date,
                 ...
                 WHERE A.policy_id >= 0
                  ...
                  AND b.share_cd = c.share_cd
                  AND A.user_id  = c.user_id
                  AND A.user_id  = v_user -- bonok :: 4.26.2016 :: FGIC SR 22230 :: changed from USER to v_user
                GROUP BY A.line_cd,      A.subline_cd,            A.iss_cd,
                         A.issue_yy,     A.pol_seq_no,            A.renew_no,
                           c.share_cd,     c.acct_trty_type,      c.share_type,
                          A.ann_tsi_amt) A
    GROUP BY A.line_cd,                A.subline_cd,                A.iss_cd,
              A.issue_yy,             A.pol_seq_no,                A.renew_no)
    LOOP
      v_policy_no      := rc_pol.line_cd||rc_pol.subline_cd||rc_pol.iss_cd||rc_pol.issue_yy||rc_pol.pol_seq_no||rc_pol.renew_no;
    ...
    GROUP BY A.line_cd,              A.subline_cd,              A.iss_cd,
     ...
     LOOP
       ...
       --A.R.C. 05.10.2006
       IF q4_exists = 'Y' THEN
          FOR ctr IN ext_risk_row.FIRST..ext_risk_row.LAST LOOP
              IF ABS(v_pol_tsi) >= ext_risk_row(ctr).range_from AND
                 ABS(v_pol_tsi) <= ext_risk_row(ctr).range_to THEN --if within the range
                 v_pol_ctr := NVL(v_pol_ctr,0) + v_pol_cntr;
                 ext_risk_row_dtl(v_pol_ctr).range_from := ext_risk_row(ctr).range_from;
                 ext_risk_row_dtl(v_pol_ctr).range_to := ext_risk_row(ctr).range_to;
                 ext_risk_row_dtl(v_pol_ctr).line_cd := rc.line_cd;
                 ext_risk_row_dtl(v_pol_ctr).subline_cd := rc.subline_cd;
                 ext_risk_row_dtl(v_pol_ctr).iss_cd := rc.iss_cd;
                 ext_risk_row_dtl(v_pol_ctr).issue_yy := rc.issue_yy;
                 ext_risk_row_dtl(v_pol_ctr).pol_seq_no := rc.pol_seq_no;
                 ext_risk_row_dtl(v_pol_ctr).renew_no := rc.renew_no;
                 ext_risk_row_dtl(v_pol_ctr).user_id := v_user; -- bonok :: 4.26.2016 :: FGIC SR 22230 :: changed from USER to v_user
                 ext_risk_row_dtl(v_pol_ctr).date_from := v_date_from;
                 ext_risk_row_dtl(v_pol_ctr).date_to := v_date_to;
                 ext_risk_row_dtl(v_pol_ctr).all_line_tag := v_line_tag;
                 ext_risk_row_dtl(v_pol_ctr).ann_tsi_amt := NVL(ext_risk_row_dtl(v_pol_ctr).ann_tsi_amt,0) + NVL(v_tsi_amt,0);--v_pol_tsi;
                 IF v_share_type  = v_reten_shr_tp THEN
                       IF v_share_cd = v_reten_shr_cd THEN
                       ext_risk_row_dtl(v_pol_ctr).net_retention := NVL(ext_risk_row_dtl(v_pol_ctr).net_retention,0) + NVL(v_prem_amt,0);
                       ext_risk_row_dtl(v_pol_ctr).net_retention_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).net_retention_tsi,0) + NVL(v_tsi_amt,0);
                       ELSE
                       ext_risk_row_dtl(v_pol_ctr).sec_net_retention_prem := NVL(ext_risk_row_dtl(v_pol_ctr).sec_net_retention_prem,0) + NVL(v_prem_amt,0);
                       ext_risk_row_dtl(v_pol_ctr).sec_net_retention_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).sec_net_retention_tsi,0) + NVL(v_tsi_amt,0);
                       END IF;
                  ELSIF v_share_type = v_facul_shr_tp THEN
                    IF v_share_cd   = v_facul_shr_cd THEN
                       ext_risk_row_dtl(v_pol_ctr).facultative := NVL(ext_risk_row_dtl(v_pol_ctr).facultative,0) + NVL(v_prem_amt,0);
                       ext_risk_row_dtl(v_pol_ctr).facultative_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).facultative_tsi,0) + NVL(v_tsi_amt,0);
                       END IF;
                  ELSIF v_share_type = v_trty_shr_tp THEN
                       --Separate Quota Share to other Treaty.
                       IF v_acct_trty_type = NVL(v_acct_trty_tp,0) THEN
                       ext_risk_row_dtl(v_pol_ctr).quota_share := NVL(ext_risk_row_dtl(v_pol_ctr).quota_share,0) + NVL(v_prem_amt,0);
                       ext_risk_row_dtl(v_pol_ctr).quota_share_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).quota_share_tsi,0) + NVL(v_tsi_amt,0);
                       ELSE
                       IF q5_exists = 'Y' THEN
                          rec_counter := 0;
                          FOR trty IN vv_trty_share_cd.FIRST..vv_trty_share_cd.LAST LOOP
                              --rec_counter := rec_counter + 1; -- aron 
                                 DBMS_OUTPUT.PUT_LINE('SHARE_CD :'||v_share_cd);
                              IF v_share_cd = vv_trty_share_cd(trty) THEN
                                 rec_counter := rec_counter + 1; -- aaron 031709
                                 IF rec_counter = 1 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty := NVL(ext_risk_row_dtl(v_pol_ctr).treaty,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty_name := vv_index_trty_name(trty);
                                  ELSIF rec_counter = 2 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty2_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty2_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty2_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty2_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty2_name := vv_index_trty_name(trty);
                                  ELSIF rec_counter = 3 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty3_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty3_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty3_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty3_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty3_name := vv_index_trty_name(trty);
                                  ELSIF rec_counter = 4 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty4_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty4_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty4_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty4_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty4_name := vv_index_trty_name(trty);
                                  ELSIF rec_counter = 5 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty5_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty5_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty5_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty5_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty5_name := vv_index_trty_name(trty);
                                  ELSIF rec_counter = 6 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty6_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty6_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty6_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty6_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty6_name := vv_index_trty_name(trty);
                                  ELSIF rec_counter = 7 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty7_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty7_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty7_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty7_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty7_name := vv_index_trty_name(trty);
                                  ELSIF rec_counter = 8 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty8_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty8_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty8_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty8_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty8_name := vv_index_trty_name(trty);
                                  ELSIF rec_counter = 9 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty9_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty9_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty9_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty9_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty9_name := vv_index_trty_name(trty);
                                  ELSIF rec_counter = 10 THEN
                                    ext_risk_row_dtl(v_pol_ctr).treaty10_prem := NVL(ext_risk_row_dtl(v_pol_ctr).treaty10_prem,0) + NVL(v_prem_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).treaty10_tsi := NVL(ext_risk_row_dtl(v_pol_ctr).treaty10_tsi,0) + NVL(v_tsi_amt,0);
                                    ext_risk_row_dtl(v_pol_ctr).trty10_name := vv_index_trty_name(trty);
                                  END IF;
                               END IF; --v_share_cd = vv_trty_share_cd(trty)
                           END LOOP;
                         END IF; --Q5_exists = 'Y'
...
--       IF ext_risk_row.FIRST IS NULL THEN
            ...
            IF ABS(v_pol_tsi) >= ext_risk_row(indx).range_from AND
               ...
               ELSIF v_share_type = v_facul_shr_tp THEN
                  IF v_share_cd   = v_facul_shr_cd THEN
                     ...
                     END IF;
               ELSIF v_share_type = NVL(v_trty_shr_tp,0) THEN
                     --Separate Quota Share to other Treaty.
                     IF v_acct_trty_type = NVL(v_acct_trty_tp,0) THEN
           ...
           IF vv_count(rec) <> 0 THEN
              ...
              IF q4_exists = 'Y' THEN
                  FOR grp IN vv_index_range.FIRST..vv_index_range.LAST LOOP
                    ...
                    IF vv_trty_share_cd(rec) = vv_index_share(grp) THEN
                       IF rec_counter = 1 THEN
                          ext_risk_row(vv_index_range(grp)).treaty_prem --nieko 02212017, SR 23886
                             := ext_risk_row(vv_index_range(grp)).treaty_prem + vv_index_prem_amt(grp);
  ...
  IF ext_risk_row.LAST IS NOT NULL THEN
       FOR transfer IN ext_risk_row.FIRST..ext_risk_row.LAST LOOP
        /*Editted by MJ for consolidation 01032013 
        DBMS_OUTPUT.PUT_LINE('asd '||ext_risk_row(transfer).net_retention);
        */
       ...
       treaty_prem   = ext_risk_row(transfer).treaty_prem, --nieko 02212017, SR 23886
       ...
       net_retention = NVL(ext_risk_row(transfer).net_retention,0), -- aaron 031809
       net_retention_tsi  = ext_risk_row(transfer).net_retention_tsi,
       net_retention_cnt  = ext_risk_row(transfer).net_retention_cnt,
       facultative        = NVL(ext_risk_row(transfer).facultative,0),
       facultative_tsi    = ext_risk_row(transfer).facultative_tsi,
       facultative_cnt    = NVL(ext_risk_row(transfer).facultative_cnt,0),
       quota_share          = NVL(ext_risk_row(transfer).quota_share,0),
  ...
  --A.R.C. 05.10.2006
     DBMS_OUTPUT.PUT_LINE('EXT_RISK_ROW_dtl.FIRST: '||TO_CHAR(ext_risk_row_dtl.FIRST));
     DBMS_OUTPUT.PUT_LINE('EXT_RISK_ROW_dtl.LAST: '||TO_CHAR(ext_risk_row_dtl.LAST));
  
  IF ext_risk_row_dtl.LAST IS NOT NULL THEN -- aaron  031809 to prevent ora-06502 
  FOR transfer_dtl IN ext_risk_row_dtl.FIRST..ext_risk_row_dtl.LAST LOOP
    --DBMS_OUTPUT.PUT_LINE(ext_risk_row_dtl(transfer_dtl).line_cd||'-'||ext_risk_row_dtl(transfer_dtl).subline_cd||'-'||ext_risk_row_dtl(transfer_dtl).iss_cd||'-'||ext_risk_row_dtl(transfer_dtl).issue_yy||'-'||ext_risk_row_dtl(transfer_dtl).pol_seq_no||'-'||ext_risk_row_dtl(transfer_dtl).renew_no);
       ...
       INSERT INTO GIPI_RISK_PROFILE_DTL(range_from, range_to, line_cd,
                                         ...
                                         trty7_name, trty8_name, trty9_name,
                                         trty10_name, ann_tsi_amt)
       VALUES(ext_risk_row_dtl(transfer_dtl).range_from, ext_risk_row_dtl(transfer_dtl).range_to, ext_risk_row_dtl(transfer_dtl).line_cd,
              ext_risk_row_dtl(transfer_dtl).subline_cd, ext_risk_row_dtl(transfer_dtl).iss_cd, ext_risk_row_dtl(transfer_dtl).issue_yy,
              ext_risk_row_dtl(transfer_dtl).pol_seq_no, ext_risk_row_dtl(transfer_dtl).renew_no, ext_risk_row_dtl(transfer_dtl).user_id,
              ext_risk_row_dtl(transfer_dtl).net_retention, ext_risk_row_dtl(transfer_dtl).quota_share, ext_risk_row_dtl(transfer_dtl).treaty,
              ext_risk_row_dtl(transfer_dtl).facultative, ext_risk_row_dtl(transfer_dtl).date_from, ext_risk_row_dtl(transfer_dtl).date_to,
              ext_risk_row_dtl(transfer_dtl).all_line_tag, ext_risk_row_dtl(transfer_dtl).treaty_tsi, ext_risk_row_dtl(transfer_dtl).treaty2_tsi,
              ext_risk_row_dtl(transfer_dtl).treaty3_tsi, ext_risk_row_dtl(transfer_dtl).treaty4_tsi, ext_risk_row_dtl(transfer_dtl).treaty5_tsi,
              ext_risk_row_dtl(transfer_dtl).treaty6_tsi, ext_risk_row_dtl(transfer_dtl).treaty7_tsi, ext_risk_row_dtl(transfer_dtl).treaty8_tsi,
              ext_risk_row_dtl(transfer_dtl).treaty9_tsi, ext_risk_row_dtl(transfer_dtl).treaty10_tsi, ext_risk_row_dtl(transfer_dtl).treaty_prem,
              ext_risk_row_dtl(transfer_dtl).treaty2_prem, ext_risk_row_dtl(transfer_dtl).treaty3_prem, ext_risk_row_dtl(transfer_dtl).treaty4_prem,
              ext_risk_row_dtl(transfer_dtl).treaty5_prem, ext_risk_row_dtl(transfer_dtl).treaty6_prem, ext_risk_row_dtl(transfer_dtl).treaty7_prem,
              ext_risk_row_dtl(transfer_dtl).treaty8_prem, ext_risk_row_dtl(transfer_dtl).treaty9_prem, ext_risk_row_dtl(transfer_dtl).treaty10_prem,
              ext_risk_row_dtl(transfer_dtl).net_retention_tsi, ext_risk_row_dtl(transfer_dtl).facultative_tsi, ext_risk_row_dtl(transfer_dtl).quota_share_tsi,
              ext_risk_row_dtl(transfer_dtl).sec_net_retention_tsi, ext_risk_row_dtl(transfer_dtl).sec_net_retention_prem, ext_risk_row_dtl(transfer_dtl).tarf_cd,
              ext_risk_row_dtl(transfer_dtl).peril_cd, ext_risk_row_dtl(transfer_dtl).peril_tsi, ext_risk_row_dtl(transfer_dtl).peril_prem,
              ext_risk_row_dtl(transfer_dtl).trty_name, ext_risk_row_dtl(transfer_dtl).trty2_name, ext_risk_row_dtl(transfer_dtl).trty3_name,
              ext_risk_row_dtl(transfer_dtl).trty4_name, ext_risk_row_dtl(transfer_dtl).trty5_name, ext_risk_row_dtl(transfer_dtl).trty6_name,
              ext_risk_row_dtl(transfer_dtl).trty7_name, ext_risk_row_dtl(transfer_dtl).trty8_name, ext_risk_row_dtl(transfer_dtl).trty9_name,
              ext_risk_row_dtl(transfer_dtl).trty10_name, ext_risk_row_dtl(transfer_dtl).ann_tsi_amt);
     END LOOP;
    END IF; -- aaron 031809
     COMMIT;
...
PROCEDURE get_poldist_ext
  ...
  */
  (p_line_cd     gipi_polbasic.line_cd%TYPE,
   ...
   p_include_expired    VARCHAR2,                 -- mark jm additional parameter for extraction filter
   p_include_endt       VARCHAR2,                 -- mark jm additional parameter for extraction filter
   p_user_id     giis_users.user_id%TYPE) -- bonok :: 4.26.2016 :: FGIC SR 22230
IS
...
BEGIN
  DELETE FROM GIPI_POLRISK_EXT1
    WHERE user_id = p_user_id; -- bonok :: 4.26.2016 :: FGIC SR 22230 :: changed from USER to p_user_id
  COMMIT;         
      ...
      SELECT   A.policy_id,          A.line_cd,                 A.subline_cd,
               ...
               SUM((b.tsi_amt*DECODE(p_basis,'AD',sign_tsi(    p_date_from,
                                                               p_date_to,
                                                            b.acct_neg_date,
                                                            A.acct_ent_date,
                                                            A.pol_flag),1))),
    ...
    FROM     gipi_polbasic A,
       ...
       WHERE     A.line_cd LIKE  p_line_cd
             ...
             --AND A.cred_branch LIKE p_cred_branch -- mark jm additional parameter for extraction filter
              AND NVL(A.cred_branch,A.iss_cd) LIKE p_cred_branch --added NVL function for 'RI'
             ...
             AND Date_Risk(b.acct_ent_date, A.eff_date, A.issue_date, A.booking_mth, A.booking_year,
                ...
                b.acct_neg_date) = 1
				 AND b.dist_flag <> '4' -- bonok :: 4.26.2016 :: FGIC SR 22230 :: to exclude negated policies
...
--     AND b.dist_flag = decode(a.pol_flag,'5','4','3');--if pol_flag is 5, dist_flag shld be 4, else dist_flag shld be 3
         ...
         FORALL i IN vv_policy_id.FIRST..vv_policy_id.LAST
                  INSERT INTO GIPI_POLRISK_EXT1
                          (policy_id,          line_cd,              subline_cd,
                           ...
                           user_id,            acct_ent_date,        acct_neg_date,
                           pol_flag)
                   VALUES
                       (vv_policy_id(i),    vv_line_cd(i),         vv_subline_cd(i),
                        ...
                        p_user_id,               vv_acct_ent_date(i),   vv_acct_neg_date(i),
                        vv_pol_flag(i));
                 COMMIT;
 ...
 FOR X IN (SELECT policy_id, line_cd, subline_cd, iss_cd, issue_yy, pol_seq_no, renew_no
             FROM GIPI_POLRISK_EXT1
   WHERE user_id = p_user_id -- bonok :: 4.26.2016 :: FGIC SR 22230 :: changed from USER to p_user_id
   GROUP BY policy_id, line_cd, subline_cd, iss_cd, issue_yy, pol_seq_no, renew_no)
 LOOP
 ...
 SELECT  A.policy_id,          A.line_cd,          A.subline_cd,
   ...
   WHERE  A.line_cd LIKE  p_line_cd
     AND A.subline_cd LIKE  p_subline_cd
   --AND A.cred_branch LIKE p_cred_branch -- mark jm additional parameter for extraction filter
     AND NVL(A.cred_branch,A.iss_cd) LIKE p_cred_branch --vin added NVL function for 'RI'
     AND A.pol_flag <> DECODE (p_include_expired,'Y',' ','X' )
     AND A.pol_flag <> '4'  -- aaron to exclude cancelled policies
   --AND A.pol_flag <> CASE p_include_expired WHEN 'Y' THEN ' ' ELSE 'X' END-- mark jm additional parameter for extraction filter
     AND NVL(A.endt_type, 'A') = 'A'
      ...
      DECODE(p_include_endt,'Y',
       DECODE(
             ...
             WHERE line_cd = A.line_cd
                  ...
                  AND issue_yy = A.issue_yy
                  AND pol_seq_no = A.pol_seq_no),1,
          DECODE(p_basis,'AD',b.acct_ent_date,'ID',A.issue_date, 'ED',A.eff_date),p_date_to),p_date_to), p_date_to),
    b.acct_neg_date) = 1
 GROUP  BY A.policy_id,          A.line_cd,          A.subline_cd,
          A.iss_cd,             A.issue_yy,       A.pol_seq_no,
         A.renew_no,           b.dist_no,b.acct_ent_date,
...
END;
PROCEDURE get_share_ext (p_user_id      giis_users.user_id%TYPE) IS
...
/*       IF v_val_pol_flag = 'Y' THEN
  ...
  */
  ...
  BEGIN
    FOR cur1 IN (
      SELECT SUM(y.tsi_amt*y.currency_rt*DECODE(p_basis,'AD',sign_tsi(p_from,
                                                             p_to,
                                                         p_and,
                                                         x.acct_ent_date,
