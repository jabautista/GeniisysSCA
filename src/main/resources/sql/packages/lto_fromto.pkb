CREATE OR REPLACE PACKAGE BODY CPI.Lto_Fromto AS 
...
--	if sql%notfound then
	   ...
	   LOOP
	    SELECT SUM(DECODE(b.coverage_cd, 1, c.prem_amt * b.currency_rt, 0)) mla_prem_amt,
         ...
         WHERE 1=1
    	   ...
    	   AND Lto_Fromto.date_ok(a.acct_ent_date, a.eff_date, a.issue_date, a.booking_mth, a.booking_year,
    ...
    */
	SELECT get_peril_stat_name(v_zone_type) peril_stat_name,
	       ...
	       COUNT(DECODE(e.coverage_cd, 1, b.claim_id, NULL)) mla_cnt,
     	   ...
     	   SUM(DECODE(e.coverage_cd, 1, c.loss_reserve, 0))
     	   - SUM(DECODE(e.coverage_cd, 1, c.losses_paid, 0)) mla_losses,
     	   SUM(DECODE(e.coverage_cd, 1, 0, c.loss_reserve))
     	   - SUM(DECODE(e.coverage_cd, 1,0, c.losses_paid)) out_mla_losses,
     	   SUM(DECODE(e.coverage_cd, 1, c.losses_paid, 0))
     	   + SUM(DECODE(e.coverage_cd, 1, c.expenses_paid, 0)) mla_pd_claims,
     	   SUM(DECODE(e.coverage_cd, 1,0, c.losses_paid))
     	   + SUM(DECODE(e.coverage_cd, 1,0, c.expenses_paid)) out_mla_pd_claims
      BULK COLLECT INTO
	       vv_peril_stat_name,
		   vv_subline,
		   vv_mla_clm_cnt,
     ...
     WHERE 1=1
       ...
       AND c.peril_cd = v_peril_cd
	       --check if loss_date of gicl_claims are within specified dates
	   AND TRUNC(b.loss_date) >= v_date_from
	   AND TRUNC(b.loss_date) <= v_date_to
  ...
  */
  FUNCTION get_subline_type(p_line_cd     VARCHAR2,
    						...
    						p_to          DATE)
  RETURN VARCHAR2 IS
    v_return    GIIS_MC_SUBLINE_TYPE.subline_type_cd%TYPE;
  BEGIN
			   ...
			   WHERE 1=1
				 ...
				 AND NOT EXISTS (SELECT 'X'
								  ...
								  WHERE 1=1
									...
									--check if dates are valid for that particular record in polbasic
									AND date_ok(r.acct_ent_date, r.eff_date, r.issue_date, r.booking_mth, r.booking_year,
                                        r.spld_acct_ent_date, r.pol_flag, p_dt_type, p_from, p_to) =  1)
			 ORDER BY x.eff_date DESC)
    LOOP
	  v_return := a.subline_type_cd;
	  EXIT;
	END LOOP;
	RETURN(v_return);
  END;
