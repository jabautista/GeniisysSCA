CREATE OR REPLACE PACKAGE BODY CPI.GIACR137A_PKG
AS
   /*
   ...
   */    
    ...
    BEGIN
        ...
        LOOP
            ...
            FOR w IN(SELECT cession_year, cession_mm, line_cd, share_cd, 
                            treaty_yy, trty_com_rt, branch_cd,
                   ...
                   GROUP BY cession_year, cession_mm, line_cd, share_cd, treaty_yy, trty_com_rt, branch_cd
                   ORDER BY cession_year, cession_mm, line_cd, share_cd, treaty_yy, trty_com_rt, branch_cd)
            LOOP
                ...
                v_rep.trty_com_rt2      := w.trty_com_rt;
                v_count                 :=  1;
                v_mm_total              :=  0;
                v_grp_count             :=  1;
                ...
                LOOP
                    ...
                    FOR e IN(SELECT cession_year, cession_mm, line_cd, share_cd, treaty_yy, 
                              ...
                              WHERE user_id = p_user_id
                               ...
                               AND trty_com_rt = q.trty_com_rt
                           GROUP BY cession_year, cession_mm, line_cd, share_cd, treaty_yy, trty_com_rt, branch_cd, prnt_ri_cd)
                    LOOP
    ...
    BEGIN
        FOR q IN(SELECT cession_year, cession_mm, line_cd, share_cd, treaty_yy, 
                        trty_com_rt, branch_cd, SUM(commission_amt) comm_per_branch
                       FROM gixx_trty_prem_comm
                      WHERE cession_mm  BETWEEN DECODE(p_quarter,1,3,2,6,3,9,4,12,NULL,3)-2 
                        AND DECODE(p_quarter,1,3,2,6,3,9,4,12,NULL,3)
                        AND user_id = p_user_id 
