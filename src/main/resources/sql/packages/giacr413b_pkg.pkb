CREATE OR REPLACE PACKAGE BODY CPI.giacr413b_pkg
AS
/*
    **  Created by   :  Ildefonso Ellarina Jr
    **  Date Created : 07.23.2013
    **  Reference By : GIACR413B - Commissions Paid
    */
   FUNCTION get_details (
      p_from_dt     VARCHAR2,
      p_to_dt       VARCHAR2,
      p_tran_post   NUMBER,
      p_branch      VARCHAR2,
      p_module_id   VARCHAR2,
      p_intm_type   VARCHAR2,
      p_user_id     VARCHAR2
   )
      RETURN get_details_tab PIPELINED
   IS
      v_list   get_details_type;
      v_is_empty VARCHAR2(1) := 'Y';
   BEGIN
      BEGIN
         SELECT param_value_v
           INTO v_list.cf_company
           FROM giis_parameters
          WHERE param_name = 'COMPANY_NAME';
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN
            v_list.cf_company := NULL;
      END;

      BEGIN
         SELECT param_value_v
           INTO v_list.cf_com_address
           FROM giac_parameters
          WHERE param_name = 'COMPANY_ADDRESS';
      EXCEPTION
         WHEN NO_DATA_FOUND
         THEN
            v_list.cf_com_address := NULL;
      END;

      BEGIN
         SELECT    'From '
                || TO_CHAR (TO_DATE (p_from_dt, 'MM/DD/YYYY'),
                            'fm Month DD, YYYY'
                           )
                || ' to '
                || TO_CHAR (TO_DATE (p_to_dt, 'MM/DD/YYYY'),
                            'fm Month DD,YYYY'
                           )
           INTO v_list.period
           FROM DUAL;
      END;

      BEGIN
         IF p_tran_post = 1
         THEN
            v_list.tran_post := 'Based on Transaction Date';
         ELSE
            v_list.tran_post := 'Based on Posting Date';
         END IF;
      END;

      FOR i IN
         (SELECT   d.intm_type, a.intm_no, d.intm_name, a.iss_cd,
                   SUM (a.comm_amt) comm, SUM (a.wtax_amt) wtax,
                   SUM (input_vat_amt) input_vat
              FROM giac_comm_payts a, giac_acctrans b, giis_intermediary d
             WHERE a.gacc_tran_id = b.tran_id
               AND a.intm_no = d.intm_no
               AND a.iss_cd = NVL (p_branch, a.iss_cd)
               --AND check_user_per_iss_cd_acctg2 (NULL, a.iss_cd, p_module_id, p_user_id) = 1 --mikel 12.12.2016;
               AND EXISTS (SELECT 'X'
                                   FROM table (security_access.get_branch_line('AC', p_module_id, p_user_id))
                                  WHERE branch_cd = a.iss_cd) --mikel 12.12.2016; SR 5874 - optimization                                                              
               AND (   (    p_tran_post = 1
                        AND TRUNC (b.tran_date) BETWEEN TO_DATE (p_from_dt,
                                                                 'MM/DD/YYYY'
                                                                )
                                                    AND TO_DATE (p_to_dt,
                                                                 'MM/DD/YYYY'
                                                                )
                       )
                    OR (    p_tran_post = 2
                        AND TRUNC (b.posting_date) BETWEEN TO_DATE
                                                                 (p_from_dt,
                                                                  'MM/DD/YYYY'
                                                                 )
                                                       AND TO_DATE
                                                                 (p_to_dt,
                                                                  'MM/DD/YYYY'
                                                                 )
                       )
                   )
               AND b.tran_flag <> 'D'
               --AND b.tran_flag <> 'CP' --mikel 12.12.2016;
               AND b.tran_class NOT IN ('CP', 'CPR') --mikel 12.12.2016; SR 5874 - excluded transactions that are processed from cancelled policies module (GIACS412)
               AND NOT EXISTS (
                      SELECT c.gacc_tran_id
                        FROM giac_reversals c, giac_acctrans d
                       WHERE c.reversing_tran_id = d.tran_id
                         AND d.tran_flag <> 'D'
                         AND d.intm_type = NVL (p_intm_type, d.intm_type)
                         AND c.gacc_tran_id = a.gacc_tran_id)
          GROUP BY d.intm_type, a.intm_no, d.intm_name, a.iss_cd
          ORDER BY 1, 3)
      LOOP
         v_is_empty := 'N';
         v_list.iss_cd := i.iss_cd;
         v_list.intm_type := i.intm_type;
         v_list.intm_no := i.intm_no;
         v_list.intm_name := i.intm_name;
         v_list.comm := i.comm;
         v_list.wtax := i.wtax;
         v_list.input_vat := i.input_vat;

         BEGIN
            SELECT iss_name
              INTO v_list.iss_name
              FROM giis_issource
             WHERE iss_cd = i.iss_cd;
         END;

         BEGIN
            v_list.net_amt := i.comm - i.wtax + i.input_vat;
         END;

         BEGIN
            SELECT intm_desc
              INTO v_list.intm_desc
              FROM giis_intm_type
             WHERE intm_type = i.intm_type;
         END;

         PIPE ROW (v_list);
      END LOOP;
      
      IF v_is_empty = 'Y' THEN
        PIPE ROW (v_list);
      END IF;

      RETURN;
   END get_details;
END giacr413b_pkg;
/