CREATE OR REPLACE PACKAGE BODY CPI.Nlto_Yearly AS 
  ...
  BEGIN
    ...
    LOOP
         ...
         v_line_cd     := c.line;
    END LOOP;
    --start insertion of extracted records to gixx_nlto_stat
	   DELETE FROM GIXX_NLTO_STAT
        WHERE user_id = /*USER*/giis_users_pkg.app_user; --replaced by pjsantos 12/15/2016, GENQA 5892
       COMMIT;
	DBMS_OUTPUT.PUT_LINE(v_subline_mc);
	DBMS_OUTPUT.PUT_LINE(v_subline_pc);
	DBMS_OUTPUT.PUT_LINE(v_subline_cv);
	DBMS_OUTPUT.PUT_LINE(v_line_cd);
	--iris b  (12.04.2002)
    ...
    */
	...
	IF SQL%NOTFOUND THEN
	   DBMS_OUTPUT.PUT_LINE('SQLNOTFOUND');
	END IF;
	IF SQL%FOUND THEN
	   DBMS_OUTPUT.PUT_LINE('SQLFOUND');
    ...
    SELECT get_coverage_desc(v_zone_type),
     ...
     WHERE 1=1
	   ...
	   AND c.subline_cd IN (v_subline_pc, v_subline_mc, v_subline_cv)
	       --filter gicl_claims by loss_date against specified year
	   AND TO_NUMBER(TO_CHAR(c.loss_date, 'YYYY')) = v_year
       GROUP BY d.peril_cd;
    IF SQL%FOUND THEN
