DROP PROCEDURE CPI.DELETION_PROCESS_GRP;

CREATE OR REPLACE PROCEDURE CPI.DELETION_PROCESS_GRP(
    ...
    p_tsi_amt           IN      GIPI_WITMPERL.tsi_amt%TYPE,     --  :b490_grp.tsi_amt;
    p_prem_amt          IN      GIPI_WITMPERL.prem_amt%TYPE,    --  :b490_grp.prem_amt;
    p_prem_rt           IN      GIPI_WITMPERL.prem_rt%TYPE,     --  :b490_grp.prem_rt;
    i_tsi_amt           IN OUT  GIPI_WITEM.tsi_amt%TYPE,        --  :b480_grp.tsi_amt;
    i_prem_amt          IN OUT  GIPI_WITEM.prem_amt%TYPE,       --  :b480_grp.prem_amt;
    i_nbt_tsi_amt       IN OUT  GIPI_WITEM.ann_tsi_amt%TYPE,    --  :b480_grp.nbt_tsi_amt;
    i_nbt_prem_amt      IN OUT  GIPI_WITEM.ann_prem_amt%TYPE,   --  :b480_grp.nbt_prem_amt;
...
IS
    ...
    **  Description    : DELETION_PROCESS_GRP program unit
	*/
    ...
    /*FOR A1 IN (
            SELECT   sum(NVL(disc_amt,0))  disc_amt
             ...
             WHERE   par_id    =  :b240.par_id
               AND   item_no   =  :b480_grp.item_no
               AND   line_cd   =  :b240.line_cd
               AND   peril_cd  =  :b490_grp.peril_cd) LOOP
