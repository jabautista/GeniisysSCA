DROP PROCEDURE CPI.LRATIO_EXTRACT_BY_INTERMEDIARY;

CREATE OR REPLACE PROCEDURE CPI.Lratio_Extract_By_Intermediary
...
BEGIN
  p_counter :=0;
    FOR curr_prem IN(
      SELECT intm_no, NVL(SUM(prem_amt),0) prem
    ...
    LOOP
      ...
      VALUES
        (p_session_id,      p_line_cd,         p_subline_cd,
		 ...
		 p_assd_no,         curr_prem.intm_no, curr_prem.prem,
		 USER);
    ...
    LOOP
	  ...
	  INSERT INTO gicl_loss_ratio_ext
        ...
        VALUES
          (p_session_id,      p_line_cd,         p_subline_cd,
           p_iss_cd,          p_peril_cd,        p_loss_date,
		   p_assd_no,         prev_prem.intm_no, prev_prem.prem,
		   USER);
       END IF;
    END LOOP;
  --current loss
  FOR curr_loss IN(
    SELECT intm_no, NVL(SUM(os_amt),0) loss
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,      p_line_cd,         p_subline_cd,
          p_iss_cd,          p_peril_cd,        p_loss_date,
		  p_assd_no,         curr_loss.intm_no, curr_loss.loss,
		  USER);
    END IF;
  END LOOP;
  --previous loss
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,      p_line_cd,         p_subline_cd,
          p_iss_cd,          p_peril_cd,        p_loss_date,
		  p_assd_no,         prev_loss.intm_no, prev_loss.loss,
		  USER);
    END IF;
  END LOOP;
  --losses paid
  FOR losses IN(
    SELECT intm_no, NVL(SUM(loss_paid),0) loss_paid
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,   p_line_cd,         p_subline_cd,
          p_iss_cd,       p_peril_cd,        p_loss_date,
		  p_assd_no,      losses.intm_no,    losses.loss_paid,
		  USER);
    END IF;
  END LOOP;
  --current recovery
  FOR curr_rec IN(
    SELECT intm_no, NVL(SUM(recovered_amt),0) recovery
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,     p_line_cd,          p_subline_cd,
          p_iss_cd,         p_peril_cd,         p_loss_date,
		  p_assd_no,        curr_rec.intm_no,   -curr_rec.recovery,
		  USER);
    END IF;
  END LOOP;
  --current recovery
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,     p_line_cd,          p_subline_cd,
          p_iss_cd,         p_peril_cd,         p_loss_date,
		  p_assd_no,        prev_rec.intm_no,   -prev_rec.recovery,
		  USER);
    END IF;
  END LOOP;
