CREATE OR REPLACE PROCEDURE CPI.RECAP_UPR_CURR_YEAR_SUMM AS
   TYPE tab_pol_id              IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.policy_id%TYPE;
   TYPE tab_item_no             IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.item_no%TYPE;
   ...
   TYPE tab_subln_tpcd          IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.subline_type_cd%TYPE;
   TYPE tab_premium_amt         IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.premium_amt%TYPE;
   TYPE tab_comm_amt            IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.commission_amt%TYPE;
   ...
   TYPE tab_trty_prem           IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.treaty_prem%TYPE;
   TYPE tab_trty_comm           IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.treaty_comm%TYPE;
   TYPE tab_trty_tsi            IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.treaty_tsi%TYPE;
   ...
   TYPE tab_locfor_sw           IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.local_foreign_sw%TYPE;
   TYPE tab_inw_ri_comm         IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.inw_ri_comm%TYPE;
   TYPE tab_cedant              IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.cedant%TYPE;
   
   --mikel 03.29.2016
   TYPE tab_eff_date            IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.eff_date%TYPE;
   TYPE tab_exp_date            IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.exp_date%TYPE;
   TYPE tab_def_prem_amt        IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.def_prem_amt%TYPE;
   ...
   FUNCTION get_bond_class(
      ...
      p_subline_cd GIAC_RECAP_CURR_DTL_EXT.subline_cd%TYPE) RETURN CHAR AS
      v_exs BOOLEAN := FALSE;
   ...
   PROCEDURE get_rowthings(
      p_line_cd IN GIAC_RECAP_CURR_DTL_EXT.line_cd%TYPE,
      p_peril_cd IN GIAC_RECAP_CURR_DTL_EXT.peril_cd%TYPE,
      p_tariff_cd IN GIAC_RECAP_CURR_DTL_EXT.tariff_cd%TYPE,
   ...
   PROCEDURE get_rowthings_others(
      ...
      p_subline_cd IN GIAC_RECAP_CURR_DTL_EXT.subline_cd%TYPE,
      p_rowno      OUT GIAC_RECAP_SUMM_EXT.rowno%TYPE,
      p_rowtitle   OUT GIAC_RECAP_SUMM_EXT.rowtitle%TYPE) IS
   BEGIN
   ...
   PROCEDURE get_rowthings_mc(  
      ...
      p_peril_cd IN GIAC_RECAP_CURR_DTL_EXT.peril_cd%TYPE,
      p_subline_cd IN GIAC_RECAP_CURR_DTL_EXT.subline_cd%TYPE,
      p_subline_lto IN GIAC_RECAP_CURR_DTL_EXT.subline_type_cd%TYPE,
      p_subline_tpcd IN GIAC_RECAP_CURR_DTL_EXT.subline_type_cd%TYPE,
      p_rowno   OUT GIAC_RECAP_SUMM_EXT.rowno%TYPE,
      p_rowtitle  OUT GIAC_RECAP_SUMM_EXT.rowtitle%TYPE) IS
      v_exs BOOLEAN := FALSE;
        ...
        FROM GIAC_RECAP_ROW;
            ...
            FOR grot IN (SELECT LINE_CD, ROWTITLE, ROWNUM 
                    ...
                    WHERE 1=1
                      ...
                      AND a.subline_cd IN (SELECT subline_cd rowtitle FROM GIAC_RECAP_CURR_DTL_EXT x
...
BEGIN
   --Truncate the table
   EXECUTE IMMEDIATE 'TRUNCATE TABLE GIAC_RECAP_CURR_SUMM_EXT';
   ...
   IF SQL%FOUND THEN
      ...
      LOOP
         ...
         INSERT INTO GIAC_RECAP_CURR_SUMM_EXT(
            rowno, rowtitle,
