CREATE OR REPLACE PROCEDURE CPI.RECAP_UPR_CURR_YEAR_DTL (p_fm_date    DATE,
                                                         p_to_date    DATE)
AS
   TYPE tab_iss_cd IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.iss_cd%TYPE;

   TYPE tab_line_cd IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.line_cd%TYPE;

   TYPE tab_subline_cd IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.subline_cd%TYPE;

   TYPE tab_policy_id IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.policy_id%TYPE;

   TYPE tab_peril_cd IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.peril_cd%TYPE;

   TYPE tab_tariff_cd IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.tariff_cd%TYPE;

   TYPE tab_subline_type_cd
      IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.subline_type_cd%TYPE;

   TYPE tab_bond_class_subline
      IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.bond_class_subline%TYPE;

   TYPE tab_premium_amt IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.premium_amt%TYPE;

   TYPE tab_commission_amt
      IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.commission_amt%TYPE;

   TYPE tab_tsi_amt IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.tsi_amt%TYPE;

   TYPE tab_ri_cd IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.ri_cd%TYPE;

   TYPE tab_local_foreign_sw
      IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.local_foreign_sw%TYPE;

   TYPE tab_treaty_prem IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.treaty_prem%TYPE;

   TYPE tab_treaty_tsi IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.treaty_tsi%TYPE;

   TYPE tab_treaty_comm IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.treaty_comm%TYPE;

   TYPE tab_facul_prem IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.facul_prem%TYPE;

   TYPE tab_facul_tsi IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.facul_tsi%TYPE;

   TYPE tab_facul_comm IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.facul_comm%TYPE;

   TYPE tab_inw_ri_comm IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.inw_ri_comm%TYPE;

   TYPE tab_item_no IS TABLE OF gipi_fireitem.item_no%TYPE;

   TYPE tab_currency_rt IS TABLE OF gipi_item.currency_rt%TYPE;

   TYPE tab_date_tag IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.date_tag%TYPE;
   ...
   FUNCTION get_cur_rt (p_policy_id GIAC_RECAP_CURR_DTL_EXT.policy_id%TYPE)
      RETURN NUMBER
   AS
   ...
   FUNCTION get_bond_class (
      p_line_cd       GIAC_RECAP_CURR_DTL_EXT.line_cd%TYPE,
      p_subline_cd    GIAC_RECAP_CURR_DTL_EXT.subline_cd%TYPE)
      RETURN CHAR
   AS
      v_exs        BOOLEAN := FALSE;
   ...
   IS
      ...
      (
         policy_id      GIAC_RECAP_CURR_DTL_EXT.policy_id%TYPE,
         peril_cd       GIAC_RECAP_CURR_DTL_EXT.peril_cd%TYPE,
         ri_cd          GIAC_RECAP_CURR_DTL_EXT.ri_cd%TYPE,
         eff_date       GIAC_RECAP_CURR_DTL_EXT.eff_date%TYPE,
         exp_date       GIAC_RECAP_CURR_DTL_EXT.exp_date%TYPE,
         def_prem_amt   GIAC_RECAP_CURR_DTL_EXT.def_prem_amt%TYPE
      );
      ...
      TYPE policy_id_tab IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.policy_id%TYPE;

      TYPE peril_cd_tab IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.peril_cd%TYPE;

      TYPE ri_cd_tab IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.ri_cd%TYPE;

      TYPE eff_date_tab IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.eff_date%TYPE;

      TYPE exp_date_tab IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.exp_date%TYPE;

      TYPE def_prem_amt_tab
         IS TABLE OF GIAC_RECAP_CURR_DTL_EXT.def_prem_amt%TYPE;
   ...
   BEGIN
      SELECT a.policy_id,
        ...
        BULK COLLECT INTO vv_policy_id,
             ...
             vv_exp_date,
             vv_def_prem_amt
        FROM GIAC_RECAP_CURR_DTL_EXT a, gipi_polbasic b, giis_line c, gipi_invoice d  --added gipi_invoice by albert 01.25.2017 (GENQA SR 5848)
...
BEGIN
   EXECUTE IMMEDIATE 'TRUNCATE TABLE GIAC_RECAP_CURR_DTL_EXT';
   ...
   THEN
      FORALL i IN vv_iss_cd.FIRST .. vv_iss_cd.LAST
         INSERT INTO GIAC_RECAP_CURR_DTL_EXT (iss_cd,
                                              line_cd,
                                              subline_cd,
                                              policy_id,
                                              peril_cd,
