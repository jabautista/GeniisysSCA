DROP PROCEDURE CPI.EXT_LOSSES;

CREATE OR REPLACE PROCEDURE CPI.Ext_Losses IS
...
FOR os IN(
  SELECT SUM(NVL(f.res_amt,0)) res_amt,
       	 ...
       	 SUM((NVL(f.reserve_expenses,0) - NVL(g.exp_paid,0)) +
   ...
   WHERE 1=1
	 ...
	 AND b.claim_id = h.claim_id(+)
	 AND b.item_no = h.item_no(+)
...
LOOP
  ...
  IF TRUNC(NVL(os.claim_close_dt, SYSDATE+1)) < TRUNC(SYSDATE) THEN
     ...
     END IF;
  ELSE
     IF os.clm_stat_cd IN ('DN', 'CC', 'WD') THEN
	    v_can_loss	   := os.loss_amt;
	 ELSE
