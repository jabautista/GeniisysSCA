DROP PROCEDURE CPI.GIPIS031_CREATE_CASUALTY_ITEM;

CREATE OR REPLACE PROCEDURE CPI.GIPIS031_CREATE_CASUALTY_ITEM (
...
AS
	...
	**  Description 	: This procedure is used for inserting records to GIPI_CASUALTY_ITEM table
	*/
...
BEGIN
            ...
            FOR B1 IN B(D1.policy_id) LOOP
                IF v_eff_date  IS NULL THEN
                    INSERT INTO gipi_wcasualty_item (
                        par_id,                    item_no,            section_line_cd,    section_subline_cd,
                        section_or_hazard_cd,    property_no_type,    capacity_cd,        property_no,
                        location,                conveyance_info,    limit_of_liability,    interest_on_premises,
                        section_or_hazard_info)
                    VALUES (
                        p_par_id,                    p_item_no,            b1.section_line_cd,            b1.section_subline_cd,
                        b1.section_or_hazard_cd,    b1.property_no_type,    b1.capacity_cd,            b1.property_no,
                        b1.location,                b1.conveyance_info,        b1.limit_of_liability,    b1.interest_on_premises,
                        b1.section_or_hazard_info);
                END IF;
    ...
    ELSE
        ...
        END LOOP;  
    END IF;
END GIPIS031_CREATE_CASUALTY_ITEM;
/
