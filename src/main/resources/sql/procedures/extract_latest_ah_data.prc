DROP PROCEDURE CPI.EXTRACT_LATEST_AH_DATA;

CREATE OR REPLACE PROCEDURE CPI.Extract_Latest_Ah_Data (p_expiry_date      gicl_claims.expiry_date%TYPE,
...
BEGIN
  FOR get_item_dtl IN (SELECT c.endt_seq_no endt_seq_no, b.item_title item_title,
                        ...
                        WHERE c.line_cd       = p_line_cd
                          ...
                          AND TRUNC(DECODE(TRUNC(NVL(b.from_date,c.eff_date)),
                              TRUNC(c.incept_date), NVL(b.from_date,p_incept_date),                                            NVL(b.from_date,c.eff_date))) <= p_loss_date
                          AND TRUNC(DECODE(NVL(b.TO_DATE,NVL(c.endt_expiry_date,                                               c.expiry_date)), c.expiry_date,NVL(b.TO_DATE,                                                     p_expiry_date), NVL(b.TO_DATE,c.endt_expiry_date)))
                              >= p_loss_date
  ...
  FOR get_item_dtl2 IN (SELECT b.item_title item_title,
                         ...
                         WHERE c.line_cd       = p_line_cd
                           ...
                           AND TRUNC(DECODE(TRUNC(NVL(b.from_date,c.eff_date)),                                                 TRUNC(c.incept_date), NVL(b.from_date,p_incept_date),                                            NVL(b.from_date,c.eff_date))) <= p_loss_date
