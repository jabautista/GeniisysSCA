DROP PROCEDURE CPI.COPY_POL_PACK_WPOLBAS;

CREATE OR REPLACE PROCEDURE CPI.copy_pol_pack_wpolbas(
    ...
    p_pack_sw               OUT gipi_pack_polbasic.pack_pol_flag%TYPE,
...
BEGIN
  ...
  **  Description  : copy_pol_pack_wpolbas program unit 
  */
  BEGIN
    SELECT pack_polbasic_policy_id_s.NEXTVAL
      INTO p_new_policy_id 
      FROM DUAL;
    p_new_pack_policy_id   := p_new_policy_id;
  EXCEPTION
    WHEN NO_DATA_FOUND THEN
     p_msg := 'Cannot generate new PACK POLICY ID.';
  END;
  FOR pol IN 
  ...
  LOOP 
    ...
    FOR  MAX_REN IN (SELECT renew_no, 
                            DECODE(pack_policy_id, p_old_pol_id, manual_renew_no, 0) manual_renew_no
                       FROM gipi_pack_polbasic
   ...
   ELSE
    ...
    END LOOP;
    -- to here             
    insert_into_pack_parlist(p_policy_id, v_assd_no, v_par_yy, 
                             p_new_par_id, p_new_pack_par_id, p_msg,
                             p_proc_renew_flag, p_user, v_same_polno_sw); --nieko 07212016 SR 22730, KB 3722, same_polno added
    --CLEAR_MESSAGE;
    ...
    INSERT INTO gipi_pack_polbasic (
                ...
                fleet_print_tag,       endt_expiry_tag,       comp_sw,            pack_policy_id ,
       ...
       VALUES ( p_new_par_id,  pol.line_cd,           pol.subline_cd,     pol.iss_cd,           
                ...
                pol.prorate_flag,      pol.auto_renew_flag,   pol.pack_pol_flag,  pol.prov_prem_tag,   
