DROP PROCEDURE CPI.PROCESS_EXPIRING_POLICIES_TEST;

CREATE OR REPLACE PROCEDURE CPI.process_expiring_policies_test
...
--      IF v_tsi != 0 THEN
	  ...
	  */
         ...
         LOOP
		   ...
		   -- compute depreciated tax_amt = dep_prem * tax_rate
		   v_dep_tax_amt := v_total_dep_prm*(a9.rate/100);
DBMS_OUTPUT.put_line (a2.policy_id||'POLICY ID'||' '||a9.line_cd||'LINE'||' '||a9.iss_cd||'ISSUE'||' '||  a9.tax_cd||'TAX'
                       ||' '||a9.tax_id||'TAX ID'||' '||a9.tax_desc||'TAX_DESC'||' '||v_dep_tax_amt||'TAX_AMT');
		   INSERT INTO giex_old_group_tax
                       (policy_id,    line_cd,     iss_cd,     tax_cd,
                       tax_id,       tax_desc,    tax_amt)
           VALUES (a2.policy_id, a9.line_cd,  a9.iss_cd,  a9.tax_cd,
