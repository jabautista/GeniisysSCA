DROP PROCEDURE CPI.LRATIO_EXTRACT_PREMIUM;

CREATE OR REPLACE PROCEDURE CPI.Lratio_Extract_Premium
  ...
  **     this is used if date parameter is policy's incept date, effectivity date or claim file date
  */
  -- hardy 011404
...
BEGIN --main
  ...
  BEGIN -- extract current year premiums written
    ...
    SELECT d.policy_id,   d.line_cd,
           ...
           (c.prem_amt * b.currency_rt) prem_amt,
    ...
    BULK COLLECT
     ...
     WHERE d.line_cd = NVL(p_line_cd, d.line_cd)
       AND check_user_per_iss_cd2(d.line_cd,d.iss_cd,'GICLS204',USER) = 1 --angelo092407
    ...
    IF SQL%FOUND THEN
       ...
       FORALL i IN vv_policy_id.FIRST..vv_policy_id.LAST
         ...
         VALUES
           (p_session_id,	vv_assd_no(i),    vv_policy_id(i),
		    vv_line_cd(i),  vv_subline_cd(i), vv_iss_cd(i),
			vv_peril_cd(i), vv_prem(i),     USER, TO_DATE(vv_date24(i),'MM-DD-YYYY'));
    END IF;
