DROP PROCEDURE CPI.LOSS_RATIO_EXT_BY_LINE_INTM2;

CREATE OR REPLACE PROCEDURE CPI.Loss_Ratio_Ext_By_Line_Intm2 (p_line_cd            giis_line.line_cd%TYPE,
...
BEGIN
  ...
  LOOP
    ...
    FOR CURR_PREM1 IN (
      SELECT NVL(SUM(c.premium_amt),0) prem_amt
        FROM gipi_polbasic d,
           gipi_comm_inv_peril c
       WHERE d.policy_id = c.policy_id
         AND d.line_cd = curr_prem.line_cd
         AND d.subline_cd = NVL(p_subline_cd, d.subline_cd)
         AND d.iss_cd = NVL(p_iss_cd, d.iss_cd)
         AND TRUNC(d.acct_ent_date) >= v_curr1_date
         AND TRUNC(d.acct_ent_date) <= v_curr2_date
    ...
    END LOOP;
    FOR PREV_PREM2 IN (
      SELECT NVL(SUM(NVL(c.premium_amt,0)),0) prem_amt
        FROM gipi_polbasic d,
             gipi_comm_inv_peril c
       WHERE d.policy_id = c.policy_id
         AND d.line_cd = curr_prem.line_cd
    ...
    FOR PAID_LOSS IN
      (SELECT SUM((NVL(losses_paid,0)+ NVL(expenses_paid,0)) * (c.shr_intm_pct/100)) loss_paid
        ...
        WHERE 1 = 1
         AND b.line_cd           = curr_prem.line_cd
    ...
    VALUES (p_session_id,       curr_prem.line_cd,  p_subline_cd,
