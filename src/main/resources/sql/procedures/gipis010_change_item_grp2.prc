DROP PROCEDURE CPI.GIPIS010_CHANGE_ITEM_GRP2;

CREATE OR REPLACE PROCEDURE CPI.GIPIS010_CHANGE_ITEM_GRP2 (
   p_par_id                IN gipi_parlist.par_id%TYPE,
   p_pack_pol_flag        IN gipi_wpolbas.pack_pol_flag%TYPE,
   p_currency_cd        IN gipi_witem.currency_cd%TYPE,  
   p_item_grp            OUT gipi_witem.item_grp%TYPE)
IS
   /*
   **  Created by    : Mark JM
   ...
   **                : Returns item_grp for record to be inserted
   */
...
BEGIN
   ...
   IF p_pack_pol_flag = 'Y' THEN
        ...
        LOOP
            IF p_item_grp IS NULL THEN
                IF p_currency_cd = c1.currency_cd THEN                    
                    p_item_grp := v_item_grp;
    ...
    p_item_grp := NVL(p_item_grp, v_item_grp);
END GIPIS010_CHANGE_ITEM_GRP2;
/
