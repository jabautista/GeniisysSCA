DROP PROCEDURE CPI.PROCESS_EXPIRING_POLICIES_;

CREATE OR REPLACE PROCEDURE CPI.process_expiring_policies_
    ...
    IF v_policy_no IS NULL OR v_policy_no = a1.policy_no THEN
       ...
       FOR ABCDE IN (
	     SELECT MAX(ENDT_SEQ_NO) ENDT_SEQ_NO
		   FROM GIPI_POLBASIC A
		  WHERE A1.line_cd    = a.line_cd
	        AND A1.subline_cd = a.subline_cd
	        AND A1.iss_cd     = a.iss_cd
	        AND A1.issue_yy   = a.issue_yy
	        AND A1.pol_seq_no = a.pol_seq_no
	        AND A1.renew_no   = a.renew_no)
	   LOOP
		 IF a1.endt_seq_no = ABCDE.endt_seq_no THEN
		 ...
		 FOR a9 IN (
		   SELECT --SUM(d.tax_amt * c.currency_rt) tax_amt2,
             ...
             WHERE b.line_cd         = a2.line_cd
               ...
               AND b.pol_flag        IN('1','2','3')
               AND NVL(b.reg_policy_sw,'Y') = DECODE(NVL(inc_special_sw,'N'),'N','Y',NVL(b.reg_policy_sw,'Y'))
			 GROUP BY d.line_cd, d.tax_cd, d.tax_id, d.iss_cd,e.tax_desc,e.rate  --added by A.R.C. 03.07.2005
             --GROUP BY d.line_cd, d.tax_cd, d.tax_id, d.iss_cd,e.tax_desc,d.rate
			 )
         LOOP
		   -- rollie 09feb2005
