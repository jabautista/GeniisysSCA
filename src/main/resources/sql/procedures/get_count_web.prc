CREATE OR REPLACE PROCEDURE CPI.Get_Count_Web
AS
   ...
   */
   ...
   --mikel 04.01.2016; 
    FUNCTION include_policy_count (
       p_policy_id    giac_recap_dtl_ext.policy_id%TYPE,
       p_peril_cd     giac_recap_dtl_ext.peril_cd%TYPE)
       RETURN BOOLEAN
    AS
       v_include   BOOLEAN := FALSE;
    BEGIN
       FOR rec
          IN (SELECT MAX (x.prem_amt) max_prem_amt --incase there are two or more basic perils with the same premium amount
                FROM gipi_itmperil x, giis_peril y
       ...
       LOOP
          FOR rec2
             IN (  SELECT 'X'
                     FROM gipi_polbasic a, gipi_itmperil b, giis_peril c
                    WHERE     a.policy_id = b.policy_id
                          AND b.line_cd = c.line_cd
                          AND b.peril_cd = c.peril_cd
                          AND b.prem_amt = rec.max_prem_amt
    ...
    BEGIN
       --rowno 23
       FOR rec IN (SELECT   line_cd, subline_cd, peril_cd, subline_type_cd, policy_id --mikel 04.01.2016; added policy_id
                       FROM giac_recap_dtl_ext b
                      WHERE b.subline_cd = v_subline_lto
                        AND b.line_cd = 'MC'
                        AND iss_cd != 'RI' --mikel 04.07.2016; RI policies are not included
                   ...
                   GROUP BY b.line_cd, b.subline_cd, b.iss_cd,
                            ...
                            , policy_id --mikel 04.01.2016; added policy_id
                     HAVING SUM (DECODE (b.date_tag,'A', 1,'C', -10000,'S', 0,0)) > 0)
       LOOP
          v_recap_subline_type := NULL;
          
          v_rowtitle := null; --mikel 03.25.2015
          ...
          THEN
             raise_application_error (-20130,'Geniisys Exception#E#No set-up for recap_subline_type in giis_mc_subline_type.');
          ...
          IF include_policy_count(rec.policy_id, rec.peril_cd) THEN --mikel 04.01.2016;
       ...
       LOOP
          ...
          THEN
             raise_application_error (-20130,'Geniisys Exception#E#No set-up for recap_line_cd in giis_subline.');
...
--                  VALUES (v_rowno, v_rowtitle, 1);
       ...
       LOOP
         ...
         SELECT rv_meaning
          ...
          WHERE rv_low_value = v_recap_line_cd;    
...
BEGIN
     ...
     WHERE 1=1
       ...
       AND EXISTS(SELECT 'X'
                   ...
                   WHERE d.rowno    = vv_rowno(i)
                     --AND d.peril_cd = c.peril_cd --mikel 04.07.2016; RI policies are not included
  ...
  LOOP
    SELECT COUNT(COUNT(*))
     ...
     AND iss_cd != 'RI' --mikel 04.07.2016; RI policies are not included
       AND c.line_cd = vv_line_cd(i)
       AND c.premium_amt IS NOT NULL                        --nieko 03252014 include only policies with premium amount
       AND 1 = 2 --mikel 04.01.2016; not included in policy count
       AND EXISTS(SELECT 'X'
                    FROM GIAC_RECAP_ROW_DTL d
                   WHERE d.rowno    = vv_rowno(i)
  ...
  --mikel 02.23.2015; for rowno 31 and 32 (MW and MI)
  FOR i IN 33..34
  LOOP
        SELECT   COUNT (COUNT (*))
          INTO vv_total (i)
          FROM giac_recap_dtl_ext c, giis_subline e
         WHERE 1 = 1
           AND iss_cd != 'RI' --mikel 04.07.2016; RI policies are not included
           AND c.line_cd = e.line_cd
           AND c.subline_cd = e.subline_cd
           AND e.recap_line_cd = vv_line_cd(i)
           AND c.premium_amt IS NOT NULL
      GROUP BY c.line_cd, c.subline_cd, c.iss_cd, c.issue_yy, c.pol_seq_no,c.renew_no
        HAVING SUM (DECODE (c.date_tag, 'A', 1, 'C', -10000, 'S', 0, 0)) > 0;
  END LOOP;
...
--  END LOOP;*/ --commented out count breakdown of CTPL non-LTO per subline, albert 12042013
  ...
  IF v_max_rowno IS NULL THEN
     RAISE_APPLICATION_ERROR(-20099,'Geniisys Exception#E#NO RECORDS IN GIAC_RECAP_ROW.');
  END IF;
  ...
  FOR i IN (SELECT 1, line_cd, subline_cd, ROWNUM -- mildred 04.15.2013
             ...
             WHERE a.line_cd = b.line_cd
               ...
               AND NVL (a.recap_line_cd, 'CPI') NOT IN ('MW', 'MI') --mikel 03.25.2015; migrant workers (MW) and micro insurance (MI)                      
             UNION
...
--                      AND d.bond_class_no = c.bond_class_subline)
  ...
  LOOP
        SELECT   COUNT (COUNT (*))
          INTO vv_total_losspd (i)
          FROM giac_recap_losspd_ext c, giis_subline e
         WHERE 1 = 1
           AND iss_cd != 'RI' --mikel 04.07.2016; RI policies are not included
           AND c.line_cd = e.line_cd
           AND c.subline_cd = e.subline_cd
...
--                      AND d.bond_class_no = c.bond_class_subline)
  ...
  LOOP
        SELECT   COUNT (COUNT (*))
          INTO vv_total_osloss (i)
          FROM giac_recap_osloss_ext c, giis_subline e
         WHERE 1 = 1
           AND iss_cd != 'RI' --mikel 04.07.2016; RI policies are not included
           AND c.line_cd = e.line_cd
           AND c.subline_cd = e.subline_cd
