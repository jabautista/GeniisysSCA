DROP PROCEDURE CPI.LOSS_RATIO_EXT_BY_INTM2;

CREATE OR REPLACE PROCEDURE CPI.Loss_Ratio_Ext_By_Intm2 (p_line_cd            giis_line.line_cd%TYPE,
...
BEGIN
  ...
  LOOP
    v_curr_prem := 0;
    FOR CURR_PREM1 IN(
      SELECT NVL(SUM(NVL(c.premium_amt,0)),0) prem
       FROM gipi_polbasic d,
            gipi_comm_inv_peril c
      WHERE d.policy_id = c.policy_id
        AND d.line_cd = NVL(p_line_cd, d.line_cd)
    ...
    LOOP
      v_curr_prem := curr_prem1.prem;
    END LOOP;
    FOR CURR_PREM2 IN(
      SELECT NVL(SUM(c.premium_amt),0) prem
       FROM gipi_polbasic d,
            gipi_comm_inv_peril c
      WHERE d.policy_id = c.policy_id
        AND d.line_cd = NVL(p_line_cd, d.line_cd)
        ...
        AND c.intrmdry_intm_no   = curr_prem.intm_no)
    LOOP
      v_curr_prem := v_curr_prem - curr_prem2.prem;
    END LOOP;
    ...
    FOR PREV_PREM2 IN (
      SELECT NVL(SUM(c.premium_amt),0) prem
       ...
       WHERE d.policy_id = c.policy_id
         ...
         AND c.intrmdry_intm_no   = curr_prem.intm_no)
    LOOP
       v_prev_prem  := v_prev_prem - prev_prem2.prem;
    END LOOP;
         ...
         FROM GICL_CLM_RES_HIST a,
              (SELECT b1.claim_id, b1.clm_res_hist_id,
                 ...
                 WHERE tran_id IS NULL
                   ...
                   AND b2.item_no  = b1.item_no
                   AND b2.peril_cd = b1.peril_cd
                   AND TO_CHAR(NVL(b2.close_date, v_curr2_date),'YYYY')
                       > v_prev_year) b,
                   (SELECT claim_id, item_no, peril_cd,
                           SUM(losses_paid) losses_paid,
                           SUM(expenses_paid) exp_paid
