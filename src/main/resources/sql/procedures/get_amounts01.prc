DROP PROCEDURE CPI.GET_AMOUNTS01;

CREATE OR REPLACE PROCEDURE CPI.GET_AMOUNTS01 (
...
AS
    ...
    **	==========	===============	============================
    **	01.27.2012	mark jm			For events that will delete existing records amounts must be reset and 
    ...
    */	
    ...
    --temp    
	v_gipi_wpolbas gipi_wpolbas%ROWTYPE;
...
BEGIN
    ...
    v_gipi_wpolbas.prem_amt := 0;
    v_gipi_wpolbas.tsi_amt := 0;
	v_expiry_date := extract_expiry2(p_line_cd, v_gipi_wpolbas.subline_cd, v_gipi_wpolbas.iss_cd, v_gipi_wpolbas.issue_yy, v_gipi_wpolbas.pol_seq_no, v_gipi_wpolbas.renew_no); -- marco 09.05.2012
    ...
    FOR SW IN(
        SELECT '1'
         ...
         WHERE B.line_cd = p_line_cd
           AND B.subline_cd = v_gipi_wpolbas.subline_cd
           AND B.iss_cd = v_gipi_wpolbas.iss_cd
           AND B.issue_yy = v_gipi_wpolbas.issue_yy
           AND B.pol_seq_no = v_gipi_wpolbas.pol_seq_no
           AND B.renew_no = v_gipi_wpolbas.renew_no
    ...
    IF v_expired_sw = 'N' THEN
        ...
        FOR AMT IN (SELECT b250.ann_tsi_amt,      b250.ann_prem_amt
                  ...
                  WHERE b250.line_cd = p_line_cd
                    AND b250.subline_cd = v_gipi_wpolbas.subline_cd
                    AND b250.iss_cd = v_gipi_wpolbas.iss_cd
                    AND b250.issue_yy = v_gipi_wpolbas.issue_yy
                    AND b250.pol_seq_no = v_gipi_wpolbas.pol_seq_no
                    AND b250.renew_no = v_gipi_wpolbas.renew_no
        ...
        LOOP            
            v_gipi_wpolbas.ann_tsi_amt := amt.ann_tsi_amt; 
            v_gipi_wpolbas.ann_prem_amt := amt.ann_prem_amt; 
            v_amt_sw := 'Y';
            EXIT;
        END LOOP;
    ...
    ELSE     
        ...
        v_gipi_wpolbas.ann_tsi_amt  := v_ann_tsi2;
        v_gipi_wpolbas.ann_prem_amt := v_ann_prem2;
