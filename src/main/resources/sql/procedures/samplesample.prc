DROP PROCEDURE CPI.SAMPLESAMPLE;

CREATE OR REPLACE PROCEDURE CPI.samplesample(p_from_date  DATE,
                                         p_to_date    DATE) AS
 ...
 TYPE tab_cred_branch             IS TABLE OF gipi_polbasic.cred_branch%TYPE;
  ...
  COMMIT;
    SELECT
          ...
          DECODE(b.acct_neg_date,p_to_date,
               (DECODE(b.share_type, '1',NVL(a.dist_tsi,0))*(-1)),
               (DECODE(b.share_type, '1',NVL(a.dist_tsi,0)))) AS nr_dist_tsi,
          DECODE(b.acct_neg_date,p_to_date,
               (DECODE(b.share_type, '1',NVL(a.dist_prem,0))*(-1)),
               (DECODE(b.share_type,'1',NVL(a.dist_prem,0)))) AS nr_dist_prem,
          DECODE(b.share_type, '1',a.dist_spct ) AS nr_dist_spct,
          DECODE(b.acct_neg_date,p_to_date,
               (DECODE(b.share_type, '2',NVL(a.dist_tsi,0))*(-1)),
               (DECODE(b.share_type, '2', NVL(a.dist_tsi,0)))) AS tr_dist_tsi,
          DECODE(b.acct_neg_date,p_to_date,
               (DECODE(b.share_type, '2',NVL(a.dist_prem,0))*(-1)),
               (DECODE(b.share_type, '2',NVL(a.dist_prem,0)))) AS tr_dist_prem,
          DECODE(b.share_type, '2',a.dist_spct ) AS tr_dist_spct,
          DECODE(b.acct_neg_date,p_to_date,
               (DECODE(b.share_type, '3',NVL(a.dist_tsi,0))*(-1)),
               (DECODE(b.share_type, '3',NVL(a.dist_tsi,0)))) AS fa_dist_tsi,
          DECODE(b.acct_neg_date,p_to_date,
               (DECODE(b.share_type, '3',NVL(a.dist_prem,0))*(-1)),
               (DECODE(b.share_type, '3',NVL(a.dist_prem,0)))) AS fa_dist_prem,
 ...
 IF SQL%FOUND THEN
      ...
      forall i IN vv_policy_id.first..vv_policy_id.last
       INSERT INTO giac_prodrep_peril_ext
            ...
            user_id                ,    last_update             ,   from_date1               ,    
            to_date1               ,    cred_branch)
