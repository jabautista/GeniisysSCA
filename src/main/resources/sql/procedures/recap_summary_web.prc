CREATE OR REPLACE PROCEDURE CPI.Recap_Summary_Web AS
   ...
   --mikel 03.29.2016
   TYPE tab_eff_date            IS TABLE OF GIAC_RECAP_DTL_EXT.eff_date%TYPE;
   TYPE tab_exp_date            IS TABLE OF GIAC_RECAP_DTL_EXT.exp_date%TYPE;
   TYPE tab_def_prem_amt        IS TABLE OF GIAC_RECAP_DTL_EXT.def_prem_amt%TYPE;
   ...
   v_line_ah  GIIS_LINE.line_cd%TYPE;--ramon 04/27/2010
   ...
   v_max_policy_id GIAC_RECAP_SUMM_EXT.policy_id%TYPE; --mikel 04.25.2013

   v_addl_row VARCHAR2(1) := 'N'; --mikel 02.23.2015
   ...
   FUNCTION get_tsi_tag(
      p_line_cd  GIIS_PERIL.line_cd%TYPE,                   --added parameter line_cd by FJ03242014
      p_peril_cd GIIS_PERIL.peril_cd%TYPE) RETURN CHAR IS
      v_exs BOOLEAN := FALSE;
      v_tag VARCHAR2(1);
   BEGIN
      FOR tsi IN (SELECT NVL(tsi_tag,'N') tag
                  ...
                  WHERE 1=1
                    AND line_cd = p_line_cd           --added by FJ 03242014
                    AND peril_cd = p_peril_cd)
      LOOP
         v_tag := tsi.tag;
         v_exs := TRUE;
   ...
   BEGIN
      ...
      END IF;*/
            ...
            FOR grot IN (SELECT LINE_CD, ROWTITLE, ROWNUM -- mildred
                    FROM
                    ...
                    WHERE 1=1
                      ...
                      /*AND a.subline_cd IN(SELECT DISTINCT subline_cd FROM GIAC_RECAP_DTL_EXT
                                           WHERE line_cd IN(Giisp.v('LINE_CODE_CA'),'AH',Giisp.v('LINE_CODE_OTHER')) -- add new parameter for line cd specific for clients
                                           )--by Ramon 04/27/2010*/ --comment out by mikel 04.29.2013;
                      --mikel 04.29.2013; select other rows
                      AND a.subline_cd IN (SELECT subline_cd rowtitle FROM giac_recap_dtl_ext x
                                            WHERE x.line_cd = b.line_cd
                                           MINUS
                                          SELECT rowtitle FROM GIAC_RECAP_ROW)
                      AND NVL (a.recap_line_cd, 'CPI') NOT IN ('MW', 'MI') --mikel 02.23.2015; migrant workers (MW) and micro insurance (MI)
   ...
   BEGIN
      ...
      FOR grot IN (SELECT line_cd, subline_cd rowtitle --mikel 04.26.2013; added ' '|| for indention
                  FROM GIIS_SUBLINE
...
BEGIN
   ...
   pop_recap_other_rows(v_line_fi,v_line_mn,v_line_mh,v_line_av,
                        v_line_su,v_line_mc,v_line_ac,v_line_en);--Ramon 04/27/2010
   ...
   SELECT policy_id, item_no,
          ...
          ,eff_date, exp_date, def_prem_amt --mikel 03.29.2016
     BULK COLLECT INTO
          ...
          ,vv_eff_date, vv_exp_date, vv_def_prem_amt --mikel 03.29.2016
   ...
   IF SQL%FOUND THEN
      ...
      LOOP
         ...
         v_tsi_tag := get_tsi_tag(vv_line_cd(rec),vv_peril_cd(rec));      --added line_cd as parameter by FJ03242014

         --added by mikel 02.23.2015
         v_addl_row := 'N';
         BEGIN
         ...
         EXCEPTION WHEN NO_DATA_FOUND THEN
            NULL;
         END;
         --end mikel 02.23.2015

             --Get the bond_class_no if the line_cd = 'SU'
             IF vv_line_cd(rec) = v_line_su THEN --'SU' THEN --albert 04.29.2016; use parameter value instead of hard coding
         ...
         --Get the rowno and the rowtitle
            ...
            IF v_addl_row = 'N' THEN
             IF vv_line_cd(rec) IN (v_line_fi,v_line_mn,v_line_mh,v_line_av,v_line_ac,v_line_en,v_line_su) THEN
                ...
                --added by ramon 04/27/2010, for 'SU'
         ...
         */
         ...
         INSERT INTO GIAC_RECAP_SUMM_EXT(
            ...
            ,eff_date, exp_date, curr_def_prem_amt) --mikel 03.29.2016
         VALUES(
            v_rowno, v_rowtitle,
            ...
            ,vv_eff_date(rec), vv_exp_date(rec), vv_def_prem_amt(rec)); --mikel 03.29.2016
      END LOOP;

      /* added by mikel 04.25.2013
      ** insert dummy rows for rowno 23
      */
      /*SELECT MAX(policy_id)
        INTO v_max_policy_id
        FROM giac_recap_summ_ext;

      FOR ins IN (SELECT rowno, rowtitle, line_cd
                 FROM giac_recap_row
                WHERE TRUNC (rowno) = 23)
       LOOP

          INSERT INTO giac_recap_summ_ext
                      (rowno, rowtitle, line_cd, policy_id)
               VALUES (ins.rowno, ins.rowtitle, ins.line_cd, v_max_policy_id);
       END LOOP;*/
      -- end mikel 04.25.2013
