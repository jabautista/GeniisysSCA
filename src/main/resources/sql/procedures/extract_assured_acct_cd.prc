DROP PROCEDURE CPI.EXTRACT_ASSURED_ACCT_CD;

CREATE OR REPLACE PROCEDURE CPI.EXTRACT_ASSURED_ACCT_CD (p_incept_date            gicl_claims.pol_eff_date%TYPE,
...
IS
  ...
  ** Commented out Abie's changes in ORDER BY
  */ 
...
BEGIN
  ...
  FOR W IN (SELECT assd_no, endt_seq_no
			 ...
			 ----ORDER BY endt_seq_no desc, eff_date DESC) -- added ent_seq_no in the order by clause, abie 09/12/2011
  ...
  LOOP
    UPDATE gicl_claims
       SET assd_no = NVL(v_assd_no,assd_no),
           acct_of_cd = NVL(v_acct_of_cd,acct_of_cd),
           assured_name = NVL(assd.assd_name,assured_name),
           assd_name2 = NVL(assd.assd_name2,assd_name2)
