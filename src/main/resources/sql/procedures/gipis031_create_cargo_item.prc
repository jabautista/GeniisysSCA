DROP PROCEDURE CPI.GIPIS031_CREATE_CARGO_ITEM;

CREATE OR REPLACE PROCEDURE CPI.GIPIS031_CREATE_CARGO_ITEM (
...
AS
	...
	**  Description 	: This procedure is used for inserting records to GIPI_CARGO table
	*/
...
BEGIN
            ...
            FOR B1 IN B(D1.policy_id) LOOP
                IF v_eff_date  IS NULL THEN
                    INSERT INTO gipi_wcargo (
                        par_id,                item_no,                rec_flag,        print_tag,
                        vessel_cd,            geog_cd,                cargo_class_cd,    voyage_no,
                        bl_awb,                origin,                    destn,            etd,
                        eta,                cargo_type,                deduct_text,    pack_method,
                        tranship_origin,    tranship_destination,    lc_no,            cpi_rec_no,
                        cpi_branch_cd)
                    VALUES (
                        p_par_id,           p_item_no,                    b1.rec_flag,        b1.print_tag,
                        b1.vessel_cd,        b1.geog_cd,                    b1.cargo_class_cd,    b1.voyage_no,
                        b1.bl_awb,            b1.origin,                    b1.destn,            b1.etd,
                        b1.eta,                b1.cargo_type,                b1.deduct_text,        b1.pack_method,
                        b1.tranship_origin,    b1.tranship_destination,    b1.lc_no,            b1.cpi_rec_no,
                        b1.cpi_branch_cd);
                END IF;
    ...
    ELSE
        ...
        END LOOP;  
    END IF;
END GIPIS031_CREATE_CARGO_ITEM;
/
