DROP PROCEDURE CPI.GET_SURVEY_AGENT;

CREATE OR REPLACE PROCEDURE CPI.get_survey_agent(
    ...
    p_survey_agent_cd           OUT     GIPI_POLBASIC.survey_agent_cd%TYPE
    ) IS
...
BEGIN
/* jen.082306
/* Retrieve survey agent cd based on the specified loss date*/
  ...
  FOR w IN (SELECT MAX(endt_seq_no) endt_seq_no 
             ...
             WHERE a.line_cd    = p_line_cd
               ...
               AND a.survey_agent_cd IS NOT NULL ) 
  LOOP
  ...
  IF v_max_endt_seq_no > 0 THEN
     ...
     ELSE
        --survey_agent_cd should be from the latest backward endt. with updates
  ...
  END IF;
  --get survey_agent_cd from records with eff_date equal to the derrived eff_date
  FOR A1 IN (SELECT b2501.survey_agent_cd survey_agent_cd
  ...
  LOOP
    p_survey_agent_cd := a1.survey_agent_cd;
    EXIT;
  END LOOP;
END;
/
