DROP PROCEDURE CPI.SEARCH_FOR_POLICY;

CREATE OR REPLACE PROCEDURE CPI.Search_For_Policy (
    ...
    p_gipi_wpolbas        OUT Gipis031_Ref_Cursor_Pkg.rc_gipi_wpolbas_type2,--Gipis031_Ref_Cursor_Pkg.rc_gipi_wpolbas_type, replaced by: Nica 12.28.2012
    ...
    */
        ...
        FOR A2 IN(
            SELECT (C.tsi_amt * A.currency_rt) tsi,
             ...
             WHERE B.line_cd      =  p_line_cd
               ...
               --AND TRUNC(NVL(B.endt_expiry_date,B.expiry_date)) >= TRUNC(SYSDATE)
               /*comment out condition for Expiry Date to allow retrieval of expired policy by MAC 04/30/2013
               AND TRUNC(DECODE(NVL(b.endt_expiry_date, b.expiry_date), b.expiry_date,
                    p_v_expiry_date, expiry_date,b.endt_expiry_date)) >= SYSDATE*/
         ORDER BY B.eff_date DESC) 
        LOOP
            ...
            ELSIF A2.prorate_flag = 2 OR a2.prorate_flag IS NULL THEN --return premium if prorate_flag is null by MAC 04/30/2013
                v_comp_prem  :=  A2.prem;
            ELSE
    ...
    FOR A1 IN (
        SELECT b250.incept_date,      b250.expiry_date,   b250.eff_date,
               ...
               b250.ref_pol_no, 	  b250.takeup_term,
			   b250.bank_ref_no,      b250.bancassurance_sw, 
               b250.banc_type_cd,     b250.area_cd, 
               b250.branch_cd,        b250.manager_cd, 
               b250.acct_of_cd,       b250.risk_tag, 
               b250.region_cd,        b250.industry_cd,  
               b250.place_cd -- additional columns added by: Nica 12.28.2012
    ...
    v_wpolbas.mortg_name         := v_tmp_mortg_name; --added by christian 03/04/13
