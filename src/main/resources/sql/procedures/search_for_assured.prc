DROP PROCEDURE CPI.SEARCH_FOR_ASSURED;

CREATE OR REPLACE PROCEDURE CPI.Search_For_Assured (
...
BEGIN
	...
	v_max_endt_seq_no := Gipi_Polbasic_Pkg.get_max_endt_seq_no(p_line_cd, p_subline_cd, p_iss_cd, p_issue_yy, p_pol_seq_no, p_renew_no, NULL, 'ASSURED');
	...
	IF NVL(v_max_endt_seq_no,0) > 0 THEN
		...
		v_max_endt_seq_no1 := Gipi_Polbasic_Pkg.get_max_endt_seq_no_back_stat(p_line_cd, p_subline_cd, p_iss_cd, p_issue_yy, p_pol_seq_no, p_renew_no, NULL, 'ASSURED');
		...
		IF v_max_endt_seq_no != NVL(v_max_endt_seq_no1,-1) THEN
			...
			v_max_eff_date1 := Gipi_Polbasic_Pkg.get_max_eff_date_back_stat(p_line_cd, p_subline_cd, p_iss_cd, p_issue_yy, p_pol_seq_no, p_renew_no, NULL, v_max_endt_seq_no1, 'ASSURED');
			
			--get max eff_date for endt 
			v_max_eff_date2 := Gipi_Polbasic_Pkg.get_endt_max_eff_date(p_line_cd, p_subline_cd, p_iss_cd, p_issue_yy, p_pol_seq_no, p_renew_no, NULL, 'ASSURED');
			v_max_eff_date := NVL(v_max_eff_date2,v_max_eff_date1); 
		ELSE
			--address should be from the latest backward endt. with updates
			v_max_eff_date := Gipi_Polbasic_Pkg.get_max_eff_date_back_stat(p_line_cd, p_subline_cd, p_iss_cd, p_issue_yy, p_pol_seq_no, p_renew_no, NULL, v_max_endt_seq_no1, 'ASSURED');
		END IF;
	ELSE
