DROP PROCEDURE CPI.COMPUTE_DEPRECIATION;

CREATE OR REPLACE PROCEDURE CPI.compute_depreciation
...
**  Description   : Compute depreciation for line MC for GICLS030
**                  
*/
  ...
  LOOP
      ...
      IF NVL(v_special_rate,NVL(v_fix_rate,0)) > 0 THEN
        ...
        VALUES
          (p_claim_id,             p_clm_loss_id,          p_line_cd,
           p_subline_cd,           p_loss_exp_type,        get_orig.loss_exp_cd,
           v_depreciation_cd,      get_orig.dtl_amt,       NVL(v_special_rate,v_fix_rate),
           p_user_id,              SYSDATE,
    ...
    FOR insert_main IN(
      SELECT SUM(NVL(loss_amt,0)) loss, SUM(NVL(ded_amt,0)) ded_amt
       ...
       WHERE claim_id = p_claim_id
         AND clm_loss_id = p_clm_loss_id
         AND loss_exp_cd = v_depreciation_cd)     
    LOOP
      ...
      VALUES
        (p_claim_id,            p_clm_loss_id,         p_line_cd,
         ...
         ABS((insert_main.ded_amt/insert_main.loss)* 100)); 
    END LOOP;
