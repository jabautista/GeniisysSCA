DROP PROCEDURE CPI.LOSS_RATIO_EXT_BY_BRANCH_INTM;

CREATE OR REPLACE PROCEDURE CPI.Loss_Ratio_Ext_By_Branch_Intm (p_line_cd            giis_line.line_cd%TYPE,
...
/*BETH 01/29/2002
**     this procedure extract amounts to be used in loss ratio(per branch) reports
...
BEGIN
  ...
  FOR curr_prem IN (
    SELECT d.iss_cd,
           NVL(SUM(c.premium_amt),0) prem_amt
  ...
  LOOP
    ...
    FOR prev_prem IN (
      SELECT NVL(SUM(c.premium_amt),0) prem_amt
        FROM gipi_polbasic d,
             gipi_comm_inv_peril c
       WHERE d.policy_id = c.policy_id
         AND d.line_cd = NVL(p_line_cd, d.line_cd)
         AND d.subline_cd = NVL(p_subline_cd, d.subline_cd)
         AND d.iss_cd = curr_prem.iss_cd
      ...
      SELECT SUM((NVL(a.loss_reserve,0) - NVL(c.losses_paid,0)) * (e.shr_intm_pct/100)) loss
        FROM gicl_clm_res_hist a,
            ...
            WHERE 1 = 1
              AND d.line_cd = NVL(p_line_cd, d.line_cd)
              AND d.subline_cd = NVL(p_subline_cd, d.subline_cd)
              AND DECODE(p_issue_param, 1, d.iss_cd, d.pol_iss_cd) = curr_prem.iss_cd
    ...
    FOR paid_loss IN
      (SELECT SUM((NVL(losses_paid,0)+ NVL(expenses_paid,0)) * (c.shr_intm_pct/100)) loss_paid
        ...
        WHERE 1 = 1
         ...
         AND DECODE(p_issue_param, 1, b.iss_cd, b.pol_iss_cd) = curr_prem.iss_cd
         AND a.claim_id          = b.claim_id
         AND tran_id IS NOT NULL
         AND NVL(cancel_tag,'N') = 'N'
    ...
    FOR prev_recovery IN(
      SELECT SUM(NVL(recovered_amt,0) * (NVL(c.premium_amt,0)
    ...
    VALUES (p_session_id,       p_line_cd,          p_subline_cd,
            curr_prem.iss_cd,   p_intm_no,          p_loss_date,
            NVL(v_curr_prem,0), NVL(v_prev_prem,0), NVL(v_paid_amt,0),
            NVL(v_curr_loss,0) + NVL(v_curr_exp,0) - NVL(v_curr_recovery,0),
            NVL(v_prev_loss,0) + NVL(v_prev_exp,0) - NVL(v_prev_recovery,0),  USER);  END LOOP;
  COMMIT;
END;
/
