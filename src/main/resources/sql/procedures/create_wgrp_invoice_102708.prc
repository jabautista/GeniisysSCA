DROP PROCEDURE CPI.CREATE_WGRP_INVOICE_102708;

CREATE OR REPLACE PROCEDURE CPI.Create_Wgrp_Invoice_102708(p_policy_id  IN NUMBER,
          ...
          DECLARE
            CURSOR c1 IS
              SELECT DISTINCT b.tax_cd, NVL(b.rate,0) rate, b.peril_sw,
                     ...
                     DECODE(b.allocation_tag,'N','N','Y') fixed_tax_alloc, takeup_alloc_tag  ---tonio
   ...
   CURSOR d2 IS
    ...
    BEGIN
     ...
     INSERT INTO gipi_winv_tax
      ...
      END IF;
           IF (c4_rec.takeup_seq_no <> 1 AND d1_rec.takeup_alloc_tag = 'F') OR
                       (c4_rec.takeup_seq_no <> p_no_of_takeup AND d1_rec.takeup_alloc_tag = 'L') THEN --tax allocated on first or last takeup: tonio
     ...
     IF (c4_rec.takeup_seq_no = 1 AND d2_rec.takeup_alloc_tag = 'F') OR
                      (c4_rec.takeup_seq_no = p_no_of_takeup AND d2_rec.takeup_alloc_tag = 'L') THEN
     INSERT INTO gipi_winv_tax
                     (par_id,              item_grp,          tax_cd,
                      line_cd,             iss_cd,            rate,
                      tax_amt,             tax_id,            tax_allocation,
      ...
      END IF;
      IF (c4_rec.takeup_seq_no <> 1 AND d2_rec.takeup_alloc_tag = 'F') OR
                       (c4_rec.takeup_seq_no <> p_no_of_takeup AND d2_rec.takeup_alloc_tag = 'L') THEN  --tax allocated on first or Last takeup: tonio 10/3/2008
     ...
     IF (c4_rec.takeup_seq_no = 1 AND c1_rec.takeup_alloc_tag = 'F') OR
                       (c4_rec.takeup_seq_no = p_no_of_takeup AND c1_rec.takeup_alloc_tag = 'L') THEN
     ...
     END IF;
     IF (c4_rec.takeup_seq_no <> 1 AND c1_rec.takeup_alloc_tag = 'F') OR
                         (c4_rec.takeup_seq_no <> p_no_of_takeup AND c1_rec.takeup_alloc_tag = 'L') THEN  --tax allocated on first or last takeup: tonio
     ...
     IF c4_rec.takeup_seq_no = v_no_of_payment THEN -- LAST RECORD
        ...
        END IF;
                    IF (c4_rec.takeup_seq_no = 1 AND c1a_rec.takeup_alloc_tag = 'F') OR
                       (c4_rec.takeup_seq_no = p_no_of_takeup AND c1a_rec.takeup_alloc_tag = 'L') THEN
     INSERT INTO gipi_winv_tax
                     (par_id,              item_grp,          tax_cd,
                      line_cd,             iss_cd,            rate,
                      tax_amt,             tax_id,            tax_allocation,
     ...
     END IF;
     IF (c4_rec.takeup_seq_no <> 1 AND c1a_rec.takeup_alloc_tag = 'F') OR
                          (c4_rec.takeup_seq_no <> p_no_of_takeup AND c1a_rec.takeup_alloc_tag = 'L') THEN  --tax allocated on first or last takeup: tonio INSERT INTO GIPI_WINV_TAX
    ...
    FOR d2_rec IN d2
     ...
     BEGIN
      ...
      c4_rec.takeup_seq_no);
        END IF;
        IF (c4_rec.takeup_seq_no <> 1 AND d2_rec.takeup_alloc_tag = 'F') OR
                              (c4_rec.takeup_seq_no <> p_no_of_takeup AND d2_rec.takeup_alloc_tag = 'L') THEN  --tax allocated on first or last takeup: tonio
