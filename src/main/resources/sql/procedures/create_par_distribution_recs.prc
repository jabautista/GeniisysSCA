DROP PROCEDURE CPI.CREATE_PAR_DISTRIBUTION_RECS;

CREATE OR REPLACE PROCEDURE CPI.CREATE_PAR_DISTRIBUTION_RECS
...
BEGIN
  ...
  */
  ...
  --:gauge.file := 'creating default distribution records...';
  --:gauge.recno := :gauge.recno + 35;
  --vbx_counter;
  ...
  FOR c1 IN (  SELECT NVL(item_grp, 1) item_grp        ,
                 ...
                 FROM gipi_witem a
                WHERE par_id = p_par_id
                -- jhing 11.07.2014 added condition to create default dist for 
                -- records with zero tsi and zero prem which is not in essence a tax endorsement but a regular endt
                AND ( (prem_amt != 0
                        OR  tsi_amt  != 0)  
                   OR (a.tsi_amt = 0 AND a.prem_amt = 0 
                        AND EXISTS (
                            SELECT 1 FROM gipi_witmperl x
                                WHERE x.par_id = a.par_id
                                    AND x.item_no = a.item_no
                         ))
                   )
                  AND item_grp = NVL(v_item_grp, a.item_grp)
  ...
  --:gauge.file  := 'processing created distribution records...';
  --vbx_counter;
