CREATE OR REPLACE PROCEDURE CPI.Recap_web (p_fm_date  DATE,
                                       p_to_date  DATE)
AS
...
-- EXEC recap(TO_DATE('1/1/2002','mm/dd/yyyy'),TO_DATE('1/31/2002','mm/dd/yyyy'));
  ...
  --end albert 03102014  
    ...
    ----------------------------------------------------------------------
  TYPE chk_ext                 IS TABLE OF NUMBER INDEX BY VARCHAR2(100);                 
  v_chk_ext1                    chk_ext;
  v_chk_ext2                    chk_ext;
  v_chk_ext3                    chk_ext;
  ----------------------------------------------------------------------
...
--  COMMIT;
  ...
  IF v_fi_line_cd IS NULL THEN
     RAISE_APPLICATION_ERROR(-20101,'Geniisys Exception#E#Line code for fire is not found in giis_parameters.');
  END IF;
...
--     RAISE_APPLICATION_ERROR(-20102,'Line code for fire is not found in giis_parameters.');
     RAISE_APPLICATION_ERROR(-20102,'Geniisys Exception#E#Line code for Motor Car is not found in giis_parameters.');
  END IF;
...
--     RAISE_APPLICATION_ERROR(-20102,'Line code for Bonds is not found in giis_parameters.');
     RAISE_APPLICATION_ERROR(-20103,'Geniisys Exception#E#Line code for Bonds is not found in giis_parameters.');     
  END IF;
  ...
  SELECT pol.line_cd        AS line_cd,
         ...
         --added by mikel 03.10.2015; to handle ri comm of spoiled RI policies     
         DECODE((SIGN(p_fm_date - pol.acct_ent_date) * 4)
              + SIGN(NVL(pol.spld_acct_ent_date, p_to_date+60) - p_to_date),
              1, 1,
             -3, 1,
              3,-1,
              4,-1, 0) * --end mikel 03.10.2015
...
--  IF SQL%FOUND THEN
  IF vv_policy_id IS NOT NULL THEN      --added by albert 04042014 
     ...
     SELECT policy_id        AS policy_id,
      ...
      LOOP
             v_chk_ext1 ( i.policy_id || '-' || i.peril_cd) := -1;            
      END LOOP;
      ...
      FOR I IN  (
       ...
       LOOP
             v_chk_ext2 ( i.policy_id || '-' || i.peril_cd) := 2;  
       END LOOP;       
       FOR I IN ( SELECT  a.peril_cd , a.policy_id,  NVL(new_comm,0) - NVL(prev_comm,0) comm
              FROM          (SELECT   a.policy_id, b.peril_cd, 
                                      NVL (SUM (NVL (b.commission_amt, 0) * ( SELECT currency_rt
                                                                                FROM gipi_item
                   ...
                   (SELECT   a.policy_id, c.peril_cd,
                             NVL (SUM (NVL (c.commission_amt, 0)  * ( SELECT currency_rt
                                                                        FROM gipi_item
                                                                       WHERE policy_id = a.policy_id
                                                                     AND ROWNUM    = 1)), 0) prev_comm
                        FROM giac_new_comm_inv a,
                             giac_new_comm_inv_peril b,
        ...
        LOOP 
              v_chk_ext3 ( i.policy_id || '-' || i.peril_cd) := i.comm;  
        END LOOP;
       
     IF SQL%FOUND THEN --mikel 04.06.2016
         ...
         --added by albert 03102014; to consider modified commissions for recap III  
           ...
           BEGIN
               v_comm_exs := v_chk_ext1( vv_sum_policy_id(i)  || '-' || vv_sum_peril_cd(i));  
           EXCEPTION
           WHEN NO_DATA_FOUND THEN
              BEGIN
                    v_comm_exs := v_chk_ext2( vv_sum_policy_id(i)  || '-' || vv_sum_peril_cd(i));
              EXCEPTION 
                 WHEN NO_DATA_FOUND THEN
                     v_comm_exs:=  NULL;
              END;            
         ...
         --added by albert 04042014; moved old codes at the end of the procedure      
           ...
           IF v_comm_exs = -1 THEN
              BEGIN
                  vv_sum_comm_amt (i):= v_chk_ext3( vv_sum_policy_id(i)  || '-' || vv_sum_peril_cd(i));
              EXCEPTION
              WHEN NO_DATA_FOUND THEN
                vv_sum_comm_amt(i) := 0;
              END;
    ...
    --           RAISE_APPLICATION_ERROR(-20105,'Policy_id '|| vv_sum_policy_id(i) ||' has no records in gipi_comm_inv_peril');
               RAISE_APPLICATION_ERROR(-20104,'Geniisys Exception#E#Policy_id '|| vv_sum_policy_id(i) ||' has no records in gipi_comm_inv_peril'); --changed error number by albert 04042014
             END;
           END IF; 
           --end albert 03072014      
         END LOOP;
         ...
         LOOP     
          BEGIN
           SELECT ROUND((vv_premium_amt(beta) / DECODE(premium_amt,0,DECODE(vv_premium_amt(beta),0,1,vv_premium_amt(beta)),premium_amt) ) * commission_amt,4)
                  ...
                  --* DECODE(vv_date_tag(beta),'S',-1,'Z',0,1) --commented out by albert 04162014
                  --added by albert 04162014; to compute for the correct commission multiplier
                  * DECODE((SIGN(p_fm_date - vv_acct_ent_date(beta)) * 4)
                  + SIGN(NVL(vv_spld_acct_ent_date(beta), p_to_date+60) - p_to_date),
                  1, 1,
                 -3, 1,
                  3,-1,
                  4,-1,
    ...
    --        RAISE_APPLICATION_ERROR(-20109,'Policy_id '|| vv_policy_id(beta) ||' has zero denominator value');
            RAISE_APPLICATION_ERROR(-20105,'Geniisys Exception#E#Policy_id '|| vv_policy_id(beta) ||' has zero denominator value');  --changed error number by albert 04042014
          END;
         END LOOP;
     END IF;
  ...
  --mikel 04.01.2016; populate unearned premium reserves (UPR) for as extract date and as of previous year
  v_start_date    := TO_DATE ('jan-' || (TO_NUMBER (TO_CHAR (p_to_date, 'YYYY')) - 1000), 'mm-yyyy');
  v_prior_to_date := LAST_DAY (TO_DATE ('dec-' || (TO_NUMBER (TO_CHAR (p_to_date, 'YYYY')) - 1), 'mm-yyyy'));
  
  --UPR for previous year
  cpi.recap_upr_prev_year_dtl(v_start_date, v_prior_to_date);
  cpi.recap_upr_prev_year_summ;
  
  --UPR for current year
  cpi.recap_upr_curr_year_dtl(v_start_date, p_to_date);
  cpi.recap_upr_curr_year_summ;
  --end mikel 04.01.2016   
  ...
  v_cnt         := 0;
      ...
      FOR perl IN
        (SELECT distfr.dist_no          AS dist_no,
               ...
               -- frperl.ri_shr_pct       AS shr_pct,-- jhing 03.17.2016 republicfullweb Sr#21959 commented out and replaced share% from giri_frps_ri
                frpsri.ri_shr_pct AS shr_pct,
                NVL(frpsri.ri_shr_pct2,frpsri.ri_shr_pct) AS shr_pct2,
          ...
          WHERE 1=1
            AND (   (    TRUNC(bndr.acc_ent_date) >= p_fm_date
                     ...
                     --added by mikel 03.10.2015; to handle re-used binders
                     AND distfr.dist_no IN ( SELECT MIN(dist_no)
                                              ...
                                              WHERE x.line_cd = y.line_cd
                                                ...
                                                AND y.fnl_binder_id = bndr.fnl_binder_id))
                     --end mikel                           
    ...
    --          RAISE_APPLICATION_ERROR(20103,'Policy_id '|| perl.policy_id ||' does not exist in gipi_polbasic');
              RAISE_APPLICATION_ERROR(-20106,'Geniisys Exception#E#Policy_id '|| perl.policy_id ||' does not exist in gipi_polbasic'); --changed error number by albert 04042014
        END;
        FOR item IN
          (SELECT decode(itmprl.dist_spct, 0, 1, itmprl.dist_spct) spct, itmprl.dist_prem prem, itmprl.line_cd, --koks 7.10.14  SR 16256    
                  decode(NVL(itmprl.dist_spct1, itmprl.dist_spct), 0, 1, NVL(itmprl.dist_spct1, itmprl.dist_spct)) spct2,  -- jhing REPUBLICFULLWEB SR# 21959
    ...
    --            RAISE_APPLICATION_ERROR(20104,'The record with policy_id='
                RAISE_APPLICATION_ERROR(-20107,'Geniisys Exception#E#The record with policy_id='     --changed error number by albert 04042014
          ...
          --vv_facul_prem(v_cnt)       := (perl.shr_pct/item.spct) * item.prem * v_currency_rt; -- jhing REPUBLICFULLWEB 21959 commented out and replaced with
          vv_facul_prem(v_cnt)       := (perl.shr_pct2/item.spct2) * item.prem * v_currency_rt;
          vv_facul_tsi(v_cnt)        := ((perl.shr_pct/item.spct) * item.tsi) * v_currency_rt;
          --vv_facul_comm(v_cnt)       := ROUND(perl.comm_rt * ((perl.shr_pct/item.spct) * item.prem),4) * v_currency_rt; -- jhing REPUBLICFULLWEB 21959 commented out and replaced with
          vv_facul_comm(v_cnt)       := ROUND(perl.comm_rt * ((perl.shr_pct2/item.spct2) * item.prem),4) * v_currency_rt;
     ...
     LOOP
       BEGIN
         ...
         EXCEPTION                          --albert 11112013
           WHEN NO_DATA_FOUND THEN
--             NULL;
             RAISE_APPLICATION_ERROR(-20108, 'Geniisys Exception#E#Bond subline code ' || vv_subline_cd(i)
