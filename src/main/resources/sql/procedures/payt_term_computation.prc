DROP PROCEDURE CPI.PAYT_TERM_COMPUTATION;

CREATE OR REPLACE PROCEDURE CPI.payt_term_computation (
...
----------------------------------------
   ...
   FOR s IN (SELECT   a.no_of_days, a.no_of_payt, a.annual_sw,
                ...
                WHERE b.par_id = p_par_id
                  AND a.payt_terms = NVL (b.payt_terms, 'COD')
             ORDER BY b.item_grp, b.takeup_seq_no)
   LOOP                                 ------------------------------------ S
/*added by glyza 04.10.08*/
  ---------------------------------------
     var_due_date := s.due_date;--var_due_date + nvl(s.no_payt_days, 0);
      IF v_variable = 'Y'
      THEN
         IF v_count = s.takeup_seq_no THEN --added by belle 11.7.2012
         ...
         ELSE
            FOR y IN (SELECT takeup_seq_no, due_date
	         ...
	         END IF;
		 END IF;
      ELSE--belle 11.07.2012 for single take-up
      
         IF s.on_incept_tag = 'N'THEN 
            v_incept_date := var_due_date;
         END IF;
      ...
      THEN
         ...
         ELSE
            ...
            ELSE
               v_no_of_payment := var_no_of_payt;
            END IF;
            v_exact_no_of_payment := v_no_of_payment; --added by belle 12.06.12
         END IF;
      ELSE
      ...
      LOOP                 ------------------------------------ S->var_inst_no
         ...
         THEN
            ...
            THEN
               ...
               END IF;
            ELSE
               var_due_date := s.due_date;--TO_DATE (p_nbt_due_date, 'mm-dd-rrrr'); Commented to handle multiple item groups. Due date is now updated first but not committed so that data can be fetched from table.
            END IF;
         ...
         THEN
            ...
            */
            ...
            ELSE
               IF v_no_payt_days IS NOT NULL
               THEN
                  IF (v_policy_days - v_no_of_days) > NVL (v_no_payt_days, 0)
                  --added by tonio may 17, 2011
                  THEN
                     /* var_due_date :=
                           var_due_date
                         + ROUND (v_no_payt_days / v_no_of_payment);*/
...
--*******************
         ...
         DBMS_OUTPUT.put_line ('out : ' || var_due_date);

         IF     (pv_date_sw = 'Y' OR NVL (pv_date_sw1, 'Y') = 'Y')
            AND p_version = '1'
         THEN
            DBMS_OUTPUT.put_line ('Insert HERE: ');
             IF var_due_date > v_expiry_date AND v_allow_exp_pol_iss = 'N' THEN --added v_allow_exp_pol_iss by robert SR 19785 07.20.15
                var_due_date := '';
            END IF; --added by PJD 8.29.2013 to set the due date as null if it exceeds the expirydate.SR 0013796
         ...
         ELSE
            DBMS_OUTPUT.put_line ('Update HERE: ');
             IF var_due_date > v_expiry_date AND v_allow_exp_pol_iss = 'N' THEN --added v_allow_exp_pol_iss by robert SR 19785 07.20.15
                var_due_date := '';
            END IF; --added by PJD 8.29.2013 to set the due date as null if it exceeds the expirydate.SR 0013796
   ...
   OPEN p_temp_inst_cur FOR
      SELECT par_id, item_grp, takeup_seq_no, inst_no, due_date, share_pct,
             prem_amt, tax_amt
        FROM gipi_winstallment
       WHERE par_id = p_par_id;
EXCEPTION
