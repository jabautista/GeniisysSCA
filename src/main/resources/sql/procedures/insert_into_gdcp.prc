DROP PROCEDURE CPI.INSERT_INTO_GDCP;

CREATE OR REPLACE PROCEDURE CPI.insert_into_gdcp(p_advice_id  IN  GIAC_DIRECT_CLAIM_PAYTS.advice_id%TYPE,
   ...
   **  Description  : Executes insert_into_gdcp program unit in GICLS043
   **                 
   */
...
BEGIN
  ...
  FOR d IN  (SELECT a.advice_id, a.claim_id, b.clm_loss_id, 
                        ...
                        DECODE(b.currency_cd, giacp.n('CURRENCY_CD'), 1, NVL(a.orig_curr_rate, a.convert_rate)) conv_rt_b,
                        DECODE(a.currency_cd, giacp.n('CURRENCY_CD'), 1, NVL(a.orig_curr_rate, a.convert_rate)) conv_rt_a 
  ...
  LOOP
    ...
    IF SQL%NOTFOUND THEN      
       p_msg_alert := 'Cannot insert into GIAC_DIRECT_CLAIM_PAYTS table.' ||
