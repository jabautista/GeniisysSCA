DROP PROCEDURE CPI.GIPIS031_VALIDATE_EFF_DATE02;

CREATE OR REPLACE PROCEDURE CPI.GIPIS031_VALIDATE_EFF_DATE02 (
...
AS
    ...
    **                                Part 2. See GIPIS031_VALIDATE_EFF_DATE03 for next part
...
BEGIN
    ...
    IF TRUNC(p_eff_date) < TRUNC(SYSDATE)  AND p_p_sysdate_sw = 'Y' THEN        
        ...
        SEARCH_FOR_POLICY2_1(p_par_id, p_line_cd, p_subline_cd, p_iss_cd, p_issue_yy, p_pol_seq_no,
            ...
            p_message, p_message_type); -- retrieved current info based on entered date
    ...
    ELSIF TRUNC(p_eff_date) >= TRUNC(p_var_max_eff_date) OR p_var_max_eff_date IS NULL THEN
...
END GIPIS031_VALIDATE_EFF_DATE02;
/
