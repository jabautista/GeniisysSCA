DROP PROCEDURE CPI.GIPIS031_POL_FLAG_INTERRUPTION;

CREATE OR REPLACE PROCEDURE CPI.GIPIS031_POL_FLAG_INTERRUPTION (
...
BEGIN
	...
	BEGIN
		...
		IF v_item > 0 OR v_peril > 0 OR v_incept_date != v_eff_date THEN
			IF NOT p_msg_alert IS NULL THEN
				p_msg_alert := p_msg_alert || CHR(10);
			END IF;
			
			IF v_incept_date != v_eff_date THEN					
				p_msg_alert := 'Flat Cancellation requires an endorsement effectivity as of inception date. Changes are about to take place.';
			ELSIF v_item > 0 AND v_peril > 0 THEN				
	...
	NULL;
END GIPIS031_POL_FLAG_INTERRUPTION;
/
