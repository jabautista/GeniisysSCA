DROP PROCEDURE CPI.GICLS032_SETUP_THREE;

CREATE OR REPLACE PROCEDURE CPI.gicls032_setup_three (
...
IS
   ...
   **  Description  : Converted procedure from GICLS032 - setup_three
   */
...
--
   FOR rlp_rec IN rlp
   LOOP
      IF rlp_rec.payee_type = p_variables.v_loss_payee_type
      THEN
      ...
      ELSIF rlp_rec.payee_type = p_variables.v_exp_payee_type
      THEN
   ...
   LOOP
      IF dnlp_rec.payee_type = p_variables.v_loss_payee_type
      THEN
      ...
      ELSIF dnlp_rec.payee_type = p_variables.v_exp_payee_type
      THEN
...
--
   ...
   LOOP
      IF rnlp_rec.payee_type = p_variables.v_loss_payee_type
      THEN
      ...
      ELSIF rnlp_rec.payee_type = p_variables.v_exp_payee_type
      THEN
         v_item_no := 4;                                              /* for DECREASE IN RESERVE FOR LOSS EXPENSE PAYABLE - RI */
         gicls032_aeg_create_acct_ent (p_claim_id,
                                       p_advice_id,
                                       p_variables,
                                       rnlp_rec.peril_cd,
