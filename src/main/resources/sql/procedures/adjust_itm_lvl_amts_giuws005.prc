DROP PROCEDURE CPI.ADJUST_ITM_LVL_AMTS_GIUWS005;

CREATE OR REPLACE PROCEDURE CPI.ADJUST_ITM_LVL_AMTS_GIUWS005(p_dist_no IN giuw_pol_dist.dist_no%TYPE) IS
  ...
  v_sum_dist_spct1   giuw_wpolicyds_dtl.ann_dist_spct%TYPE;
...
BEGIN
  ...
  **  Description  : ADJUST_ITEM_LEVEL_AMTS program unit
  */
    ...
    BEGIN
      ...
      ************************************************************************** */
      ...
      FOR c10 IN (SELECT ROUND(SUM(NVL(dist_tsi, 0)), 2)     dist_tsi     , 
       ...
       ROUND(SUM(NVL(dist_spct1, 0)), 9)    dist_spct1    ,
