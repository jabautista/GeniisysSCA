DROP PROCEDURE CPI.CREATE_NEGATED_RECORDS_FLAT;

CREATE OR REPLACE PROCEDURE CPI.Create_Negated_Records_Flat (
...
BEGIN
    ...
    v_max_endt_seq_no := Gipi_Polbasic_Pkg.get_max_endt_seq_no(p_line_cd, p_subline_cd, p_iss_cd,
        p_issue_yy, p_pol_seq_no, p_renew_no, NULL, 'FLAT_CANCELLATION');
    ...
    IF v_max_endt_seq_no > 0 THEN
        ...
        IF v_max_endt_seq_no != NVL(v_max_endt_seq_no1,-1) THEN             
            v_max_eff_date1 := Gipi_Polbasic_Pkg.get_max_eff_date_back_stat(p_line_cd, p_subline_cd, p_iss_cd,
                p_issue_yy, p_pol_seq_no, p_renew_no, NULL, v_max_endt_seq_no1, 'FLAT_CANCELLATION');
        ...
        ELSE
            v_max_eff_date := Gipi_Polbasic_Pkg.get_max_eff_date_back_stat(p_line_cd, p_subline_cd, p_iss_cd,
                p_issue_yy, p_pol_seq_no, p_renew_no, NULL, v_max_endt_seq_no1, 'FLAT_CANCELLATION2');
        END IF;
    ELSE
    ...
    BEGIN
        ...
        Gipi_Witem_Pkg.GET_TSI_PREM_AMT(p_par_id, p_tsi_amt, p_prem_amt);        
        p_ann_tsi_amt  := 0;
        p_ann_prem_amt := 0;           
    ...
    SELECT *
      FROM GIPI_WITMPERL
     WHERE par_id = p_par_id;
END Create_Negated_Records_Flat;
/
