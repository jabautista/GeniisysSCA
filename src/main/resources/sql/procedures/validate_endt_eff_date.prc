DROP PROCEDURE CPI.VALIDATE_ENDT_EFF_DATE;

CREATE OR REPLACE PROCEDURE CPI.Validate_Endt_Eff_Date (
...
BEGIN
    p_ann_tsi_amt := null;
    p_ann_prem_amt := null; --edited by d.alcantara, 03.20.2012, changed from 0 to null
    ...
    IF TRUNC(v_eff_date2) < TRUNC(SYSDATE) AND p_sysdate_sw = 'Y' THEN
        p_ann_tsi_amt := 0;
        p_ann_prem_amt := 0;
        Search_For_Policy2(p_par_id, p_line_cd, p_subline_cd, p_iss_cd, p_issue_yy,    p_pol_seq_no, p_renew_no, v_eff_date2, 
