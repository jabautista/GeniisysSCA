DROP PROCEDURE CPI.POPULATE_WARRANTIES;

CREATE OR REPLACE PROCEDURE CPI.POPULATE_WARRANTIES(
    p_old_pol_id       IN  gipi_pack_polbasic.pack_policy_id%TYPE,
    p_proc_summary_sw  IN  VARCHAR2,
    p_new_par_id       IN  gipi_wpolwc.par_id%TYPE,
    p_msg             OUT  VARCHAR2
)  
IS
  --rg_id               RECORDGROUP;
...
BEGIN 
  ...
  **  Description  : POPULATE_WARRANTIES program unit 
  */
