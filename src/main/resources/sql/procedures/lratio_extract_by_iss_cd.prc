DROP PROCEDURE CPI.LRATIO_EXTRACT_BY_ISS_CD;

CREATE OR REPLACE PROCEDURE CPI.Lratio_Extract_By_Iss_Cd
...
BEGIN
  p_counter :=0;
    FOR curr_prem IN(
      SELECT iss_cd, NVL(SUM(prem_amt),0) prem
    ...
    LOOP
      ...
      VALUES
        (p_session_id,     p_line_cd,         p_subline_cd,
         curr_prem.iss_cd, p_peril_cd,        p_loss_date,
    ...
    LOOP
	 ...
	 INSERT INTO gicl_loss_ratio_ext
       ...
       VALUES
         (p_session_id,     p_line_cd,         p_subline_cd,
          prev_prem.iss_cd, p_peril_cd,        p_loss_date,
  ...
  --current loss
  FOR curr_loss IN(
    SELECT iss_cd, NVL(SUM(os_amt),0) loss
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,     p_line_cd,         p_subline_cd,
          curr_loss.iss_cd, p_peril_cd,        p_loss_date,
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,     p_line_cd,         p_subline_cd,
          prev_loss.iss_cd, p_peril_cd,        p_loss_date,
  ...
  --losses paid
  FOR losses IN(
    SELECT iss_cd, NVL(SUM(loss_paid),0) loss_paid
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,  p_line_cd,         p_subline_cd,
          losses.iss_cd, p_peril_cd,        p_loss_date,
  ...
  --current recovery
  FOR curr_rec IN(
    SELECT iss_cd, NVL(SUM(recovered_amt),0) recovery
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,    p_line_cd,          p_subline_cd,
          curr_rec.iss_cd, p_peril_cd,         p_loss_date,
  ...
  LOOP
    ...
    IF v_exists = 'N' THEN
       ...
       VALUES
         (p_session_id,       p_line_cd,          p_subline_cd,
          prev_rec.iss_cd,    p_peril_cd,         p_loss_date,
