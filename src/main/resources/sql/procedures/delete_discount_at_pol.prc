DROP PROCEDURE CPI.DELETE_DISCOUNT_AT_POL;

CREATE OR REPLACE PROCEDURE CPI.DELETE_DISCOUNT_AT_POL(
...
BEGIN
	...
	**  Description 	: DELETE_DISCOUNT_AT_POL program unit
	*/
  ...
  IF cnt_disc > 0 THEN
    FOR delete_pol IN(SELECT SUM(ROUND( (NVL(b930.disc_amt,0) * b480.currency_rt),2 )) disc_amt,
                       ...
                       WHERE b930.par_id     = p_par_id
                         ...
                         AND b480.item_no    = b930.item_no) LOOP
        UPDATE GIPI_WPOLBAS
           SET prem_amt     = (prem_amt     + delete_pol.disc_amt) - delete_pol.surc_amt,
               ann_prem_amt = (ann_prem_amt + (delete_pol.disc_amt / NVL(v_prorate, 1))) - delete_pol.surc_amt
...
--update discounted ri_comm_amt
   ...
   IF p_delete_sw = 'Y' THEN
	 	  LAST_DISCOUNT(p_par_id, '3', p_SEQUENCE);
   END IF;		                   
