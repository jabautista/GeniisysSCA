DROP PROCEDURE CPI.LRATIO_EXTRACT_PREMIUM_INTM2;

CREATE OR REPLACE PROCEDURE CPI.Lratio_Extract_Premium_Intm2
  ...
  BEGIN -- extract current year premiums written
    ...
    BEGIN --select all records from gipi_polbasic for record
      ...
      BULK COLLECT
       ...
       WHERE d.policy_id = c.policy_id
         AND check_user_per_iss_cd2(d.line_cd,d.line_cd,'GICLS204',USER) = 1 --angelo092507
    ...
    BEGIN--select all records from gipi_polbasic for record
     ...
     --insert record on table gicl_lratio_curr_prem_ext
         ...
         FORALL i IN vv_policy_id.first..vv_policy_id.last
	       INSERT INTO gicl_lratio_curr_prem_ext
             (session_id, 	    assd_no,      	     policy_id,        spld_sw,
              ...
              prem_amt,         intm_no,             user_id, date_for_24th)
           VALUES
	         (p_session_id,	    vv_assd_no(i),       vv_policy_id(i),  'Y',
              vv_line_cd(i),    vv_subline_cd(i),    vv_iss_cd(i),     vv_peril_cd(i),
              -vv_prem(i),      vv_intm_no(i),       USER, TO_DATE(vv_date24(i),'MM-DD-YYYY'));
	     --after insert refresh arrays by deleting data
	     vv_assd_no.DELETE;
	     vv_policy_id.DELETE;
	     vv_prem.DELETE;
      ...
      END IF;
    END;--select all records from gipi_polbasic for record
	    --with spolied accounting entry date within the current year
  ...
  FOR del_spld IN(
    SELECT a.policy_id
     ...
     WHERE a.session_id = p_session_id
       AND check_user_per_iss_cd2(a.line_cd,a.line_cd,'GICLS204',USER) = 1 --angelo092507
