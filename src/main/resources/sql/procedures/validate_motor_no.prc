DROP PROCEDURE CPI.VALIDATE_MOTOR_NO;

CREATE OR REPLACE PROCEDURE CPI.Validate_Motor_No (
...
BEGIN
  ...
  **  Description  : Validate_Motor_No program unit
  */
...
IF p_par_type='P' THEN   
  IF v_cnt_item=1 THEN-- for PAR w/ 1 item
   ...
   IF v_motor_cnt > 1 THEN--if it occurs more than 1, serial already exist in other PAR
  	  v_exist:='Y';
      Check_Exist_Par_Policy(p_par_id,v_msg_alert,'PAR','MOTOR_NO',v_motor_no);
   END IF	;
   ...
   IF v_exist='N' THEN--if it does not occurs in other PAR, then check other Policy
   		...
   		LOOP
		   v_exist:='Y';
  	       Check_Exist_Par_Policy(p_par_id,v_msg_alert,'POLICY','MOTOR_NO',v_motor_no);
   		   EXIT;
   		END LOOP;
   END IF;
  		...
  		LOOP
  			...
  			LOOP
			    ...
			    IF v_motor_cnt > 1 THEN
  				    v_exist:='Y';
				    Check_Exist_Par_Policy(p_par_id,v_msg_alert,'PAR','MOTOR_NO',c.motor_no);-- adpascual 05112012 changed from v_msg_alert1 to v_msg_alert
   			    END IF	;
  			    ...
  			    IF v_exist='N' THEN
   				    ...
   				    LOOP
			  		    v_exist:='Y';
					    Check_Exist_Par_Policy(p_par_id,v_msg_alert1,'POLICY','MOTOR_NO',c.motor_no);
   					    EXIT;
   				    END LOOP;
  			    END IF;
  			END LOOP;						
  						...
  						LOOP
  								...
  								FOR D IN (SELECT policy_id FROM GIPI_VEHICLE--to be compare to the policy_no of PAR to be posted
   								...
   								LOOP
				   								...
				   								LOOP
 						  								v_exist:='Y';
													  	Check_Exist_Par_Policy(p_par_id,v_msg_alert,'POLICY','MOTOR_NO',c.motor_no);-- adpascual 05112012 changed from v_msg_alert1 to v_msg_alert
   														EXIT;
   												END LOOP;
											    IF v_exist = 'Y' THEN
											    	 EXIT;
  								...
  								IF v_exist='N' THEN
  									...
  									LOOP						
  												FOR E IN (SELECT 1 FROM GIPI_WPOLBAS
				   								...
				   								LOOP
 						  								v_exist:='Y';
													  	Check_Exist_Par_Policy(p_par_id,v_msg_alert1,'PAR','MOTOR_NO',c.motor_no);
   														EXIT;
   												END LOOP;
											    IF v_exist = 'Y' THEN
											    	 EXIT;
