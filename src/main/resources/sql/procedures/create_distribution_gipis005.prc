DROP PROCEDURE CPI.CREATE_DISTRIBUTION_GIPIS005;

CREATE OR REPLACE PROCEDURE CPI.CREATE_DISTRIBUTION_GIPIS005(b_par_id  IN NUMBER,
                                                             p_dist_no IN NUMBER)
/*
  ...
  LOOP
    ...
    IF p_exist = 1 THEN
      ...
      LOOP
        ...
        GIRI_WDISTFRPS_PKG.DEL_GIRI_WDISTFRPS(pi_dist_no);  
      END LOOP;
      ...
      GIUW_PERILDS_DTL_PKG.del_giuw_perilds_dtl(pi_dist_no);       
      GIUW_ITEMPERILDS_DTL_PKG.del_giuw_itemperilds_dtl(pi_dist_no);        
      GIUW_ITEMDS_DTL_PKG.del_giuw_itemds_dtl(pi_dist_no);       
      GIUW_POLICYDS_DTL_PKG.del_giuw_policyds_dtl(pi_dist_no);       
      GIUW_PERILDS_PKG.del_giuw_perilds(pi_dist_no);       
      GIUW_ITEMPERILDS_PKG.del_giuw_itemperilds(pi_dist_no);       
      GIUW_ITEMDS_PKG.del_giuw_itemds(pi_dist_no);       
      GIUW_POLICYDS_PKG.del_giuw_policyds(pi_dist_no);
          ...
          IF takeup_val = v_no_of_payment THEN --------------------------------------------- IF: last loop record (y)
            INSERT INTO giuw_pol_dist
                 ...
                 VALUES (p2_dist_no,    b_par_id,       p_policy_id,    p_endt_type,                                                            
                         ...
                         'N',           NVL(v_tsi_amt,0), NVL(v_prem_amt,0) - (ROUND((NVL(v_prem_amt,0)/ v_no_of_payment),2) * (v_no_of_payment - 1)),
                         NVL(v_ann_tsi_amt,0));
