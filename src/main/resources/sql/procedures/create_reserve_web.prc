DROP PROCEDURE CPI.CREATE_RESERVE_WEB;

CREATE OR REPLACE PROCEDURE CPI.Create_Reserve_web(p_claim_id             NUMBER,
                                                   p_item_no            NUMBER,
                                                   p_grouped_item_no    NUMBER,
                                                   p_peril_cd            NUMBER,
                                                   p_loss_reserve       NUMBER,
                                                   p_expense_reserve    NUMBER,
...
BEGIN
  ...
  INSERT INTO GICL_CLM_RES_HIST
    (claim_id,             clm_res_hist_id,    hist_seq_no,
     ...
     user_id,            last_update,        loss_reserve,    expense_reserve,    
     /*Commented out by MJ 12/28/2012: Redundant column
     expense_reserve,    
     */
     dist_sw,           booking_month,   
  ...
  VALUES
    (p_claim_id,         v_clm_res_hist,    v_hist_seq_no,
     ...
     USER,               SYSDATE,           p_loss_reserve,  p_expense_reserve,
     /*Commented out by MJ 12/28/2012: Redundant value
     v_prev_exp_res,     
     */
     'Y',                v_booking_month,
 ...
 BEGIN
  ...
  UPDATE gicl_clm_reserve
     SET loss_reserve = p_loss_reserve,
         expense_reserve = p_expense_reserve     
   WHERE claim_id = p_claim_id
     AND item_no  = p_item_no
     AND peril_cd = p_peril_cd;
 EXCEPTION
   WHEN NO_DATA_FOUND THEN     
           INSERT INTO gicl_clm_reserve(claim_Id, item_no, peril_cd, grouped_item_no, loss_reserve, expense_reserve, currency_cd, convert_rate)
         VALUES(p_claim_id, p_item_no, p_peril_cd, p_grouped_item_no, p_loss_reserve, p_expense_reserve, p_currency_cd, p_convert_rate);
 END;     
  -- call procedure which will generate records in claims distribution tables
  -- gicl_reserve_ds and gicl_reserve_rids
  Process_Reserve_Distribution2(p_claim_id,        p_item_no,               p_grouped_item_no,
                               p_peril_cd,      p_loss_reserve,      v_prev_exp_res,                                                      
