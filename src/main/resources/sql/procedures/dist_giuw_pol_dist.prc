DROP PROCEDURE CPI.DIST_GIUW_POL_DIST;

CREATE OR REPLACE PROCEDURE CPI.dist_giuw_pol_dist(
  ...
  FOR b1 IN (SELECT no_of_takeup, yearly_tag
	...
	IF v_no_of_payment = 1 THEN -------------------------------------------------------- IF: Single takeup (x)
	  ...
	  INSERT INTO giuw_pol_dist
	              ...
	              last_upd_date,post_flag , takeup_seq_no /* added takeup by jhing 11.05.2014 */)
	       VALUES (v_dist_no,p_par_id, --change to blongterm
	              NVL(p_bond_tsi_amt,0),
	              NVL(p_prem_amt,0),
	              NVL(p_bond_tsi_amt,0),
	              '1',1,v_eff_date,v_expiry_date,SYSDATE,USER,
	              SYSDATE,'O' ,v_no_of_payment /* added v_no_of_payment by jhing 11.05.2014 */ );
