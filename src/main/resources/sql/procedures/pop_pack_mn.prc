DROP PROCEDURE CPI.POP_PACK_MN;

CREATE OR REPLACE PROCEDURE CPI.POP_PACK_MN(p_par_id       IN  GIPI_WITEM.par_id%TYPE,
...
**  Description  : Equivalent to the Program Unit POP_PACK_MN in GIPIS096 module
*/
...
BEGIN
    ...
    --BETH 02192001 latest amount for item should be retrieved from the latest endt record
      ...
      IF v_max_endt_seq_no = v_max_endt_seq_no1 THEN                                 
       -- Check the latest endorsement with an existing item being edited
        FOR Z1 IN (
          SELECT   b.item_title    item_title,
                   b.ann_tsi_amt   ann_tsi_amt,
                   b.ann_prem_amt  ann_prem_amt,
        ...
        LOOP
              ...
              b480.rec_flag     :=  'C';
              FOR v IN (SELECT a.vessel_cd, vessel_name
                         FROM GIPI_CARGO A, GIIS_VESSEL B
                        WHERE policy_id = Z1.policy_id
                          AND item_no = p_item_no
                          AND a.vessel_cd = b.vessel_cd) 
              LOOP
                  b340.vessel_cd := v.vessel_cd;
                  EXIT;
              END LOOP;                 
           EXIT;
        END LOOP;
      ...
      END IF;  
        ...
        /* Vida -->>>07/07/97    */
        /* If item_no is new/additional then default currency to Peso */
        IF v_exist = 'N' THEN
          b480.rec_flag        := 'A';
    ...
    */
    ...
    --to insert records to gipi_wcargo
    INSERT INTO GIPI_WCARGO
    (par_id, item_no, rec_flag, print_tag,vessel_cd)
    
    VALUES( p_par_id,       p_item_no,  b480.rec_flag, 
            b340.print_tag, b340.vessel_cd);
