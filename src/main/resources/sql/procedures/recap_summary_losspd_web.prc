CREATE OR REPLACE PROCEDURE CPI.RECAP_SUMMARY_LOSSPD_WEB AS
   ...
   /*Added by Aliza collection table for OS Loss that is not existing. Will be used for PY and CY 04/04/2016*/
   TYPE tab_osloss              IS TABLE OF GIAC_RECAP_LOSS_CUR_DTL_EXT.OS_LOSS%TYPE;
   TYPE tab_osexp               IS TABLE OF GIAC_RECAP_LOSS_CUR_DTL_EXT.OS_EXPENSE%TYPE;
   ...
   --added by aliza 04/04/2016
   v_ret_os_loss_cy NUMBER (16,2);
   v_ret_os_exp_cy NUMBER (16,2);
   v_ret_os_loss_py NUMBER (16,2);
   v_ret_os_exp_py NUMBER (16,2);
   v_ret_loss_inc NUMBER (16,2);
   v_ret_exp_inc NUMBER (16,2);
   --end of code added by aliza 04/04/2016
   ...
   v_addl_row VARCHAR2(1) := 'N'; --mikel 02.25.2015 EDSA!
   ...
   BEGIN
      ...
      FOR OCTPL IN (SELECT b.rowtitle, a.rowno + b.rowno rowno
                    ...
                    WHERE a.line_cd = b.line_cd
                      ...
                      AND b.rowtitle = NVL(p_rowtitle, b.rowtitle)) --mikel 03.25.2015; to handle grouping for CTPL NON-LTO
      LOOP
      ...
      IF p_subline_cd = p_subline_lto THEN
       ...
       END LOOP;
         ...
         IF v_exs = FALSE THEN
           ...
           EXCEPTION            --added exception as temp by albert 04.28.2016
      ...
      FOR grrd IN (SELECT a.rowno, a.rowtitle
       ...
       IF v_exs = FALSE THEN
         ...
         IF v_ctpl = TRUE THEN
            ...
            LOOP
               --p_rowno := c2.rowno;
                  --p_rowtitle := c2.rowtitle; --mikel 03.25.2015; replaced by codes below

                   -- start -- mikel 03.25.2015
                      ...
                      AND a.subline_cd IN (
                              ...
                              SELECT rowtitle
                               ...
                               WHERE rowno > (SELECT rowno
                                                FROM giac_recap_row
                                               WHERE rowtitle = 'OTHERS'))
                      AND NVL (a.recap_line_cd, 'CPI') NOT IN ('MW', 'MI') --mikel 02.25.2015 EDSA!; migrant workers (MW) and micro insurance (MI)
...
BEGIN
   ...
   IF SQL%FOUND THEN
     ...
     LOOP
        --Initialize the values for the amounts
           ...
           v_ret_loss_inc       := 0;
           v_ret_exp_inc        := 0;
            --end of code added by aliza G. 04/04/2016

        v_rowno              := NULL;
        v_rowtitle           := NULL;

        --added by mikel 02.25.2015 EDSA!
         v_addl_row := 'N';
         BEGIN
         ...
         EXCEPTION WHEN NO_DATA_FOUND THEN
            NULL;
         END;
         --end mikel 02.25.2015 EDSA!

             --Get the bond_class_no if the line_cd = 'SU'
             IF vv_line_cd(rec) = v_line_su THEN --'SU' THEN --mikel 05.02.2016;SR 5518 use parameter value instead of hard coding
       ...
       --Get the rowno and the rowtitle
          ...
          IF v_addl_row = 'N' THEN
             ...
             END IF;
          --mikel 02.25.2015 EDSA!; call additional row
   ...
   END IF;
         ...
         /*    Start code added by Aliza G. 2016/04/06
            Codes for OS summarizing for Current Year
      */
       ...
       vv_peril_tp.delete;
       vv_tariff_cd.delete;
       vv_subln_tpcd.delete;
       vv_nr_loss.delete;
       vv_nr_exp.delete;
       vv_ri_cd.delete;
       vv_locfor_sw.delete;
       vv_treaty_loss.delete;
       vv_treaty_exp.delete;
       vv_facul_loss.delete;
       vv_facul_exp.delete;
       vv_cedant.delete;


         --Build the collection
   SELECT b.iss_cd, b.line_cd, b.subline_cd, b.claim_id,
          b.pol_seq_no, b.renew_no, a.item_no, a.peril_cd,
          c.tarf_cd tariff_cd,d.subline_type_cd, e.peril_type,
          0,0, null, null,0,0,0,0,
          b.ri_cd, a.os_loss, a.os_expense
     BULK COLLECT INTO vv_iss_cd,vv_line_cd,vv_subline_cd,vv_claim_id,
              vv_polseq_no,vv_renew_no,vv_item_no,vv_peril_cd,
              vv_tariff_cd,vv_subln_tpcd,vv_peril_tp,
              vv_nr_loss,vv_nr_exp,vv_ri_cd,vv_locfor_sw, vv_treaty_loss,vv_treaty_exp,vv_facul_loss,vv_facul_exp,
              vv_cedant, vv_osloss, vv_osexp
     FROM giac_recap_loss_cur_dtl_ext a, gicl_claims b, gicl_fire_dtl c, gicl_motor_car_dtl d,giis_peril e
       WHERE a.claim_id = b.claim_id AND a.claim_id = c.claim_id (+) and  a.item_no = d.claim_id(+) and a.item_no = d.item_no  (+) and a.peril_cd = e.peril_cd and a.line_cd = e.line_cd;
   --Check if theres data
   ...
   IF SQL%FOUND THEN
     ...
     LOOP
        --Initialize the values for the amounts
           ...
           v_ret_loss_oth       := 0;
           v_ret_exp_oth        := 0;
           v_ret_os_loss_cy     := 0; --added by aliza G. 04/04/2016
           v_ret_os_exp_cy      := 0; --added by aliza G. 04/04/2016
       ...
       INSERT INTO giac_recap_losspd_summ_ext(
                rowno, rowtitle, iss_cd, line_cd, ri_cd, cedant_cd, claim_id,
                subline_cd, pol_seq_no, renew_no,
                gross_loss, gross_exp, inw_grs_loss_auth, inw_grs_exp_auth,
                inw_grs_loss_asean, inw_grs_exp_asean, inw_grs_loss_oth, inw_grs_exp_oth,
                loss_auth, exp_auth, loss_asean, exp_asean, loss_oth, exp_oth,
                ret_loss_auth, ret_exp_auth, ret_loss_asean, ret_exp_asean, ret_loss_oth, ret_exp_oth,RET_OS_LOSS_CY,RET_OS_EXP_CY,RET_OS_LOSS_PY,RET_OS_EXP_PY)
       VALUES(
          v_rowno, v_rowtitle, vv_iss_cd(rec), vv_line_cd(rec), vv_ri_cd(rec),vv_cedant(rec),vv_claim_id(rec),
          vv_subline_cd(rec), vv_polseq_no(rec),vv_renew_no(rec),
          v_gross_loss, v_gross_exp, v_inw_grs_loss_auth, v_inw_grs_exp_auth,
                v_inw_grs_loss_asean, v_inw_grs_exp_asean, v_inw_grs_loss_oth, v_inw_grs_exp_oth,
                v_loss_auth, v_exp_auth, v_loss_asean, v_exp_asean, v_loss_oth, v_exp_oth,
                v_ret_loss_auth, v_ret_exp_auth, v_ret_loss_asean, v_ret_exp_asean, v_ret_loss_oth, v_ret_exp_oth, vv_osloss(rec), vv_osexp(rec),0,0);

    END LOOP;
      --COMMIT;
     END IF;

   --Codes for OS summarizing for Previous Year
   ...
   SELECT b.iss_cd, b.line_cd, b.subline_cd, b.claim_id,
     ...
     BULK COLLECT INTO vv_iss_cd,vv_line_cd,vv_subline_cd,vv_claim_id,
              ...
              vv_nr_loss,vv_nr_exp,vv_ri_cd,vv_locfor_sw, vv_treaty_loss,vv_treaty_exp,vv_facul_loss,vv_facul_exp,
              vv_cedant, vv_osloss, vv_osexp
     FROM giac_recap_loss_prev_dtl_ext a, gicl_claims b, gicl_fire_dtl c, gicl_motor_car_dtl d,giis_peril e
       WHERE a.claim_id = b.claim_id AND a.claim_id = c.claim_id (+) and  a.item_no = d.claim_id(+) and a.item_no = d.item_no  (+) and a.peril_cd = e.peril_cd and a.line_cd = e.line_cd;
   --Check if theres data
     ...
     LOOP
       ...
       VALUES(
          ...
          v_gross_loss, v_gross_exp, v_inw_grs_loss_auth, v_inw_grs_exp_auth,
                v_inw_grs_loss_asean, v_inw_grs_exp_asean, v_inw_grs_loss_oth, v_inw_grs_exp_oth,
                v_loss_auth, v_exp_auth, v_loss_asean, v_exp_asean, v_loss_oth, v_exp_oth,
                v_ret_loss_auth, v_ret_exp_auth, v_ret_loss_asean, v_ret_exp_asean, v_ret_loss_oth, v_ret_exp_oth, 0,0,vv_osloss(rec), vv_osexp(rec));
