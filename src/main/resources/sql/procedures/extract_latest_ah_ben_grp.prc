DROP PROCEDURE CPI.EXTRACT_LATEST_AH_BEN_GRP;

CREATE OR REPLACE PROCEDURE CPI.Extract_Latest_Ah_Ben_Grp (p_expiry_date      gicl_claims.expiry_date%TYPE,
  ...
  LOOP
    FOR get_info IN (SELECT c.endt_seq_no endt_seq_no,
                     ...
                     WHERE c.line_cd            = p_line_cd
                       ...
                       AND TRUNC(DECODE(TRUNC(NVL(g.from_date,c.eff_date)),TRUNC(c.incept_date),                            NVL(g.from_date,p_incept_date), NVL(g.from_date,c.eff_date) ))
                           <= p_loss_date
                       AND TRUNC(DECODE(NVL(g.TO_DATE,NVL(c.endt_expiry_date, c.expiry_date)),                              c.expiry_date,NVL(g.TO_DATE,p_expiry_date),
    ...
    FOR get_info2 IN (SELECT c.endt_seq_no endt_seq_no,
                       ...
                       WHERE c.policy_id          = g.policy_id
                         ...
                         AND c.pol_flag           IN ('1','2','3','X')
                         AND TRUNC(DECODE(TRUNC(NVL(g.from_date,c.eff_date)),
                             TRUNC(c.incept_date), NVL(g.from_date,p_incept_date),                                    NVL(g.from_date,c.eff_date))) <= p_loss_date
