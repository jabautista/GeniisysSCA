DROP PROCEDURE CPI.SETUP_TWO_GICLB001;

CREATE OR REPLACE PROCEDURE CPI.setup_two_giclb001(
...
IS
    ...
    **  Description     :  SETUP_TWO program unit; 
    **                     Setup TWO - OS_BOOKING_EXP = 'Y' and module entry for expenses is null      
    */

  /* 
  ** UNPAID LOSSES -DIRECT
  */
  cursor ULDL is (SELECT sum(nvl(a.os_loss,0) + nvl(a.os_expense,0)) os_loss, b.peril_cd, 
                         a.acct_intm_cd, 
  ...
  ** UNPAID LOSSES -RI
  */
  cursor ULRL is (SELECT sum(nvl(a.os_loss,0) + nvl(a.os_expense,0)) os_loss, b.peril_cd, a.acct_intm_cd, 
                         c.line_cd, c.subline_cd
                    FROM gicl_claims c, gicl_clm_res_hist b, gicl_take_up_hist a
                   WHERE c.claim_id = b.claim_id
                     AND b.claim_id = a.claim_id
