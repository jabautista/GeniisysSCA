DROP PROCEDURE CPI.GENERATE_RN_NO_PCK_GIEXS006;

CREATE OR REPLACE PROCEDURE CPI.generate_rn_no_pck_giexs006
...
BEGIN
 ...
 END IF;
  ...
  UPDATE giis_rn_seq
     SET rn_seq_no = v_rn_seq_no
   WHERE rownum     = v_rownum;
  IF t_pack_policy_id.exists(1) THEN
     FOR INDX IN t_pack_policy_id.FIRST..t_pack_policy_id.LAST LOOP
        INSERT INTO giex_pack_rn_no
         (pack_policy_id,         line_cd,  iss_cd,
     rn_yy,             rn_seq_no)
   VALUES
         (t_pack_policy_id(indx), t_line_cd(indx), t_iss_cd(indx),
