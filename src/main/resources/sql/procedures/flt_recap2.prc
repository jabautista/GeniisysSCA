DROP PROCEDURE CPI.FLT_RECAP2;

CREATE OR REPLACE PROCEDURE CPI.flt_recap2 (p_fm_date  DATE,
                                   p_to_date  DATE)
AS
...
--dbms_output.put_line('comm_pip: '||v_comm_pip);
  ...
  FOR trty IN (SELECT pol.iss_cd,
		      ...
		      gdl.peril_cd,
                      NULL,
                      NULL,
		      gdl.premium_amt premium_amt,
		      decode( (sign(p_fm_date - pol.acct_ent_date) * 4)
                      + sign(nvl(pol.spld_acct_ent_date, p_to_date+60) - p_to_date),
			  1, 1,
			 -3, 1,
			  3,-1,
			  4,-1, 0) * ids.dist_tsi tsi_amt,
		      gdl.commission_amt commission_amt,
		      gri.ri_cd,
		      gri.local_foreign_sw,
                      ipr.item_no
                ...
                WHERE gtc.cession_id     = gdl.cession_id
                  AND gtc.cession_year   >= v_fm_treaty_year
                  AND gtc.cession_mm     >= v_fm_treaty_mm
                  AND gtc.cession_year   <= v_to_treaty_year
                  AND gtc.cession_mm     <= v_to_treaty_mm
                  ...
                  AND gtc.line_cd        = nvl(ids.line_cd,ids.line_cd)   -- optimization purposes
      ...
      --Insert the date
       INSERT INTO giac_recap_dtl_flt_ext
         (iss_cd,                 line_cd,            subline_cd,
          ...
          ri_cd,                  local_foreign_sw)
       VALUES
         (v_iss_cd,            v_line_cd,     v_subline_cd,
          v_policy_id,         v_peril_cd,    v_tariff_cd,
          v_subline_type_cd,   v_facul_prem,  v_facul_comm,
          v_ri_cd,             v_local_foreign_sw);
     END LOOP; -- item
