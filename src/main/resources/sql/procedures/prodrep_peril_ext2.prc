DROP PROCEDURE CPI.PRODREP_PERIL_EXT2;

CREATE OR REPLACE PROCEDURE CPI.prodrep_peril_ext2 (
   p_from_date   DATE,
   p_to_date     DATE,
   p_user_id     VARCHAR2
)
...
BEGIN
   ...
   SELECT b.policy_id AS policy_id, b.policy_no AS policy_no,
          a.line_cd AS line_cd, b.subline_cd AS subline_cd,
          a.share_cd AS share_cd, b.share_type AS share_type,
          b.trty_name AS trty_name, b.trty_yy AS trty_yy,
          a.dist_no AS dist_no, a.dist_seq_no AS dist_seq_no,
          a.peril_cd AS peril_cd, c.peril_type AS peril_type,
          /*added currency_rt dexter 01092013*/
          DECODE (b.acct_neg_date,
                  p_to_date, (  DECODE (b.share_type,
                                        '1', NVL (a.dist_tsi, 0)
                                       )
                              * (-1)
                              * d.currency_rt
                  ...
                  (DECODE (b.share_type,
                           '1', NVL (a.dist_tsi, 0) * d.currency_rt
                          )
                  )
                 ) AS nr_dist_tsi,
          DECODE (b.acct_neg_date,
                  ...
                  (DECODE (b.share_type,
                           '1', NVL (a.dist_prem, 0) * d.currency_rt
                          )
                  )
                 ) AS nr_dist_prem,
          ...
          DECODE (b.acct_neg_date,
                  ...
                  (DECODE (b.share_type,
                           '2', NVL (a.dist_tsi, 0) * d.currency_rt
                          )
                  )
                 ) AS tr_dist_tsi,
          DECODE (b.acct_neg_date,
                  ...
                  (DECODE (b.share_type,
                           '2', NVL (a.dist_prem, 0) * d.currency_rt
                          )
                  )
                 ) AS tr_dist_prem,
          ...
          DECODE (b.acct_neg_date,
                  ...
                  (DECODE (b.share_type,
                           '3', NVL (a.dist_tsi, 0) * d.currency_rt
                          )
                  )
                 ) AS fa_dist_tsi,
          DECODE (b.acct_neg_date,
                  ...
                  (DECODE (b.share_type,
                           '3', NVL (a.dist_prem, 0) * d.currency_rt
                          )
                  )
                 ) AS fa_dist_prem,
          ...
          TO_CHAR (b.pol_spld_acct_ent_date, 'MM-DD-YYYY')
                                                    AS pol_spld_acct_ent_date,
   ...
   BULK COLLECT INTO vv_policy_id, vv_policy_no,
          vv_line_cd, vv_subline_cd,
          vv_share_cd, vv_share_type,
          vv_trty_name, vv_trty_yy,
          vv_dist_no, vv_dist_seq_no,
          vv_peril_cd, vv_peril_type,
          vv_nr_dist_tsi,
          vv_nr_dist_prem,
          vv_nr_dist_spct,
          vv_tr_dist_tsi,
    ...
    WHERE a.dist_no = b.dist_no
      ...
      ** Optimized this part of the query
      ** Removed the concatenation
      
      AND TO_CHAR (a.dist_no) || TO_CHAR (a.dist_seq_no) IN (         
             SELECT TO_CHAR (a.dist_no) || TO_CHAR (a.dist_seq_no)    
      ************************************/
      AND (TO_CHAR (a.dist_no), TO_CHAR (a.dist_seq_no)) IN (       -- MJ Fabroa 2014-11-04: Updated line
             SELECT TO_CHAR (a.dist_no), TO_CHAR (a.dist_seq_no)    -- MJ Fabroa 2014-11-04: Updated line
               FROM giuw_policyds a, giuw_pol_dist b
              WHERE a.dist_no = b.dist_no
                AND (   (    b.acct_ent_date >= p_from_date
                         AND b.acct_ent_date <= p_to_date
                        )
                     OR (    b.acct_neg_date >= p_from_date
      ...
      FORALL i IN vv_policy_id.FIRST .. vv_policy_id.LAST
         INSERT INTO giac_prodrep_peril_ext
              ...
              VALUES (vv_policy_id (i), vv_line_cd (i), vv_subline_cd (i),
                      ...
                      vv_pol_seq_no (i), vv_renew_no (i),
                      vv_endt_iss_cd (i), vv_endt_yy (i),
                      TO_DATE (vv_pol_acct_ent_date (i), 'MM-DD-YYYY'),
                      TO_DATE (vv_pol_spld_acct_ent_date (i), 'MM-DD-YYYY'),
                      p_user_id, TO_DATE (v_date, 'MM-DD-YYYY'),
