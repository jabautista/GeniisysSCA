DROP PROCEDURE CPI.VALIDATE_EXISTING_DIST_REC2;

CREATE OR REPLACE PROCEDURE CPI.VALIDATE_EXISTING_DIST_REC2(
...
BEGIN	
  ...
  LOOP
    v_hdr_sw := 'N';
    FOR B IN (SELECT ROUND(SUM(c110.prem_amt * b.currency_rt), 2) prem,     -- added round : shan 06.23.2014
    								 ROUND(SUM(c110.tsi_amt * b.currency_rt), 2) tsi,       -- added round : shan 06.23.2014
                     ROUND(SUM(c110.ann_tsi_amt * b.currency_rt), 2) ann_tsi            -- added round : shan 06.23.2014
                FROM giuw_wpolicyds c110, gipi_invoice b /*--gipi_item b mark jm 10.13.09 used gipi_invoice instead of gipi_item */
  ...
  LOOP
    ...
    END IF;
  END LOOP;  
END VALIDATE_EXISTING_DIST_REC2;
/
