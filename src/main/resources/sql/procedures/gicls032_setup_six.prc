DROP PROCEDURE CPI.GICLS032_SETUP_SIX;

CREATE OR REPLACE PROCEDURE CPI.gicls032_setup_six (
...
IS
   ...
   **  Description  : Converted procedure from GICLS032 - setup_six
   */   
   ...
   */
   ...
   IS
      (SELECT   MAX (a.take_up_hist) hist,
                d.shr_loss_res_amt * DECODE (b.currency_cd, p_variables.v_local_currency, 1, c.convert_rate) disbmt_amt, a.peril_cd,
   ...
   IS
      (SELECT   MAX (a.take_up_hist) hist,
          ...
          WHERE a.claim_id = e.claim_id
            ...
            AND b.payee_class_cd = p_payee_class_cd
            AND b.payee_cd = p_payee_cd
       GROUP BY d.shr_loss_ri_res_amt,
                a.peril_cd,
                --b.clm_loss_id,
                d.acct_trty_type,
                d.line_cd,
