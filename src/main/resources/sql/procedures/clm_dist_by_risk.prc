DROP PROCEDURE CPI.CLM_DIST_BY_RISK;

CREATE OR REPLACE PROCEDURE CPI.clm_dist_by_risk (
...
--Cursor for peril distribution in underwriting table.
   ...
   CURSOR cur_dist (v_risk_cd gipi_fireitem.risk_cd%TYPE, v_loss_date DATE)
   IS
                  ...
                  SELECT 1
                   ...
                   WHERE sub_a.line_cd = a.line_cd
                     ...
                     AND EXISTS (SELECT 1 FROM GICL_FIRE_DTL b_id
                                         WHERE b_id.claim_id= v_claim_id
...
BEGIN
   ...
   LOOP
      ...
      FOR dist IN cur_dist (v_risk_cd, v_loss_date)
      LOOP
         v_ctr := v_ctr + 1;
         v_share_cd.EXTEND;
         v_share_type.EXTEND;
