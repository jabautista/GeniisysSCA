DROP PROCEDURE CPI.CREATE_WINVOICE_112108;

CREATE OR REPLACE PROCEDURE CPI.Create_Winvoice_112108(p_policy_id  IN NUMBER,
   ...
   CURSOR d2 IS
        ...
        -- on issue_date else on eff_date (loth 032200)
          BEGIN
            ...
            IF d1_rec.tax_cd = p_doc_stamps AND v_param_old_doc  = 'Y' THEN --added by bdarusin, copied from old create_winvoice
              ...
              IF d1_rec.tax_cd = Giacp.n('EVAT') THEN --EVAT
        ...
        FOR d2_rec IN d2
              ...
              END IF;
                 ...
                 --  A.R.C. 04.10.2007
              -- add code to consider VAT_TAG
              IF d2_rec.tax_cd = Giacp.n('EVAT') THEN --EVAT
                    FOR v1 IN (SELECT NVL(vat_tag,3) vat_tag
                                 FROM GIIS_ASSURED b,
                                      GIPI_WPOLBAS a
                                WHERE b.assd_no = a.assd_no
   ...
   ELSE
          ...
          ELSE
              p_tax_amt := FLOOR(prem_amt_per_group / 4) * (0.5);
                ...
                --  A.R.C. 04.10.2007
            -- add code to consider VAT_TAG
            IF c2_rec.tax_cd = Giacp.n('EVAT') THEN --EVAT
                    FOR v1 IN (SELECT NVL(vat_tag,3) vat_tag
                                 FROM GIIS_ASSURED b,
                                      GIPI_WPOLBAS a
                                WHERE b.assd_no = a.assd_no
          ...
          END IF;
                ...
                --  A.R.C. 04.10.2007
            -- add code to consider VAT_TAG
            IF c2a_rec.tax_cd = Giacp.n('EVAT') THEN --EVAT
                    FOR v1 IN (SELECT NVL(vat_tag,3) vat_tag
                                 FROM GIIS_ASSURED b,
                                      GIPI_WPOLBAS a
                                WHERE b.assd_no = a.assd_no
    ...
    IF d1_rec.tax_cd = Giacp.n('EVAT') THEN --EVAT
     ...
     CURSOR c5 IS
                  ...
                  BEGIN
                   ...
                   --  A.R.C. 04.10.2007
             -- add code to consider VAT_TAG
               IF c1_rec.tax_cd = Giacp.n('EVAT') THEN --EVAT
                       FOR v1 IN (SELECT NVL(vat_tag,3) vat_tag
                                           FROM GIIS_ASSURED b,
                                                GIPI_WPOLBAS a
                                          WHERE b.assd_no = a.assd_no
     ...
     CURSOR c5 IS
       ...
       IF c1a_rec.tax_cd = Giacp.n('EVAT') THEN --EVAT
                       FOR v1 IN (SELECT NVL(vat_tag,3) vat_tag
                                          FROM GIIS_ASSURED b,
                                             GIPI_WPOLBAS a
                                          WHERE b.assd_no = a.assd_no
