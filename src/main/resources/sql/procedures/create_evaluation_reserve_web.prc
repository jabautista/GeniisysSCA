DROP PROCEDURE CPI.CREATE_EVALUATION_RESERVE_WEB;

CREATE OR REPLACE PROCEDURE CPI.Create_Evaluation_Reserve_web(p_claim_id     	NUMBER,
	   	  		  									  p_item_no			NUMBER,
...
BEGIN
  ...
  IF v_booking_month IS NULL OR v_booking_year IS NULL THEN 
     RAISE_APPLICATION_ERROR(-20631,'Geniisys Exception#E#Cannot generate booking date.');
  END IF;
