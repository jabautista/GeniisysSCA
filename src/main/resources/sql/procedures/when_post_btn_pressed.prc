CREATE OR REPLACE PROCEDURE CPI.When_Post_Btn_Pressed(
                    p_par_id                IN  GIPI_PARLIST.par_id%TYPE,
                    p_line_cd               IN  GIPI_PARLIST.line_cd%TYPE,
                    p_iss_cd                IN  GIPI_PARLIST.iss_cd%TYPE,
                    p_user_id               IN  VARCHAR2,
                    p_MSG_ALERT             OUT VARCHAR2,    
                    p_change_stat           OUT VARCHAR2,
                    p_msg_type              OUT VARCHAR2,
                    p_cred_branch_conf      IN  VARCHAR2,
                    p_chk_dflt_intm_sw      IN  VARCHAR2, --benjo 09.07.2016 SR-5604
                    p_msg_alert2            OUT VARCHAR2,
                    p_module_id             GIIS_MODULES.module_id%TYPE DEFAULT NULL      
                    )
       IS
    v_eff_date                          DATE;
    v_peril_name                        GIIS_PERIL.peril_name%TYPE;
    v_line_cd                           GIPI_POLBASIC.line_cd%TYPE;
    v_subline_cd                        GIPI_POLBASIC.subline_cd%TYPE;
    v_iss_cd                            GIPI_POLBASIC.iss_cd%TYPE;
    v_issue_yy                          GIPI_POLBASIC.issue_yy%TYPE;
    v_pol_seq_no                        GIPI_POLBASIC.pol_seq_no%TYPE;
    v_renew_no                          GIPI_POLBASIC.renew_no%TYPE;
    v_pol_flag                          GIPI_POLBASIC.pol_flag%TYPE;
    v_ann_tsi_amt                       GIPI_POLBASIC.ann_tsi_amt%TYPE;
    v_endt_type                         GIPI_POLBASIC.endt_type%TYPE;
    v_allow                             GIIS_PARAMETERS.param_value_v%TYPE := 'Y';
    v_dist                              GIUW_POL_DIST.auto_dist%TYPE;
    alert_button                        NUMBER;
    v_par_type                          GIPI_PARLIST.par_type%TYPE;
    v_exist                             VARCHAR2(1)                        := 'N';
    v_op_switch                         VARCHAR2(1)                        := 'N';
    v_booking_year                      GIPI_POLBASIC.booking_year%TYPE;          -- added by kim 01/05/05
    v_booking_mth                       GIPI_POLBASIC.booking_mth%TYPE;              -- added by kim 01/05/05
    v_update                            GIIS_PARAMETERS.param_value_v%TYPE;       -- added by kim 01/05/05
    v_booked_tag                        GIIS_BOOKING_MONTH.booked_tag%TYPE;       -- added by kim 01/05/05
    v_req_deduct                        GIIS_PARAMETERS.param_value_v%TYPE;       -- added by ohwver 06/20/06 parameter container variable
    v_ver_flag                          NUMBER;                                   --ver flag variable
    v_ver_flag1                         NUMBER                              := 0; --ver flag variable
    v_deduct_msg                        VARCHAR2(32767); --issa@fpac06.23.2006, changed size from 100 to max to prevent error
    v_post_limit                        GIIS_POSTING_LIMIT.post_limit%TYPE;                -- connie 09/22/06
    v_all_amt_sw                        GIIS_POSTING_LIMIT.all_amt_sw%TYPE;                -- connie 09/22/06
    v_item_no                           GIPI_WITEM.item_no%TYPE;                                    -- connie 09/22/06
    v_underwriter                       GIPI_PARLIST.underwriter%TYPE; -- added by aaron 060507
    v_amt1                              NUMBER;    -- mark jm 01.05.09 for amount comparison
    v_amt2                              NUMBER; -- mark jm 01.05.09 for amount comparison
    v_prem_seq_no                       GIPI_WINVOICE.prem_seq_no%TYPE; -- added by: Nica 11.30.2012 - for FPAC 8532
    v_doc_stamps                        GIPI_WINV_TAX.tax_amt%TYPE; -- added by: Nica 11.30.2012 - for FPAC 8532
    v_pol_id                            GIPI_WPOLBAS.cancelled_endt_id%TYPE;
    v_cancel_type                       VARCHAR2(1); -- aaron 050409
    v_line                              GIPI_PARLIST.line_cd%TYPE;                --added by ailene 12/17/08 
    v_open_policy                       GIIS_SUBLINE.open_policy_sw%TYPE;              --added by ailene 12/17/08 
    v_valid                             NUMBER;   
    v_menu_line                         GIIS_LINE.line_cd%TYPE;
    v_msg_alert                         VARCHAR2(32000);
    v_msg_icon                          VARCHAR2(1); 
    v_takeup_term                        gipi_wpolbas.takeup_term%type;
	v_val_post_limit					NUMBER := 1; --added by cherrie 02.12.2014
   	v_perl_exist						VARCHAR(1) := 'N'; --added by cherrie 02.12.2014
    v_temp_tax                          gipi_winv_tax.tax_amt%TYPE ; -- jhing 01.23.2015
    v_temp_prem_amt                     gipi_winvoice.prem_amt%TYPE ; -- jhing 01.23.2015
    v_dst_tax_cd                        giac_parameters.param_value_n%TYPE := giacp.n ('DOC_STAMPS') ; -- jhing 01.23.2015 
    v_return_nondef_tax                 GIIS_PARAMETERS.param_value_v%TYPE := NVL(giisp.v('RETURN_NONDEF_TAX'),'1'); --added by robert GENQA 4844 08.19.15
    v_validate_mop                      NUMBER := NULL; -- ADDED BY MarkS 5.16.2016 for SR-22084
    v_latitude                          gipi_wfireitm.latitude%TYPE; -- Added by Jerome 11.16.2016 SR 5749
    v_longitude                         gipi_wfireitm.longitude%TYPE; -- Added by Jerome 11.16.2016 SR 5749
    v_load_tag                          gipi_parlist.load_tag%TYPE; -- bonok :: 1.11.2017 :: for quick policy issuance
BEGIN
  /*
  **  Created by   : Jerome Orio
  **  Date Created : March 24, 2010
  **  Reference By : (GIPIS055 - POST PAR)
  **  Description  : WHEN-BUTTON-PRESSED trigger in post_button
  */
    /*
    **  Modified by  : Gzelle
    **  Date Created : 09.04.2013
    **  Modification : Added p_module_id parameter to determine if procedure is called from Batch Posting.
    **              If called from Batch Posting, insert error to giis_post_error_log
    */   
        /*
        **  Modified by  : Kenneth L.
        **  Date Created : 02.17.2014
        **  Modification : modified out of p_msg 
        **  
        */ 
     --A.R.C. 11.23.2006
     --to check if co-insurer exists
     IF NVL(Giisp.V('CHECK_CO_INSURER'),'N') = 'Y' THEN
        FOR c2 IN (SELECT 1
                                  FROM GIPI_WPOLBAS A
                                WHERE 1=1
                                  AND co_insurance_sw <> 1
                                  AND par_id = p_par_id
                                  AND NOT EXISTS (SELECT 1
                                                    FROM GIPI_MAIN_CO_INS z
                                                   WHERE z.par_id = A.par_id))
        LOOP
            --gipis207_pkg.pre_post_error2(p_par_id, 'The PAR has no Co-insurance.', p_module_id);  
            --p_MSG_ALERT := 'The PAR has no Co-insurance.';
            IF p_module_id = 'GIPIS207'
            THEN
                gipis207_pkg.pre_post_error2(p_par_id, 'The PAR has no Co-insurance.', p_module_id);  
                p_MSG_ALERT := 'Y';
            ELSE
                p_MSG_ALERT := 'The PAR has no Co-insurance.';
            END IF;
            RETURN;
        END LOOP;     
     END IF;
 -- ADDED BY MarkS 5.16.2016 for SR-22084
    SELECT NVL(param_value_n,1)
    INTO v_validate_mop
    FROM cpi.giis_parameters
    WHERE param_name = 'VALIDATE_MOP';
  --END SR-22084

    /**
    * Validation for crediting branch
    * Added by : Andrew Robes
    * Modified by : Gzelle 02-26-2014
    * Date : 07-22-2011
    */
    BEGIN
      FOR i IN (SELECT cred_branch
                  FROM GIPI_WPOLBAS
                 WHERE par_id = p_par_id)
      LOOP
        IF NVL(Giisp.V('MANDATORY_CRED_BRANCH'),'N') = 'N' AND i.cred_branch IS NULL AND p_cred_branch_conf <> 'Y' THEN
            --p_msg_type := 'confirm';  
            --p_msg_alert := 'The PAR has no Crediting Branch. Do you want to continue posting?';
            IF p_module_id = 'GIPIS207'     --Gzelle 02-26-2014
            THEN
                gipis207_pkg.pre_post_error2(p_par_id, 'The PAR has no Crediting Branch. Please post this PAR individually.', p_module_id); 
                p_msg_alert := 'Y';
            ELSE
                p_msg_type := 'confirm';
                p_msg_alert := 'The PAR has no Crediting Branch. Do you want to continue posting?';
            END IF;
            RETURN;
        ELSIF NVL(Giisp.V('MANDATORY_CRED_BRANCH'),'N') = 'Y' AND i.cred_branch IS NULL THEN
            --gipis207_pkg.pre_post_error2(p_par_id, 'The PAR has no Crediting Branch.', p_module_id);  
            --p_msg_alert := 'The PAR has no Crediting Branch.';
            IF p_module_id = 'GIPIS207'
            THEN
                gipis207_pkg.pre_post_error2(p_par_id, 'Crediting Branch is mandatory, please check Basic Information details.', p_module_id); 
                p_msg_alert := 'Y';
            ELSE
                p_msg_alert := 'Crediting Branch is mandatory, please check basic information details.';
            END IF;
            RETURN;
        END IF;                         
      END LOOP;
    END;

    /* benjo 09.07.2016 SR-5604 */
    IF NVL (giisp.v ('REQUIRE_DEFAULT_INTM_PER_ASSURED'), 'N') = 'Y' AND p_chk_dflt_intm_sw <> 'Y' THEN
      FOR i IN (SELECT par_id, assd_no, line_cd
                  FROM gipi_parlist
                 WHERE par_id = p_par_id)
      LOOP
         FOR x IN (SELECT intm_no
                     FROM giis_assured_intm
                    WHERE assd_no = i.assd_no AND line_cd = i.line_cd)
         LOOP
            FOR y IN (SELECT intrmdry_intm_no
                        FROM gipi_wcomm_invoices
                       WHERE par_id = i.par_id)
            LOOP
               IF x.intm_no <> y.intrmdry_intm_no THEN
                  p_msg_type := 'confirm';
                  p_msg_alert := 'Default Intermediary for the assured has been changed. Would you like to continue?';
                  RETURN;
               END IF;
            END LOOP;
         END LOOP;
      END LOOP;
    END IF;
    
    BEGIN --Added by Jerome 11.16.2016 SR 5749
      IF p_line_cd = 'FI' THEN
         FOR a IN (SELECT latitude, longitude
                      FROM gipi_wfireitm
                     WHERE par_id = p_par_id)
          LOOP
            v_latitude := a.latitude;
            v_longitude := a.longitude; 
            
            IF NVL(giisp.v('REQUIRE_LAT_LONG'),'N') = 'Y' THEN
               IF v_latitude IS NULL AND v_longitude IS NULL THEN
                   p_msg_alert := 'Cannot post Par. Latitude and Longitude are required.'; 
               END IF;
            END IF;
          END LOOP;
      END IF;
    END;
    -- mark jm 01.05.09 compare the amounts if they are the reverse of each other starts here (like pcic enhancement implementation)
     
    BEGIN  -- aaron 050409
      FOR X IN (SELECT cancel_type, cancelled_endt_id
                  FROM GIPI_WPOLBAS
                 WHERE par_id = p_par_id)
      LOOP
          v_cancel_type := X.cancel_type;
          v_pol_id := X.cancelled_endt_id;           
      END LOOP;
    END;
     
         
     IF v_cancel_type IN ('3','4') THEN --adpascual 06.05.2012 ucpbgen sr 9503
         -- get the policy_id / cancelled_endt_id
         SELECT    cancelled_endt_id
         INTO        v_pol_id
         FROM        GIPI_WPOLBAS
         WHERE        par_id = p_par_id;
         
         
         -- gipi_witem v. gipi_item
         SELECT    SUM(tsi_amt) tsi_amt, SUM(prem_amt) prem_amt
         INTO        v_amt1, v_amt2
         FROM        GIPI_ITEM
         WHERE        policy_id = v_pol_id;
     
         FOR A IN (SELECT     SUM(tsi_amt) tsi_amt, SUM(prem_amt) prem_amt
                             FROM         GIPI_WITEM
                             WHERE        par_id = p_par_id) LOOP
             IF ((A.tsi_amt + v_amt1) != 0) THEN 
                 --gipis207_pkg.pre_post_error2(p_par_id, 'TSI Amounts of Item/s are not equal.', p_module_id);  
                 --p_MSG_ALERT := 'TSI Amounts of Item/s are not equal.';    
                 IF p_module_id = 'GIPIS207'
                 THEN
                     gipis207_pkg.pre_post_error2(p_par_id, 'TSI Amounts of Item/s are not equal.', p_module_id); 
                     p_MSG_ALERT := 'Y';
                 ELSE
                     p_MSG_ALERT := 'TSI Amounts of Item/s are not equal.';
                 END IF;
                 RETURN;
             ELSE
                 IF ((A.prem_amt + v_amt2) != 0) THEN
                        --gipis207_pkg.pre_post_error2(p_par_id, 'Premium Amounts of Item/s are not equal.', p_module_id);  
                        --p_MSG_ALERT := 'Premium Amounts of Item/s are not equal.'; 
                        IF p_module_id = 'GIPIS207'
                        THEN
                            gipis207_pkg.pre_post_error2(p_par_id, 'Premium Amounts of Item/s are not equal.', p_module_id); 
                            p_MSG_ALERT := 'Y';
                        ELSE
                            p_MSG_ALERT := 'Premium Amounts of Item/s are not equal.';
                        END IF;   
                     RETURN;   
                 END IF;
             END IF;         
         END LOOP;
         
         -- gipi_witmperl v. gipi_itmperil
         SELECT    SUM(tsi_amt) tsi_amt, SUM(prem_amt) prem_amt
         INTO        v_amt1, v_amt2
         FROM        GIPI_ITMPERIL
         WHERE        policy_id = v_pol_id;
     
         FOR A IN (SELECT     SUM(tsi_amt) tsi_amt, SUM(prem_amt) prem_amt
                             FROM         GIPI_WITMPERL
                             WHERE        par_id = p_par_id) LOOP
             IF ((A.tsi_amt + v_amt1) != 0) THEN
                 --gipis207_pkg.pre_post_error2(p_par_id, 'TSI Amounts of Item Peril are not equal.', p_module_id);  
                 --p_MSG_ALERT := 'TSI Amounts of Item Peril are not equal.';
                 IF p_module_id = 'GIPIS207'
                 THEN
                     gipis207_pkg.pre_post_error2(p_par_id, 'TSI Amounts of Item Peril are not equal.', p_module_id); 
                     p_MSG_ALERT := 'Y';
                 ELSE
                     p_MSG_ALERT := 'TSI Amounts of Item Peril are not equal.';
                 END IF;     
                 RETURN;
             ELSE
                 IF ((A.prem_amt + v_amt2) != 0) THEN
                     --gipis207_pkg.pre_post_error2(p_par_id, 'Premium Amounts of Item Peril are not equal.', p_module_id);  
                     --p_MSG_ALERT := 'Premium Amounts of Item Peril are not equal.';  
                     IF p_module_id = 'GIPIS207'
                     THEN
                        gipis207_pkg.pre_post_error2(p_par_id, 'Premium Amounts of Item Peril are not equal.', p_module_id); 
                        p_MSG_ALERT := 'Y';
                     ELSE
                        p_MSG_ALERT := 'Premium Amounts of Item Peril are not equal.';
                     END IF;       
                     RETURN;
                 END IF;
             END IF;
         END LOOP;
         
         -- gipi_winvoice v. gipi_invoice
         SELECT    /*SUM(tax_amt) tax_amt, */ SUM(prem_amt) prem_amt /*, prem_seq_no, iss_cd */ -- edited by Nica 11.30.2012 - for FPAC 8532
         INTO        /*v_amt1,*/ v_amt2 /*, v_prem_seq_no, v_iss_cd */
         FROM        GIPI_INVOICE
         WHERE        policy_id = v_pol_id ; 
         --GROUP BY prem_seq_no, iss_cd; -- jhing 01.23.2015 commented out group and retrieval of tax. Tax comparison will be separated
         
         -- jhing 01.23.2015 commented out 
         /* added by Nica 11.30.2012 - in compliance with Revision No. GEN-2011-492_GIPIS055_V01_12142011*/
         /*   BEGIN
                SELECT tax_amt
                  INTO v_doc_stamps
                  FROM GIPI_INV_TAX
                WHERE iss_cd = v_iss_cd
                  AND prem_seq_no = v_prem_seq_no
                  AND tax_cd = (SELECT GIACP.n('DOC_STAMPS')
                                FROM dual);
            EXCEPTION
                WHEN NO_DATA_FOUND THEN
                    v_doc_stamps := 0;
            END; */
         /*****/
     
         FOR A IN (SELECT     SUM(tax_amt) tax_amt, SUM(prem_amt) prem_amt
                             FROM         GIPI_WINVOICE
                             WHERE        par_id = p_par_id) LOOP
             -- jhing 01.23.2015 commented out checking on tax. Will separate this into another block of code 
             /* IF ((A.tax_amt + v_amt1) != 0) THEN
                -- added condition to consider the tax_amt of documentary stamps
                IF((a.tax_amt * -1) != (v_amt1 - v_doc_stamps)) THEN
                    --gipis207_pkg.pre_post_error2(p_par_id, 'Tax Amounts of Invoice are not equal.', p_module_id);  
                    --p_MSG_ALERT := 'Tax Amounts of Invoice are not equal.';    
                    IF p_module_id = 'GIPIS207'
                    THEN
                       gipis207_pkg.pre_post_error2(p_par_id, 'Tax Amounts of Invoice are not equal.', p_module_id); 
                       p_MSG_ALERT := 'Y';
                    ELSE
                       p_MSG_ALERT := 'Tax Amounts of Invoice are not equal.';
                    END IF;  
                    RETURN;
                END IF;
             ELSE */ 
                 IF ((A.prem_amt + v_amt2) != 0) THEN
                     --gipis207_pkg.pre_post_error2(p_par_id, 'Premium Amounts of Invoice are not equal.', p_module_id);  
                     --p_MSG_ALERT := 'Premium Amounts of Invoice are not equal.'; 
                     IF p_module_id = 'GIPIS207'
                     THEN
                        gipis207_pkg.pre_post_error2(p_par_id, 'Premium Amounts of Invoice are not equal.', p_module_id); 
                        p_MSG_ALERT := 'Y';
                     ELSE
                        p_MSG_ALERT := 'Premium Amounts of Invoice are not equal.';
                     END IF;    
                     RETURN;
                 END IF;
            -- END IF; -- jhing 01.23.2015 commented out             
         END LOOP;         
         
         -- jhing 01.23.2015 added code to re-handle difference of tax between cancelled endorsement and cancellation PAR. This code 
         -- will consider multiple bill groups.This code assumes no changes on bill grouping 
         -- will be done on the cancellation PAR since endorsement cancellation/COI cancellation expects that the PAR will contain exact 
         -- reversal of amounts with minor exceptions such as DST computation ( which may or may not exists on the cancellation PAR 
         -- depending on the requirements of the client. 
         
         FOR canld_pol IN (SELECT   a.takeup_seq_no, a.item_grp, a.prem_amt, b.tax_cd, b.tax_amt
                            FROM gipi_invoice a, gipi_inv_tax b
                           WHERE a.policy_id = v_pol_id
                             AND a.iss_cd = b.iss_cd
                             AND a.prem_seq_no = b.prem_seq_no
                        ORDER BY a.item_grp, a.takeup_seq_no, b.tax_cd)
         LOOP
                
            v_temp_tax := 0 ;
            v_temp_prem_amt := 0 ;  
            
            FOR cur_tax_par IN (SELECT b.tax_amt , a.prem_amt 
                                      FROM gipi_winvoice a , gipi_winv_tax b  
                                     WHERE a.par_id = p_par_id 
                                        AND a.par_id = b.par_id (+) 
                                        AND a.item_grp = b.item_grp (+) 
                                        AND a.takeup_seq_no = b.takeup_seq_no (+)
                                        AND a.item_grp (+) = canld_pol.item_grp  
                                        AND a.takeup_seq_no (+) =  canld_pol.takeup_seq_no 
                                        AND b.tax_cd (+) = canld_pol.tax_cd )
            
            LOOP 
                v_temp_tax := NVL(cur_tax_par.tax_amt,0) ; 
                v_temp_prem_amt := NVL(cur_tax_par.prem_amt,0) ; 
                EXIT;
            END LOOP;
         
            IF (canld_pol.tax_amt != (v_temp_tax * -1 ) ) THEN 
                IF NOT (canld_pol.tax_cd = v_dst_tax_cd   AND v_temp_prem_amt < 0  ) THEN 
                    --added by robert GENQA 4844 08.19.15
					v_exist := 'N';
					
					FOR abc IN (( SELECT a.tax_cd
								  FROM gipi_inv_tax a, gipi_invoice b, giis_tax_charges c
								 WHERE policy_id = v_pol_id
								   AND a.prem_seq_no = b.prem_seq_no
								   AND a.iss_cd = b.iss_cd
								   AND a.tax_cd = c.tax_cd
								   AND a.line_cd = c.line_cd
								   AND a.iss_cd = c.iss_cd
								   AND (   DECODE (v_return_nondef_tax,'1', refund_sw,'3', primary_sw,1) =
											  DECODE (v_return_nondef_tax, '1', 'Y','3', 'Y',1)
										OR primary_sw = DECODE (v_return_nondef_tax,'1', 'Y')
									   )
								MINUS
								SELECT tax_cd
								  FROM gipi_winv_tax a
								 WHERE par_id = p_par_id)
							   UNION ALL
							   (SELECT tax_cd
								  FROM gipi_winv_tax a
								 WHERE par_id = p_par_id
								MINUS
								SELECT a.tax_cd
								  FROM gipi_inv_tax a, gipi_invoice b, giis_tax_charges c
								 WHERE policy_id = v_pol_id
								   AND a.prem_seq_no = b.prem_seq_no
								   AND a.iss_cd = b.iss_cd
								   AND a.tax_cd = c.tax_cd
								   AND a.line_cd = c.line_cd
								   AND a.iss_cd = c.iss_cd
								   AND (   DECODE (v_return_nondef_tax,'1', refund_sw,'3', primary_sw,1) =
											  DECODE (v_return_nondef_tax, '1', 'Y','3', 'Y',1)
										OR primary_sw = DECODE (v_return_nondef_tax,'1', 'Y')
									   )))
				    LOOP
					   v_exist := 'Y';
				    END LOOP;
					
					IF v_exist = 'Y' THEN
						IF p_module_id = 'GIPIS207'
						THEN
						   gipis207_pkg.pre_post_error2(p_par_id, 'Tax Amounts of Invoice are not equal.', p_module_id); 
						   p_MSG_ALERT := 'Y';
						ELSE
						   p_MSG_ALERT := 'Tax Amounts of Invoice are not equal.';
						END IF;  
						RETURN;  
				    END IF;  
					--end robert GENQA 4844 08.18.15                                
                END IF; 
            END IF; 
         
         END LOOP;
         
         
         -- end of added code jhing 01.23.2015
         
         
         -- gipi_wcomm_invoices v. gipi_comm_invoice
         SELECT    SUM(commission_amt) commission_amt, SUM(premium_amt) premium_amt
         INTO        v_amt1, v_amt2
         FROM        GIPI_COMM_INVOICE
         WHERE        policy_id = v_pol_id;
     
         FOR A IN (SELECT     SUM(commission_amt) commission_amt, SUM(premium_amt) premium_amt
                             FROM         GIPI_WCOMM_INVOICES
                             WHERE        par_id = p_par_id) LOOP
             IF ((A.commission_amt + v_amt1) != 0) THEN
                 --gipis207_pkg.pre_post_error2(p_par_id, 'Commission Amounts of Commission Invoice are not equal.', p_module_id);  
                 --p_MSG_ALERT := 'Commission Amounts of Commission Invoice are not equal.';    
                 IF p_module_id = 'GIPIS207'
                 THEN
                    gipis207_pkg.pre_post_error2(p_par_id, 'Commission Amounts of Commission Invoice are not equal.', p_module_id); 
                    p_MSG_ALERT := 'Y';
                 ELSE
                    p_MSG_ALERT := 'Commission Amounts of Commission Invoice are not equal.';
                 END IF;
                 RETURN;
             ELSE
                 IF ((A.premium_amt + v_amt2) != 0) THEN
                     --gipis207_pkg.pre_post_error2(p_par_id, 'Premium Amounts of Commission Invoice are not equal.', p_module_id);  
                     --p_MSG_ALERT := 'Premium Amounts of Commission Invoice are not equal.';
                     IF p_module_id = 'GIPIS207'
                     THEN
                        gipis207_pkg.pre_post_error2(p_par_id, 'Premium Amounts of Commission Invoice are not equal.', p_module_id); 
                        p_MSG_ALERT := 'Y';
                     ELSE
                        p_MSG_ALERT := 'Premium Amounts of Commission Invoice are not equal.';
                     END IF;    
                     RETURN;
                 END IF;
             END IF;
         END LOOP;         
     END IF;    
     
     -- mark jm 01.05.09 compare the amounts if they are the reverse of each other ends here (like pcic enhancement implementation)

/* Modified by       : aaron
** Date Modified    : 060507
** Remarks          : validate if user = gipi_parlist.underwriter, before posting...
*/

FOR X IN (SELECT underwriter, load_tag
            FROM GIPI_PARLIST
           WHERE par_id = p_par_id)
LOOP
    v_underwriter := X.underwriter;
    v_load_tag := x.load_tag; -- bonok :: 1.11.2017 :: for quick policy issuance
END LOOP;

IF v_underwriter <> p_user_id THEN --USER THEN
  --gipis207_pkg.pre_post_error2(p_par_id, 'Unable to post PAR. The PAR should be re-assigned to you before you could continue with the posting.', p_module_id);  
  --p_MSG_ALERT := 'Unable to post PAR. The PAR should be re-assigned to you before you could continue with the posting.';
  IF p_module_id = 'GIPIS207'
  THEN
     gipis207_pkg.pre_post_error2(p_par_id, 'Unable to post PAR. The PAR should be re-assigned to you before you could continue with the posting.', p_module_id); 
     p_MSG_ALERT := 'Y';
  ELSE
     p_MSG_ALERT := 'Unable to post PAR. The PAR should be re-assigned to you before you could continue with the posting.';
  END IF;   
  RETURN;
END IF;

----------------------------------


/*
** Modified by     : connie
** Date Modified : 09/22/06
** Modifications : validate if user is allowed to post a policy
*/
/* commented out by cherrie 02.12.2014
  BEGIN
      SELECT post_limit, all_amt_sw
        INTO v_post_limit, v_all_amt_sw
      FROM GIIS_POSTING_LIMIT
     WHERE line_cd = p_line_cd
       AND iss_cd = p_iss_cd
       AND UPPER(posting_user) = p_user_id; --USER;
  EXCEPTION
      WHEN NO_DATA_FOUND THEN
          --gipis207_pkg.pre_post_error2(p_par_id, 'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user '||giis_users_pkg.app_user||'.', p_module_id);  
          --p_MSG_ALERT := 'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user '||giis_users_pkg.app_user||'.';
          IF p_module_id = 'GIPIS207'
          THEN
             gipis207_pkg.pre_post_error2(p_par_id, 'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user '||giis_users_pkg.app_user||'.', p_module_id); 
             p_MSG_ALERT := 'Y';
          ELSE
             p_MSG_ALERT := 'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user '||giis_users_pkg.app_user||'.';
          END IF;
          RETURN;
  END;
  IF v_all_amt_sw IS NULL OR v_all_amt_sw = 'N' THEN --unchecked or 'N'
      IF v_post_limit IS NULL OR v_post_limit = 0 THEN
          --gipis207_pkg.pre_post_error2(p_par_id, 'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user '||giis_users_pkg.app_user||'.', p_module_id);  
          --p_MSG_ALERT := 'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user '||giis_users_pkg.app_user||'.';
          IF p_module_id = 'GIPIS207'
          THEN
             gipis207_pkg.pre_post_error2(p_par_id, 'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user '||giis_users_pkg.app_user||'.', p_module_id); 
             p_MSG_ALERT := 'Y';
          ELSE
             p_MSG_ALERT := 'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user '||giis_users_pkg.app_user||'.';
          END IF;
          RETURN;
      ELSIF v_post_limit IS NOT NULL OR v_post_limit <> 0 THEN
          --A.R.C. 11.23.2006
          --FOR p1 IN (SELECT item_no, ann_tsi_amt
          FOR p1 IN (SELECT SUM(ann_tsi_amt*currency_rt) ann_tsi_amt          
                                    FROM GIPI_WITEM
                                    WHERE par_id = p_par_id)
            LOOP
                --v_item_no := p1.item_no;
                v_ann_tsi_amt := p1.ann_tsi_amt;
                IF NVL(v_ann_tsi_amt,0) > NVL(v_post_limit,0) THEN
                   --gipis207_pkg.pre_post_error2(p_par_id, 'Total TSI amount exceeds the allowable TSI amount of the user '||giis_users_pkg.app_user||'. Reassign the PAR to another user with higher authority.', p_module_id);  
                   --p_MSG_ALERT := 'Total TSI amount exceeds the allowable TSI amount of the user '||giis_users_pkg.app_user||'. Reassign the PAR to another user with higher authority.';
                   IF p_module_id = 'GIPIS207'
                   THEN
                      gipis207_pkg.pre_post_error2(p_par_id, 'Total TSI amount exceeds the allowable TSI amount of the user '||giis_users_pkg.app_user||'. Reassign the PAR to another user with higher authority.', p_module_id); 
                      p_MSG_ALERT := 'Y';
                   ELSE
                      p_MSG_ALERT := 'Total TSI amount exceeds the allowable TSI amount of the user '||giis_users_pkg.app_user||'. Reassign the PAR to another user with higher authority.';
                   END IF;                     
                   RETURN; 
                END IF;
            END LOOP;
        END IF;
  END IF; 
  */  --cherrie 02.12.2014
  
   --added by cherrie 02.12.2014 | changes were base on the latest CS version (gipis055 v. 1.16.2014 11:24am)
   SELECT par_type
	 INTO v_par_type
	 FROM GIPI_PARLIST
	WHERE par_id = p_par_id;
   
   --      edited by gab 09.23.2015   
--   FOR p1 IN (SELECT SUM (ann_tsi_amt * currency_rt) ann_tsi_amt
--                FROM gipi_witem
--               WHERE par_id = p_par_id)
    FOR p1 IN (SELECT ann_tsi_amt
                FROM gipi_wpolbas
               WHERE par_id = p_par_id)
--  end edited

   LOOP
      v_ann_tsi_amt := p1.ann_tsi_amt;
   END LOOP;

   IF v_par_type = 'P'
   THEN
      v_val_post_limit :=
         validate_posting_limit (p_user_id,
                                 p_iss_cd,
                                 p_line_cd,
                                 v_par_type,
                                 v_ann_tsi_amt
                                );
   ELSIF v_par_type = 'E'
   THEN
      FOR itmperl IN (SELECT 1
                        FROM gipi_witmperl
                       WHERE par_id = p_par_id)
      LOOP
         v_perl_exist := 'Y';
      END LOOP;

      IF v_perl_exist = 'Y'
      THEN
         v_val_post_limit :=
            validate_posting_limit (p_user_id,
                                    p_iss_cd,
                                    p_line_cd,
                                    v_par_type,
                                    v_ann_tsi_amt
                                   );
      ELSE
         NULL;
      END IF;
   END IF;

   IF (v_val_post_limit = 1)
   THEN
      NULL;                                                     -- do nothing
   ELSIF (v_val_post_limit = 0)
   THEN
      p_msg_alert :=
         --'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user '; commented out by Gzelle 05182015 SR 3803
         'Total TSI amount exceeds the allowable TSI amount of the user ' || p_user_id || '. Reassign the PAR to another user with higher authority.';  --replaced with this message - Gzelle 
      RETURN;
   ELSIF (v_val_post_limit = 3) --benjo 10.08.2015 GENQA-SR-4992
   THEN
      p_msg_alert := 'User has no authority to post a policy. Reassign the PAR to another user or set-up the posting limit of user.';
      RETURN;
   END IF;
-- end of added codes | cherrie 02.12.2014
  
  
  --added by iris bordey (09.20.2002)
  --Validation for posting policy with pending claims on particular item and peril(s).
  --Disallow posting if there are pending claims on specified item and peril(s).
  FOR  dt IN (SELECT TRUNC(eff_date) eff_date, line_cd, subline_cd, iss_cd, 
                     pol_seq_no, issue_yy, renew_no, pol_flag,
                     ann_tsi_amt, endt_type,
                     booking_year, booking_mth, -- added by kim 01/05/05
                     nvl(takeup_term,'ST') takeup_term /*vjp 08032011*/    
                FROM GIPI_WPOLBAS
               WHERE par_id = p_par_id)
  LOOP
      v_eff_date     := dt.eff_date;
      v_line_cd      := dt.line_cd;
      v_subline_cd   := dt.subline_cd;
      v_iss_cd       := dt.iss_cd;
      v_issue_yy     := dt.issue_yy;
      v_pol_seq_no   := dt.pol_seq_no;
      v_renew_no     := dt.renew_no;
      v_pol_flag     := dt.pol_flag;
      v_ann_tsi_amt  := dt.ann_tsi_amt;
      v_endt_type    := dt.endt_type;
      v_booking_year := dt.booking_year;
      v_booking_mth  := dt.booking_mth;
      v_takeup_term  := dt.takeup_term;/*vjp 08032011*/    
        
      EXIT;
  END LOOP;
  

   
  -- Added by gracey 061604
    -- check if posting is allowed for undistributed policies
        
    -- modified by gmi 020608
    /* 
         Revise checking of parameter ALLOW_POSTING_OF UNDIST.  Before, if value = 'N', the 
       user is not allowed to post if the policy is not fully distributed.  For multi-year, 
       even if the policy is not yet fully distributed (since this is possible), posting 
       should be allowed if the distribution for the current year/effective year is distributed   
    */    
    /*FOR a IN (SELECT param_value_v
                FROM giis_parameters
               WHERE param_name = 'ALLOW_POSTING_OF_UNDIST')
    LOOP*/
   IF NVL(v_load_tag, 'XX') <> 'Q' THEN -- bonok :: 1.11.2017 :: for quick policy issuance.  if load_tag is Q, skip checking of ALLOW_POSTING_OF_UNDIST
      v_allow := Giisp.v('ALLOW_POSTING_OF_UNDIST');
   END IF;
    /*    EXIT;
    END LOOP;    */

  FOR b IN (SELECT NVL(auto_dist,'N') auto_dist, eff_date, expiry_date
                FROM GIUW_POL_DIST
               WHERE par_id = p_par_id
                 AND 
                  (TRUNC(SYSDATE) > TRUNC(expiry_date)
                              OR TRUNC(SYSDATE) BETWEEN TRUNC(eff_date) AND TRUNC(expiry_date)) 
                              OR dist_no = (SELECT MIN(dist_no)
                                              FROM GIUW_POL_DIST
                                             WHERE par_id = p_par_id)) -- long term add ons..
    LOOP        
        v_dist := b.auto_dist;
        IF v_dist = 'N' THEN
            EXIT;
        END IF;    
    END LOOP;    
  FOR d IN (SELECT A.par_type par_type, c.op_flag op_flag
                FROM GIPI_PARLIST A, GIPI_WPOLBAS b, GIIS_SUBLINE c
               WHERE A.par_id = b.par_id
                 AND b.subline_cd = c.subline_cd
                 AND b.line_cd = c.line_cd
                 AND A.par_id = p_par_id)
    LOOP
        v_par_type := d.par_type;
      v_op_switch := d.op_flag;
        EXIT;
    END LOOP;    

  FOR d IN (SELECT 1
              FROM GIPI_WITMPERL
             WHERE par_id = p_par_id)
    LOOP
        v_exist := 'Y';
        EXIT;
    END LOOP;
    --**-- gmi 09/26/05 --**--
    FOR d1 IN (SELECT 1
              FROM GIPI_WITMPERL_GROUPED
             WHERE par_id = p_par_id)
    LOOP
        v_exist := 'Y';
        EXIT;
    END LOOP;
  --** --gmi-- **--

/*Modified by : John Oliver Mendoza
**    date    : 062020006
**Modification: this validates if all the items of the given PAR have at least 1 record
                              in GIPI_DEDUCTIBLES, if not posting will not continue.
*/
  --added by VJ, 111709
  BEGIN
      SELECT NVL(menu_line_cd,line_cd) line_cd
        INTO v_menu_line
        FROM GIIS_LINE
       WHERE line_cd = v_line_cd;
  END;
  IF v_menu_line <> 'SU' THEN

    -- ohwver start 062006--
    
    /*FOR ver_rec1 in (SELECT param_value_v
                       FROM giis_parameters
                      WHERE param_name = 'REQUIRE_DEDUCTIBLES')
    LOOP*/
   IF NVL(v_load_tag, 'XX') <> 'Q' THEN -- bonok :: 1.11.2017 :: for quick policy issuance.  if load_tag is Q, skip checking of REQUIRE_DEDUCTIBLES
      v_req_deduct := Giisp.v('REQUIRE_DEDUCTIBLES');--ver_rec1.param_value_v;
   END IF;
    --END LOOP;
    
  IF v_par_type = 'P' THEN
    IF NVL(v_req_deduct,'N') = 'Y' AND v_op_switch = 'N' THEN --VJP 062711
         --check if policy deductibles exist christian 03/09/2013
         FOR ver_rec3 IN (SELECT 'VER'
                            FROM GIPI_WDEDUCTIBLES
                           WHERE par_id = p_par_id
                             AND item_no = 0)
         LOOP
           v_ver_flag:=1;
         END LOOP;
         
         IF v_ver_flag = 1 THEN
            v_ver_flag :=0;                       
         ELSE        
            FOR ver_rec2 IN (SELECT item_no 
                                    FROM GIPI_WITEM
                                   WHERE par_id = p_par_id)
             LOOP
                 FOR ver_rec3 IN (SELECT 'VER'
                                                      FROM GIPI_WDEDUCTIBLES
                                                     WHERE par_id = p_par_id
                                                       AND item_no = ver_rec2.item_no)
                 LOOP
                         v_ver_flag:=1;
                 END LOOP;
                 IF v_ver_flag = 1 THEN
                        v_ver_flag :=0;                       
                 ELSE
                         v_deduct_msg:=v_deduct_msg||ver_rec2.item_no||', ';
                       v_ver_flag :=0;
                       v_ver_flag1 := v_ver_flag1+1;
                 END IF;
             END LOOP;
             IF v_ver_flag1 > 0 THEN
               v_deduct_msg:=SUBSTR(v_deduct_msg,1,INSTR(v_deduct_msg,', ',-1)-1);
                 --gipis207_pkg.pre_post_error2(p_par_id, 'The ff. must have at least 1 deductible: Item no(s) '||v_deduct_msg||'.', p_module_id);  
                 --p_MSG_ALERT := 'The ff. must have at least 1 deductible: Item no(s) '||v_deduct_msg||'.';
                 IF p_module_id = 'GIPIS207'
                 THEN
                    gipis207_pkg.pre_post_error2(p_par_id, 'The ff. must have at least 1 deductible: Item no(s) '||v_deduct_msg||'.', p_module_id); 
                    p_MSG_ALERT := 'Y';
                 ELSE
                    p_MSG_ALERT := 'The ff. must have at least 1 deductible: Item no(s) '||v_deduct_msg||'.';
                 END IF;   
                 RETURN;           
             END IF;--end if v_ver_flag1 > 0
         END IF;
    END IF;--end if nvl(v_req_deduct,'N') = 'Y'
  ELSIF v_par_type = 'E' AND v_pol_flag <> '4' AND v_ann_tsi_amt <> 0 THEN --added condition v_pol_flag and v_ann_tsi_amt GEN-2012-116_GIPIS055_V01_03122012 niknok 
    IF NVL(v_req_deduct,'N') = 'Y' AND v_op_switch = 'N' THEN  --VJP 062711
         --check if policy deductibles exist christian 03/09/2013
         FOR ver_rec3 IN (SELECT 'VER'
                            FROM GIPI_WDEDUCTIBLES
                           WHERE par_id = p_par_id
                             AND item_no = 0)
         LOOP
           v_ver_flag:=1;
         END LOOP;
         
         IF v_ver_flag = 1 THEN
            v_ver_flag :=0;                       
         ELSE   
             FOR ver_rec2 IN (SELECT item_no 
                                FROM GIPI_WITEM
                               WHERE par_id = p_par_id
                                 AND rec_flag = 'A')
             LOOP
                 FOR ver_rec3 IN (SELECT 'VER'
                                                      FROM GIPI_WDEDUCTIBLES
                                                   WHERE par_id = p_par_id
                                                       AND item_no = ver_rec2.item_no)
                 LOOP
                     v_ver_flag:=1;
                 END LOOP;
                 IF v_ver_flag = 1 THEN
                   v_ver_flag :=0;                       
                 ELSE
                     v_deduct_msg:=v_deduct_msg||ver_rec2.item_no||', ';
                     v_ver_flag :=0;
                     v_ver_flag1 := v_ver_flag1+1;
                 END IF;
             END LOOP;
             IF v_ver_flag1 > 0 THEN
                 v_deduct_msg:=SUBSTR(v_deduct_msg,1,INSTR(v_deduct_msg,', ',-1)-1);
                 --gipis207_pkg.pre_post_error2(p_par_id, 'The ff. must have at least 1 deductible: Item no(s) '||v_deduct_msg||'.', p_module_id);  
                 --p_MSG_ALERT := 'The ff. must have at least 1 deductible: Item no(s) '||v_deduct_msg||'.';
                 IF p_module_id = 'GIPIS207'
                 THEN
                    gipis207_pkg.pre_post_error2(p_par_id, 'The ff. must have at least 1 deductible: Item no(s) '||v_deduct_msg||'.', p_module_id); 
                    p_MSG_ALERT := 'Y';
                 ELSE
                    p_MSG_ALERT := 'The ff. must have at least 1 deductible: Item no(s) '||v_deduct_msg||'.';
                 END IF;  
                 RETURN;               
             END IF;--end if v_ver_flag1 > 0
         END IF;
    END IF;--end if nvl(v_req_deduct,'N') = 'Y'
  END IF;--end if v_par_type
  END IF;
  --end vj
  --ohwver end--   
  
  --glyza 060608
    FOR X IN (SELECT NVL(auto_dist,'N') auto_dist, eff_date, expiry_date
                FROM GIUW_POL_DIST
               WHERE par_id = p_par_id)
    LOOP
        IF NVL(v_allow,'Y') = 'N' THEN
            IF X.auto_dist = 'N' THEN 
                --gipis207_pkg.pre_post_error2(p_par_id, 'Distribute the PAR before posting the policy.', p_module_id);  
                --p_MSG_ALERT := 'Distribute the PAR before posting the policy.';
                IF p_module_id = 'GIPIS207'
                THEN
                   gipis207_pkg.pre_post_error2(p_par_id, 'Distribute the PAR before posting the policy.', p_module_id); 
                   p_MSG_ALERT := 'Y';
                ELSE
                   p_MSG_ALERT := 'Distribute the PAR before posting the policy.';
                END IF; 
                RETURN;
                EXIT;
            END IF;
        END IF;
    END LOOP;
    
  IF v_par_type = 'P' THEN
     IF NVL(v_allow,'Y') = 'N' AND NVL(v_dist,'N') = 'N' AND v_op_switch = 'N' THEN
        --gipis207_pkg.pre_post_error2(p_par_id, 'Distribute the PAR before posting the policy.', p_module_id);  
        --p_MSG_ALERT := 'Distribute the PAR before posting the policy.';
        IF p_module_id = 'GIPIS207'
        THEN
           gipis207_pkg.pre_post_error2(p_par_id, 'Distribute the PAR before posting the policy.', p_module_id); 
           p_MSG_ALERT := 'Y';
        ELSE
           p_MSG_ALERT := 'Distribute the PAR before posting the policy.';
        END IF; 
        RETURN;
     END IF;    
  ELSIF v_par_type = 'E' AND v_exist = 'Y' THEN
     IF NVL(v_allow,'Y') = 'N' AND NVL(v_dist,'N') = 'N' AND v_op_switch = 'N' THEN
        --gipis207_pkg.pre_post_error2(p_par_id, 'Distribute the PAR before posting the policy.', p_module_id);  
        --p_MSG_ALERT := 'Distribute the PAR before posting the policy.';
        IF p_module_id = 'GIPIS207'
        THEN
           gipis207_pkg.pre_post_error2(p_par_id, 'Distribute the PAR before posting the policy.', p_module_id); 
           p_MSG_ALERT := 'Y';
        ELSE
           p_MSG_ALERT := 'Distribute the PAR before posting the policy.';
        END IF; 
        RETURN;
     END IF;    
  END IF;   
 
    v_update := Giisp.v('UPDATE_BOOKING');--a.param_value_v;

    -- Added by kim 01/05/05
    -- checks if the value of the booked tag of the PAR
    FOR A IN (SELECT booked_tag
                            FROM GIIS_BOOKING_MONTH
                         WHERE booking_year = v_booking_year
                           AND booking_mth  = v_booking_mth)
    LOOP
        v_booked_tag := A.booked_tag;
        EXIT;
    END LOOP;

    -- added by kim 01/05/05
    -- if the value of the parameter update_booking is set to Y
    -- and the booked_tag of the booking date is set to Y
    -- then it updates the booking date to the next available booking date    
    
    IF v_update = 'Y' AND v_booked_tag = 'Y' THEN
        FOR C IN (SELECT BOOKING_YEAR, 
                         TO_CHAR(TO_DATE('01-' || SUBSTR(booking_mth,1, 3) || '-' || booking_year, 'DD-MON-YYYY'), 'MM'), 
                         BOOKING_MTH 
                    FROM GIIS_BOOKING_MONTH
                   WHERE (NVL(BOOKED_TAG, 'N') != 'Y')
                        AND (BOOKING_YEAR > v_booking_year
                      OR (BOOKING_YEAR = v_booking_year 
                 AND TO_NUMBER(TO_CHAR(TO_DATE('01-'||SUBSTR(booking_mth,1, 3)||'-'|| booking_year, 'DD-MON-YYYY'), 'MM'))>= TO_NUMBER(TO_CHAR(TO_DATE('01-'||SUBSTR(v_booking_mth,1, 3)||'-'||v_booking_year, 'DD-MON-YYYY'), 'MM')))) 
              ORDER BY 1, 2 ) 
      LOOP
        v_booking_year := TO_NUMBER(C.BOOKING_YEAR);       
        v_booking_mth  := C.BOOKING_MTH;       
        EXIT;   
      END LOOP; 
      
        --gipis207_pkg.pre_post_error2(p_par_id, 'Booking date has been closed. Will now update the booking date to the next available booking month.', p_module_id);  
        --p_msg_alert2 := 'Booking date has been closed. Will now update the booking date to the next available booking month.';
        IF p_module_id = 'GIPIS207'
        THEN
           --gipis207_pkg.pre_post_error2(p_par_id, 'Booking date has been closed. Will now update the booking date to the next available booking month.', p_module_id); 
           p_msg_alert2 := 'Booking date has been closed. Will now update the booking date to the next available booking month.';
        ELSE
           p_msg_alert2 := 'Booking date has been closed. Will now update the booking date to the next available booking month.';
        END IF; 
     
      UPDATE GIPI_WPOLBAS 
         SET booking_mth  = v_booking_mth,
             booking_year = v_booking_year
       WHERE par_id = p_par_id;
         
     /*vjp 08032011*/        
      IF v_takeup_term = 'ST' THEN
        UPDATE gipi_winvoice
           SET multi_booking_mm  = v_booking_mth,
               multi_booking_yy  = v_booking_year
         WHERE par_id = p_par_id;
      END IF;
        
      COMMIT;   
    END IF;
    
    /*added by dannel 10162006
        validation for plate_no,serial_no and motor_no    */
    IF p_line_cd='MC' THEN
       IF v_msg_alert IS NULL THEN
          Validate_Carnap(p_par_id,v_msg_alert);
          IF v_msg_alert IS NOT NULL THEN
            --gipis207_pkg.pre_post_error2(p_par_id, v_msg_alert, p_module_id);  
            --p_MSG_ALERT := v_msg_alert;
            IF p_module_id = 'GIPIS207'
            THEN
               gipis207_pkg.pre_post_error2(p_par_id, v_msg_alert, p_module_id); 
               p_MSG_ALERT := v_msg_alert;
            ELSE
               p_MSG_ALERT := v_msg_alert;
            END IF; 
            RETURN;
          END IF;
       END IF;
       IF v_msg_alert IS NULL THEN
          Validate_Plate_No(p_par_id,v_msg_alert, v_msg_icon, v_par_type);
          IF v_msg_icon = 'E' THEN
            --gipis207_pkg.pre_post_error2(p_par_id, v_msg_alert, p_module_id);  
            --p_MSG_ALERT := v_msg_alert;
            IF p_module_id = 'GIPIS207'
            THEN
               gipis207_pkg.pre_post_error2(p_par_id, v_msg_alert, p_module_id); 
               p_MSG_ALERT := v_msg_alert;
            ELSE
               p_MSG_ALERT := v_msg_alert;
            END IF;
            RETURN;
          END IF;
       END IF;
       IF v_msg_alert IS NULL THEN
             Validate_Serial_Motor(p_par_id,v_msg_alert);
          IF v_msg_alert IS NOT NULL THEN
            --gipis207_pkg.pre_post_error2(p_par_id, v_msg_alert, p_module_id);  
            --p_MSG_ALERT := v_msg_alert;
            IF p_module_id = 'GIPIS207'
            THEN
               gipis207_pkg.pre_post_error2(p_par_id, v_msg_alert, p_module_id); 
               p_MSG_ALERT := v_msg_alert;
            ELSE
               p_MSG_ALERT := v_msg_alert;
            END IF;
            RETURN;
          END IF;
       END IF;
       IF v_msg_alert IS NULL THEN
             Validate_Serial_No(p_par_id,v_msg_alert,v_par_type);
       END IF;
       IF v_msg_alert IS NULL THEN
          Validate_Motor_No(p_par_id,v_msg_alert,v_par_type);
       END IF;
       IF v_msg_alert IS NULL THEN
             Validate_Coc_Serial_No(p_par_id,v_msg_alert,v_par_type);
       END IF;
       
    END IF;
 
  -- for SOA
  --DECLARE
      --v_endt_type GIPI_WPOLBAS.endt_type%TYPE:='N';
  BEGIN
      SELECT DISTINCT 'A'
        INTO v_endt_type
        FROM GIPI_WINVOICE
       WHERE par_id = p_par_id;
  EXCEPTION
      WHEN NO_DATA_FOUND THEN
      v_endt_type:= 'N';
  END;
  --end SOA
 
  IF v_pol_flag <> '4' AND v_ann_tsi_amt = 0 AND v_par_type = 'E' AND v_endt_type <> 'N'/* aron 081707 removed comment out--- comment out dannel 10162006*/ THEN
     p_change_stat := 'Y';  
     v_pol_flag := '4';
  END IF;

       --- added by ailene 12/17/2008
      --- check open policy details
      ---    start
      --- codes moved by VJ; to consider affecting endts only. 120209
  
  FOR X IN ( SELECT NVL(b.menu_line_cd, b.line_cd)line_cd
                             FROM GIPI_PARLIST A, GIIS_LINE b
                            WHERE par_id = p_par_id
                              AND A.line_cd =b.line_cd)
  LOOP
      v_line := X.line_cd;
  END LOOP;
  
  FOR i IN ( SELECT b.open_policy_sw
                         FROM GIPI_WPOLBAS A, GIIS_SUBLINE b
                        WHERE A.line_cd = b.line_cd
                          AND A.subline_cd = b.subline_cd
                          AND A.par_id = p_par_id
                         )
  LOOP
      v_open_policy := i.open_policy_sw;
  END LOOP;
 
  IF v_line = 'MN' AND v_open_policy = 'Y' AND v_endt_type = 'A' THEN
        v_valid := Validate_Mop.validate_risknote_dtl(p_par_id);
  END IF;
  
  IF v_valid = 2 THEN
      --gipis207_pkg.pre_post_error2(p_par_id, 'Risk Note'||'s duration should be within the open policy'||'s duration', p_module_id);  
      --p_MSG_ALERT := 'Risk Note'||'s duration should be within the open policy'||'s duration';
      IF p_module_id = 'GIPIS207'
      THEN
         gipis207_pkg.pre_post_error2(p_par_id, 'Risk Note'||'s duration should be within the open policy'||'s duration', p_module_id); 
         p_MSG_ALERT := 'Y';
      ELSE
         p_MSG_ALERT := 'Risk Note'||'s duration should be within the open policy'||'s duration';
      END IF;
      RETURN;
  ELSIF v_valid = 3 THEN
      --gipis207_pkg.pre_post_error2(p_par_id, 'Etd/Eta should be within the open policy'||'s duration', p_module_id);  
      --p_MSG_ALERT := 'Etd/Eta should be within the open policy'||'s duration';
      --EDITED BY MarkS 05.16.2016 added condition to check parameter validate_mop to compare etd with mop policy or rn SR-22084
      IF v_validate_mop = 1 THEN
          IF p_module_id = 'GIPIS207'
          THEN
              gipis207_pkg.pre_post_error2(p_par_id, 'ETD/ETA should be within the RISK NOTE'||'s duration', p_module_id); 
              p_MSG_ALERT := 'Y';
          ELSE
              p_MSG_ALERT := 'ETD/ETA should be within the RISK NOTE'||'s duration';
          END IF;
      ELSE
          IF p_module_id = 'GIPIS207'
          THEN
              gipis207_pkg.pre_post_error2(p_par_id, 'ETD/ETA should be within the OPEN POLICY'||'s duration', p_module_id); 
              p_MSG_ALERT := 'Y';
          ELSE
              p_MSG_ALERT := 'ETD/ETA should be within the OPEN POLICY'||'s duration';
          END IF;
      END IF;
      --end SR-22084
      RETURN;
  ELSIF v_valid = 4 THEN
      --gipis207_pkg.pre_post_error2(p_par_id, 'Total Sum Insured should be within the open policy'||'s limit of liability', p_module_id);  
      --p_MSG_ALERT := 'Total Sum Insured should be within the open policy'||'s limit of liability';
      IF p_module_id = 'GIPIS207'
      THEN
         gipis207_pkg.pre_post_error2(p_par_id, 'Total Sum Insured should be within the open policy'||'s limit of liability', p_module_id); 
         p_MSG_ALERT := 'Y';
      ELSE
         p_MSG_ALERT := 'Total Sum Insured should be within the open policy'||'s limit of liability';
      END IF;
      RETURN;
  ELSIF v_valid = 1 THEN 
      NULL;
  END IF;
  
  --end  
  /*bdarusin, jan172003
    display a warning before cancelling a paid policy*/
          
  IF v_pol_flag = '4' AND v_ann_tsi_amt = 0 AND v_par_type = 'E' AND v_endt_type <> 'N'/* aron 081707 removed comment out--- comment out dannel 10162006*/  THEN
      FOR A IN (SELECT SUM(c.total_payments) paid_amt
                  FROM GIPI_POLBASIC A, GIPI_INVOICE b, GIAC_AGING_SOA_DETAILS c
                 WHERE A.line_cd     = v_line_cd
                   AND A.subline_cd  = v_subline_cd
                   AND A.iss_cd      = v_iss_cd
                   AND A.issue_yy    = v_issue_yy
                         AND A.pol_seq_no  = v_pol_seq_no
                         AND A.renew_no    = v_renew_no
                         AND A.pol_flag IN ('1','2','3')
                         AND A.policy_id = b.policy_id
                         AND b.iss_cd = c.iss_cd
                         AND b.prem_seq_no = c.prem_seq_no) LOOP
          IF A.paid_amt <> 0 THEN
            NULL;
          END IF;
        END LOOP;
  END IF;
END;
/
