DROP PROCEDURE CPI.SETUP_FIVE_GICLB001;

CREATE OR REPLACE PROCEDURE CPI.setup_five_giclb001(
...
IS
    ...
    **  Description     :  SETUP_FIVE program unit; 
    **                     Setup FIVE - OS_BOOKING_EXP = 'Y' and module entry for expenses is null      
    */

  /* 
  ** UNPAID LOSSES - NET DIRECT
  */
  cursor ULDL is (SELECT sum((a.os_loss * (d.shr_pct/100)) + (a.os_expense * (d.shr_pct/100))) os_loss, 
                         b.peril_cd, a.acct_intm_cd, 
  ...
  */
  cursor ULRL is (SELECT sum((a.os_loss * (d.shr_pct/100)) + (a.os_expense * (d.shr_pct/100))) os_loss, 
                         b.peril_cd, a.acct_intm_cd, 
                         c.line_cd, c.subline_cd
                    FROM gicl_claims c, gicl_clm_res_hist b, gicl_take_up_hist a,
                         gicl_reserve_ds d 
  ...
  */
  cursor ULDLG is (SELECT sum(nvl(a.os_loss,0) + nvl(a.os_expense,0)) os_loss, b.peril_cd, 
                         a.acct_intm_cd, 
                         c.line_cd, c.subline_cd
                    FROM gicl_claims c, gicl_clm_res_hist b, gicl_take_up_hist a
                   WHERE c.claim_id = b.claim_id
  ...
  */
  cursor ULRLG is (SELECT sum(nvl(a.os_loss,0) + nvl(a.os_expense,0)) os_loss, b.peril_cd, a.acct_intm_cd, 
                         c.line_cd, c.subline_cd
                    FROM gicl_claims c, gicl_clm_res_hist b, gicl_take_up_hist a
                   WHERE c.claim_id = b.claim_id
                     AND b.claim_id = a.claim_id
